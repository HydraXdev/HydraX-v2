...
class EliteGuardBalanced:
    ...
    def detect_liquidity_sweep_reversal(self, symbol: str) -> Optional[PatternSignal]:
        ...
        try:
            ...
            sweep_threshold_pips = 2.5  # Minimum sweep for scalping
            if symbol == 'XAUUSD':
                sweep_threshold_pips = 250  # Gold needs bigger moves
            elif 'JPY' in symbol:
                sweep_threshold_pips = 3.0  # JPY pairs need slightly more
            ...
            if bearish_sweep_strength > 0:
                # Strong rejection = large upper wick + close below swept level
                has_rejection = (upper_wick_ratio > 0.4 and  # 40%+ upper wick
                               current['close'] < swept_level)  # Closed below swept level
                ...
                if has_rejection:
                    ...
                    volume_bonus = min(10, (volume_ratio - 1.0) * 5) if volume_ratio >= 1.2 else 0
            ...
            # BULLISH SETUP: Swept lows, strong rejection upward
                ...
                # Strong rejection = large lower wick + close above swept level
                has_rejection = (lower_wick_ratio > 0.4 and  # 40%+ lower wick
                               current['close'] > swept_level)  # Closed above swept level
                ...
                if test_candle['close'] > test_candle['open']:  # Bullish OB
                    ...
                    if ob_low <= current['low'] <= ob_high:
                        ...
                        if current['close'] > ob_mid:
                            ...
                            # Volume bonus
                            volume_ratio = current.get('volume', 1000) / test_candle.get('volume', 1000)
    ...
    def detect_momentum_breakout(self, symbol: str) -> Optional[PatternSignal]:
        ...
        try:
            ...
            if candle_range < 5 or candle_body < 3:
                return None
            direction = None
            breakout_strength = 0
