...
# Load mission link signing config
...
def _verify_link_token(tok):
    try:
        raw = base64.urlsafe_b64decode(tok + "===")
        msg, sig = raw.rsplit(b".", 1)
        if not hmac.compare_digest(hmac.new(SIGN_KEY, msg, hashlib.sha256).digest(), sig):
            return None
        payload = json.loads(msg.decode())
        if payload.get("exp", 0) < int(time.time()): return None
        return payload  # {uid, sid, exp}
    except Exception:
        return None
...
_CLASS_RULES = _json.loads(_os.environ.get("CLASS_FIRE_RULES_JSON",'{"RAPID":["PRESS_PASS","NIBBLER","FANG","COMMANDER"],"SNIPER":["FANG","COMMANDER"]}'))
def _bitten_can_fire(user_tier:str, pattern_class:str)->bool:
    try:
        allow = _CLASS_RULES.get(pattern_class, [])
        return (user_tier or "PRESS_PASS") in allow
    except: return False
def _bitten_user_tier(db_path, user_id):
    try:
        ...
        conn.close()
        return row[0] if row and row[0] else "PRESS_PASS"
    except: return "PRESS_PASS"
...
# ## TELEGRAM_LOGIN_HELPER_START
def _verify_telegram_login(query_params, bot_token):
    """
    ...
    # Check auth_date is within 5 minutes (300 seconds)
    try:
    ...
    # Verify hash according to Telegram docs
    hash_value = query_params.pop('hash', '')
    data_check_arr = []
    
    # Sort params and create check string
    for key in sorted(query_params.keys()):
        data_check_arr.append(f"{key}={query_params[key]}")
...
def _map_telegram_user_to_bitten_user(tg_user_data, db_path):
    """
    Map Telegram user to BITTEN user_id
    ...
    tg_username = tg_user_data.get('username', '').lower()
    tg_first_name = tg_user_data.get('first_name', '')
    ...
    try:
        ...
        # Try direct Telegram ID mapping first
        try:
            row = conn.execute("SELECT user_id FROM users WHERE telegram_id=?", (tg_id,)).fetchone()
            if row:
                conn.close()
        ...
        # Auto-create new user with Telegram ID as user_id
        user_id = str(tg_id)
        try:
            # Create user with default NIBBLER tier
            conn.execute("""
                INSERT INTO users (user_id, tier, risk_pct_default, max_concurrent, 
                                   daily_dd_limit, cooldown_s, balance_cache, telegram_id, xp, streak)
                VALUES (?, 'NIBBLER', 2.0, 1, 6.0, 30, 100.0, ?, 0, 0)
            """, (user_id, tg_id))
            conn.commit()
            print(f"[TG-LOGIN] Auto-created user {user_id} for Telegram user @{tg_username or tg_id}")
        except sqlite3.IntegrityError:
            # User already exists, just update telegram_id if needed
            try:
                conn.execute("UPDATE users SET telegram_id=? WHERE user_id=?", (tg_id, user_id))
                conn.commit()
            except:
                pass
        ...
        print(f"[TG-LOGIN] Error mapping user: {e}")
        return None

def _create_session_cookie(user_id, ttl_minutes=15):
    """
    Create a signed session cookie for user_id
    """
    payload = {
        'user_id': str(user_id),
    ...
    msg = json.dumps(payload, separators=(',',':'), sort_keys=True).encode()
    sig = hmac.new(SIGN_KEY, msg, hashlib.sha256).digest()
    cookie_value = base64.urlsafe_b64encode(msg + b'.' + sig).decode().rstrip('=')
    return cookie_value

def _verify_session_cookie(cookie_value):
    """
    Verify and extract user_id from session cookie
    ...
    try:
        ...
        for user_row in users:
            ...
            try:
                ...
                # Check slot availability for AUTO mode
                try:
                    from src.bitten_core.fire_mode_database import FireModeDatabase
                    fire_mode_db = FireModeDatabase()
                    
                    # Get user's fire mode settings
                    mode_info = fire_mode_db.get_user_mode(user_id)
                    
                    # Check if slots are available
                    if mode_info['current_mode'] == 'AUTO':
                        if mode_info['slots_in_use'] >= mode_info['max_slots']:
                            result["skipped"][user_id] = f"slots_full_{mode_info['slots_in_use']}/{mode_info['max_slots']}"
                            logger.info(f"AUTO: User {user_id} slots full: {mode_info['slots_in_use']}/{mode_info['max_slots']}")
                            continue
                except Exception as slot_err:
                    logger.warning(f"Could not check slots for {user_id}: {slot_err}")
                    # Fall back to old logic - check for any pending fires
                    cur.execute("""
                        SELECT COUNT(*) as pending_count FROM fires 
                        WHERE user_id = ? 
                          AND status IN ('SENT', 'QUEUED')
                          AND created_at > ?
                    """, (user_id, now - 900))  # 15 minutes
                    
                    pending_count = cur.fetchone()[0] if cur.fetchone() else 0
                    if pending_count >= 3:  # Default max 3 slots
                        result["skipped"][user_id] = f"too_many_pending_{pending_count}"
                        continue
                ...
                try:
                    # Occupy slot before enqueueing
                    try:
                        from src.bitten_core.fire_mode_database import FireModeDatabase
                        fire_mode_db = FireModeDatabase()
                        if not fire_mode_db.occupy_slot(user_id, mission_id, symbol):
                            logger.warning(f"AUTO: Failed to occupy slot for {user_id}")
                            # Continue anyway - slot tracking is supplementary
                    except Exception as slot_err:
                        logger.warning(f"AUTO: Could not occupy slot: {slot_err}")
...
def api_signals():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            signal_type = signal_data.get("signal_type") or "PRECISION_STRIKE"
            if not signal_data.get("signal_type"):
                signal_data["signal_type"] = signal_type
                print("[CONTRACT:WARN] Missing signal_type; defaulted to PRECISION_STRIKE")
...
def telegram_login():
    ...
    Verifies Telegram login data and creates session cookie
    """
    try:
        ...
        # Get all query parameters (make a copy to avoid modifying original)
        query_params = dict(request.args)
        signal_id = None
        
        # Extract signal_id from state parameter if present
        state = query_params.pop('state', '')
        if state:
            try:
                import json
                state_data = json.loads(state)
                signal_id = state_data.get('sid')
            except:
                # Try URL param format
                try:
                    import urllib.parse
                    state_data = urllib.parse.parse_qs(state)
                    signal_id = state_data.get('sid', [None])[0]
                except:
                    pass
        
        print(f"[TG-LOGIN] Received login attempt with params: {list(query_params.keys())}")
        
        # Verify Telegram login data
        tg_user_data = _verify_telegram_login(query_params, bot_token)
        if not tg_user_data:
            print(f"[TG-LOGIN] Invalid Telegram authentication data")
            error_html = '''
            <!DOCTYPE html>
            <html><head><title>Login Error</title><meta name="viewport" content="width=device-width, initial-scale=1">
            <style>body{background:#0a0a0a;color:#ff0066;font-family:monospace;padding:20px;text-align:center;}
            .error{background:#1a1a1a;border:1px solid #ff0066;padding:30px;margin:20px auto;max-width:500px;}</style>
            </head><body><div class="error"><h2>⚠️ LOGIN ERROR</h2><p>Invalid or expired Telegram authentication.</p>
            <p>Please try again from the Telegram alert.</p></div></body></html>
            '''
            return error_html, 400
        
        # Map Telegram user to BITTEN user
        DB_PATH = os.environ.get("BITTEN_DB", "/root/HydraX-v2/bitten.db")
        user_id = _map_telegram_user_to_bitten_user(tg_user_data, DB_PATH)
        
        if not user_id:
            print(f"[TG-LOGIN] Could not map Telegram user {tg_user_data.get('username', tg_user_data.get('id'))} to BITTEN user")
            # Show upgrade/registration page instead of failing
            upgrade_html = '''
            <!DOCTYPE html>
            <html><head><title>Account Required</title><meta name="viewport" content="width=device-width, initial-scale=1">
            <style>body{background:#0a0a0a;color:#ff0066;font-family:monospace;padding:20px;text-align:center;}
            .info{background:#1a1a1a;border:1px solid #00ff66;padding:30px;margin:20px auto;max-width:500px;}
            .btn{background:#ff0066;color:white;padding:10px 20px;text-decoration:none;border-radius:5px;display:inline-block;margin:10px;}</style>
            </head><body><div class="info"><h2>🔒 BITTEN Account Required</h2>
            <p>To view missions, you need a BITTEN account linked to your Telegram.</p>
            <p>Contact support to link your account or upgrade to access signals.</p>
            <a href="/upgrade" class="btn">Learn More</a></div></body></html>
            '''
            return upgrade_html, 200
        
        # Create session cookie
        session_cookie = _create_session_cookie(user_id, ttl_minutes=15)
        
        print(f"[TG-LOGIN] Successfully verified user={user_id} from Telegram user {tg_user_data.get('username', tg_user_data.get('id'))}")
        
        # Redirect to /brief with signal_id if provided
        redirect_url = '/brief'
        if signal_id:
            redirect_url += f'?signal_id={signal_id}'
        
        response = redirect(redirect_url)
        response.set_cookie('bitten_session', session_cookie, max_age=15*60, httponly=True, secure=False)
        
        return response
        
    except Exception as e:
        print(f"[TG-LOGIN] Error processing Telegram login: {e}")
        return "Login processing error", 500

@app.route('/brief')
def brief_mission():
    """Create per-user mission when clicking from Telegram"""
    try:
        # Normalize signal_id (URL decode and trim)
        import urllib.parse
        raw_signal_id = request.args.get('signal_id', '').strip()
        signal_id = urllib.parse.unquote_plus(raw_signal_id).strip()
        ...
        print(f"[BRIEF] qparams signal_id={signal_id!r} uid={uid!r} token={'yes' if token else 'no'}")
        
        if token:
            payload = _verify_link_token(token)
            if payload and payload.get("sid") == signal_id:
                user_id = payload.get("uid")
                print(f"[BRIEF] Token verified: user_id={user_id}, signal_id={signal_id}")
            else:
                print(f"[BRIEF] Token verification failed for signal_id={signal_id}")
                user_id = None
        else:
            # Get user_id from multiple sources
            user_id = uid or request.args.get('user_id')
        ...
        if not user_id:
            # First try new session cookie format
            session_cookie = request.cookies.get('bitten_session')
            if session_cookie:
                user_id = _verify_session_cookie(session_cookie)
                if user_id:
                    print(f"[BRIEF] Authenticated via session cookie: user_id={user_id}")
        ...
        # If no user_id, check if they came from Telegram with initData
        if not user_id:
            # Extract Telegram user ID from initData (Telegram Web App)
            ...
            if init_data:
                try:
                    ...
                    if 'user' in parsed:
                        ...
                        print(f"[BRIEF] Got user_id from Telegram initData: {user_id}")
                except Exception as e:
                    print(f"[BRIEF] Failed to parse initData: {e}")
        
        # Default to the Commander if still no user_id (temporary for testing)
        if not user_id:
            user_id = "7176191872"  # Commander user for now
            print(f"[BRIEF] Defaulting to Commander user: {user_id}")
        
        if not signal_id:
            error_html = '''
            <!DOCTYPE html>
            <html><head><title>Mission Error</title><meta name="viewport" content="width=device-width, initial-scale=1">
            <style>body{background:#0a0a0a;color:#ff0066;font-family:monospace;padding:20px;text-align:center;}
            .error{background:#1a1a1a;border:1px solid #ff0066;padding:30px;margin:20px auto;max-width:500px;}</style>
            </head><body><div class="error"><h2>⚠️ MISSION ERROR</h2><p>No signal_id provided in mission link.</p></div></body></html>
            '''
            return error_html, 400
            
        # Create or find mission for authenticated users only
        DB_PATH = os.environ.get("BITTEN_DB", "/root/HydraX-v2/bitten.db")
        
        # Try to find signal data from multiple sources
        signal_data = None
        mission_id = None
        
        # 1) First check if mission already exists
        if user_id != "VIEW_ONLY":
            try:
                with get_bitten_db() as conn:
                    existing_mission = conn.execute(
                        'SELECT mission_id, payload_json FROM missions WHERE signal_id = ? AND (user_id = ? OR mission_id LIKE ?)',
                        (signal_id, user_id, f'%{user_id}%')
                    ).fetchone()
                    if existing_mission:
                        mission_id = existing_mission['mission_id']
                        payload_json = existing_mission['payload_json']
                        if payload_json:
                            signal_data = json.loads(payload_json)
                            print(f"[BRIEF] Found existing mission: {mission_id}")
            except Exception as e:
                print(f"[BRIEF] Error checking existing mission: {e}")
        
        # 2) If no mission/signal found, try to find in Redis alerts stream
        if not signal_data:
            try:
                import redis
                r = redis.Redis(decode_responses=True)
                # Search recent alerts for this signal_id
                alerts = r.xrevrange("alerts", count=100)
                for alert_id, fields in alerts:
                    event_data = json.loads(fields.get("event", "{}"))
                    if event_data.get("signal_id") == signal_id:
                        signal_data = event_data
                        print(f"[BRIEF] Found signal in Redis alerts: {signal_id}")
                        break
            except Exception as e:
                print(f"[BRIEF] Error checking Redis alerts: {e}")
        
        # 3) Try signals table as fallback
        if not signal_data:
            try:
                with get_bitten_db() as conn:
                    ...
                    if signal_row:
                        ...
                        print(f"[BRIEF] Found signal in signals table: {signal_id}")
            except Exception as e:
                print(f"[BRIEF] Error checking signals table: {e}")
        
        # 4) If still no signal found, return user-friendly HTML error
        if not signal_data:
            error_html = f'''
            <!DOCTYPE html>
            <html><head><title>Mission Not Found</title><meta name="viewport" content="width=device-width, initial-scale=1">
            <style>body{{background:#0a0a0a;color:#ff0066;font-family:monospace;padding:20px;text-align:center;}}
            .error{{background:#1a1a1a;border:1px solid #ff0066;padding:30px;margin:20px auto;max-width:500px;border-radius:10px;}}
            .hint{{color:#0ff;font-size:0.9em;margin-top:20px;}}</style>
            </head><body><div class="error"><h2>🔍 MISSION NOT FOUND</h2>
            <p>Signal ID: <code>{signal_id}</code></p>
            <p>This mission link may be stale or the signal never persisted.</p>
            <div class="hint">Check if the alert is recent or contact support if the link was just created.</div>
            </div></body></html>
            '''
            return error_html, 404
        
        # 5) Create mission on-demand if needed for authenticated users
        if user_id != "VIEW_ONLY" and not mission_id:
            try:
                with get_bitten_db() as conn:
                    mission_id = f"msn_{signal_id}"
                    # Create minimal mission record (idempotent)
                    rows_affected = conn.execute('''
                        INSERT OR IGNORE INTO missions(mission_id, signal_id, payload_json, status, created_at, user_id)
                        VALUES (?, ?, ?, 'PENDING', ?, ?)
                    ''', (mission_id, signal_id, json.dumps(signal_data), int(time.time()), user_id)).rowcount
                    conn.commit()
                    
                    # Only count if we actually created a new mission (not already existed)
                    if rows_affected > 0:
                        print(f"[BRIEF-FALLBACK] Created mission on-demand for {signal_id}")
                        # Increment fallback counter and track signal_id
                        try:
                            r = redis.Redis(host='127.0.0.1', port=6379, decode_responses=True)
                            r.incr('bitten:brief_fallback_count')
                            r.sadd('bitten:brief_fallback_signals', signal_id)
                        except Exception as redis_e:
                            # Don't break mission creation if Redis fails
                            print(f"[BRIEF-FALLBACK] Redis counter failed: {redis_e}")
                    else:
                        print(f"[BRIEF] Mission already existed: {mission_id}")
            except Exception as e:
                print(f"[BRIEF] Error creating mission: {e}")
                mission_id = None
        ...
        # Get live balance for authenticated users
        if user_id != "VIEW_ONLY":
        ...
        <body>
            ...
            <div class="mission">
                ...
                <div class="balance">
                    {'Live Balance: ${:.2f}<br>Live Equity: ${:.2f}'.format(balance, equity) if user_id != 'VIEW_ONLY' else 'Login to see balance'}
                </div>
                {'<button class="fire-btn" onclick="executeFire()">FIRE</button>' if user_id != 'VIEW_ONLY' else '<button class="fire-btn" disabled>FIRE (Login Required)</button>'}
            </div>
            <script>
                function executeFire() {{
                    ...
                    .then(r => r.json())
                    .then(data => {{
                        if (data.fire_id || data.status === 'QUEUED') alert('✅ FIRE command sent! ID: ' + (data.fire_id || 'QUEUED'));
                        else if (data.error) alert('❌ Error: ' + data.error);
                        else if (data.message) alert('⚠️ ' + data.message);
...
def fire_mission():
    ...
    # Add idempotency key if missing (skip symbol for now, will add after loading mission)
    if not _req.get("idem"):
        import time
        _req["idem"] = f"{_req.get('fire_id','') or 'fire'}:{_req['user_id']}:{int(time.time())}"
    
    # Get mission data to extract symbol
    mission_id = _req.get("mission_id")
    if mission_id:
        try:
            import sqlite3
            DB_PATH = os.environ.get("BITTEN_DB", "/root/HydraX-v2/bitten.db")
            conn = sqlite3.connect(DB_PATH)
            conn.row_factory = sqlite3.Row
            mission = conn.execute("SELECT payload_json FROM missions WHERE mission_id=?", (mission_id,)).fetchone()
            if mission and mission['payload_json']:
                mission_data = json.loads(mission['payload_json'])
                _req['symbol'] = mission_data.get('symbol', mission_data.get('pair', 'EURUSD'))
            conn.close()
        except Exception as e:
            logger.warning(f"Could not extract symbol from mission: {e}")
            _req['symbol'] = 'EURUSD'  # Default fallback
    
    # --- FIRE SHADOW PUBLISH moved to after payload construction ---
    # Shadow publish now happens after fire_payload is built with all parameters
    ...
    try:
        ...
        # Check if already fired on this exact mission (no time limit)
        already_fired = cur.execute("""
            SELECT 1 FROM fires
            WHERE mission_id=? AND user_id=?
            LIMIT 1
        """, (mission_id, user_id)).fetchone()
        
        if already_fired:
            conn.close()
            return jsonify({"error": "Already fired on this mission"}), 409
        ...
        stop_pips = float(mission_data.get('stop_pips', mission_data.get('stop_loss_pips', 20)) or 20)  # Default 20 pips
        target_pips = float(mission_data.get('target_pips', mission_data.get('take_profit_pips', 40)) or 40)  # Default 40 pips
        ...
        # If entry_price is 0, try to get current market price from Redis
        # But still calculate reasonable SL/TP levels
        if entry_price == 0:
            # Try to get current price from Redis tick data
            try:
                import redis
                r = redis.Redis(host='127.0.0.1', port=6379, decode_responses=True)
                tick_data = r.get(f"tick:{symbol}")
                if tick_data:
                    tick = json.loads(tick_data)
                    # Use ask for BUY, bid for SELL as entry estimate
                    if direction.upper() == "BUY":
                        entry_price = float(tick.get('ask', 0))
                    else:
                        entry_price = float(tick.get('bid', 0))
            except:
                pass
            
            # If still no entry price, use reasonable defaults based on symbol
            if entry_price == 0:
                # Default prices for common symbols
                symbol_defaults = {
                    'EURUSD': 1.10, 'GBPUSD': 1.28, 'USDJPY': 147.0,
                    'GBPJPY': 188.0, 'USDCAD': 1.37, 'XAUUSD': 2650.0,
                    'BTCUSD': 65000.0
                }
                entry_price = symbol_defaults.get(symbol.upper(), 1.0)
            
            # Now calculate absolute SL/TP from estimated entry
            if direction.upper() == "BUY":
                sl = entry_price - stop_pips * pip
                tp = entry_price + target_pips * pip
            else:
            ...
            # Absolute levels when entry is known
            if direction.upper() == "BUY":
        ...
        # Enqueue to IPC
        try:
            # Get the actual target_uuid from request or EA instances
            target_uuid = data.get('target_uuid')
            if not target_uuid:
                # Get fresh EA for user
                ea_row = cur.execute(
                    '''SELECT target_uuid FROM ea_instances 
                       WHERE user_id = ? AND (strftime('%s','now') - last_seen) < 180
                       ORDER BY last_seen DESC LIMIT 1''',
                ...
                if ea_row:
                    target_uuid = ea_row[0] if isinstance(ea_row, tuple) else ea_row['target_uuid']
                else:
                    target_uuid = 'COMMANDER_DEV_001'  # Fallback for Commander
            ...
            _sig_class = _bitten_signal_class_from_id(_dbp, signal_id if 'signal_id' in locals() else mission['signal_id'])
            if not _bitten_can_fire(_user_tier, _sig_class):
                ...
                return jsonify({'ok': False, 'upgrade_required': True, 'pattern_class': _sig_class,
                               'message': f'Upgrade to FANG ($89) or COMMANDER ($129) to fire {_sig_class} signals.',
                               'upgrade_url': os.environ.get('UPGRADE_URL_BASE','https://yourdomain/upgrade')}), 403
            
            # --- FIRE SHADOW PUBLISH (Redis Streams) with complete payload ---
            try:
                import os as _os_shadow
                if _os_shadow.environ.get("FIRE_TO_REDIS","1") == "1":
                    stream = f"fire.{fire_payload['target_uuid']}"
                    _bitten_fire_xadd(_os_shadow.environ.get("REDIS_HOST","127.0.0.1"), _os_shadow.environ.get("REDIS_PORT","6379"), stream, fire_payload)
            except Exception as _e:
                try: print("fire shadow publish err:", _e, flush=True)
                except: pass
            # --- END FIRE SHADOW PUBLISH ---
            
            logger.info(f"🔥 Enqueueing fire payload: {json.dumps(fire_payload)}")
            enqueue_fire(fire_payload)
...
@app.route('/api/trades/<user_id>')
def get_user_trades(user_id):
    """Get complete trade history and statistics for user"""
    try:
        from trade_history_tracker import get_user_trade_data
        
        # Get comprehensive trade data
        trade_data = get_user_trade_data(user_id)
        
        # Add real-time price updates for open positions
        for position in trade_data.get('positions', {}).get('open', []):
            # Subscribe to real-time updates for this position
            position['live_updates'] = True
        ...
        logger.error(f"Error getting trade data for user {user_id}: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def war_room():
    ...
    try:
        ...
        try:
            ...
            # Get user's actual positions and stats
            positions = tracker.get_user_positions(user_id)
            real_stats = tracker.get_user_statistics(user_id)
            
            # Update stats with real data if available
            if real_stats['total_trades'] > 0:
                total_trades = real_stats['total_trades']
                win_rate = real_stats['win_rate'] / 100.0  # Convert to decimal
                current_streak = real_stats['current_streak']
                total_pnl = real_stats['total_pips'] * 10  # Approximate USD conversion
            
            # Format recent trades for display
            recent_kills = []
            
            # Add open positions first
            for pos in positions.get('open', [])[:3]:
                recent_kills.append({
                    "pair": pos['symbol'],
                    ...
                    "time": "OPEN",
                    "tcs": 0,
                    "profit": pos.get('current_pnl', 0)
                })
            
            # Add recent closed trades
            for pos in positions.get('closed', [])[:5]:
                ...
                recent_kills.append({
                    ...
                    "direction": pos['direction'],
                    "entry": f"{pos['entry_price']:.5f}",
                    "time": datetime.fromtimestamp(pos['closed_at']).strftime('%H:%M') if pos['closed_at'] else 'Unknown',
                    "tcs": 0,
                    "profit": pos.get('final_pnl', 0)
                })
                
        except Exception as e:
            logger.warning(f"Could not load real trade data: {e}")
...
def tier_comparison():
    ...
    <body>
        ...
        <div class="tier-grid">
            <div class="tier-card">
                <h3>🎫 PRESS PASS</h3>
                <div class="tier-price">PRESS PASS (7 days)</div>
                <p>7-day trial</p>
...
@app.route("/api/_mint_token", methods=["POST"])
def _mint():
    # Require X-Admin: 1 header to guard (simple)
    if request.headers.get("X-Admin") != "1":
        return jsonify({"ok": False, "err": "forbidden"}), 403
    data = request.get_json(force=True) or {}
    return jsonify({"ok": True, "t": _mint_link_token(data.get("user_id"), data.get("signal_id"))})
...
def upgrade_page():
    return '''<h2>🚀 Upgrade to FANG or COMMANDER</h2>
