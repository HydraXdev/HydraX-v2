...
class AdaptiveMLEngine:
    def __init__(self):
        ...
        self.pair_performance = defaultdict(
            lambda: {"win_rate": 0.0, "total_trades": 0, "streak": 0, "last_update": time.time()}
        )
    ...
    def calculate_adaptive_confidence(self, signal_data):
        ...
        try:
            ...
            if abs(adjustment) > 5:
                logger.info(
                    f"ML Adjustment: {symbol} {pattern} {base_confidence:.1f}% → {adjusted_confidence:.1f}% (Δ{adjustment:+.1f}%)"
                )
    ...
    def get_recommended_signals(self, current_signals):
        ...
        for signal in current_signals:
            ...
            composite_score = (
                ml_confidence * 0.4 + signal["pattern_win_rate"] * 100 * 0.3 + signal["pair_win_rate"] * 100 * 0.3
            )
        ...
        for signal in ranked_signals:
            ...
            if signal["ml_confidence"] >= 85 and signal["pattern_win_rate"] >= 0.4 and signal["pair_win_rate"] >= 0.35:
            ...
            try:
                ...
                if subscriber.poll(1000):  # 1 second timeout
                    ...
                    if message.startswith("ELITE_GUARD_SIGNAL"):
                        ...
                        logger.info(
                            f"ML Enhanced: {signal_data['symbol']} {signal_data['pattern']} "
