...
class AdminSlotManager:
    ...
    def get_effective_slot_limits(self, user_id: str, user_tier: str) -> Dict[str, int]:
        ...
        if override_data:
            ...
            effective_total = (
                total_override if total_override is not None else (tier_total or (effective_manual + effective_auto))
            )

            logger.info(
                f"🎯 User {user_id} has slot overrides: Manual={effective_manual}, Auto={effective_auto}, Total={effective_total}, Reason: {reason}"
            )
        else:
        ...
        return {
            ...
            "tier_defaults": {
                "manual": tier_manual,
                "auto": tier_auto,
                "total": tier_total or (tier_manual + tier_auto),
            },
        }

    def set_user_slot_override(
        self,
        user_id: str,
        admin_user: str,
        manual_slots: Optional[int] = None,
        auto_slots: Optional[int] = None,
    ...
    def remove_user_slot_override(self, user_id: str, admin_user: str, reason: str = "Override removed") -> bool:
        ...
        try:
            ...
            cursor.execute(
                "SELECT manual_slot_override, auto_slot_override, total_slot_override FROM user_slot_overrides WHERE user_id = ?",
                (user_id,),
            )
            override_data = cursor.fetchone()
    ...
    def update_tier_defaults(
        ...
        try:
            ...
            cursor.execute(
                "SELECT manual_slots, auto_slots, total_slots FROM tier_slot_defaults WHERE tier = ?", (tier.upper(),)
            )
...
def main():
    ...
    if args.action == "get" and args.user_id:
        ...
        for entry in history:
            print(
                f"  {entry['timestamp']}: {entry['user_id']} - {entry['action']} {entry['slot_type']} {entry['old_value']}→{entry['new_value']} by {entry['admin_user']}"
            )
        ...
        for override in overrides:
            print(
                f"  {override['user_id']}: Manual={override['manual_slot_override']}, Auto={override['auto_slot_override']}, Total={override['total_slot_override']}"
            )
