...
class AdvancedSMCDetector:
    ...
    def detect_liquidity_grab(self, symbol: str, candles: List[MarketData]) -> Optional[SMCPattern]:
        ...
        if (
            current.high > swing_high.high
            and current.close < swing_high.high - (swing_high.high - swing_high.low) * 0.3
        ):
            ...
            if risk_pips > 0 and reward_pips / risk_pips >= self.min_rr_ratio:
                confidence = self._calculate_confidence(
                    ...
                    symbol,
                    candles,
                    {"direction": "SELL", "grab_strength": current.high - swing_high.high},
                )
        ...
        # Check for liquidity grab below low (bullish setup)
        elif current.low < swing_low.low and current.close > swing_low.low + (swing_low.high - swing_low.low) * 0.3:
            ...
            if risk_pips > 0 and reward_pips / risk_pips >= self.min_rr_ratio:
                confidence = self._calculate_confidence(
                    ...
                    symbol,
                    candles,
                    {"direction": "BUY", "grab_strength": swing_low.low - current.low},
                )
    ...
    def detect_break_of_structure(self, symbol: str, candles: List[MarketData]) -> Optional[SMCPattern]:
        ...
        if structure == MarketStructure.BULLISH:
            ...
            if (
                current.close > resistance_level and current.high > resistance_level and current.close > current.open
            ):  # Bullish candle
                ...
                if risk_pips > 0 and reward_pips / risk_pips >= self.min_rr_ratio:
                    confidence = self._calculate_confidence(
                        PatternType.BREAK_OF_STRUCTURE, symbol, candles, {"direction": "BUY", "structure": "BULLISH"}
                    )
        ...
        # For bearish structure, look for breaks below support
            ...
            if (
                current.close < support_level and current.low < support_level and current.close < current.open
            ):  # Bearish candle
                ...
                if risk_pips > 0 and reward_pips / risk_pips >= self.min_rr_ratio:
                    confidence = self._calculate_confidence(
                        PatternType.BREAK_OF_STRUCTURE, symbol, candles, {"direction": "SELL", "structure": "BEARISH"}
                    )
    ...
    def detect_inducement_trap(self, symbol: str, candles: List[MarketData]) -> Optional[SMCPattern]:
        ...
        if false_break_candles and current.close < key_resistance and current.close < current.open:  # Rejection candle
            ...
            if risk_pips > 0 and risk_pips <= 15:  # Tight stop for traps
                confidence = self._calculate_confidence(
                    ...
                    symbol,
                    candles,
                    {"direction": "SELL", "trap_strength": max(c.high for c in false_break_candles) - key_resistance},
                )
        ...
        if false_break_candles and current.close > key_support and current.close > current.open:  # Bullish rejection
            ...
            if risk_pips > 0 and risk_pips <= 15:  # Tight stop for traps
                confidence = self._calculate_confidence(
                    ...
                    symbol,
                    candles,
                    {"direction": "BUY", "trap_strength": key_support - min(c.low for c in false_break_candles)},
                )
    ...
    def _calculate_confidence(
        self, pattern_type: PatternType, symbol: str, candles: List[MarketData], metadata: Dict
    ) -> float:
        """
        ...
        final_confidence = (
            base_conf * win_rate_adj + session_bonus + volume_surge + volatility_factor + mtf_confluence + pattern_bonus
        )
