...
class AdvancedStopHuntingProtection:
    ...
    def calculate_adaptive_stop_randomization(
        self, symbol: str, base_stop: float, current_atr: float, volatility_multiplier: float
    ) -> float:
        """
        ...
        adjusted_range = (int(random_range[0] * broker_multiplier), int(random_range[1] * broker_multiplier))
        ...
        self.protection_stats["avg_randomization_pips"] = (
            self.protection_stats["avg_randomization_pips"] * (self.protection_stats["stops_randomized"] - 1)
            + random_pips
        ) / self.protection_stats["stops_randomized"]

        self.logger.debug(
            f"🎲 {symbol} stop randomized: {random_pips:.1f} pips (volatility: {volatility_multiplier:.2f}x)"
        )
    ...
    def calculate_rolling_spread_baseline(self, symbol: str, tick_data: List, lookback_minutes: int = 60) -> float:
        ...
        if len(tick_data) < 10:
            ...
            # Cache the fallback
            self.spread_baseline_cache[cache_key] = {"baseline": baseline, "timestamp": current_time}
            return baseline
    ...
    def check_correlated_pair_spreads(self, primary_symbol: str, all_tick_data: Dict) -> Dict:
        ...
        if not related_pairs:
            return {"safe_to_trade": True, "correlation_health": 1.0, "affected_pairs": [], "recommendation": "TRADE"}
    ...
    def calculate_hunting_adjusted_position_size(
        self,
        base_size: float,
        symbol: str,
        ...
        if final_multiplier < 1.0:
            ...
            self.protection_stats["avg_position_reduction"] = (
                self.protection_stats["avg_position_reduction"] * (self.protection_stats["position_sizes_reduced"] - 1)
                + reduction_pct
    ...
    def get_final_trade_recommendation(
        self, correlation_check: Dict, spread_analysis: Dict, position_adjustment: Dict
    ) -> str:
        """
    ...
    def apply_comprehensive_protection(
        self,
        signal: Dict,
        ...
        position_adjustment = self.calculate_hunting_adjusted_position_size(
            base_position_size, symbol, correlation_check, spread_analysis, volatility_multiplier, news_env
        )
