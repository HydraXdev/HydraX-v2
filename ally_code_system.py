...
class AllyCodeManager:
    ...
    def create_founder_codes(self):
        ...
        founder_codes = [f"FOUNDER{str(i).zfill(2)}" for i in [1, 10, 20, 30, 40, 50, 60, 70, 80, 90]]
    ...
    def create_influencer_code(
        self, influencer_name: str, commission_rate: float = 0.5, discount_percent: float = 50.0, max_uses: int = 100
    ) -> str:
    ...
    def bind_founder_to_telegram(self, founder_code: str, telegram_id: str, tier: str) -> Dict:
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (telegram_id, founder_code, datetime.now().isoformat(), tier, 89.0, True, 0.0),
        )
    ...
    def use_ally_code(
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (code.upper(), user_id, email, telegram_id, stripe_session_id, datetime.now().isoformat(), ip_address),
        )
        ...
        return {"valid": True, "used": True, "code_type": validation["code_type"], "discount_info": validation}
