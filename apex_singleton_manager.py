...
Ensures only ONE instance of engine can run at a time
...
class SingletonManager:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        logger = logging.getLogger("_SINGLETON")
    ...
    def acquire_lock(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ Acquired exclusive lock for engine")
            return True
        except IOError:
    ...
    def kill_existing_processes(self, exclude_pid: Optional[int] = None) -> int:
        """Kill all processes except the excluded PID"""
        killed_count = 0
        ...
        for proc in apex_processes:
            ...
            try:
                self.logger.warning(f"🔫 Killing duplicate process PID={proc.pid}")
                proc.terminate()
                time.sleep(0.5)
    ...
    def ensure_singleton(self) -> bool:
        """Main method to ensure only one instance runs"""
        current_pid = os.getpid()
        
        # Step 1: Try to acquire exclusive lock
        if not self.acquire_lock():
            self.logger.error("❌ Another instance has the lock. Exiting.")
            return False
        ...
        if other_processes:
            self.logger.warning(f"⚠️ Found {len(other_processes)} other processes running!")
            
            # Kill all other processes
            killed = self.kill_existing_processes(exclude_pid=current_pid)
            self.logger.info(f"🎯 Killed {killed} duplicate processes")
            ...
            if other_remaining:
                self.logger.error(f"❌ Still {len(other_remaining)} processes running!")
                self.release_lock()
                return False
        ...
        self.logger.info(f"✅ Singleton established - PID: {current_pid}")
        return True
...
def enforce_singleton():
    """Helper function to be called at the start of engine"""
    manager = SingletonManager()
    if not manager.ensure_singleton():
        print("❌ Failed to establish singleton - another instance is running!")
        sys.exit(1)
    return manager
