...
import asyncio
import time
from logging.logger import app_logger, forexvps_logger, performance_logger, security_logger, setup_logging
...
from database.models import Mission, Signal, User, create_tables, get_db
from forexvps.client import ForexVPSClient, execute_trade_forexvps
from security.auth import TelegramAuth, auth_manager, check_rate_limit, get_current_user, require_tier, validate_input
...
async def request_logging_middleware(request: Request, call_next):
    ...
    # Log request start
    app_logger.info(f"Request started: {request.method} {request.url.path}", request_id=request_id)
    ...
    try:
        ...
        performance_logger.log_request_time(
            endpoint=f"{request.method} {request.url.path}", response_time=response_time
        )
        ...
        response_time = time.time() - start_time
        app_logger.error(
            f"Request failed: {request.method} {request.url.path} " f"in {response_time:.3f}s - {str(e)}",
            request_id=request_id,
        )
        raise
...
async def detailed_health_check(current_user: User = Depends(require_tier("COMMANDER"))):
    ...
    return {"status": "healthy", "details": "Admin health check implementation"}
...
async def login(telegram_id: str, username: str = None):
    ...
    try:
        ...
        token = auth_manager.create_access_token(
            {"telegram_id": user.telegram_id, "username": user.username, "tier": user.tier}
        )
...
async def receive_signal(
    signal_data: Dict[str, Any], background_tasks: BackgroundTasks, current_user: User = Depends(require_tier("SCOUT"))
):
    ...
    try:
        ...
        return {"status": "success", "signal_id": signal.signal_id, "message": "Signal received and processing"}
...
async def execute_trade(
    trade_request: Dict[str, Any], current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        # Execute trade via ForexVPS
        forexvps_result = await execute_trade_forexvps(user_id=current_user.telegram_id, trade_data=trade_request)
...
async def war_room(request: Request, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "war_room.html", {"request": request, "user": user_stats, "title": "War Room - Command Center"}
    )
