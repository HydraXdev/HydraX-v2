...
# Import singleton protection - but skip in testing mode
try:
    from apex_singleton_manager import enforce_singleton
    # Only enforce singleton for production, not testing
    SINGLETON_AVAILABLE = True
except ImportError:
    SINGLETON_AVAILABLE = False

# Import REAL MT5 data connection via file bridge
try:
    ...
    print("⚠️ MT5 file bridge not available")
...
class ProductionV6Enhanced:
    """Enhanced production engine with smart timer integration - TESTING COPY"""
    
    def __init__(self, testing_mode=False):
        self.testing_mode = testing_mode
        
        # Only enforce singleton in production mode
        if not testing_mode and SINGLETON_AVAILABLE:
        ...
        # Initialize apex_data_config attribute
        self.apex_data_config = None
        self.load_apex_data_feed_config()
        
        # Initialize smart timer system
        self.smart_timer = SmartTimerEngine()
        
        # Skip MT5 initialization in testing mode
        if not testing_mode:
    ...
    def load_apex_data_feed_config(self):
        ...
        try:
            ...
            if self.testing_mode:
                self.logger.warning(f"⚠️ data feed config not found in testing mode: {e}")
                # In testing mode, create minimal config
                self.apex_data_config = {
                    'login': 'testing_mode',
                    'server': 'test_server'
                }
            else:
    ...
    def initialize_apex_data_feed(self):
        """Initialize REAL MT5 connection via socket bridge for data feed"""
        if not MT5_BRIDGE_AVAILABLE:
            if self.testing_mode:
                self.logger.warning("⚠️ MT5 bridge not available in testing mode - using mock connection")
                self.mt5_connection = False  # Mark as not connected for testing
                return False
            else:
                self.logger.critical("🚨 SOCKET BRIDGE NOT AVAILABLE - CANNOT START")
                raise Exception("CRITICAL: Socket bridge required - cannot operate without real MT5 connection")
        ...
        try:
            # Initialize socket connection to MT5 bridge
            self.mt5_bridge_host = "localhost"
            self.mt5_bridge_port = 9000
            
            # Test connection to MT5 bridge
            test_result = self._test_mt5_bridge_connection()
            if test_result:
                self.logger.info(f"🚀 REAL DATA FEED CONNECTED VIA BRIDGE: {self.apex_data_config['login']}@{self.apex_data_config['server']}")
                self.mt5_connection = True  # Mark as connected
                return True
            else:
                raise Exception("MT5 bridge connection test failed")
                
        except Exception as e:
            self.logger.error(f"❌ data feed connection failed: {e}")
            self.mt5_connection = None
            raise Exception(f"CRITICAL: Cannot establish MT5 bridge connection - {e}")
    
    def _test_mt5_bridge_connection(self) -> bool:
        """Test connection to MT5 bridge server"""
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(5)  # 5 second timeout
            sock.connect((self.mt5_bridge_host, self.mt5_bridge_port))
            
            # Send ping command
            ping_request = {
                "command": "ping",
                "account": self.apex_data_config['login'],
                "server": self.apex_data_config['server']
            }
            
            sock.send(json.dumps(ping_request).encode() + b'\n')
            response = sock.recv(1024).decode().strip()
            sock.close()
            
            response_data = json.loads(response)
            if response_data.get('status') == 'connected':
                self.logger.info(f"✅ MT5 Bridge ping successful: {response_data}")
                return True
            else:
                self.logger.error(f"❌ MT5 Bridge ping failed: {response_data}")
                return False
                
        except Exception as e:
            self.logger.error(f"❌ MT5 Bridge connection test failed: {e}")
            return False
    ...
    def _get_market_data_from_bridge(self, symbol: str) -> Dict:
        ...
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(10)  # 10 second timeout for data requests
            sock.connect((self.mt5_bridge_host, self.mt5_bridge_port))
            ...
            sock.send(json.dumps(data_request).encode() + b'\n')
            ...
            if response_data.get('success'):
                ...
                raise Exception(f"Bridge returned error: {response_data.get('error', 'Unknown error')}")
                
        except Exception as e:
            self.logger.error(f"❌ Failed to get market data from bridge for {symbol}: {e}")
            raise Exception(f"Bridge communication failed: {e}")
    ...
    def _generate_signal_with_real_context(self, market_context: Dict) -> Optional[Dict]:
        """Generate signal using real market context for testing"""
        try:
            symbol = market_context['symbol']
            timestamp = market_context['timestamp']
            price = market_context['close']
            volatility = market_context['volatility']
            session = market_context['session']
            
            # Use actual production signal generation logic
            technical_score = self._calculate_technical_score(market_context)
            pattern_strength = self._calculate_pattern_strength(market_context)
            momentum_score = self._calculate_momentum_score(market_context)
            volatility_score = self._calculate_volatility_score(volatility)
            session_score = self._calculate_session_score(session)
            
            # Calculate TCS using production formula
            tcs = (technical_score * 0.3 + 
            ...
            if tcs < self.base_thresholds['min_tcs_confidence']:
                return None
            
            # Determine direction based on technical analysis
            direction = 'buy' if technical_score > momentum_score else 'sell'
            
            # Calculate stop loss and take profit using production rules
            risk_pips = max(15, volatility * 10000 * 1.5)  # Minimum 15 pips
            reward_ratio = 1.8  # Conservative 1:1.8 ratio
            reward_pips = risk_pips * reward_ratio
            
            if direction == 'buy':
                stop_loss = price - (risk_pips * 0.0001)
                take_profit = price + (reward_pips * 0.0001)
            else:
                stop_loss = price + (risk_pips * 0.0001)
                take_profit = price - (reward_pips * 0.0001)
    ...
    def _calculate_technical_score(self, context: Dict) -> float:
        """Calculate technical analysis score from real market data"""
        try:
            price = context['close']
            high = context['high']
            low = context['low']
            ...
            # Simple but realistic technical scoring
            range_position = (price - low) / (high - low) if high != low else 0.5
            volume_score = min(100, volume / 1000)  # Normalize volume
            
            # Combine factors
            technical_score = (range_position * 60) + (volume_score * 40)
            return min(100, max(0, technical_score))
            
        except:
            return 50.0  # Neutral score
    
    def _calculate_pattern_strength(self, context: Dict) -> float:
        """Calculate pattern strength from real market data"""
        try:
            # Simple pattern recognition
            open_price = context['open']
            ...
            pattern_strength = body_ratio * 100
            
            return min(100, max(0, pattern_strength))
            
        except:
            return 50.0
    
    def _calculate_momentum_score(self, context: Dict) -> float:
        """Calculate momentum from real market data"""
        try:
            open_price = context['open']
            close_price = context['close']
            ...
            if close_price > open_price:
                momentum = ((close_price - open_price) / open_price) * 10000  # In pips
            else:
                momentum = ((open_price - close_price) / open_price) * 10000
    ...
    def _calculate_volatility_score(self, volatility: float) -> float:
        """Score volatility for signal generation"""
        try:
            # Optimal volatility range
            if 0.0001 <= volatility <= 0.0003:
                return 80.0  # Good volatility
            elif volatility < 0.0001:
                return 30.0  # Too low
            else:
                return 60.0  # High but acceptable
        except:
            return 50.0
    
    def _calculate_session_score(self, session: str) -> float:
        ...
        return session_scores.get(session, 70.0)
