...
def index():
    ...
    try:
        ...
        <body>
            <div id="bitten-hud">
                <h1>🎯 BITTEN TACTICAL TRADING HUD</h1>
...
def mission_briefing():
    ...
    try:
        ...
        <html>
        <head>
            <title>{{ mission.title }} - BITTEN</title>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <style>
                body { font-family: 'Courier New', monospace; background: #000; color: #00ff00; margin: 0; padding: 20px; }
                .hud { border: 2px solid #00ff00; padding: 20px; margin: 10px 0; }
                .signal-type { font-size: 24px; font-weight: bold; text-align: center; margin-bottom: 20px; }
                .signal-info { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin: 15px 0; }
                .info-item { padding: 10px; border: 1px solid #00ff00; }
                .countdown { font-size: 20px; color: #ff3333; text-align: center; margin: 20px 0; }
                .fire-button { background: #ff3333; color: white; border: none; padding: 15px 30px; font-size: 18px; width: 100%; cursor: pointer; }
                .fire-button:disabled { background: #666; cursor: not-allowed; }
                .tactical-notes { margin: 20px 0; }
                .tactical-notes ul { padding-left: 20px; }
            </style>
            <script>
        ...
        <body>
            <div class="hud">
                <div class="signal-type">{{ signal.signal_type }} - {{ signal.symbol }} {{ signal.direction }}</div>
                <div class="countdown" id="countdown"></div>
                
                <div class="signal-info">
                    <div class="info-item"><strong>TCS Score:</strong> {{ signal.tcs_score }}%</div>
                    <div class="info-item"><strong>Entry:</strong> {{ signal.entry_price }}</div>
                    <div class="info-item"><strong>Stop Loss:</strong> {{ signal.stop_loss }}</div>
                    <div class="info-item"><strong>Take Profit:</strong> {{ signal.take_profit }}</div>
                    <div class="info-item"><strong>R:R Ratio:</strong> {{ signal.risk_reward_ratio }}</div>
                    <div class="info-item"><strong>User Tier:</strong> {{ user.tier }}</div>
                </div>
                
                <div class="tactical-notes">
                    <h3>📋 MISSION BRIEFING:</h3>
                    <p>{{ mission.description }}</p>
                    {% if mission.tactical_notes %}
                    <h4>🎯 TACTICAL NOTES:</h4>
                    <ul>
                        {% for note in mission.tactical_notes %}
                        <li>{{ note }}</li>
                        {% endfor %}
                    </ul>
                    {% endif %}
                ...
                <button class="fire-button" id="fire-btn" onclick="fireMission()">
                    🔥 FIRE MISSION
                </button>
            </div>
            ...
            <script>
                function fireMission() {
                    if (confirm('Execute this mission?')) {
                        document.getElementById('fire-btn').innerHTML = '🚀 EXECUTING...';
                        document.getElementById('fire-btn').disabled = true;
                        // Fire mission logic would go here
                        setTimeout(() => {
                            document.getElementById('fire-btn').innerHTML = '✅ MISSION FIRED';
                        }, 2000);
                    }
                }
            </script>
