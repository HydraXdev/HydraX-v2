...
function Beacon({ label, ok = true, value }: BeaconProps) {
  return (
    <div className="flex items-center gap-1 text-xs text-zinc-400 m-0 p-0">
      <div
        className={`h-2.5 w-2.5 rounded-full m-0 p-0 ${ok ? "bg-emerald-400" : "bg-red-500"}`}
      />
      <span className="uppercase tracking-wide m-0 p-0">{label}</span>
      {value && (
        <span className="tabular-nums text-zinc-300 m-0 p-0">{value}</span>
      )}
    </div>
  );
}
...
function TradeLane({ trade, now }: TradeLaneProps) {
  ...
  const ageTint =
    agingSec > 3 * 3600 ? "ring-yellow-600/50" : "ring-emerald-700/40";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 m-0 p-0">
          ...
          <div className="w-full sm:w-40 m-0 p-0">
            <div className="flex items-center justify-between m-0 p-0">
              <div className="text-lg font-bold text-emerald-300 m-0 p-0">
                {trade.pair}
              </div>
              <span className="border border-zinc-700 text-zinc-300 bg-zinc-900/40 px-2 py-0.5 text-xs rounded m-0">
                {trade.lots} LOTS
              </span>
            </div>
          ...
          <div className="relative flex-1 m-0 p-0">
            <div
              className={`relative h-12 bg-zinc-900 border border-emerald-700/50 ring-1 ${ageTint} rounded m-0 p-0`}
            >
              {/* SL gate */}
              <div className="absolute left-0 top-0 bottom-0 w-1 bg-red-600 rounded-l m-0 p-0" />
              <div className="absolute left-2 top-1/2 -translate-y-1/2 text-[11px] text-red-400 m-0 p-0">
                SL {trade.stopLoss}
              </div>

              {/* Fill */}
              <div
                className={`absolute top-0 bottom-0 transition-all duration-300 rounded-l m-0 p-0 ${isPos ? "bg-emerald-600/60" : "bg-red-600/60"}`}
                style={{ width: pct(progress) }}
              />

              {/* Cursor (NOW) */}
              <div
                className="absolute top-0 bottom-0 w-1 bg-yellow-400 m-0 p-0"
                style={{ left: pct(progress) }}
              />
              <div
                className="absolute -translate-y-1/2 top-1/2 text-[11px] text-yellow-300 font-bold whitespace-nowrap m-0 p-0"
                style={{ left: `calc(${pct(progress)} + 6px)` }}
              >
                {trade.current}
              </div>
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 text-[11px] text-emerald-400 m-0 p-0">
                TP {trade.takeProfit}
              </div>
            </div>
            <div className="mt-1 text-[11px] text-zinc-400 m-0 p-0">
              ENTRY {trade.entry}
            </div>
          </div>
          ...
          <div className="w-full sm:w-44 text-right border-2 border-emerald-700/50 bg-black/60 p-3 rounded m-0">
            ...
            <div
              className={`text-2xl font-bold tabular-nums m-0 p-0 ${isPos ? "text-emerald-300" : "text-red-400"}`}
            >
              {fmtSigned(trade.equity)}
            </div>
            <div className="text-[11px] text-zinc-400 m-0 p-0">USD</div>
            {trade.history && trade.history.length > 1 && (
              <div className="mt-1 text-emerald-400/70 m-0 p-0">
                <Sparkline data={trade.history} />
              </div>
            )}
          </div>
        </div>
        ...
        if (positionsRes.ok) {
          ...
          const openTrades: Trade[] =
            positionsData.positions?.map((pos: any) => ({
              ...
              startTime: new Date(
                pos.open_time || pos.created_at || Date.now(),
              ),
    ...
    <div className="min-h-screen bg-zinc-950 text-emerald-300 font-mono m-0 p-0">
      ...
      <div className="border-b border-emerald-800/50 bg-gradient-to-b from-zinc-900 to-black m-0 p-0">
        <div className="mx-auto max-w-7xl p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 m-0">
          <div className="m-0 p-0">
            <div
              className="text-2xl sm:text-3xl font-extrabold tracking-[0.15em] m-0 p-0"
              style={{ color: "#8b7355" }}
            >
              ⬢ B.I.T.T.E.N
            </div>
            <div className="text-emerald-400/70 text-xs sm:text-sm m-0 p-0">
          ...
          <div className="flex items-center gap-4 text-zinc-400 flex-wrap m-0 p-0">
            ...
            <time className="tabular-nums text-zinc-200 m-0 p-0">
              {now.toLocaleTimeString(undefined, { hour12: false })}
            </time>
            <span className="text-zinc-400 m-0 p-0">
              {now.toLocaleDateString(undefined, {
                month: "short",
                day: "numeric",
              })}
            </span>
            <span className="border border-yellow-600/60 text-yellow-300 bg-yellow-900/20 px-2 py-1 text-xs rounded m-0">
              LEVEL: {level}
            </span>
          </div>
        </div>
            ...
            <div className="grid sm:grid-cols-3 gap-3 m-0 p-0">
              <div className="m-0 p-0">
                <div className="flex items-center justify-between m-0 p-0">
                  ...
                  <span className="text-2xl font-bold tabular-nums m-0 p-0">
                    {fmtUSD(balance)}
                  </span>
                </div>
                <div className="mt-2 text-[11px] text-zinc-400 m-0 p-0">
                  Base capital
                </div>
              </div>
              <div className="m-0 p-0">
                <div className="flex items-center justify-between m-0 p-0">
                  ...
                  <span
                    className={`text-2xl font-bold tabular-nums m-0 p-0 ${totalPL >= 0 ? "text-emerald-300" : "text-red-400"}`}
                  >
                    {fmtUSD(equity)}
                  </span>
                </div>
                <div className="mt-2 m-0 p-0">
                  <div className="text-[11px] text-zinc-400 mb-1 m-0 p-0">
                    Live Δ from balance
                  </div>
                  <div className="h-2 bg-zinc-800 rounded-full overflow-hidden m-0 p-0">
                    <div
                      className={`${dd >= 0 ? "bg-emerald-500" : "bg-red-500"} h-2 transition-all m-0 p-0`}
                      style={{ width: `${Math.min(100, Math.abs(dd) * 100)}%` }}
                    />
                  </div>
                </div>
              </div>
              <div className="m-0 p-0">
                ...
                <div className="mt-2 text-[11px] text-zinc-400 m-0 p-0">
                  Capacity remaining: {Math.max(0, slots - trades.length)}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl p-3 sm:p-4 space-y-3 m-0">
        {loading ? (
          ...
          <>
            {trades.map((t) => (
              <TradeLane key={t.id} trade={t} now={now} />
            ))}

            {/* Empty slots */}
            {Array.from({ length: Math.max(0, slots - trades.length) }).map(
              (_, idx) => (
                <div
                  key={`empty-${idx}`}
                  className="bg-black/30 border border-zinc-800 rounded-lg m-0 p-0"
                >
                  <div className="p-3 sm:p-4 m-0">
                    <div className="flex items-center gap-3 sm:gap-4 h-10 m-0 p-0">
                      <div className="w-20 sm:w-32 text-zinc-600 text-xs sm:text-sm m-0 p-0">
                        SLOT {trades.length + idx + 1}
                      </div>
                      <div className="flex-1 bg-zinc-900/60 h-full grid place-items-center text-zinc-600 text-xs sm:text-sm rounded m-0 p-0">
                        [ AVAILABLE ]
                      </div>
                      <div className="w-16 sm:w-40 text-right text-zinc-600 text-xs sm:text-sm m-0 p-0">
                        —
                      </div>
                    </div>
                  </div>
        ...
        <div className="bg-black/60 border border-emerald-700/50 rounded-lg m-0 p-0">
          <div className="p-2 text-center text-[12px] text-emerald-400 m-0">
            ...
            <span className="inline-flex items-center gap-4 flex-wrap justify-center m-0 p-0">
              <span className="inline-flex items-center gap-1 m-0 p-0">
                <div className="h-2.5 w-2.5 rounded-full bg-emerald-400 m-0 p-0" />{" "}
                OPERATIONAL
              </span>
              <span className="inline-flex items-center gap-1 m-0 p-0">
                🔒 SECURE
