...
function MissionPageContent() {
  ...
  async function fetchLatestMission() {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://134.199.204.67:8888';
      ...
      if (data.missions && data.missions.length > 0) {
        ...
        const missionResponse = await fetch(`${apiUrl}/api/mission/${latestMission.signal_id}`);
        ...
        if (missionData.success && missionData.mission) {
          ...
          else if (symbol.includes('XAU') || symbol.includes('GOLD')) pipSize = 0.1;
          else if (symbol.includes('XAG') || symbol.includes('SILVER')) pipSize = 0.001;

          // Calculate actual SL/TP prices from pips
          const sl = mission.stop_loss || (entry - (stopPips * pipSize * (mission.direction === 'BUY' ? 1 : -1)));
          const tp = mission.take_profit || (entry + (targetPips * pipSize * (mission.direction === 'BUY' ? 1 : -1)));
          ...
          const alert = {
            ...
            pattern: mission.pattern_type || mission.pattern || 'TACTICAL STRIKE',
            patternType: mission.pattern_type || mission.pattern || 'TACTICAL STRIKE',
            timeframe: mission.timeframe || 'M5',
            session: mission.session || 'LONDON',
            timestamp: typeof mission.created_at === 'string' ? mission.created_at : new Date((mission.created_timestamp || Date.now() / 1000) * 1000).toISOString(),
  ...
  useEffect(() => {
    ...
    async function fetchMissionSession() {
      try {
        ...
        const response = await fetch(`${apiUrl}/api/mission_session/${missionSessionId}`);
        const data = await response.json();
        ...
        if (data.success && data.mission) {
          ...
          console.log('[Mission] Risk calculation:', { lotSize, stopPips, pipValue, riskPerTrade, potentialReward });
          ...
          const alert = {
            ...
            timeframe: mission.timeframe || 'M5',
            session: mission.session || 'LONDON',
            timestamp: new Date(mission.created_at * 1000 || Date.now()).toISOString(),
  ...
  useEffect(() => {
    ...
    const socketUrl = process.env.NEXT_PUBLIC_SOCKET_URL || 'http://134.199.204.67:8888';
    ...
    return (
      <div className="min-h-screen bg-[#0a0e1a] flex items-center justify-center">
        <div className="text-center max-w-md px-4">
          ...
          <h2 className="text-2xl font-bold text-[#f56565] mb-2">Mission Session Expired</h2>
    ...
    return (
      <div className="min-h-screen bg-[#0a0e1a] flex items-center justify-center">
        <div className="text-center max-w-md px-4">
          ...
          <h2 className="text-2xl font-bold text-[#fbbf24] mb-2">Order Already Executed</h2>
          <p className="text-[#cbd5e0] mb-6">
  ...
  if (!tokenValid && errorMessage) {
    return (
      <div className="min-h-screen bg-[#0a0e1a] flex items-center justify-center">
        <div className="text-center max-w-md px-4">
          <div className="text-6xl mb-4">🚫</div>
          <h2 className="text-2xl font-bold text-[#f56565] mb-2">Access Denied</h2>
    ...
    return (
      <div className="min-h-screen bg-[#0a0e1a] flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-[#34d399] border-t-transparent rounded-full animate-spin mx-auto mb-4" aria-label="Loading" />
  ...
  const tradeAlertUserData = {
    balance: 10005.00, // TODO: Get from user profile
    activeTrades: userProfile?.capacity?.used || 2,
    maxTrades: userProfile?.capacity?.total || 6,
    riskPerTrade: userProfile?.riskPerTrade || 100.00,
    potentialReward: (userProfile?.riskPerTrade || 100) * (missionAlert?.riskReward || 1.5)
  };
