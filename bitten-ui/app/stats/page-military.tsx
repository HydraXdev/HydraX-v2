...
export default function StatsPage() {
  ...
  const balance = 10005.00;
  const equity = 10152.40;
  const totalPnL = 147.40;
  ...
  const recentOps = [
    { id: 1, when: '2h ago', title: 'EUR/USD — London Breakout', delta: 97.2, tag: 'WIN' },
    { id: 2, when: '6h ago', title: 'GBP/JPY — Session Continuation', delta: 45.5, tag: 'WIN' },
    { id: 3, when: '1d ago', title: 'Z-Trap Reversal', delta: -36.75, tag: 'LOSS' },
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-green-400 p-2 sm:p-4 font-mono">
      ...
      <div className="border-2 border-green-500 bg-black/50 p-3 mb-3">
        <div className="text-sm font-bold text-green-400 mb-2">TOTAL ACCOUNT GROWTH</div>
        <div className="grid grid-cols-3 gap-3 mb-2">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-green-400">${equity.toFixed(2)}</div>
          </div>
          <div>
      ...
      <div className="border-2 border-yellow-500 bg-black/70 p-3 mb-3">
        <div className="text-sm font-bold text-yellow-400 mb-2">KEY PERFORMANCE</div>
        <div className="grid grid-cols-3 gap-2">
          {kpis.map((k) => (
            <div key={k.label} className="border border-green-700 bg-black/50 p-2">
      ...
      <div className="border-2 border-blue-500 bg-black/70 p-3 mb-3">
        <div className="text-sm font-bold text-blue-400 mb-2">RECENT OPERATIONS</div>
        <div className="space-y-2">
          {recentOps.map(op => (
            <div key={op.id} className="border border-green-700 bg-black/50 p-2 flex justify-between items-center">
              <div>
                <div className="text-sm text-green-400">{op.title}</div>
                <div className="text-xs text-green-600">{op.when} • {op.tag}</div>
              </div>
              <div className={`text-sm font-bold ${op.delta >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="border-2 border-purple-500 bg-black/70 p-3 mb-3">
        <div className="text-sm font-bold text-purple-400 mb-2">DISTRIBUTION BY PAIR</div>
          ...
          {pairDist.map(pair => (
            <div key={pair.name} className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <div className="h-2 bg-green-500" style={{ width: `${pair.value * 10}px` }}></div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button onClick={() => router.push('/war-room')} className="border-2 border-yellow-500 bg-yellow-900/30 p-3 hover:bg-yellow-900/50 transition-colors text-yellow-400 font-bold">
          🏛️ WAR ROOM
        </button>
        <button onClick={() => router.push('/notebook')} className="border-2 border-purple-500 bg-purple-900/30 p-3 hover:bg-purple-900/50 transition-colors text-purple-400 font-bold">
          📓 NOTEBOOK
        </button>
      </div>
    </div>
