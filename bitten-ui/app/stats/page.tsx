      ...
      try {
        ...
        if (statsRes.ok) {
          ...
          const realKpis: KPI[] = [
            {
              label: "WIN RATE",
              value: `${Math.round(statsData.win_rate || 0)}%`,
              sub: "Last 90 days",
            },
            {
              label: "AVG R:R",
              value: (statsData.avg_rr || 0).toFixed(2),
              sub: "Risk/Reward",
            },
            {
              label: "PROFIT FACTOR",
              value: (statsData.profit_factor || 0).toFixed(2),
              sub: "Net profit",
            },
            {
              label: "BEST TRADE",
              value: `$${(statsData.best_trade || 0).toFixed(2)}`,
              sub: statsData.best_trade_symbol || "N/A",
            },
            {
              label: "LONGEST STREAK",
              value: `${statsData.longest_streak || 0} wins`,
              sub: "Best run",
            },
            {
              label: "AVG HOLD",
              value: statsData.avg_hold_time || "0h 0m",
              sub: "Position time",
            },
          ];
          setKpis(realKpis);
          ...
          if (statsData.recent_trades) {
            setRecentOps(
              statsData.recent_trades.map((trade: any, idx: number) => ({
                id: idx + 1,
          ...
          if (statsData.pair_distribution) {
            setPairDist(
              statsData.pair_distribution.map((pair: any) => ({
                name: pair.symbol,
                value: pair.count,
              })),
            );
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-green-400 p-2 sm:p-4 font-mono m-0">
      ...
      <div className="border-2 border-green-500 bg-black/50 p-2 sm:p-3 mb-3 m-0">
        <div className="text-xs sm:text-sm font-bold text-green-400 mb-2 m-0 p-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mb-2 m-0 p-0">
          <div className="m-0 p-0">
            ...
            <div className="text-base sm:text-lg font-bold text-green-400 m-0 p-0">
              ${balance.toFixed(2)}
            </div>
          </div>
          <div className="m-0 p-0">
          ...
          <div className="m-0 p-0">
            ...
            <div
              className={`text-base sm:text-lg font-bold m-0 p-0 ${totalPnL >= 0 ? "text-green-400" : "text-red-400"}`}
            >
              {totalPnL >= 0 ? "+" : ""}
              {growthPct.toFixed(2)}%
            </div>
      ...
      <div className="border-2 border-yellow-500 bg-black/70 p-2 sm:p-3 mb-3 m-0">
        <div className="text-xs sm:text-sm font-bold text-yellow-400 mb-2 m-0 p-0">
          KEY PERFORMANCE
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 m-0 p-0">
          {kpis.map((k) => (
            <div
              key={k.label}
              className="border border-green-700 bg-black/50 p-2 m-0"
            >
              <div className="text-xs text-green-600 m-0 p-0">{k.label}</div>
              <div className="text-lg sm:text-xl font-bold text-green-400 m-0 p-0">
                {k.value}
              </div>
              {k.sub && (
                <div className="text-xs text-gray-500 m-0 p-0">{k.sub}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="border-2 border-blue-500 bg-black/70 p-2 sm:p-3 mb-3 m-0">
        <div className="text-xs sm:text-sm font-bold text-blue-400 mb-2 m-0 p-0">
          RECENT OPERATIONS
        </div>
        <div className="space-y-2 m-0 p-0">
          {recentOps.length === 0 ? (
            <div className="text-center text-gray-500 text-sm m-0 p-2">
            ...
            recentOps.map((op) => (
              <div
                key={op.id}
                className="border border-green-700 bg-black/50 p-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1 sm:gap-0 m-0"
              >
                <div className="m-0 p-0">
                  <div className="text-xs sm:text-sm text-green-400 m-0 p-0">
                    {op.title}
                  </div>
                  <div className="text-xs text-green-600 m-0 p-0">
                    {op.when} • {op.tag}
                  </div>
                </div>
                <div
                  className={`text-sm font-bold m-0 p-0 ${op.delta >= 0 ? "text-green-400" : "text-red-400"}`}
                >
      ...
      <div className="border-2 border-purple-500 bg-black/70 p-2 sm:p-3 mb-3 m-0">
        <div className="text-xs sm:text-sm font-bold text-purple-400 mb-2 m-0 p-0">
        ...
        <div className="space-y-1 m-0 p-0">
          {pairDist.length === 0 ? (
            ...
            pairDist.map((pair) => (
              <div
                key={pair.name}
                className="flex justify-between items-center m-0 p-0"
              >
                <div className="text-xs sm:text-sm text-green-400 m-0 p-0">
                  {pair.name}
                </div>
                <div className="flex items-center gap-2 m-0 p-0">
                  <div
                    className="h-2 bg-green-500 m-0 p-0"
                    style={{ width: `${Math.min(pair.value * 10, 100)}px` }}
                  ></div>
                  <div className="text-xs sm:text-sm text-green-400 m-0 p-0">
                    {pair.value}
                  </div>
