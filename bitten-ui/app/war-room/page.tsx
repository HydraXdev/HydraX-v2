...
export default function WarRoomPage() {
  ...
  const user = {
    ...
    streakCurrent: 3,
    streakBest: 8,
    referralEarnings: 284.50,
    referralCount: 5
  }

  // Mock kill cards (recent trade history)
  const killCards = [
    { id: '1', symbol: 'EURUSD', outcome: 'WIN', pips: 23.5, amount: 235, pattern: 'VCB_BREAKOUT', timestamp: Date.now() - 3600000 },
    { id: '2', symbol: 'GBPJPY', outcome: 'WIN', pips: 41.2, amount: 412, pattern: 'LIQUIDITY_SWEEP', timestamp: Date.now() - 7200000 },
    { id: '3', symbol: 'USDJPY', outcome: 'LOSS', pips: -18.0, amount: -180, pattern: 'ORDER_BLOCK', timestamp: Date.now() - 10800000 },
    { id: '4', symbol: 'XAUUSD', outcome: 'WIN', pips: 15.3, amount: 153, pattern: 'SWEEP_RETURN', timestamp: Date.now() - 14400000 }
  ]

  // Mock badges
  const badges = [
    { id: '1', name: 'Streak Master', description: '5+ wins in a row', icon: Flame, earned: true },
    { id: '2', name: 'Pattern Expert', description: 'Master all patterns', icon: Target, earned: true },
    { id: '3', name: 'Risk Manager', description: 'Never exceed 6% daily risk', icon: Shield, earned: false },
    { id: '4', name: 'Elite Trader', description: 'Reach COMMANDER rank', icon: Crown, earned: true }
  ]
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-tactical text-mint neon-glow">War Room</h1>
                <p className="text-sm text-secondary font-code">Tactical HQ - {user.rank} Level {user.level}</p>
              </div>
            </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'warroom' ? (
            ...
            >
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <div className="text-2xl font-tactical text-mint neon-glow">{user.rank}</div>
                      <div className="text-lg font-tactical text-cyan">Level {user.level}</div>
                    </div>
                  </div>

                  <div className="text-right">
                    <div className="text-sm text-secondary mb-1">XP Progress</div>
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-code text-mint">{user.totalWins}</div>
                    <div className="text-xs text-secondary">WINS</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-code text-danger">{user.totalLosses}</div>
                    <div className="text-xs text-secondary">LOSSES</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-code text-warning">{user.streakCurrent}</div>
                    <div className="text-xs text-secondary">CURRENT STREAK</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-code text-cyan">{user.streakBest}</div>
              ...
              >
                <h3 className="text-lg font-tactical text-mint mb-4">Command Control</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-xs text-secondary">
                      {autoFireEnabled ? 'Enabled - High confidence signals fire automatically' : 'Disabled - Manual execution required'}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-tactical text-warning">Risk Level</span>
                      <span className="text-lg font-code text-mint">{riskAdjuster}%</span>
                    </div>
                    <input
                      type="range"
                      min="1"
                      ...
                      step="0.5"
                      value={riskAdjuster}
                      onChange={(e) => setRiskAdjuster(parseFloat(e.target.value))}
                      className="w-full h-2 bg-secondary rounded-lg appearance-none cursor-pointer"
                    />
                    <div className="text-xs text-secondary">
                      Risk per trade: ${(balance * (riskAdjuster / 100)).toFixed(0)}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-tactical text-cyan">Trades Available</span>
                      <span className="text-lg font-code text-mint">{3 - liveMissions.length}/3</span>
                    </div>
                    <div className="w-full h-2 bg-secondary rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-mint to-cyan transition-all duration-300"
                        style={{ width: `${((3 - liveMissions.length) / 3) * 100}%` }}
                      />
                    </div>
                    <div className="text-xs text-secondary">
                  ...
                  {killCards.map((card, index) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="font-tactical text-lg">{card.symbol}</div>
                      ...
                      <div className="mb-2">
                        ...
                        <div className="text-sm text-secondary">
                          {card.outcome === 'WIN' ? '+' : ''}{card.pips} pips
                        </div>
                      </div>
                      <div className="text-xs text-secondary">
                    ...
                    return (
                      ...
                      >
                        <div className={`w-12 h-12 mx-auto mb-2 rounded-lg flex items-center justify-center ${
                          badge.earned ? 'bg-mint/20 border border-mint/30' : 'bg-secondary/50'
                        }`}>
                          <IconComponent className={`w-6 h-6 ${badge.earned ? 'text-mint' : 'text-secondary'}`} />
                        </div>
                        <div className={`font-tactical text-sm ${badge.earned ? 'text-mint' : 'text-secondary'}`}>
                          {badge.name}
                        </div>
                        <div className="text-xs text-muted mt-1">
                          {badge.description}
            ...
            >
              ...
              <motion.div
                ...
                className="hud-panel-accent p-6"
              >
                <h3 className="text-lg font-tactical text-mint mb-4">Account Status</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div className="text-center">
                    <div className="text-2xl font-code text-mint">${balance.toLocaleString()}</div>
                    <div className="text-sm text-secondary">Balance</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-code text-cyan">{winRate.toFixed(1)}%</div>
                    <div className="text-sm text-secondary">Win Rate</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-code text-warning">{liveMissions.length}</div>
                    <div className="text-sm text-secondary">Active Positions</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-code text-danger">{activeRisk.toFixed(1)}%</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <motion.div
                  ...
                  className="hud-panel p-6"
                >
                  <h3 className="text-lg font-tactical text-cyan mb-4">Subscription</h3>
                ...
                <motion.div
                  ...
                  className="hud-panel p-6"
                >
                  <h3 className="text-lg font-tactical text-warning mb-4">Referral Program</h3>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-mint font-code">${user.referralEarnings}</span>
                    </div>
                    <div className="flex items-center justify-between">
