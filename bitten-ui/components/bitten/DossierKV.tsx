...
export function DossierKV({
  ...
  return (
    <div className={`flex items-baseline justify-between py-2 border-b border-[#2d3748]/50 ${className}`}>
      <dt className="text-sm text-[#4a5568]">{label}</dt>
      <dd className={`text-sm font-mono tabular-nums ${colorClasses[valueColor]}`}>
        {value}
      </dd>
    </div>
  );
    ...
    <dl className={`space-y-0 ${className}`}>
      {title && (
        <div className="mb-2 pb-2 border-b border-[#34d399]/30">
          <h3 className="text-xs font-tactical text-[#34d399] uppercase tracking-wider">{title}</h3>
        </div>
      )}
      {children}
    </dl>
