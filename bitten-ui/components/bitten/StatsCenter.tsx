...
export function StatsCenter({
  ...
  const equityPath = useMemo(() => {
    ...
    const points = equitySeries.map((point, i) => {
      ...
      const y =
        innerHeight -
        ((point.equity - yExtent[0]) / (yExtent[1] - yExtent[0])) * innerHeight;
      return `${x},${y}`;
    });
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-lg font-mono ${growth >= 0 ? "text-[#34d399]" : "text-[#ef4444]"}`}
              >
                {growth >= 0 ? "+" : ""}
                {fmtPercent(growth / 100)}
              </span>
            </div>
          </div>
          ...
          >
            ...
            {equitySeries.length > 0
              ? fmtUSD(equitySeries[equitySeries.length - 1].equity)
              : "Loading"}
        ...
        >
          {kpis.map((kpi, idx) => (
            ...
            >
              ...
              <div className="text-2xl font-mono font-bold text-[#cbd5e0] mb-1">
                {kpi.value}
              </div>
              {kpi.sub && (
                <div className="text-xs text-[#4a5568]">{kpi.sub}</div>
              )}
            </div>
          ))}
        </motion.section>
              ...
              .map((event, idx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-2 h-2 rounded-full ${event.tag === "WIN" ? "bg-[#34d399]" : event.tag === "LOSS" ? "bg-[#ef4444]" : "bg-[#718096]"}`}
                    />
                    <div>
                      <div className="text-sm font-medium text-[#cbd5e0]">
                        {event.title}
                      </div>
                      <div className="text-xs text-[#4a5568]">
                        {fmtRelative(event.when)}
                      </div>
                    </div>
                  </div>
                  <div
                    className={`text-sm font-mono font-bold ${event.delta >= 0 ? "text-[#34d399]" : "text-[#ef4444]"}`}
                  >
                    {event.delta >= 0 ? "+" : ""}
                    {fmtUSD(event.delta)}
                  </div>
