        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="w-full sm:w-40">
            <div className="flex items-center justify-between">
              ...
              <span className="border border-zinc-700 text-zinc-300 bg-zinc-900/40 px-2 py-0.5 text-xs rounded">
                {trade.lots} LOTS
              </span>
            </div>
          ...
          <div className="relative flex-1">
            <div
              className={`relative h-12 bg-zinc-900 border border-emerald-700/50 ring-1 ${ageTint} rounded`}
            >
              {/* SL gate */}
              <div className="absolute left-0 top-0 bottom-0 w-1 bg-red-600 rounded-l" />
              <div className="absolute left-2 top-1/2 -translate-y-1/2 text-[11px] text-red-400">
                SL {trade.stopLoss}
              </div>

              {/* Fill */}
              <div
                className={`absolute top-0 bottom-0 transition-all duration-300 rounded-l ${isPos ? "bg-emerald-600/60" : "bg-red-600/60"}`}
              ...
              />
              <div
                className="absolute -translate-y-1/2 top-1/2 text-[11px] text-yellow-300 font-bold whitespace-nowrap"
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 text-[11px] text-emerald-400">
                TP {trade.takeProfit}
              </div>
            </div>
          ...
          <div className="w-full sm:w-44 text-right border-2 border-emerald-700/50 bg-black/60 p-3 rounded">
            ...
            <div
              className={`text-2xl font-bold tabular-nums ${isPos ? "text-emerald-300" : "text-red-400"}`}
            >
    ...
    <div className="min-h-screen bg-zinc-950 text-emerald-300 font-mono">
      ...
      <div className="border-b border-emerald-800/50 bg-gradient-to-b from-zinc-900 to-black">
        <div className="mx-auto max-w-7xl p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          <div>
            <div
              className="text-2xl sm:text-3xl font-extrabold tracking-[0.15em]"
              style={{ color: "#8b7355" }}
            >
              ⬢ B.I.T.T.E.N
            </div>
          ...
          <div className="flex items-center gap-4 text-zinc-400 flex-wrap">
            ...
            <time className="tabular-nums text-zinc-200">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-2xl font-bold tabular-nums ${totalPL >= 0 ? "text-emerald-300" : "text-red-400"}`}
                  >
                    {fmtUSD(equity)}
                  </span>
                </div>
                <div className="mt-2">
                  ...
                  <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                    <div
                      className={`${dd >= 0 ? "bg-emerald-500" : "bg-red-500"} h-2 transition-all`}
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold tabular-nums">
                    {openPositions}/{maxSlots}
                  </span>
                </div>
                <div className="mt-2 text-[11px] text-zinc-400">
                  Capacity remaining: {Math.max(0, maxSlots - openPositions)}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl p-3 sm:p-4 space-y-3">
        ...
        {/* Empty slots */}
        {Array.from({ length: Math.max(0, maxSlots - openPositions) }).map(
          (_, i) => (
            <div
              key={`empty-${i}`}
            ...
            >
              <div className="p-3 sm:p-4">
                <div className="flex items-center gap-3 sm:gap-4 h-10">
                  <div className="w-20 sm:w-32 text-zinc-600 text-xs sm:text-sm">
                    SLOT {openPositions + i + 1}
                  </div>
                  <div className="flex-1 bg-zinc-900/60 h-full grid place-items-center text-zinc-600 text-xs sm:text-sm rounded">
                    [ AVAILABLE ]
                  </div>
                  <div className="w-16 sm:w-40 text-right text-zinc-600 text-xs sm:text-sm">
                    —
                  </div>
                </div>
              </div>
