...
import type { BusTopics, TopicName, UserProfile, AlertData, LiveTrade, SystemStatus } from './contracts';
...
export function startMockEvents(emit: EmitFn) {
  if (process.env.NEXT_PUBLIC_USE_MOCKS !== '1') {
    console.warn('[MockSource] Not starting - NEXT_PUBLIC_USE_MOCKS not set to 1');
    return () => {};
  }
  ...
  intervals.push(setInterval(() => {
    if (mockTrades.length > 0) {
      ...
      trade.equity = ((trade.current - trade.entry) / trade.entry) * trade.lots * 100000;
  ...
  intervals.push(setInterval(() => {
    ...
    const newPoint = {
      ...
      equity: newEquity,
      dd: newEquity < lastPoint.balance ? ((lastPoint.balance - newEquity) / lastPoint.balance) * 100 : 0,
    };
