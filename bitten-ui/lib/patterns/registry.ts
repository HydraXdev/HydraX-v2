...
export const PATTERN_REGISTRY = {
  ...
  // Inactive patterns
  BLIND_SPOT: { name: 'BLIND SPOT', signals: 0, avgConfidence: 0, color: '#6b7280', description: 'No recent signals' },
  TRAPDOOR_SSR: { name: 'TRAPDOOR SSR', signals: 0, avgConfidence: 0, color: '#6b7280', description: 'No recent signals' },
  PRESSURE_VALVE: { name: 'PRESSURE VALVE', signals: 0, avgConfidence: 0, color: '#6b7280', description: 'No recent signals' },
  EMA_RSI_BB_VWAP: { name: 'EMA RSI BB VWAP', signals: 0, avgConfidence: 0, color: '#6b7280', description: 'No recent signals' },
  EMA_RSI_SCALP: { name: 'EMA RSI SCALP', signals: 0, avgConfidence: 0, color: '#6b7280', description: 'No recent signals' },
} as const;
...
export interface PatternMarker {
  t: number; // epoch ms
  price: number;
  kind: 'sweep' | 'reclaim' | 'break' | 'wickReject' | 'kalmanSignal' | 'squeeze' | 'vcb' | 'touch' | 'midpoint' | 'momentum';
  label?: string;
}
...
export function domainToPixels(
  ...
  const x = ((t - bounds.xStartTs) / (bounds.xEndTs - bounds.xStartTs)) * dimensions.width;
  const y = dimensions.height - ((price - bounds.yMin) / (bounds.yMax - bounds.yMin)) * dimensions.height;
  return { x, y };
}
