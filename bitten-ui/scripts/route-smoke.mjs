...
async function testRoute(route) {
  ...
  try {
    const { stdout } = await execAsync(`curl -s -I --max-time 10 "${url}"`, {
      timeout: TIMEOUT,
    });
    ...
    if (status === "200") {
      console.log(
        `${colors.green}✓${colors.reset} ${route.name.padEnd(25)} ${colors.green}200 OK${colors.reset} (${elapsed}ms)`,
      );
      return { success: true, status, elapsed, url };
    } else {
      console.log(
        `${colors.red}✗${colors.reset} ${route.name.padEnd(25)} ${colors.red}${status || "UNKNOWN"}${colors.reset} (${elapsed}ms)`,
      );
    ...
    const elapsed = Date.now() - startTime;
    console.log(
      `${colors.red}✗${colors.reset} ${route.name.padEnd(25)} ${colors.red}ERROR${colors.reset} (${elapsed}ms) - ${error.message}`,
    );
...
async function runSmokeTest() {
  console.log(
    `${colors.bold}${colors.blue}🧪 BITTEN UI Route Smoke Test${colors.reset}`,
  );
...
async function main() {
  ...
  if (!serverUp) {
    console.log(
      `${colors.red}💥 Server not responding at ${baseUrl}${colors.reset}`,
    );
    console.log(
      `${colors.yellow}💡 Make sure the dev server is running: npm run dev${colors.reset}`,
    );
    process.exit(1);
  }
