...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(log_config["file_path"]), logging.StreamHandler()],
)
...
try:
    ...
    def bit_enhance(msg, situation=None):
        return msg

    def bit_trade_reaction(success, profit=0, symbol=""):
        return ""

    def bit_welcome(tier="NIBBLER"):
        return "Welcome to BITTEN."

    def bit_daily_wisdom():
...
def safe_edit_message_text(bot, chat_id, message_id, text, **kwargs):
    return send_with_retry(
        lambda **kw: bot.edit_message_text(**kw), chat_id=chat_id, message_id=message_id, text=text, **kwargs
    )
...
try:
    from src.bitten_core.personalized_mission_brain import create_personalized_missions_for_signal, get_mission_brain
...
try:
    ...
    class FireLoopVerifier:
        def run_single_check(self):
            return {"overall_status": "❌ SYSTEM OFFLINE"}
    ...
    class CloneLogWatcher:
        ...
        def display_comprehensive_status(self):
            print("❌ Clone watcher unavailable")
    ...
    try:
        ...
        try:
            ...
            if signals:
                ...
                logger.info(
                    f"🐍 VENOM signal: {best_signal['pair']} {best_signal['signal_type']} @ {best_signal['confidence']}%"
                )
                return best_signal
            else:
    ...
    try:
        ...
        try:
            result = requests.post(
                "http://localhost:8888/api/fire", json={"mission_id": mission["mission_id"]}, timeout=10
            )
        ...
        try:
            ...
            from fire_router import TradeDirection, TradeRequest
...
class BittenProductionBot:
    ...
    def send_adaptive_response(self, chat_id, message_text, user_tier="NIBBLER", user_action=None):
        ...
        if self.unified_bot:
            try:
                ...
                loop.run_until_complete(
                    self.unified_bot.send_unified_message(chat_id, message_text, user_tier, user_action)
                )
                loop.close()
                return True
            except Exception as e:
        ...
        if self.adaptive_bot:
            try:
                ...
                asyncio.set_event_loop(loop)
                loop.run_until_complete(
                    self.adaptive_bot.send_adaptive_message(chat_id, message_text, user_tier, user_action)
                )
                loop.close()
                return True
            except Exception as e:
    ...
    def create_quick_keyboard(self):
        ...
        keyboard = ReplyKeyboardMarkup(
            keyboard=[[KeyboardButton("🔫 FIRE"), KeyboardButton("💰 CREDITS"), KeyboardButton("📱 MENU")]],
    ...
    def create_full_keyboard(self):
        ...
        keyboard = ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton("🔫 FIRE"), KeyboardButton("📊 STATUS"), KeyboardButton("💰 CREDITS")],
                [KeyboardButton("🎯 TACTICAL"), KeyboardButton("📚 HELP"), KeyboardButton("🏆 RECRUIT")],
                [KeyboardButton("📱 MENU"), KeyboardButton("❌ HIDE")],
            ],
            resize_keyboard=True,
            one_time_keyboard=False,
            is_persistent=True,
...
*"Knowledge is power in the markets." - BITTEN Protocol*"""
                        ...
                        self.bot.send_message(
                            chat_id=message.chat.id, text=status_msg, parse_mode="Markdown", reply_markup=keyboard
                        )
...
*"The numbers never lie." - BITTEN Analytics*"""
                        ...
                        self.bot.send_message(
                            chat_id=message.chat.id, text=stats_msg, parse_mode="Markdown", reply_markup=keyboard
                        )
                    ...
                    try:
                        ...
                        if is_new_user and len(parts) == 1:
                            ...
                            if keyboard_data:
                                ...
                                self.send_adaptive_response(
                                    message.chat.id, welcome_msg, user_tier, "cinematic_welcome"
                                )
...
Ready to dominate the markets? 🚀"""
                                ...
                                self.send_adaptive_response(
                                    message.chat.id, normal_welcome, user_tier, "normal_welcome"
                                )
...
Ready to dominate the markets? 🚀"""
                            ...
                            self.send_adaptive_response(
                                message.chat.id, fallback_welcome, user_tier, "fallback_welcome"
                            )
                    else:
...
✨ Earn +8 XP for trade reflections: /notebook"""
                                            ...
                                            self.send_adaptive_response(
                                                message.chat.id, reminder_msg, user_tier, "journal_reminder"
                                            )
                                        except Exception as e:
                    ...
                    if mission:
                        ...
                        if mode_info["current_mode"] == "SELECT" and not mission.get("expired", False):
                            ...
                            self.bot.send_message(
                                message.chat.id, semi_msg, reply_markup=keyboard, parse_mode="Markdown"
                            )
                        else:
                    ...
                    if BIT_AVAILABLE:
                        ...
                        self.send_adaptive_response(
                            message.chat.id, "BIT is currently offline. Check back later.", user_tier, "error"
                        )
                    ...
                    if int(uid) in COMMANDER_IDS:
                        ...
                        self.send_adaptive_response(
                            message.chat.id, "🐍 Generating live VENOM v7.0 signal...", user_tier, "signal_generation"
                        )
                        ...
                        if venom_signal:
                            ...
                            try:
                                ...
                                logger.info(
                                    f"🐍 Live VENOM signal generated: {venom_signal['pair']} @ {venom_signal['confidence']:.1f}%"
                                )
                            except Exception as e:
                        ...
                        self.send_adaptive_response(
                            message.chat.id,
                            "❌ Live signal generation restricted to commanders.",
                            user_tier,
                            "unauthorized_access",
                        )
                    ...
                    if int(uid) in COMMANDER_IDS:
                        ...
                        self.send_adaptive_response(
                            message.chat.id, "🐍 VENOM v7.0 Market Scan initiated...", user_tier, "scan_start"
                        )
                        ...
                        self.send_adaptive_response(
                            message.chat.id,
                            "❌ VENOM scanning restricted to commanders.",
                            user_tier,
                            "unauthorized_access",
                        )
                    ...
                    if int(uid) in COMMANDER_IDS:
                        ...
                        self.send_adaptive_response(
                            message.chat.id,
                            "❌ GHOSTED report restricted to commanders.",
                            user_tier,
                            "unauthorized_access",
                        )
                    ...
                    if int(uid) in COMMANDER_IDS:
                        try:
                            ...
                            if len(parts) < 2:
                                self.send_adaptive_response(
                                    message.chat.id, "❌ Usage: /givegold @username", user_tier, "usage_error"
                                )
                                return
                            ...
                            if not target_telegram_id:
                                self.send_adaptive_response(
                                    message.chat.id,
                                    f"❌ User @{username} not found in system.",
                                    user_tier,
                                    "user_not_found",
                                )
                                return
                            ...
                            if not user_info:
                                self.send_adaptive_response(
                                    message.chat.id,
                                    f"❌ User @{username} not registered.",
                                    user_tier,
                                    "user_not_registered",
                                )
                                return
                            ...
                            if user_info.get("offshore_opt_in", False):
                                self.send_adaptive_response(
                                    message.chat.id,
                                    f"ℹ️ Gold access already active for @{username}.",
                                    user_tier,
                                    "already_active",
                                )
                                return
                            ...
                            if success:
                                ...
                                try:
                                    self.bot.send_message(
                                        int(target_telegram_id), gold_welcome, parse_mode="MarkdownV2"
                                    )
                                ...
                                self.send_adaptive_response(
                                    message.chat.id,
                                    f"✅ Gold access granted to @{username}.",
                                    user_tier,
                                    "gold_access_granted",
                                )
                            else:
                                self.send_adaptive_response(
                                    message.chat.id, "❌ Failed to update user profile.", user_tier, "update_failed"
                                )

                        except Exception as e:
                            logger.error(f"Error in /givegold command: {e}")
                            self.send_adaptive_response(
                                message.chat.id, f"❌ Error: {str(e)}", user_tier, "command_error"
                            )
                    else:
                        self.send_adaptive_response(
                            message.chat.id,
                    ...
                    if int(uid) in COMMANDER_IDS:
                        try:
                            ...
                            if len(parts) < 2:
                                self.send_adaptive_response(
                                    message.chat.id, "❌ Usage: /givecrypto @username", user_tier, "usage_error"
                                )
                                return
                            ...
                            if user_region == "US":
                                self.send_adaptive_response(
                                    message.chat.id,
                                    "❌ Cannot grant C.O.R.E. access to US-based accounts for compliance reasons.",
                                    user_tier,
                                    "regulatory_restriction",
                                )
                                return
                            ...
                            if user_info.get("crypto_opt_in", False):
                                self.send_adaptive_response(
                                    message.chat.id,
                                    f"ℹ️ C.O.R.E. access already active for @{username}.",
                                    user_tier,
                                    "already_active",
                                )
                                return
                            ...
                            if success:
                                ...
                                try:
                                    self.bot.send_message(
                                        int(target_telegram_id), crypto_welcome, parse_mode="MarkdownV2"
                                    )
                                    logger.info(
                                        f"C.O.R.E. access granted to {target_telegram_id} (@{username}) by {uid}"
                                    )
                                except Exception as e:
                                ...
                                self.send_adaptive_response(
                                    message.chat.id,
                                    f"✅ C.O.R.E. access granted to @{username}.",
                                    user_tier,
                                    "crypto_access_granted",
                                )
                            else:
                                self.send_adaptive_response(
                                    message.chat.id, "❌ Failed to update user profile.", user_tier, "update_failed"
                    ...
                    try:
                        ...
                        if user_tier not in ["FANG", "COMMANDER"]:
                            ...
                            self.send_adaptive_response(
                                message.chat.id, upgrade_msg, user_tier, "bitmode_upgrade_required"
                            )
                            return
                        ...
                        if len(parts) == 1:
                            ...
                            if command in ["ON", "ENABLE", "TRUE", "1"]:
                                ...
                                if success:
                                    ...
                                    self.send_adaptive_response(
                                        message.chat.id, success_msg, user_tier, "bitmode_enabled"
                                    )
                                else:
                                ...
                                if success:
                                    ...
                                    self.send_adaptive_response(
                                        message.chat.id, disable_msg, user_tier, "bitmode_disabled"
                                    )
                                else:
                    ...
                    try:
                        ...
                        if status_result["success"] and status_result.get("has_press_pass"):
                            ...
                            if assignment_result["success"]:
                                ...
                                self.send_adaptive_response(
                                    message.chat.id, welcome_msg, "PRESS_PASS", "presspass_activation"
                                )
                                ...
                                if assignment_result.get("error") == "vault_full":
                                    error_msg += (
                                        f"All demo accounts are currently assigned. Please try again in a few hours."
                                    )
                                else:
                            ...
                            [
                                [
                                    InlineKeyboardButton(
                                        "📱 QUICK MENU (Persistent)", callback_data="menu_quick_persistent"
                                    ),
                                    InlineKeyboardButton(
                                        "🎯 FULL MENU (Persistent)", callback_data="menu_full_persistent"
                                    ),
                                ],
                                [
                                    InlineKeyboardButton(
                                        "⚡ ADVANCED MENU (Inline)", callback_data="menu_advanced_inline"
                                    ),
...
Select your preferred menu style:"""
                        ...
                        self.bot.send_message(
                            message.chat.id, menu_text, reply_markup=menu_selection, parse_mode="Markdown"
                        )
...
"""
                    ...
                    self.bot.send_message(
                        message.chat.id, war_room_msg, reply_markup=keyboard, parse_mode=None  # No Markdown parsing
                    )
                    ...
                    if self.drill_system:
                        ...
                        self.send_adaptive_response(
                            message.chat.id, "❌ Drill report system not available.", user_tier, "system_unavailable"
                        )
                    ...
                    if self.drill_system:
                        ...
                        self.send_adaptive_response(
                            message.chat.id, "❌ Weekly summary system not available.", user_tier, "system_unavailable"
                        )
                    ...
                    if DRILL_SYSTEM_AVAILABLE:
                        ...
                        self.send_adaptive_response(
                            message.chat.id,
                            "❌ Tactical strategy system not available.",
                            user_tier,
                            "system_unavailable",
                        )
...
Copy and share your link to start earning!"""
                        ...
                        self.send_adaptive_response(
                            message.chat.id, "❌ Recruitment system not available.", user_tier, "system_unavailable"
                        )
...
Use /recruit to get your referral link and start earning!"""
                        ...
                        self.send_adaptive_response(
                            message.chat.id, "❌ Credits system not available.", user_tier, "system_unavailable"
                        )
...
```"""
                    ...
                    message.text.startswith("/notebook")
                    or message.text.startswith("/journal")
                    or message.text.startswith("/notes")
                ):
...
"""
                        ...
                        milestone_text = ""
                        if next_milestone:
                            progress_bar = "█" * int(next_milestone["percentage"] / 10) + "░" * (
                                10 - int(next_milestone["percentage"] / 10)
                            )
                            milestone_text = f"""
...
*"Every trade tells a story. Every story builds a trader." - Norman's Legacy*"""
                        ...
                        self.bot.send_message(
                            chat_id=message.chat.id, text=notebook_msg, parse_mode="Markdown", reply_markup=keyboard
                        )
                    ...
                    try:
                        ...
                        from src.bitten_core.url_signing import (
                            get_latest_mission_for_user,
                            url_signer,
                        ...
                        if len(parts) > 1:
                            ...
                            try:
                                ...
                                if not valid_access:
                                    ...
                                    self.send_adaptive_response(
                                        message.chat.id, error_msg, user_tier, "mission_access_denied"
                                    )
                                    return
                            except Exception as e:
...
*"Intelligence is the difference between victory and defeat." - Norman's Wisdom*"""
                        ...
                        self.bot.send_message(
                            chat_id=message.chat.id, text=brief_msg, parse_mode="Markdown", reply_markup=keyboard
                        )
...
*"The war room is where legends are born." - Commander's Creed*"""
                        ...
                        self.bot.send_message(
                            chat_id=message.chat.id, text=war_msg, parse_mode="Markdown", reply_markup=keyboard
                        )
                        ...
                        if len(parts) > 1:
                            ...
                            try:
                                ...
                                if not valid_access:
                                    ...
                                    self.send_adaptive_response(
                                        message.chat.id, error_msg, user_tier, "ticket_access_denied"
                                    )
                                    return
                            except Exception as e:
...
*"Real-time data. Real-time decisions. Real profits." - BITTEN Protocol*"""
                        ...
                        self.bot.send_message(
                            chat_id=message.chat.id, text=live_msg, parse_mode="Markdown", reply_markup=keyboard
                        )
                    ...
                    try:
                        ...
                        from src.bitten_core.url_signing import format_help_message, url_signer
...
Choose your tactical interface:"""
                        ...
                        self.bot.send_message(
                            chat_id=message.chat.id, text=hud_msg, parse_mode="Markdown", reply_markup=keyboard
                        )
...
• Credits automatically apply to your next bill"""
                            ...
                            self.send_adaptive_response(
                                message.chat.id, "❌ Credits temporarily unavailable", user_tier, "credits_error"
                            )
                    else:
                        self.send_adaptive_response(
                            message.chat.id, "❌ Credits system not available.", user_tier, "system_unavailable"
                    ...
                    if DRILL_SYSTEM_AVAILABLE:
                        try:
                            ...
                            self.send_adaptive_response(
                                message.chat.id,
                                "❌ Tactical system temporarily unavailable",
                                user_tier,
                                "tactical_error",
                            )
                    else:
...
Share your link to earn $10 per successful referral!"""
                            ...
                            self.send_adaptive_response(
                                message.chat.id, "❌ Recruitment temporarily unavailable", user_tier, "recruit_error"
                            )
                    else:
                        self.send_adaptive_response(
                            message.chat.id, "❌ Recruitment system not available.", user_tier, "system_unavailable"
...
Select an option to continue..."""
                        ...
                        self.bot.send_message(message.chat.id, menu_text, reply_markup=keyboard, parse_mode="Markdown")

                    except Exception as e:
                        logger.error(f"Advanced menu error: {e}")
                        self.send_adaptive_response(
                            message.chat.id, "❌ Advanced menu temporarily unavailable", user_tier, "menu_error"
                        )
...
Use `/help` for the complete command list."""
                ...
                self.send_adaptive_response(
                    message.chat.id, "❌ Command processing error. Please try again.", user_tier, "error"
                )

        @self.bot.callback_query_handler(
            func=lambda call: call.data.startswith("mode_")
            or call.data.startswith("slots_")
            or call.data.startswith("semi_fire_")
            or call.data.startswith("menu_")
            or call.data.startswith("combat_")
            or call.data.startswith("field_")
            or call.data.startswith("tier_")
            or call.data.startswith("xp_")
            or call.data.startswith("help_")
            or call.data.startswith("tool_")
            or call.data.startswith("bot_")
            or call.data.startswith("notebook_")
            or call.data.startswith("onboard_")
        )
        def handle_all_callbacks(call):
            ...
            try:
                ...
                # Handle menu system callbacks
                    ...
                    "menu_quick_persistent",
                    "menu_full_persistent",
                    "menu_advanced_inline",
                    "menu_hide_all",
                ]:
        ...
        def block_unauthorized_messages(message):
            ...
            if message.text and message.text.startswith("/"):
                ...
                if command not in self.ALLOWED_COMMANDS:
                    ...
                    self.bot.send_message(
                        message.chat.id, "🔒 Command not recognized. Use `/help` for available commands."
                    )
                    return
            ...
            if message.text and not message.text.startswith("/"):
                ...
                if UNIFIED_PERSONALITY_AVAILABLE and self.unified_bot:
                    try:
                        ...
                        # Regular interaction
                        self.send_adaptive_response(
                            message.chat.id, message.text or "Hello!", user_tier, "general_message"
                        )
...
This menu will stay visible for quick access! 🚀"""
                ...
                self.bot.send_message(call.message.chat.id, "Quick menu activated below! ⬇️", reply_markup=keyboard)
...
Use ❌ HIDE to remove the keyboard when needed."""
                ...
                # Send a new message with the persistent keyboard
                self.bot.send_message(call.message.chat.id, "Full menu activated below! ⬇️", reply_markup=keyboard)
...
*Remember: Every master trader keeps a journal!*"""
                ...
                keyboard = InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "📓 Open Notebook", url=f"https://joinbitten.com/notebook/{call.from_user.id}"
                            )
                        ],
                        [InlineKeyboardButton("❌ Close", callback_data="menu_close")],
                    ]
                ...
                try:
                    ...
                    self.bot.send_message(
                        call.message.chat.id, help_response, reply_markup=keyboard, parse_mode="Markdown"
                    )
                ...
                try:
                    ...
                    if dashboard["recent_entries"]:
                        ...
                        for entry in dashboard["recent_entries"][-3:]:
                            entry_date = (
                                entry.get("created_at", "Unknown")[:10] if entry.get("created_at") else "Unknown"
                            )
                            xp_earned = entry.get("xp_earned", 0)
                            recent_text += (
                                f"• {entry.get('title', 'Untitled')[:30]}... (+{xp_earned} XP) - {entry_date}\n"
                            )
...
Use /help for command list or visit the webapp for detailed information."""
            ...
            if "field_manual" in callback_data:
                ...
                nav_keyboard.row(InlineKeyboardButton("❌ Close", callback_data="menu_close"))
            ...
            try:
                ...
                # If editing fails, send new message
                self.bot.send_message(call.message.chat.id, response, parse_mode="Markdown", reply_markup=nav_keyboard)
...
"""
            ...
            return {"login": login_id, "balance": 0.00, "currency": "USD", "leverage": 500, "broker": "Unknown"}
        except Exception as e:
    ...
    def _register_account_with_core(self, uid: str, account_info: dict) -> bool:
        ...
        try:
            ...
            try:
                response = requests.post("http://localhost:8888/api/register_account", json=core_payload, timeout=5)
...
📲 You'll receive a confirmation when it's online and ready."""
    ...
    def _send_connect_usage_with_keyboard(self, chat_id: str, user_tier: str) -> None:
        ...
        try:
            ...
            keyboard = InlineKeyboardMarkup()
            webapp_button = InlineKeyboardButton(text="🌐 Use WebApp", url="https://joinbitten.com/connect")
        ...
        try:
            ...
            for user_profile in eligible_users:
                try:
                    ...
                    if chat_id:
                        self.bot.send_message(
                            chat_id=chat_id, text=message_text, parse_mode="MarkdownV2", reply_markup=buttons
                        )
    ...
    def _format_core_signal_message(self, signal: dict) -> str:
        ...
        try:
            ...
            message = f"🔥 *C\\.O\\.R\\.E\\. SIGNAL: {symbol_escaped}*\n" f"🕒 {pattern_escaped} \\– Score: {score}/100"
    ...
    def _create_mission_brief_button(self, uuid: str) -> "InlineKeyboardMarkup":
        ...
        try:
            ...
            brief_button = InlineKeyboardButton(
                text="📄 Mission Brief", url=f"https://core-missions.bitten.trading/brief/{uuid}"
            )
    ...
    def _start_heartbeat(self):
        ...
        def write_heartbeat():
            while getattr(self, "running", True):
                try:
                    heartbeat = {"ts": int(time.time()), "ok": True, "uptime": int(time.time() - self.start_time)}
