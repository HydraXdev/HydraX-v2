...
try:
    ...
    from src.bitten_core.intel_bot_personalities import BotPersonality, IntelBotPersonalities
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/root/HydraX-v2/bitten_voice_personality_bot.log"), logging.StreamHandler()],
)
...
class BittenVoicePersonalityBot:
    ...
    def setup_handlers(self):
        ...
        def handle_nexus(message):
            ...
            if PERSONALITY_SYSTEMS_AVAILABLE and hasattr(self, "nexus"):
                response = self.nexus.get_celebration_response(
                    "execution_success", {"symbol": "EURUSD", "profit_pips": 25}
                )
            else:
        ...
        def handle_doc(message):
            ...
            if PERSONALITY_SYSTEMS_AVAILABLE and hasattr(self, "doc"):
                response = self.doc.get_risk_warning(
                    {"risk_level": "moderate", "account_balance": 10000, "current_risk": 200}
                )
            else:
        ...
        def handle_personalities_menu(message):
            ...
            keyboard.add(types.InlineKeyboardButton("👁️ OBSERVER", callback_data="personality_observer"))
