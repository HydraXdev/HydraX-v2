...
class CitadelAdaptiveThrottle:
    ...
    def _initialize_state(self):
        ...
        try:
            ...
            if last_signal_time:
                ...
                self.throttle_logger.info(
                    f"INIT - Last signal: {datetime.fromtimestamp(last_signal_time).isoformat()}, Minutes ago: {self.current_state.minutes_since_signal:.1f}, TCS: {self.current_state.current_tcs}"
                )
            else:
    ...
    def _update_tcs_threshold(self):
        ...
        if abs(new_tcs - self.current_state.current_tcs) > 0.1 or new_reason != self.current_state.reason_code:
            ...
            self.throttle_logger.info(
                f"TCS_CHANGE - {old_tcs:.1f} → {new_tcs:.1f} | Reason: {old_reason} → {new_reason} | Minutes: {minutes_elapsed:.1f} | Tier: {new_tier}"
            )
    ...
    def _update_citadel_state(self):
        ...
        try:
            ...
            self.throttle_logger.info(
                f"CITADEL_UPDATE - TCS: {self.current_state.current_tcs} | Tier: {self.current_state.tier_level} | Reason: {self.current_state.reason_code}"
            )
    ...
    def _send_pressure_override_alert(self):
        ...
        try:
            ...
            payload = {"chat_id": self.commander_user_id, "text": message, "parse_mode": "Markdown"}
    ...
    def on_signal_completed(self, signal_id: str, timestamp: Optional[float] = None):
        ...
        # Log reset
        self.throttle_logger.info(
            f"SIGNAL_RESET - {old_tcs:.1f} → {self.config.baseline} | Signal: {signal_id} | Reason: {old_reason} → SIGNAL_RESET"
        )
    ...
    def get_current_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_signal_timestamp": self.current_state.last_signal_timestamp,
            "last_signal_time": (
                datetime.fromtimestamp(self.current_state.last_signal_timestamp).isoformat()
                if self.current_state.last_signal_timestamp
                else None
            ),
            "minutes_since_signal": self.current_state.minutes_since_signal,
            "threshold_changed_at": self.current_state.threshold_changed_at,
        ...
        while self.monitoring_active:
            try:
                ...
                if current_last_signal and (
                    self.current_state.last_signal_timestamp is None
                    or current_last_signal > self.current_state.last_signal_timestamp
                ):
