...
class CrossTimeframeValidator:
    ...
    def __init__(self):
        ...
        self.tf_hierarchy = [TimeFrame.M5, TimeFrame.M15, TimeFrame.H1, TimeFrame.H4]
        ...
        try:
            ...
            for tf in self.tf_hierarchy:
                ...
                if tf_key in tf_data:
                    tf_analysis[tf_key] = self._analyze_timeframe(tf_data[tf_key], direction, tf)
                else:
            ...
            passes_validation = self._check_validation_rules(alignment_results, signal_type)
            ...
            result = {
                ...
                "recommendation": self._generate_recommendation(alignment_results, conflicts, signal_type),
            }

            logger.info(
                f"Cross-TF validation: {alignment_results['aligned_count']}/4 aligned, "
    ...
    def _analyze_timeframe(
        self, tf_data: Dict[str, Any], signal_direction: str, timeframe: TimeFrame
    ) -> Dict[str, Any]:
    ...
    def _analyze_momentum(self, tf_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        momentum_data = {"strength": "neutral", "divergence": False, "exhaustion": False}
    ...
    def _analyze_structure(self, tf_data: Dict[str, Any], direction: str) -> Dict[str, Any]:
        ...
        structure_data = {"pattern": "none", "key_level": False, "break_of_structure": False}
    ...
    def _check_alignment(self, trend: TrendDirection, momentum: Dict, structure: Dict, signal_direction: str) -> bool:
        """Check if timeframe aligns with signal direction."""
        # Trend alignment
        trend_aligned = (
            (trend == TrendDirection.BULLISH and signal_direction == "BUY")
            or (trend == TrendDirection.BEARISH and signal_direction == "SELL")
            or trend == TrendDirection.NEUTRAL
        )

        # Momentum alignment
        momentum_aligned = (
            momentum["strength"] not in ["overbought", "oversold"]
            or (momentum["strength"] == "oversold" and signal_direction == "BUY")
            or (momentum["strength"] == "overbought" and signal_direction == "SELL")
        )
    ...
    def _check_conflict(self, trend: TrendDirection, signal_direction: str, timeframe: TimeFrame) -> bool:
        ...
        if timeframe in [TimeFrame.H1, TimeFrame.H4]:
            ...
            if (
                ...
                or trend == TrendDirection.BEARISH
    ...
    def _calculate_tf_confidence(self, trend: TrendDirection, momentum: Dict, structure: Dict) -> float:
    ...
    def _calculate_alignment(self, tf_analysis: Dict[str, Dict], direction: str) -> Dict[str, Any]:
    ...
    def _identify_conflicts(self, tf_analysis: Dict[str, Dict], direction: str) -> List[str]:
        ...
        for tf_name, analysis in tf_analysis.items():
            ...
            if (
                direction == "BUY"
                and momentum.get("strength") == "overbought"
                or direction == "SELL"
                and momentum.get("strength") == "oversold"
            ):
            ...
            if momentum.get("divergence"):
                ...
                if direction == "BUY" and div_type == "bearish" or direction == "SELL" and div_type == "bullish":
    ...
    def _identify_confluences(self, tf_analysis: Dict[str, Dict]) -> List[str]:
        ...
        aligned_trends = [tf for tf, data in tf_analysis.items() if data.get("trend") != "neutral"]
        ...
        key_level_tfs = [tf for tf, data in tf_analysis.items() if data.get("structure", {}).get("key_level", False)]
        ...
        bullish_momentum = sum(
            1 for data in tf_analysis.values() if data.get("momentum", {}).get("strength") == "bullish"
        )
        bearish_momentum = sum(
            1 for data in tf_analysis.values() if data.get("momentum", {}).get("strength") == "bearish"
        )
    ...
    def _generate_recommendation(self, alignment: Dict, conflicts: List[str], signal_type: str) -> str:
    ...
    def _get_tf_details(self, tf_data: Dict, trend: TrendDirection, momentum: Dict) -> str:
...
if __name__ == "__main__":
    ...
    # Test signal
    test_signal = {"pair": "EURUSD", "direction": "BUY", "signal_type": "TREND_FOLLOWING"}
