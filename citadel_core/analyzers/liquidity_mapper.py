...
class LiquidityMapper:
    ...
    def analyze(self, signal: Dict[str, Any], market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            liquidity_proximity = self._check_liquidity_proximity(entry_price, sl_clusters, order_blocks, pip_value)

            # Determine trap probability
            trap_analysis = self._analyze_trap_formation(sweep_analysis, liquidity_proximity, direction, market_data)
            ...
            result = {
                ...
                "liquidity_score": self._calculate_liquidity_score(sweep_analysis, trap_analysis, liquidity_proximity),
            }

            logger.info(
                f"Liquidity analysis for {pair}: Sweep={sweep_analysis['sweep_detected']}, "
    ...
    def _detect_liquidity_sweep(self, market_data: Dict[str, Any], direction: str) -> Dict[str, Any]:
        ...
        if real_sweeps:
            ...
            for sweep in real_sweeps:
                ...
                if (direction == "BUY" and sweep["type"] == "support_sweep") or (
                    ...
                    quality = (
                        "high" if sweep["magnitude"] > 0.0003 else "medium" if sweep["magnitude"] > 0.0001 else "low"
                    )
        ...
        if not candles or len(candles) < 10:
            return {"sweep_detected": False, "sweep_type": None, "quality": "none", "time_since": None}
    ...
    def _map_sl_clusters(
        self, entry_price: float, pair_type: str, pip_value: float, market_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for distance in typical_distances:
            ...
            cluster_strength = self._calculate_cluster_strength(cluster_above, market_data, "resistance")
            ...
            cluster_strength = self._calculate_cluster_strength(cluster_below, market_data, "support")
    ...
    def _identify_order_blocks(
        self, market_data: Dict[str, Any], entry_price: float, direction: str
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(1, len(candles) - 1):
            ...
            if direction == "BUY":
                ...
                strong_bull_after = next_candle and (next_candle["close"] - next_candle["open"]) > 2 * abs(
            ...
            # Bearish order block (last bullish before bearish move)
                ...
                strong_bear_after = next_candle and (next_candle["open"] - next_candle["close"]) > 2 * abs(
                    current["close"] - current["open"]
                )
        ...
        nearby_obs = [ob for ob in order_blocks if abs(ob["mid"] - entry_price) < entry_price * 0.01]

        return sorted(nearby_obs, key=lambda x: x["strength"], reverse=True)[:3]

    def _check_liquidity_proximity(
        self, entry_price: float, sl_clusters: List[Dict], order_blocks: List[Dict], pip_value: float
    ) -> Dict[str, Any]:
        """Check proximity to liquidity zones."""
        proximity_data = {"near_cluster": False, "near_order_block": False, "danger_zone": False}
        ...
        proximity_data["danger_zone"] = proximity_data.get("cluster_distance_pips", 999) < 15 or proximity_data.get(
            "in_order_block", False
        )

        return proximity_data

    def _analyze_trap_formation(
        self, sweep_analysis: Dict, liquidity_proximity: Dict, direction: str, market_data: Dict
    ) -> Dict[str, Any]:
        ...
        return {"probability": probability, "type": trap_type, "score": trap_score}
    ...
    def _check_psychological_level(self, price: float, pair_type: str) -> Dict[str, Any]:
        ...
        return {"at_level": strength != "none", "strength": strength, "type": level_type}

    def _calculate_cluster_strength(self, price: float, market_data: Dict, zone_type: str) -> int:
    ...
    def _calculate_liquidity_score(self, sweep_analysis: Dict, trap_analysis: Dict, liquidity_proximity: Dict) -> int:
...
if __name__ == "__main__":
    ...
    # Test signal
    test_signal = {"pair": "EURUSD", "direction": "BUY", "entry_price": 1.0850, "sl": 1.0820, "tp": 1.0900}
