...
class MarketRegimeAnalyzer:
        ...
        for event_name, schedule in self.high_impact_events.items():
            ...
            if self._is_event_upcoming(current_time, schedule):
                ...
                if self._event_affects_pair(event_name, pair):
                    upcoming_events.append({"name": event_name, "time_until": "Within 2 hours", "impact": "HIGH"})
    ...
    def _event_affects_pair(self, event: str, pair: str) -> bool:
        ...
        event_currencies = {"NFP": ["USD"], "FOMC": ["USD"], "ECB": ["EUR"], "BOE": ["GBP"]}
    ...
    def _check_ma_alignment(self, ma_fast: List[float], ma_slow: List[float], ma_long: List[float]) -> str:
