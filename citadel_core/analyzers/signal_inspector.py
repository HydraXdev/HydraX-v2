...
class SignalInspector:
    ...
    def analyze(self, signal: Dict[str, Any], market_data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            signal_type = self._classify_signal_pattern(entry_price, direction, market_data)

            # Detect entry structure
            entry_structure = self._analyze_entry_structure(entry_price, direction, market_data)

            # Check for volatility zones
            volatility_zone = self._check_volatility_zone(pair, market_data)

            # Identify potential traps
            trap_risk = self._assess_trap_risk(signal_type, entry_structure, market_data)
    ...
    def _classify_signal_pattern(self, entry: float, direction: str, market_data: Optional[Dict]) -> str:
        ...
        # Reversal detection
        price_at_extreme = (direction == "SELL" and abs(current_price - recent_high) < recent_high * 0.001) or (
    ...
    def _analyze_entry_structure(self, entry: float, direction: str, market_data: Optional[Dict]) -> str:
    ...
    def _assess_trap_risk(self, signal_type: str, entry_structure: str, market_data: Optional[Dict]) -> str:
...
if __name__ == "__main__":
    ...
    test_market = {
        ...
        "atr": 0.0045,
        "avg_atr": 0.0035,
        "timeframes": {"M15": {"structure": "Testing support zone"}, "H1": {"structure": "Bullish trend intact"}},
    }
