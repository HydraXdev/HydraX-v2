...
class CitadelDataStreamEnhancer:
    ...
    def __init__(self):
        ...
        self.support_resistance = defaultdict(
            lambda: {"support_levels": [], "resistance_levels": [], "last_updated": 0}
        )

        # Liquidity tracking
        self.liquidity_zones = defaultdict(
            lambda: {"high_liquidity": [], "stop_clusters": [], "sweep_candidates": [], "order_blocks": []}
        )

        # Volume analysis
        self.volume_profile = defaultdict(
            lambda: {"volume_by_price": defaultdict(int), "average_volume": 0, "volume_spikes": []}
        )
        ...
        # ATR and volatility
        self.volatility_data = defaultdict(
            lambda: {"atr_14": 0, "atr_history": deque(maxlen=50), "volatility_percentile": 0}
        )
        ...
        if len(self.tick_history[symbol]) >= 100:
            ...
            if tick.volume > profile["average_volume"] * 2:
                profile["volume_spikes"].append(
                    {"price": tick.mid_price, "volume": tick.volume, "timestamp": tick.timestamp}
                )
    ...
    def _find_confluence_zones(self, levels: Dict, tf_data: Dict) -> List[Dict]:
        ...
        for level in all_levels:
            ...
            if confluence_count >= 2:
                confluences.append({"price": level, "strength": confluence_count, "factors": factors})
