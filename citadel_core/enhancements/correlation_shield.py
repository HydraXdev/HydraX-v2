...
class CorrelationShield:
    ...
    def analyze_signal_correlations(
        self, active_signals: List[Dict[str, Any]], new_signal: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for active in active_signals:
            ...
            # Check for conflicts
            conflict_type = self._check_conflict(new_pair, new_direction, active_pair, active_direction, correlation)
                ...
                conflicts.append(
                    {
                        ...
                        "description": self._get_conflict_description(
                            new_pair, new_direction, active_pair, active_direction, correlation
                        ),
                    }
                )
            else:
                # Check for synergies
                synergy_type = self._check_synergy(new_pair, new_direction, active_pair, active_direction, correlation)
            ...
            # Track net exposure
            self._update_net_exposure(net_exposure, new_pair, new_direction, active_pair, active_direction, correlation)

        # Calculate overall risk level
        risk_level = self._calculate_risk_level(conflicts, synergies)

        # Generate recommendations
        recommendations = self._generate_recommendations(conflicts, synergies, net_exposure, new_signal)
        ...
        return {
            ...
            "position_adjustment": self._suggest_position_adjustment(conflicts, synergies),
        }
    ...
    def _share_currency(self, pair1: str, pair2: str) -> bool:
        ...
        if len(pair1) >= 6 and len(pair2) >= 6:
            ...
            return base1 == base2 or base1 == quote2 or quote1 == base2 or quote1 == quote2
        return False

    def _check_conflict(self, pair1: str, dir1: str, pair2: str, dir2: str, correlation: float) -> Optional[str]:
    ...
    def _check_synergy(self, pair1: str, dir1: str, pair2: str, dir2: str, correlation: float) -> Optional[str]:
    ...
    def _get_conflict_description(self, pair1: str, dir1: str, pair2: str, dir2: str, correlation: float) -> str:
    ...
    def _get_synergy_description(self, pair1: str, dir1: str, pair2: str, dir2: str, correlation: float) -> str:
    ...
    def _calculate_risk_level(self, conflicts: List[Dict], synergies: List[Dict]) -> str:
    ...
    def _generate_recommendations(
        self, conflicts: List[Dict], synergies: List[Dict], net_exposure: Dict[str, float], new_signal: Dict[str, Any]
    ) -> List[str]:
        ...
        if conflicts:
            ...
            if worst_conflict["severity"] == "CRITICAL":
                recommendations.append("🚨 CRITICAL: Consider skipping this signal or closing conflicting position")
            elif worst_conflict["severity"] == "HIGH":
                recommendations.append("⚠️ Reduce position size by 50% due to correlation conflict")
            else:
                recommendations.append("📊 Monitor closely - moderate correlation exposure")
        ...
        if synergies:
            ...
            if amplified:
                recommendations.append("💡 Concentrated exposure detected - ensure total risk is managed")
            if hedged:
                recommendations.append("🛡️ Natural hedge present - consider larger position")

        # Net exposure recommendations
        high_exposure_pairs = [p for p, exp in net_exposure.items() if exp > 2]
        if high_exposure_pairs:
            recommendations.append(f"📈 High net exposure on: {', '.join(high_exposure_pairs)}")

        return recommendations

    def _get_correlation_map(self, active_signals: List[Dict], new_signal: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _suggest_position_adjustment(self, conflicts: List[Dict], synergies: List[Dict]) -> Dict[str, Any]:
        ...
        return {"multiplier": round(multiplier, 2), "reason": " | ".join(reasons) if reasons else "Standard position"}
    ...
    def _calculate_cluster_correlation(self, cluster: List[Dict]) -> float:
        ...
        for i, signal1 in enumerate(cluster):
            for signal2 in cluster[i + 1 :]:
                correlation = self._get_correlation(signal1.get("pair", ""), signal2.get("pair", ""))
    ...
    def _get_concentration_warning(self, clusters: List[Dict]) -> Optional[str]:
        ...
        if largest["risk_concentration"] > 3:
            ...
            return f"⚠️ Moderate concentration: {largest['size']} related positions"
