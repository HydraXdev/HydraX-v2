...
class MicroStructure:
    ...
    def detect_institutional_footprints(
        ...
        iceberg_detection = self._detect_iceberg_orders(price_data, volume_data, depth_data)
        if iceberg_detection["detected"]:
            footprints.append(iceberg_detection)

        # Detect absorption/exhaustion
        absorption_pattern = self._detect_absorption_pattern(price_data, volume_data)
        if absorption_pattern["detected"]:
            footprints.append(absorption_pattern)

        # Detect smart money reversal
        reversal_pattern = self._detect_smart_money_reversal(price_data, volume_data)
        ...
        position_building = self._detect_position_building(price_data, volume_data)

        # Generate interpretation
        interpretation = self._interpret_footprints(footprints, order_flow, position_building)

        # Calculate institutional probability
        inst_probability = self._calculate_institutional_probability(footprints, order_flow)
        ...
        return {
            ...
            "trading_implications": self._get_trading_implications(footprints, order_flow),
            "key_levels": self._identify_institutional_levels(price_data, footprints),
        }
    ...
    def analyze_market_depth_imbalance(self, depth_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "strength": strength,
            "large_orders": {"bid_side": len(large_bids), "ask_side": len(large_asks)},
            "clustering": {"bids": bid_clustering, "asks": ask_clustering},
            "interpretation": self._interpret_depth_imbalance(bid_ratio, inst_bias, large_bids, large_asks),
        }

    def identify_smart_money_zones(
        self, price_data: List[Dict[str, Any]], volume_data: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _detect_whale_activity(self, price_data: List[Dict], volume_data: List[Dict]) -> Dict[str, Any]:
        ...
        if volume_ratio >= self.thresholds["whale_volume_multiplier"]:
            ...
            return {
                "detected": True,
                "type": (
                    InstitutionalFootprint.WHALE_ACCUMULATION
                    if direction == "buying"
    ...
    def _detect_iceberg_orders(
        self, price_data: List[Dict], volume_data: List[Dict], depth_data: Optional[Dict]
    ) -> Dict[str, Any]:
    ...
    def _detect_absorption_pattern(self, price_data: List[Dict], volume_data: List[Dict]) -> Dict[str, Any]:
        ...
        if total_volume > 0 and avg_range > 0:
            ...
            if (
                price_range < avg_range * 0.5
                and total_volume > statistics.mean([v.get("volume", 0) for v in volume_data[:-3]]) * 2
            ):
                # Determine if accumulation or distribution
                close_position = (
                    (recent_candles[-1]["close"] - recent_candles[-1]["low"])
                    / (recent_candles[-1]["high"] - recent_candles[-1]["low"])
                    if recent_candles[-1]["high"] != recent_candles[-1]["low"]
                    else 0.5
                )
    ...
    def _detect_smart_money_reversal(self, price_data: List[Dict], volume_data: List[Dict]) -> Dict[str, Any]:
        ...
        if recent_volumes[-2] > avg_volume * self.thresholds["exhaustion_volume_spike"]:
            ...
            # Bearish reversal
            if (
                spike_candle["close"] > spike_candle["open"]
                and reversal_candle["close"] < reversal_candle["open"]
                and reversal_candle["close"] < spike_candle["low"]
            ):
            ...
            # Bullish reversal
                ...
                spike_candle["close"] < spike_candle["open"]
                and reversal_candle["close"] > reversal_candle["open"]
                and reversal_candle["close"] > spike_candle["high"]
            ):
    ...
    def _analyze_order_flow(self, price_data: List[Dict], volume_data: List[Dict]) -> Dict[str, Any]:
    ...
    def _detect_position_building(self, price_data: List[Dict], volume_data: List[Dict]) -> Dict[str, Any]:
    ...
    def _interpret_footprints(self, footprints: List[Dict], order_flow: Dict, position_building: Dict) -> str:
    ...
    def _calculate_institutional_probability(self, footprints: List[Dict], order_flow: Dict) -> float:
        """Calculate probability of institutional involvement."""
        if not footprints:
            base_probability = 0.2
        else:
            # Average reliability of detected footprints
            base_probability = statistics.mean([f.get("reliability", 0.5) for f in footprints])

        # Adjust for order flow
        if order_flow["pattern"] in [
            OrderFlowPattern.AGGRESSIVE_BUYING.value,
    ...
    def _get_trading_implications(self, footprints: List[Dict], order_flow: Dict) -> List[str]:
        ...
        whale_footprints = [
            f
            for f in footprints
            if f.get("type") in [InstitutionalFootprint.WHALE_ACCUMULATION, InstitutionalFootprint.WHALE_DISTRIBUTION]
        ]
        if whale_footprints:
        ...
        reversal_footprints = [f for f in footprints if f.get("type") == InstitutionalFootprint.SMART_MONEY_REVERSAL]
    ...
    def _identify_institutional_levels(self, price_data: List[Dict], footprints: List[Dict]) -> List[Dict[str, Any]]:
        ...
        if len(price_data) >= 10:
            ...
            if volume_weighted_prices:
                ...
                levels.append({"price": round(vwap, 5), "type": "vwap", "description": "Volume-weighted average price"})
    ...
    def _interpret_depth_imbalance(self, bid_ratio: float, inst_bias: str, large_bids: List, large_asks: List) -> str:
    ...
    def _find_volume_nodes(self, price_data: List[Dict], volume_data: List[Dict]) -> List[Dict]:
    ...
    def _has_institutional_characteristics(self, node: Dict, price_data: List[Dict]) -> bool:
