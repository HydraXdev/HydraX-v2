...
class NewsAmplifier:
    ...
    def enhance_signal_context(self, signal: Dict[str, Any], upcoming_events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        recommendations = self._generate_news_strategy(signal, event_analyses, volatility_class)

        # Check for event conflicts
        conflicts = self._check_event_conflicts(event_analyses)

        # Historical context
        historical_context = self._get_historical_context(event_analyses, signal)
        ...
        return {
            ...
            "risk_adjustments": self._calculate_risk_adjustments(overall_impact, volatility_class),
            "timing_advice": self._get_timing_advice(event_analyses),
            "shield_modifications": self._suggest_shield_modifications(overall_impact, volatility_class),
        }

    def analyze_pre_news_positioning(self, signal: Dict[str, Any], event: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def predict_post_news_scenarios(self, event: Dict[str, Any], pair: str) -> List[Dict[str, Any]]:
        """
    ...
    def _analyze_event_impact(self, event: Dict[str, Any], signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        is_affected = any(curr in signal_pair for curr in currencies) or any(
        ...
        if is_affected:
            ...
            impact = NewsImpact.LOW if self._is_correlated_impact(signal_pair, currencies) else NewsImpact.NONE
    ...
    def _generate_news_strategy(
        self, signal: Dict[str, Any], event_analyses: List[Dict], volatility: VolatilityExpectation
    ) -> List[str]:
        """Generate strategic recommendations based on news."""
        recommendations = []

        # Check for critical events
        critical_events = [e for e in event_analyses if e["impact"] == NewsImpact.CRITICAL]
        ...
        if critical_events:
            ...
            if minutes_until < 30:
                ...
                recommendations.append(
                    f"⚠️ HIGH IMPACT: {closest_event['name']} approaching - " f"Tighten stops or reduce position"
                )
            else:
                recommendations.append(f"📅 Plan exit before {closest_event['name']} - " f"Close at least 30 min prior")

        # Volatility-based recommendations
        if volatility == VolatilityExpectation.EXTREME:
            recommendations.append("🌪️ Extreme volatility expected - Use wider stops or avoid entry")
        elif volatility == VolatilityExpectation.HIGH:
            recommendations.append("📊 High volatility period - Consider 50% position size")

        # Add approach recommendations
        for event in event_analyses[:2]:  # Top 2 events
            if event["best_approach"]:
                recommendations.append(f"💡 {event['event']}: {event['best_approach'].replace('_', ' ').title()}")
    ...
    def _check_event_conflicts(self, event_analyses: List[Dict]) -> List[str]:
        ...
        high_impact_events = [e for e in event_analyses if e["impact"].value >= NewsImpact.HIGH.value]
        ...
        if len(high_impact_events) > 1:
            ...
            for i, event1 in enumerate(high_impact_events):
                for event2 in high_impact_events[i + 1 :]:
                    time_diff = abs((event1["time_until"] - event2["time_until"]).total_seconds() / 60)
        ...
        for curr, events in currencies_affected.items():
            if len(events) > 1:
                conflicts.append(f"🔄 Multiple {curr} events: {', '.join(events)}")

        return conflicts

    def _get_historical_context(self, event_analyses: List[Dict], signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"historical_note": "Limited historical data for this event combination"}

    def _calculate_risk_adjustments(self, impact: NewsImpact, volatility: VolatilityExpectation) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": self._get_risk_adjustment_text(position_size_multiplier, stop_loss_multiplier),
        }
    ...
    def _suggest_shield_modifications(self, impact: NewsImpact, volatility: VolatilityExpectation) -> Dict[str, Any]:
        """Suggest modifications to shield scoring based on news."""
        modifications = {"score_adjustment": 0, "reasons": []}
    ...
    def _get_no_news_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timing_advice": {"status": "clear", "advice": "No news conflicts", "action": "proceed_normally"},
            "shield_modifications": {"score_adjustment": 0, "reasons": []},
        }
...
if __name__ == "__main__":
    ...
    # Test signal
    signal = {"pair": "EURUSD", "direction": "BUY", "entry_price": 1.0850}

    # Test upcoming events
    events = [
        {"type": "ECB", "time": datetime.now() + timedelta(hours=2), "currencies": ["EUR"]},
        {"type": "CPI", "time": datetime.now() + timedelta(hours=4), "currencies": ["USD"]},
    ]
