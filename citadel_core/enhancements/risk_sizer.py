...
class DynamicRiskSizer:
    ...
    def calculate_position_size(
        self, signal: Dict[str, Any], shield_analysis: Dict[str, Any], risk_profile: RiskProfile
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            base_risk = self._calculate_base_risk(shield_score, classification, risk_profile)

            # Apply special condition multipliers
            adjusted_risk = self._apply_special_conditions(base_risk, risk_factors, quality_factors)

            # Apply risk mode adjustment
            final_risk = adjusted_risk * self.mode_adjustments[risk_profile.risk_mode]

            # Ensure within bounds
            final_risk = max(risk_profile.min_risk_percent, min(risk_profile.max_risk_percent, final_risk))

            # Calculate actual position size
            position_details = self._calculate_position_details(signal, risk_profile.account_balance, final_risk)

            # Generate sizing recommendation
            recommendation = self._generate_recommendation(shield_score, classification, final_risk, base_risk)
            ...
            return {
                ...
                "sizing_factors": self._get_sizing_factors(risk_factors, quality_factors),
    ...
    def calculate_basket_allocation(self, signals: List[Dict[str, Any]], risk_profile: RiskProfile) -> Dict[str, Any]:
        """
        ...
        sorted_signals = sorted(signals, key=lambda s: s.get("citadel_shield", {}).get("score", 0), reverse=True)
        ...
        allocations = []
        total_score = sum(s.get("citadel_shield", {}).get("score", 0) for s in sorted_signals)
    ...
    def suggest_scaling_strategy(self, signal: Dict[str, Any], shield_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if classification == "SHIELD_APPROVED" and shield_score >= 9.0:
            return {
                ...
                "entry_triggers": ["Immediate entry (40%)", "After 10 pip momentum (35%)", "On first pullback (25%)"],
            }
            ...
            return {
                ...
                "entry_triggers": ["Immediate entry (60%)", "After confirmation (40%)"],
            }
            ...
            return {
                ...
                "entry_triggers": ["Wait for confirmation candle"],
            }
        else:
            return {
                ...
                "entry_triggers": ["Only after strong confirmation"],
            }

    def _calculate_base_risk(self, shield_score: float, classification: str, risk_profile: RiskProfile) -> float:
    ...
    def _apply_special_conditions(
        self, base_risk: float, risk_factors: List[Dict], quality_factors: List[Dict]
    ) -> float:
    ...
    def _calculate_position_details(
        self, signal: Dict[str, Any], account_balance: float, risk_percent: float
    ) -> Dict[str, Any]:
        ...
        if entry == 0 or stop_loss == 0:
            return {"position_size": 0, "risk_amount": risk_amount, "lots": 0, "units": 0}
    ...
    def _generate_recommendation(
        self, shield_score: float, classification: str, final_risk: float, base_risk: float
    ) -> str:
        ...
        if classification == "SHIELD_APPROVED" and shield_score >= 9.0:
            ...
            return f"🛡️ Strong setup - Full position size appropriate. " f"Risk {final_risk:.1f}% with confidence."
        elif classification == "SHIELD_ACTIVE":
            return (
                f"✅ Solid setup - Standard position size suggested. " f"Risk {final_risk:.1f}% for balanced exposure."
            )
        elif classification == "VOLATILITY_ZONE":
            return (
    ...
    def _get_sizing_factors(self, risk_factors: List[Dict], quality_factors: List[Dict]) -> List[str]:
...
def get_position_size_recommendation(
    signal: Dict[str, Any], shield_analysis: Dict[str, Any], account_balance: float = 10000, risk_mode: str = "NORMAL"
) -> Dict[str, Any]:
    """
    ...
    profile = RiskProfile(account_balance=account_balance, risk_mode=risk_mode)
...
if __name__ == "__main__":
    # Test risk sizer
    test_signal = {"pair": "EURUSD", "entry_price": 1.0850, "sl": 1.0820, "tp": 1.0910}

    test_shield = {
        "shield_score": 8.5,
        "classification": "SHIELD_APPROVED",
        "risk_factors": [],
        "quality_factors": [{"factor": "post_sweep_entry", "description": "Entry after sweep"}],
    }

    result = get_position_size_recommendation(test_signal, test_shield, account_balance=10000, risk_mode="NORMAL")
