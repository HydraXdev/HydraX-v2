...
class SessionFlow:
    ...
    def analyze_institutional_flow(self, pair: str, current_time: Optional[datetime] = None) -> Dict[str, Any]:
        """
        ...
        behavior_analysis = self._analyze_institutional_behavior(session, pair, current_time)

        # Predict session flow
        flow_prediction = self._predict_session_flow(session, current_time, behavior_analysis)

        # Generate trading recommendations
        recommendations = self._generate_session_recommendations(session, pair, is_optimal_pair, behavior_analysis)
    ...
    def predict_session_transitions(self, current_time: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        if time_until.total_seconds() > 3600:  # More than 1 hour
            return {"transition_imminent": False, "next_session": next_session.value, "time_until": time_until}

        # Analyze transition patterns
        current_session = self._identify_session(current_time)

        transition_patterns = self._get_transition_patterns(current_session, next_session)
    ...
    def get_historical_session_performance(self, pair: str, session: SessionType) -> Dict[str, Any]:
        """
    ...
    def _analyze_institutional_behavior(
        self, session: SessionType, pair: str, current_time: datetime
    ) -> Dict[str, float]:
    ...
    def _predict_session_flow(
        self, session: SessionType, current_time: datetime, behavior_analysis: Dict[str, float]
    ) -> Dict[str, Any]:
        """Predict how session will likely unfold."""
        profile = self.session_profiles.get(session, {})
        session_progress = self._get_session_progress(current_time, session)

        predictions = {"next_hour": "", "session_end": "", "key_times": [], "volatility_forecast": ""}
    ...
    def _generate_session_recommendations(
        self, session: SessionType, pair: str, is_optimal: bool, behavior_analysis: Dict[str, float]
    ) -> List[str]:
        ...
        if is_optimal:
            recommendations.append(f"✅ {pair} is optimal for {session.value} session")
        else:
        ...
        if dominant_behavior[0] == "stop_hunting":
            recommendations.append("🎯 Expect stop hunts - enter after liquidity sweep")
        elif dominant_behavior[0] == "trend_initiation":
            recommendations.append("📈 Favor breakouts - trade with initial momentum")
        elif dominant_behavior[0] == "range_trading":
            recommendations.append("↔️ Range conditions - fade extremes with tight stops")
        elif dominant_behavior[0] == "position_squaring":
            recommendations.append("🔄 Position unwinding - avoid new positions")
    ...
    def _identify_key_levels(self, session: SessionType, pair: str) -> Dict[str, Any]:
        ...
        levels = {"immediate": [], "session_targets": [], "notes": []}
    ...
    def _get_session_progress(self, current_time: datetime, session: SessionType) -> float:
    ...
    def _get_session_time_remaining(self, current_time: datetime, session: SessionType) -> timedelta:
    ...
    def _get_suitability_reason(self, pair: str, session: SessionType, is_optimal: bool) -> str:
    ...
    def _identify_session_risks(self, session: SessionType, current_time: datetime) -> List[str]:
    ...
    def _get_next_session_transition(self, current_time: datetime) -> Tuple[SessionType, timedelta]:
        ...
        session_order = [SessionType.ASIAN, SessionType.LONDON, SessionType.NEW_YORK, SessionType.NY_CLOSE]
    ...
    def _get_transition_patterns(self, current: SessionType, next_session: SessionType) -> Dict[str, Any]:
        ...
        pattern = transition_patterns.get(
