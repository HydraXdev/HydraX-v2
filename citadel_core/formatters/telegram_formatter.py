...
class TelegramShieldFormatter:
    ...
    def __init__(self):
        ...
        self.templates = {
            "SHIELD_APPROVED": {"header": "🛡️ SHIELD APPROVED", "color": "green", "emphasis": "strong"},
            "SHIELD_ACTIVE": {"header": "✅ SHIELD ACTIVE", "color": "blue", "emphasis": "moderate"},
            "VOLATILITY_ZONE": {"header": "⚠️ VOLATILITY ZONE", "color": "yellow", "emphasis": "caution"},
            "UNVERIFIED": {"header": "🔍 UNVERIFIED", "color": "gray", "emphasis": "minimal"},
        }
    ...
    def format_enhanced_signal(
        self, signal: Dict[str, Any], shield_analysis: Dict[str, Any], compact: bool = False
    ) -> str:
        """
        ...
        try:
            ...
            if compact:
                return self._format_compact(
                    pair, direction, entry, tp, sl, classification, score, emoji, shield_analysis
                )
            else:
    ...
    def _format_compact(
        ...
        message_parts = [f"📍 {pair} {direction_emoji} {direction} @ {entry}", f"🎯 TP: {tp} | 🛑 SL: {sl}"]
    ...
    def format_shield_insight(self, shield_analysis: Dict[str, Any]) -> str:
        ...
        """
        lines = ["🛡️ **CITADEL Shield Analysis**", f"Score: {shield_analysis.get('shield_score', 0)}/10", ""]
        ...
        # Learning tip
        lines.extend(["", "💡 **What This Means:**", self._get_educational_tip(shield_analysis)])

        # Confidence level
        confidence = shield_analysis.get("confidence", 0)
        lines.extend(["", f"🎯 Shield Confidence: {int(confidence * 100)}%"])

        return "\n".join(lines)

    def format_shield_summary(self, signals_analyzed: int, classifications: Dict[str, int]) -> str:
        """
    ...
    def _get_educational_tip(self, analysis: Dict[str, Any]) -> str:
        ...
        if classification == "SHIELD_APPROVED":
            ...
            return tips.get(classification, {}).get(
                "default", "Continue learning to recognize these patterns yourself."
            )
    ...
    def _format_basic_signal(self, signal: Dict[str, Any]) -> str:
        ...
        direction_emoji = "📈" if direction == "BUY" else "📉"
        return f"📍 {pair} {direction_emoji} {direction} @ {entry}\n" f"🎯 TP: {tp} | 🛑 SL: {sl}"
...
if __name__ == "__main__":
    ...
    # Test signal
    test_signal = {"pair": "EURUSD", "direction": "BUY", "entry_price": 1.0850, "tp": 1.0900, "sl": 1.0820}
