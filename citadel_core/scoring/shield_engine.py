...
class ShieldScoringEngine:
    ...
    def calculate_score(
        ...
        try:
            ...
            final_score = self._calculate_final_score(base_score, risk_factors, quality_factors)

            # Determine classification
            classification = self._classify_shield_status(final_score)

            # Generate explanation
            explanation = self._generate_explanation(components, risk_factors, quality_factors, final_score)
            ...
            result = {
                ...
                "recommendation": self._generate_recommendation(classification, final_score, risk_factors),
            }
    ...
    def _score_timing_factors(self, regime: Dict[str, Any], signal: Dict[str, Any]) -> List[ScoreComponent]:
    ...
    def _identify_risk_factors(
        self, signal_analysis: Dict, regime: Dict, liquidity: Dict, tf: Dict
    ) -> List[Dict[str, Any]]:
    ...
    def _identify_quality_factors(
        ...
        # Trend continuation
        if signal_analysis.get("signal_type") == "CONTINUATION" and regime.get("trend_strength", 0) >= 3:
            quality_factors.append(
                {
        ...
        if liquidity.get("psychological_level", {}).get("at_level") and liquidity.get("order_block_nearby"):
            quality_factors.append(
                {
    ...
    def _calculate_final_score(self, base_score: float, risk_factors: List[Dict], quality_factors: List[Dict]) -> float:
    ...
    def _get_shield_emoji(self, classification: str) -> str:
        ...
        emojis = {"SHIELD_APPROVED": "🛡️", "SHIELD_ACTIVE": "✅", "VOLATILITY_ZONE": "⚠️", "UNVERIFIED": "🔍"}
    ...
    def _generate_explanation(
        ...
        positive_comps = sorted(
            [c for c in components if c.category == "positive" and c.value > 0.5], key=lambda x: x.value, reverse=True
        )[:2]

        if positive_comps:
            explanations.append("Strengths: " + ", ".join([c.reason for c in positive_comps]))

        # Top negative factors
        negative_comps = sorted([c for c in components if c.category == "negative"], key=lambda x: x.value)[:2]

        if negative_comps:
            explanations.append("Cautions: " + ", ".join([c.reason for c in negative_comps]))

        # Major bonuses/penalties
        if quality_factors:
            explanations.append("Bonuses: " + quality_factors[0]["description"])

        if risk_factors:
            explanations.append("Risks: " + risk_factors[0]["description"])

        return " | ".join(explanations)

    def _calculate_confidence(self, components: List[ScoreComponent], score: float) -> float:
    ...
    def _generate_recommendation(self, classification: str, score: float, risk_factors: List[Dict]) -> str:
