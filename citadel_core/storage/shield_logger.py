...
class ShieldLogger:
    ...
    def log_shield_analysis(self, signal_id: str, signal_data: Dict[str, Any], shield_analysis: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Logged shield analysis for {signal_id}: "
    ...
    def log_trade_outcome(
        self, signal_id: str, user_id: int, outcome: str, pips_result: float, user_followed_shield: bool
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Logged outcome for {signal_id}: {outcome} " f"({pips_result} pips), followed={user_followed_shield}"
            )
            return True
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                return {
                    ...
                    "shield_compliance": {
                        ...
                        "follow_rate": round(
                            (
                                (compliance[0] / (compliance[0] + compliance[1]) * 100)
                                if (compliance[0] + compliance[1]) > 0
                                else 0
                            ),
                            1,
                        ),
                        "win_rate_when_followed": round(
                            (compliance[2] / compliance[0] * 100) if compliance[0] > 0 else 0, 1
                        ),
                        "win_rate_when_ignored": round(
                            (compliance[3] / compliance[1] * 100) if compliance[1] > 0 else 0, 1
                        ),
                    },
                }
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    score_performance[bucket] = {"signals": count, "win_rate": round(win_rate, 1)}
                ...
                for row in cursor.fetchall():
                    ...
                    for factor in factors:
                        risk_factor_impact.append(
                            {"factor": factor, "occurrences": count, "win_rate": round(win_rate, 1)}
                        )

                # Identify threshold optimization opportunities
                improvements = {
                    "score_bucket_performance": score_performance,
                    "problematic_risk_factors": sorted(risk_factor_impact, key=lambda x: x["win_rate"])[:5],
                    "recommendations": [],
                }
                ...
                if score_performance.get(7, {}).get("win_rate", 0) < 60:
                    improvements["recommendations"].append(
                        "Consider raising SHIELD_ACTIVE threshold - " "Score 7 signals underperforming"
                    )

                if score_performance.get(8, {}).get("win_rate", 0) < 70:
                    improvements["recommendations"].append(
                        "Review SHIELD_APPROVED criteria - " "Score 8 signals not meeting expectations"
                    )
    ...
    def _add_to_cache(self, signal_id: str, signal_data: Dict, shield_analysis: Dict):
    ...
    def _generate_user_recommendation(self, trust_score: float, stats: Tuple) -> str:
...
if __name__ == "__main__":
    ...
    test_signal = {"pair": "EURUSD", "direction": "BUY"}
    test_analysis = {
        "shield_score": 8.5,
