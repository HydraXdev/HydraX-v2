...
class CitadelProtection:
    ...
    def __init__(self):
        ...
        # Statistics
        self.stats = {"signals_delayed": 0, "sweeps_detected": 0, "sweeps_avoided": 0, "post_sweep_entries": 0}
                ...
                self.completed_sweeps[symbol].append(
                    {
                        ...
                        "completion_time": datetime.now(),
                        "pips_swept": (sweep["zone"].level - sweep.get("bottom_price", sweep["start_price"]))
                        / pip_size,
                    }
                )
                del self.active_sweeps[symbol]
    ...
    def is_post_sweep_opportunity(self, signal: Dict) -> bool:
        ...
        if symbol in self.completed_sweeps:
            recent_sweeps = [
                s for s in self.completed_sweeps[symbol] if (datetime.now() - s["completion_time"]).seconds < 300
            ]  # Last 5 minutes
        ...
        for delayed in self.delayed_signals[:]:  # Copy to allow removal
            ...
            if delayed.symbol in current_tick_data:
                if self.check_sweep_completion(delayed.symbol, current_tick_data[delayed.symbol]):
                    ...
                    logger.info(
                        f"🚀 CITADEL RELEASE: {delayed.symbol} {delayed.direction} after sweep completion (+{self.POST_SWEEP_BOOST}% confidence)"
                    )
