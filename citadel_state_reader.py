    ...
    def get_adjustments(self, symbol: str) -> Dict[str, Any]:
        ...
        if global_config:
            ...
            if "hour_restriction" in global_config:
                ...
                if restriction.get("restricted_hour") == current_hour:
                    adjustments["tcs_penalty"] = max(
                        adjustments["tcs_penalty"], restriction.get("min_tcs_required", 85.0) - 70.0
                    )
        ...
        if symbol_config:
            ...
            if "tcs_penalty" in symbol_config:
                adjustments["tcs_penalty"] = max(adjustments["tcs_penalty"], float(symbol_config["tcs_penalty"]))

            if "entry_delay_ms" in symbol_config:
                adjustments["entry_delay_ms"] = max(adjustments["entry_delay_ms"], int(symbol_config["entry_delay_ms"]))

            if "min_runtime_minutes" in symbol_config:
                adjustments["min_runtime_minutes"] = max(
                    adjustments["min_runtime_minutes"], int(symbol_config["min_runtime_minutes"])
                )
