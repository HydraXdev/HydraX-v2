...
class CitadelSweepPredictionModel:
    ...
    def update_market_data(self, tick_data: Dict):
        ...
        self.volume_profile[symbol].append({"price": current_price, "volume": volume, "timestamp": timestamp})
    ...
    def update_liquidity_levels(self, symbol: str):
        ...
        self.liquidity_levels[symbol] = [
            level for level in self.liquidity_levels[symbol] if current_time - level.detected_at < self.max_level_age
        ]
        ...
        for new_level in new_levels:
            ...
            for existing_level in self.liquidity_levels[symbol]:
                if (
                    abs(new_level.price - existing_level.price) < self.get_pip_size(symbol) * 2
                    and new_level.level_type == existing_level.level_type
                ):
    ...
    def extract_sweep_features(self, symbol: str) -> Dict:
        ...
        features = {
            ...
            "volatility_ratio": (
                (np.std(prices[-10:]) / np.std(prices[-20:])) if len(prices) >= 20 and np.std(prices[-20:]) > 0 else 1
            ),
            # Volume features
    ...
    def predict_sweep(self, symbol: str) -> Optional[SweepPrediction]:
        ...
        try:
            ...
            feature_names = [
                ...
                "levels_above_count",
                "levels_below_count",
                "avg_level_strength_above",
                "avg_level_strength_below",
                "higher_highs_count",
                "lower_lows_count",
    ...
    def analyze_prediction_factors(
        self, symbol: str, features: Dict, sweep_direction: str
    ...
    def get_model_performance_metrics(self) -> Dict:
        ...
        if self.feature_importance:
            ...
            metrics["top_features"] = [
                {"feature": name, "importance": round(importance, 3)} for name, importance in sorted_features[:5]
            ]
