...
def get_threshold_status():
    ...
    try:
        ...
        if not throttle:
            return jsonify({"error": "CITADEL adaptive throttle not available", "status": "disabled"}), 503
...
def get_threshold_history():
    ...
    try:
        ...
        if not throttle:
            return jsonify({"error": "CITADEL adaptive throttle not available"}), 503
            ...
            for line in recent_lines:
                ...
                if "TCS_CHANGE" in line or "SIGNAL_RESET" in line or "PRESSURE_OVERRIDE" in line:
                    ...
                    if len(parts) >= 3:
                        ...
                        history.append(
                            {"timestamp": timestamp_str, "level": level, "message": message, "log_entry": line}
                        )
        ...
        return jsonify({"history": history, "count": len(history), "timestamp": time.time()}), 200
...
def signal_completed_webhook():
    ...
    try:
        ...
        if not signal_id:
            return jsonify({"error": "signal_id parameter required"}), 400
...
def get_throttle_config():
    ...
    try:
        ...
        config_data = {
            ...
            "decay_schedule": [
                {"minutes": minutes, "tcs_threshold": tcs, "reason_code": reason}
                for minutes, tcs, reason in throttle.decay_schedule
            ],
        }

        return jsonify({"config": config_data, "timestamp": time.time()}), 200
...
def health_check():
    ...
    try:
        ...
        return jsonify({"healthy": False, "error": str(e), "timestamp": time.time()}), 500
