                ...
                for line in f:
                    ...
                    if line and line != "[]":
                        try:
                            ...
                            if (
                                "ELITE_GUARD" in signal.get("signal_id", "")
                                and not signal.get("completed", False)
                                and signal.get("entry_price", 0) > 0
                            ):
...
def get_credit_referral_stats():
    ...
    try:
        ...
        stats = {
            ...
            "credits_this_month": admin_stats.get(
                "total_credits_issued", 0
            ),  # Would be filtered by month in production
            "conversion_rate": round(
                (admin_stats.get("total_referrals", 0) / max(1, admin_stats.get("total_codes_generated", 1))) * 100, 1
            ),
        }
...
def throne_authenticate():
    ...
    return jsonify({"success": True, "access_level": user["access_level"], "redirect": "/throne/dashboard"})
...
def throne_dashboard():
    ...
    return render_template(
        "throne_dashboard.html", access_level=session.get("access_level"), commander_id=session.get("commander_id")
    )
...
def api_sitrep():
    ...
    try:
        ...
        sitrep = {
            ...
            "active_users_by_mode": {"MANUAL": 12, "SEMI_AUTO": 8, "FULL_AUTO": 5},
...
def api_execute_command():
    ...
    try:
        ...
        throne_db.log_command(
            commander_id, command, parameters.get("target"), parameters, json.dumps(result), request.remote_addr
        )
...
def apex_status():
    ...
    try:
        ...
        return jsonify({"running": apex_running, "pid": apex_pid, "config": config, "signal_count": signal_count})
    except Exception as e:
...
def control_apex():
    ...
    try:
        ...
        if action == "start":
            # Start apex engine
            subprocess.Popen(
                ["python3", "/root/HydraX-v2/apex_v5_lean.py"],
        ...
        return jsonify(
            {
                ...
                "hit_rates": {"fired_win_rate": 0, "ghost_win_rate": 0, "true_win_rate": 0, "impact": 0},
                "tcs_breakdown": {},
                "top_missed": [],
                "ghost_mode": {"active": False, "total_ghosted": 0, "effectiveness": 0},
            }
        )
...
def api_subscription_analytics():
    ...
    try:
        ...
        analytics = {
            ...
            "conversion": {"press_to_paid": 18.5, "avg_time_to_upgrade": "4.2 days", "trending_tier": "FANG"},
            "revenue_chart": [
...
def api_signal_outcomes():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": stats, "description": "Real-time Elite Guard signal outcomes (SL vs TP hits)"}
        )
...
def api_signal_monitor_active():
    ...
    try:
        ...
        return jsonify({"success": True, "active_signals": active_data, "count": len(active_data)})

    except Exception as e:
        logger.error(f"Active signals API error: {e}")
        return jsonify({"success": False, "error": str(e), "active_signals": [], "count": 0})
...
def api_signal_monitor_completed():
    ...
    try:
        ...
        return jsonify({"success": False, "error": str(e), "completed_signals": [], "count": 0})
...
def api_get_venom_risk_config():
    ...
    try:
        ...
        loss_ratio = (
            config["current_daily_loss"] / config["max_daily_loss_cap_percent"]
            if config["max_daily_loss_cap_percent"] > 0
            else 0
        )
        trades_ratio = (
            config["trades_today"] / config["max_trades_per_session"] if config["max_trades_per_session"] > 0 else 0
        )
...
def api_venom_risk_status():
    ...
    try:
        ...
        if not os.path.exists(risk_config_file):
            return jsonify({"risk_status": "UNKNOWN", "risk_color": "#888", "message": "Risk config not found"})
        ...
        return jsonify({"risk_status": "ERROR", "risk_color": "#ff0040", "message": f"Error: {str(e)}"})
    ...
    try:
        ...
        for file_path in mission_files:
            try:
                ...
                if mission.get("source") == "venom_scalp_master" and mission.get("status") == "active":
...
def get_ml_status():
    ...
    try:
        ...
        return jsonify(
            {"accuracy": 0, "last_retrain": "Unknown", "total_cycles": 0, "training_status": "Error", "error": str(e)}
        )
                    ...
                    try:
                        ...
                        if pips > best_pips:
                            ...
                            best_trade = {
                                ...
                                "runtime_minutes": (
                                    entry.get("runtime_minutes") or entry.get("runtime_seconds", 0) / 60
                                    if entry.get("runtime_seconds")
                                    else 0
                                ),
                            ...
                            if timestamp_str:
                                ...
                                if timestamp >= yesterday:
                                    ...
                                    if execution_count > 0 or len(users_fired) > 0:
                                        ...
                                        if first_execution:
                                            try:
                                                fire_time = datetime.fromisoformat(
                                                    first_execution.replace("Z", "+00:00")
                                                )
        ...
        return jsonify(
            {
                ...
                "user_engagement": {"active_fire_users_24h": 0, "total_fires_24h": 0},
                "error": str(e),
            }
        )
...
def api_elite_guard_analytics():
    ...
    try:
        ...
        if citadel_stats["shield_approved_total"] > 0:
            shield_approved_rate = (
                citadel_stats["shield_approved_fired"] / citadel_stats["shield_approved_total"]
            ) * 100

        if citadel_stats["shield_unverified_total"] > 0:
            shield_unverified_rate = (
                citadel_stats["shield_unverified_fired"] / citadel_stats["shield_unverified_total"]
            ) * 100
        ...
        return jsonify(
            {
                "elite_guard_overview": {"signals_generated_24h": 0, "signals_fired_24h": 0, "completion_rate": "0%"},
                "pattern_performance": {},
                "citadel_shield_impact": {},
                "error": str(e),
            }
...
def api_expiring_signals():
    ...
    try:
        ...
        return jsonify({"critical_signals": [], "expiry_trends": {}, "error": str(e)})
...
def api_fire_command_analytics():
    ...
    try:
        ...
        try:
            ...
            log_files = ["/root/HydraX-v2/bitten_production_bot.log", "/root/HydraX-v2/webapp_server.log"]
        ...
        return jsonify(
            {
                "execution_metrics": {"fire_attempts_24h": 0, "fire_successes_24h": 0, "success_rate": "0%"},
                "failure_analysis": {},
                "error": str(e),
            }
        )
...
def ml_performance():
    ...
    try:
        ...
        if os.path.exists(ml_file):
            with open(ml_file, "r") as f:
                ...
                sorted_patterns = sorted(
                    pattern_performance.items(), key=lambda x: x[1]["expected_value"], reverse=True
                )
        ...
        gate_file = sorted([f for f in os.listdir("/root/elite_guard") if f.startswith("active_gates_")], reverse=True)
        if gate_file:
        ...
        return jsonify({"ml_performance": ml_stats, "last_updated": datetime.now().strftime("%H:%M:%S")})
...
def health_check():
    ...
    try:
        ...
        health_status["checks"]["service"] = {"status": "ok", "message": "Service is running"}
        ...
        try:
            ...
            if last_command:
                ...
                health_status["checks"]["last_command"] = {"status": "ok", "message": "No commands executed yet"}
...
def handle_connect():
    ...
    emit(
        ...
        {"commander_id": session.get("commander_id"), "access_level": session.get("access_level")},
    )
