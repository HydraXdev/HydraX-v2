...
for pattern in pattern_stats:
    ...
    if pattern_stats[pattern]["confidences"]:
        pattern_stats[pattern]["avg_confidence"] = sum(pattern_stats[pattern]["confidences"]) / len(
            pattern_stats[pattern]["confidences"]
        )
...
overall_row = (
    f"{'OVERALL':<25} {total_signals:<6} {total_wins:<5} {total_losses:<5} {total_pending:<5} {overall_win_rate:<6.1f}"
)
print(overall_row)
print()
...
for sig_class, stats in class_stats.items():
    ...
    print(
        f"{sig_class:<12} Total: {stats['total']:<4} Wins: {stats['wins']:<3} Losses: {stats['losses']:<3} Win Rate: {stats['win_rate']:<6.1f}%"
    )
...
if qualified_patterns:
    ...
    for pattern, stats in best:
        print(
            f"   {pattern}: {stats['win_rate']:.1f}% win rate ({stats['wins']}/{stats['wins']+stats['losses']} signals)"
        )
