...
class ConfidenceCalibrator:
    ...
    def _load_calibration_map(self) -> Dict:
        ...
        try:
            ...
            return {"calibration_offsets": {}, "last_updated": 0, "version": "1.0"}
    ...
    def analyze_confidence_calibration(self, days_back: int = None) -> Dict:
        ...
        try:
            ...
            for bucket_min, bucket_max in self.confidence_buckets:
                bucket_signals = [s for s in signals if bucket_min <= s["confidence"] < bucket_max]
    ...
    def _analyze_bucket(self, signals: List[Dict], bucket_min: float, bucket_max: float) -> Dict:
        ...
        is_significantly_miscalibrated = not (confidence_interval[0] <= expected_win_rate <= confidence_interval[1])
        ...
        if abs(calibration_error) > self.confidence_tolerance and is_significantly_miscalibrated:
            recommended_adjustment = (
                -calibration_error
            )  # Negative because we adjust confidence down if win rate is lower
        else:
    ...
    def _calculate_confidence_interval(
        self, wins: int, total: int, confidence_level: float = 0.95
    ) -> Tuple[float, float]:
    ...
    def get_calibration_status(self) -> Dict:
        ...
        try:
            ...
            miscalibrated_buckets = sum(
                1 for bucket in analysis["bucket_analysis"].values() if bucket["is_significantly_miscalibrated"]
            )

            avg_calibration_error = np.mean(
                [abs(bucket["calibration_error"]) for bucket in analysis["bucket_analysis"].values()]
            )
    ...
    def run_calibration_update(self) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Calibration update completed. Applied {adjustment_result.get('adjustments_applied', 0)} adjustments."
            )
            return summary
