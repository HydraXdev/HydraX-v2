...
def setup_logging(
    ...
    if enable_console:
        ...
        if enable_structured:
            console_format = (
                "%(asctime)s | %(levelname)-8s | %(component)-15s | " "%(filename)s:%(lineno)d | %(message)s"
            )
...
def setup_component_files(
    base_level: int, max_file_size: int, backup_count: int
    ...
    for component, file_path in component_files.items():
        ...
        handler = logging.handlers.RotatingFileHandler(
            file_path, maxBytes=max_file_size, backupCount=backup_count, encoding="utf-8"
        )
        ...
        formatter = StructuredFormatter(
            "%(timestamp)s | %(levelname)-8s | %(component)-15s | " "%(filename)s:%(lineno)d | %(message)s"
        )
...
def log_system_info():
    ...
    env_vars = ["ENGAGEMENT_DB_URL", "ENGAGEMENT_LOG_LEVEL", "ENGAGEMENT_DEBUG", "REDIS_URL", "FUSION_DASHBOARD_PORT"]
...
def setup_default_logging():
    ...
    config = setup_logging(level=level, enable_console=True, enable_file=True, enable_structured=True)
