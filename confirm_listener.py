...
def update_fire(m):
    ...
    try:
        ...
        if target_uuid:
            ...
            cur.execute(
                "UPDATE fires SET status=?, ticket=?, price=? WHERE fire_id=?", (db_status, ticket, price, fire_id)
            )
        ...
        if db_status == "FILLED" and rows_affected > 0:
            ...
            if fire_data:
                ...
                if FSM_AVAILABLE and ticket and symbol and direction:
                    try:
                        ...
                        if actual_entry and actual_sl:
                            ...
                            exit_profile_manager.on_position_open(
                                ...
                                tp_px=(
                                    tp if tp else float(actual_entry) + (float(actual_entry) - float(actual_sl)) * 10
                                ),  # Far TP for runner
                            ...
                            LOG.info(
                                f"✅ FSM REGISTERED ticket={ticket} {symbol} {direction} entry={actual_entry:.5f} r_pips={r_pips} tier={tier} timer={max_hold_min}min"
                            )
                        else:
                    ...
                    if FSM_AVAILABLE:
                        LOG.warning(
                            f"Skipping FSM registration - missing data: ticket={ticket}, symbol={symbol}, direction={direction}"
                        )
...
def handle_enhanced_heartbeat(m):
    ...
    try:
        ...
        if db_count != open_positions_count:
            LOG.warning("📊 POSITION MISMATCH: EA reports %d, DB shows %d - syncing...", open_positions_count, db_count)
        ...
        if len(positions) < db_count:
            ...
            for db_fire_id, symbol, user_id in db_positions:
                if db_fire_id not in ea_fire_ids:
                    LOG.warning("🔍 Position %s (%s) missing from EA - possibly closed manually", db_fire_id, symbol)
                    ...
                    closed_positions.append({"fire_id": db_fire_id, "symbol": symbol, "user_id": user_id})
            ...
            if closed_positions and EVENT_BUS_AVAILABLE and event_producer:
                for pos in closed_positions:
                    try:
                        ...
                        LOG.info(
                            "📡 Event Bus: Published slot release for %s (user %s)", pos["fire_id"], pos["user_id"]
                        )
                    except Exception as e:
...
def handle_position_monitoring(m):
    ...
    try:
        ...
        if command_type == "position_opened":
            ...
            cur.execute(
                "UPDATE fires SET status = 'FILLED', ticket = ?, price = ? WHERE fire_id = ?", (ticket, price, fire_id)
            )
...
def main():
    ...
    while True:
        ...
        if msg_type in ("confirmation", "close_confirmation", "filled", "failed"):
            ...
            LOG.info(
                "🎯 HYBRID_EVENT: fire_id=%s event=%s partial=%s%% trail=%s pips",
                m.get("fire_id", "?"),
                m.get("event", "?"),
                m.get("percent", "?"),
