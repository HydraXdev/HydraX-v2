...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/root/HydraX-v2/logs/core_crypto_integration.log"), logging.StreamHandler()],
)
...
class CORECryptoIntegrationBridge:
    ...
    def _update_webapp_display(self, crypto_signal: Dict):
        ...
        try:
            ...
            payload = {"signal_data": crypto_signal, "source": "CORE_CRYPTO_ENGINE", "timestamp": time.time()}
        ...
        return {
            **self.integration_stats,
            ...
            "last_signal_age_seconds": (
                time.time() - self.integration_stats["last_signal_time"]
                if self.integration_stats["last_signal_time"] > 0
                else 0
            ),
