...
class CryptoEnhancedAnalyzer:
    ...
    def __init__(self, zmq_port=5560):
        ...
        self.crypto_symbols = ["BTCUSD", "ETHUSD", "XRPUSD", "ADAUSD", "LTCUSD", "BCHUSD", "EOSUSD", "XLMUSD"]

        # Forex symbols for correlation analysis
        self.forex_symbols = ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "NZDUSD", "USDCAD"]
        ...
        try:
            ...
            for symbol in self.all_symbols:
                if len(self.m5_data[symbol]) >= self.min_correlation_periods:
                    ...
                    price_series[symbol] = [
                        candle.close for candle in list(self.m5_data[symbol])[-self.min_correlation_periods :]
                    ]
    ...
    def _classify_volume_profile(
        self, volumes: List[float], prices: List[float], avg_volume: float, volume_std: float
    ) -> VolumeProfile:
    ...
    def _analyze_symbol_for_signal(self, symbol: str) -> Optional[MultiTimeframeSignal]:
        ...
        try:
            ...
            if len(self.m1_data[symbol]) < 20 or len(self.m5_data[symbol]) < 10 or len(self.m15_data[symbol]) < 5:
                return None
            ...
            bullish_signals = sum(
                [
                    signal.m1_score > 0.5,
                    signal.m5_score > 0.5,
                    signal.m15_score > 0.5,
                    signal.volume_score > 0.6,
                    signal.pattern_score > 0.6,
    ...
    def get_analysis_status(self) -> Dict[str, Any]:
        ...
        try:
            status = {
                ...
                "support_resistance": {"symbols_analyzed": len(self.support_resistance_levels)},
                "timestamp": time.time(),
            }
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            logger.info(
                f"📊 System Status: {status['symbols_tracked']} symbols, "
