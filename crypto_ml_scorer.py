...
class CryptoMLScorer:
        ...
        try:
            ...
            if tick_entry["volume"] > 0:
                self.volume_data[symbol].append(
                    {"timestamp": tick_entry["timestamp"], "volume": tick_entry["volume"], "price": tick_entry["price"]}
                )
    ...
    def extract_features(self, symbol: str, base_score: float, timeframe_data: Optional[Dict] = None) -> MarketFeatures:
        """
        ...
        try:
            features = MarketFeatures(symbol=symbol, timestamp=time.time(), base_score=base_score)
    ...
    def predict_signal_quality(self, features: MarketFeatures) -> Tuple[float, Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(
                f"🎯 ML Prediction for {features.symbol}: {enhanced_score:.1f} "
    ...
    def record_signal_outcome(self, signal_id: str, features: MarketFeatures, outcome: bool, outcome_data: Dict):
    ...
    def _load_or_create_model(self):
        ...
        try:
            if model_path.exists():
                ...
                logger.info(
                    f"✅ Model loaded - Samples: {self.model_sample_count}, "
        ...
        while True:
            try:
                ...
                if days_since_training >= self.config["retrain_days"]:
                    logger.info(
                        f"🔄 Starting periodic model retraining (last trained {days_since_training:.1f} days ago)"
                    )
                    self.retrain_model()
    ...
    def retrain_model(self) -> bool:
        ...
        try:
            ...
            cv_scores = cross_val_score(
                new_model, X_train_scaled, y_train, cv=self.config["cv_folds"], scoring="accuracy"
            )
            test_accuracy = accuracy_score(y_test, new_model.predict(X_test_scaled))

            logger.info(
                f"📊 Model Performance - CV Score: {cv_scores.mean():.3f}±{cv_scores.std():.3f}, "
            ...
            if cv_scores.mean() >= self.config["score_threshold"]:
                ...
                logger.warning(
                    f"❌ New model quality insufficient: {cv_scores.mean():.3f} < {self.config['score_threshold']}"
                )
                return False
    ...
    def get_model_stats(self) -> Dict[str, Any]:
        ...
        return {
            "model_version": self.model_version,
            "trained_at": (
                datetime.fromtimestamp(self.model_trained_at).isoformat() if self.model_trained_at > 0 else None
            ),
            "sample_count": self.model_sample_count,
    ...
    def enhance_signal_score(
        self, symbol: str, base_score: float, timeframe_data: Optional[Dict] = None
        ...
        try:
            ...
            logger.info(
                f"🚀 Signal enhanced: {symbol} {base_score:.1f} → {final_score:.1f} "
