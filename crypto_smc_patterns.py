...
class CryptoLiquiditySweepDetector:
    ...
    def detect_liquidity_sweep(
        self, symbol: str, m1_data: List[CryptoCandle], m5_data: List[CryptoCandle]
    ) -> Optional[CryptoSMCSignal]:
        """
...
class CryptoOrderBlockDetector:
    ...
    def detect_order_block_bounce(
        self, symbol: str, m5_data: List[CryptoCandle], m15_data: List[CryptoCandle]
    ) -> Optional[CryptoSMCSignal]:
        """
        ...
        try:
            ...
            logger.info(
                f"   Key Level: {key_level:.2f} | Distance: {min_distance*100:.2f}% | Volume: {volume_surge:.1f}x"
            )
...
class CryptoFVGDetector:
    ...
    def detect_fair_value_gap(self, symbol: str, m5_data: List[CryptoCandle]) -> Optional[CryptoSMCSignal]:
        ...
        try:
            ...
            logger.info(
                f"   Gap: {best_gap['gap_low']:.2f} - {best_gap['gap_high']:.2f} | Size: {best_gap['size_pct']:.2f}%"
            )
        ...
        try:
            ...
            for i in range(2, len(recent_candles) - 2):
                ...
                if (
                    ...
                    and candle.low < prev1.low
                    and candle.low < next1.low
                    and candle.low < next2.low
                ):
                    swing_lows.append(
                        {"price": candle.low, "timestamp": candle.timestamp, "index": i, "strength": candle.volume}
                    )
            ...
            logger.info(
                f"   Broken Level: {break_detected['broken_level']:.2f} | Break: {break_detected['break_pct']:.2f}%"
            )
...
class CryptoCHoCHDetector:
    ...
    def detect_change_of_character(
        self, symbol: str, m15_data: List[CryptoCandle], m30_data: List[CryptoCandle]
    ) -> Optional[CryptoSMCSignal]:
        """
        ...
        try:
            ...
            logger.info(
                f"   Trend Change: {current_trend} → {choch_detected['type']} | Divergence: {choch_detected['volume_divergence']:.1f}%"
            )
...
class CryptoSMCEngine:
    ...
    def process_crypto_candle(self, symbol: str, timeframe: str, ohlcv_data: Dict):
        ...
        try:
            ...
            logger.debug(
                f"🕯️ {symbol} {timeframe} candle: O:{candle.open:.2f} H:{candle.high:.2f} L:{candle.low:.2f} C:{candle.close:.2f}"
            )
    ...
    def scan_crypto_patterns(self, symbol: str) -> List[CryptoSMCSignal]:
        ...
        try:
            ...
            logger.debug(
                f"📊 {symbol} data: M1:{len(m1_candles)} M5:{len(m5_candles)} M15:{len(m15_candles)} M30:{len(m30_candles)}"
            )
            ...
            logger.info(
                f"🎯 {symbol}: {len(patterns)} patterns detected, {len(quality_patterns)} above quality threshold"
            )
    ...
    def log_to_truth_tracker(self, signal: Dict):
        ...
        try:
            ...
            response = requests.post(self.truth_tracker_url, json=truth_entry, timeout=2)
...
if __name__ == "__main__":
    ...
    test_data = {
        "BTCUSD": {"price": 67245.50, "volume": 125.5, "spread": 25.0},
        "ETHUSD": {"price": 3456.78, "volume": 890.2, "spread": 2.5},
        "XRPUSD": {"price": 0.6234, "volume": 15000.0, "spread": 0.0008},
    }
