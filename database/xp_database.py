...
class XPDatabase:
    ...
    async def initialize_tables(self):
        ...
        async with self.get_connection() as conn:
            ...
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_press_pass_resets_reset_at ON press_pass_resets(reset_at DESC)"
            )
    ...
    async def add_xp(
        self, user_id: int, amount: int, description: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Tuple[XPBalance, XPTransaction]:
        """
    ...
    async def reset_press_pass_xp(self, user_id: int) -> Optional[PressPassReset]:
        ...
        async with self.transaction() as conn:
            ...
            """,
                ...
                xp_to_wipe,
                0,
                "Press Pass nightly XP reset",
                json.dumps({"reset_type": "nightly"}),
            )
            ...
            for user_id in user_ids:
                ...
                if row and row["current_balance"] > 0:
                    ...
                    """,
                        ...
                        0,
                        "Press Pass nightly XP reset",
                        json.dumps({"reset_type": "bulk_nightly"}),
                    )
    ...
    async def get_xp_statistics(self) -> Dict[str, Any]:
        ...
        async with self.get_connection() as conn:
            ...
            return {
                ...
                "top_holders": [{"user_id": row["user_id"], "balance": row["current_balance"]} for row in top_holders],
                "activity_24h": {
