...
class DirectionGate:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if symbol not in positions:
                    positions[symbol] = {"net_lots": 0.0, "tickets": [], "direction": None}
                ...
                if direction == "BUY":
                    ...
                    positions[symbol]["direction"] = (
                        Direction.BUY if positions[symbol]["net_lots"] > 0 else Direction.SELL
                    )
                else:
                    positions[symbol]["net_lots"] -= volume
                    positions[symbol]["direction"] = (
                        Direction.SELL if positions[symbol]["net_lots"] < 0 else Direction.BUY
                    )
                positions[symbol]["last_trade_time"] = last_update

                # Track positions for FIFO closing
                positions[symbol]["tickets"].append(
                    {"fire_id": fire_id, "volume": volume, "direction": direction, "last_update": last_update}
                )
    ...
    def check_direction(self, symbol: str, direction: Direction, lots: float) -> Tuple[GateAction, Dict]:
        ...
        if not self.ALLOW_OPPOSITE:
            ...
            logger.warning(
                f"🚫 BLOCKING HEDGE: {symbol} trying to {direction.value} with existing {current_direction.value} position of {net_lots:.2f} lots"
            )
            ...
            if self.REDUCE_ONLY:
                ...
                if symbol in self.position_cache:
                    tickets_to_close = [t["ticket"] for t in self.position_cache[symbol]["tickets"]]
    ...
    def get_close_commands(self, symbol: str) -> List[Dict]:
        ...
        # FIFO - oldest first
        tickets = sorted(self.position_cache[symbol]["tickets"], key=lambda x: x["created_at"])
    ...
    def update_position_closed(self, symbol: str, ticket: int):
        ...
        if symbol in self.position_cache:
            ...
            self.position_cache[symbol]["tickets"] = [
                t for t in self.position_cache[symbol]["tickets"] if t["ticket"] != ticket
            ]
