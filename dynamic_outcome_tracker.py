...
class DynamicOutcomeTracker:
    ...
    def __init__(self, 
        ...
        self.max_tracking_hours = 4  # Maximum tracking duration
    ...
    def _check_signal_outcome(self, tracking_info: Dict, bid: float, ask: float) -> bool:
        ...
        try:
            ...
            # Check for TP hit
            if direction == 'BUY' and current_price >= tp_level:
                self._record_outcome(tracking_info, 'WIN', tracking_info['target_pips'], current_price)
                return True
            elif direction == 'SELL' and current_price <= tp_level:
                self._record_outcome(tracking_info, 'WIN', tracking_info['target_pips'], current_price)
                return True
            
            # Check for SL hit
            if direction == 'BUY' and current_price <= sl_level:
                self._record_outcome(tracking_info, 'LOSS', -tracking_info['stop_pips'], current_price)
                return True
            elif direction == 'SELL' and current_price >= sl_level:
                self._record_outcome(tracking_info, 'LOSS', -tracking_info['stop_pips'], current_price)
                return True
    ...
    def get_tracking_statistics(self, days_back: int = 7) -> Dict:
        ...
        try:
            ...
            # Timing statistics
            avg_tracking_duration = sum(s['tracking_duration'] for s in tracked_signals) / total_signals / 60
            avg_timeout_used = sum(s['dynamic_timeout_used'] or 0 for s in tracked_signals) / total_signals
