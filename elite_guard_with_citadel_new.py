...
class EliteGuardWithCitadel:
    """Elite Guard with proper tick processing and candle building"""
    ...
    def __init__(self):
        ...
        # Market data storage - FIXED to match working version
        self.tick_data = defaultdict(lambda: deque(maxlen=200))
        ...
        self.MIN_CONFIDENCE = 30 # Very low for testing
        self.COOLDOWN_MINUTES = 5 # Down from 15 minutes
                ...
                if candle_range > 0:
                    ...
                    if body / candle_range > 0.6:  # Strong body (reduced from 0.7)
                        ...
                        if block_low <= current_price <= block_low + (block_range * 0.4):
                            ...
                            confidence = 65 + (momentum * 0.2) + (volume_quality * 0.1)
    ...
    def build_candles_from_tick(self, symbol: str, tick_data: dict):
        ...
        try:
            ...
            for minute in completed_minutes:
                ...
                # Build a new M5 candle from the LAST 5 M1 candles
                if len(self.m1_data[symbol]) >= 5:
                    # Only add if we don't have this M5 yet or need to update
                    last_5_m1 = list(self.m1_data[symbol])[-5:]
                    m5_timestamp = last_5_m1[0]['timestamp']
                    
                    # Check if we already have this M5 candle
                    if not self.m5_data[symbol] or self.m5_data[symbol][-1]['timestamp'] != m5_timestamp:
    ...
    def apply_ml_filter(self, signal, session: str) -> tuple[bool, str, float]:
        """Apply ML filtering with balanced thresholds"""
    ...
    def scan_for_patterns(self):
        """Scan all symbols for patterns with ML filtering"""
        ...
        for symbol in symbols_to_scan:
            ...
            if patterns:
                ...
                if best_pattern.confidence >= 45:  # Lowered from 55
                    # Generate signal
                    ...
                    if protected_signal and protected_signal.get('confidence', 0) >= 60:
