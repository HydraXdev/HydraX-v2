...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/tmp/elite_guard_zmq_relay.log"), logging.StreamHandler()],
)
...
class EliteGuardZMQRelay:
        ...
        while self.running:
            try:
                ...
                if success:
                    ...
                    logger.info(
                        f"✅ Signal relayed: {signal_data.get('signal_id')} "
    ...
    def _convert_to_webapp_format(self, elite_signal: Dict) -> Dict:
        ...
        webapp_signal = {
            ...
            "pattern_type": elite_signal.get(
                "pattern_type", elite_signal.get("pattern", "UNKNOWN")
            ),  # CRITICAL: BittenCore needs this for AUTO fire
        ...
        if (webapp_signal.get("stop_loss") is None or webapp_signal.get("stop_loss") == 0) or (
            ...
            logger.error(
                f"   Entry: {webapp_signal.get('entry_price')}, SL: {webapp_signal.get('stop_loss')}, TP: {webapp_signal.get('take_profit')}"
            )
            logger.error(
                f"   Original signal had: stop_loss={elite_signal.get('stop_loss')}, take_profit={elite_signal.get('take_profit')}"
            )
