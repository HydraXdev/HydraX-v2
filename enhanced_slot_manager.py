...
class EnhancedSlotManager:
    ...
    def handle_position_closed_event(self, event_data):
        ...
        if user_id in self.user_slots:
            ...
            self.user_slots[user_id]["positions"] = [
                pos for pos in self.user_slots[user_id]["positions"] if pos.get("fire_id") != fire_id
            ]

            self.user_slots[user_id]["last_update"] = int(time.time())

            logger.info(
                f"✅ Released slot for user {user_id}: {self.user_slots[user_id]['used']}/{self.user_slots[user_id]['total']} used"
            )
    ...
    def get_user_slot_status(self, user_id):
        ...
        return self.user_slots.get(user_id, {"used": 0, "total": 3, "positions": [], "last_update": int(time.time())})
    ...
    def can_user_fire(self, user_id):
        ...
        logger.info(
            f"🎯 User {user_id} slot check: {slot_status['used']}/{slot_status['total']} used, {available} available"
        )
