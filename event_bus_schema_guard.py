...
class EventBusSchemaGuard:
    ...
    def setup_logging(self):
        logging.basicConfig(
            level=logging.INFO,
            format='%(asctime)s [SCHEMA_GUARD] %(levelname)s: %(message)s'
        )
        ...
        for field, expected_types in schema["field_types"].items():
            if field in event_data:
                ...
                if not isinstance(value, expected_types):
                    violations.append(f"Field {field} has wrong type: {type(value).__name__}, expected {expected_types}")
    ...
    def safe_emit_with_validation(self, event_type: str, event_data: Dict[str, Any], 
                                 emit_func, dlq_func=None) -> bool:
        """
