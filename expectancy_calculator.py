...
class ExpectancyCalculator:
    ...
    def calculate_expectancy(self, pattern_type: str, symbol: str = None, window_size: int = None) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Calculated expectancy for {pattern_type}: {expectancy:.4f} (Safety: {safety_analysis['safety_status']})"
            )
            return result

        except Exception as e:
            logger.error(f"Error calculating expectancy for {pattern_type}: {e}")
            return {"pattern_type": pattern_type, "expectancy": None, "status": "ERROR", "error": str(e)}
        ...
        try:
            ...
            report = {
                ...
                "summary": {
                    ...
                    "safe_zone_patterns": len(
                        [p for p in all_expectancies.values() if p.get("safety_status") == "SAFE_ZONE"]
                    ),
                    "unprofitable_patterns": len(
                        [p for p in all_expectancies.values() if p.get("expectancy", 0) < -self.safety_zone_pct]
                    ),
                },
            }
...
def main():
    ...
    for pattern, data in expectancies.items():
        if data.get("expectancy") is not None:
            print(
                f"  {pattern}: EV={data['expectancy']:.4f}, WR={data['win_rate']:.2%}, Status={data.get('safety_status', 'UNKNOWN')}"
            )
