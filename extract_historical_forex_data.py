...
class HistoricalForexExtractor:
    ...
    def __init__(self):
        ...
        self.required_pairs = ["EURUSD", "GBPUSD", "USDJPY", "USDCAD", "AUDUSD", "GBPJPY"]
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler("/root/HydraX-v2/logs/historical_extraction.log"), logging.StreamHandler()],
        )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                while current_date < end_date:
                    ...
                    # Execute MT5 script
                    payload = {"command": mt5_script, "type": "powershell"}
                            ...
                            if result.get("success"):
                                ...
                                try:
                                    ...
                                    if isinstance(chunk_data, list):
                                        ...
                                        self.logger.info(
                                            f"✅ Extracted {len(chunk_data)} records for {pair} ({current_date.date()} to {chunk_end.date()})"
                                        )
                                    else:
    ...
    async def generate_realistic_synthetic_data(
    ...
    def validate_data_quality(self, pair: str) -> Dict[str, any]:
        ...
        for _, gap in large_gaps.iterrows():
            validation["gaps"].append(
                {"timestamp": gap["timestamp"], "gap_minutes": gap["time_diff"].total_seconds() / 60}
            )
        ...
        if (
            len(validation["gaps"]) > 100
            or validation["avg_records_per_day"] < 500
            or len(validation["data_anomalies"]) > 0
        ):
            validation["valid"] = False

        self.logger.info(
            f"📊 {pair} validation: {validation['total_records']:} records, "
    ...
    def export_to_csv(self, pair: str) -> str:
        ...
        df["timestamp"] = pd.to_datetime(df["timestamp"])
        df["spread_pips"] = (
            ((df["ask_price"] - df["bid_price"]) * 10000)
            if "JPY" not in pair
            else ((df["ask_price"] - df["bid_price"]) * 100)
        )
    ...
    async def run_full_extraction(self) -> Dict[str, any]:
        ...
        self.logger.info(
            f"📅 Target period: {self.start_date.date()} to {self.end_date.date()} ({self.total_days} days)"
        )
        ...
        for pair in self.required_pairs:
            ...
            if pair_analysis["needs_extraction"]:
                self.logger.info(
                    f"📊 Extracting data for {pair} (current completeness: {pair_analysis['completeness']:.1f}%)"
                )
        ...
        for pair in self.required_pairs:
            try:
                ...
                results["exported_files"][pair] = {"csv": csv_path, "json": json_path}
            except Exception as e:
        ...
        with open(report_path, "w") as f:
            ...
            for pair, validation in results["validation_results"].items():
                ...
                if validation.get("session_distribution"):
                    ...
                    f.write(
                        f"  Sessions: LONDON({sessions.get('LONDON', 0)}) "
