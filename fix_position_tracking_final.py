...
def clear_all_tracked_positions():
    ...
    try:
        ...
        if open_count > 0:
            ...
            actual_count = input(f"\n❓ How many positions do you ACTUALLY have open in MT5? (database shows {open_count}): ").strip()
    ...
    try:
        ...
        if positions:
            ...
            for fire_id, symbol, direction, ticket, last_update in positions:
                logger.info("  • %s: %s %s (ticket %s) - %s", 
                           fire_id, symbol, direction, ticket, last_update)
        else:
