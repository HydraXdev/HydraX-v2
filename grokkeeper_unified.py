...
class GrokkeeperML:
    ...
    def prepare_features(self, df):
        ...
        if "win" in df.columns:
            ...
            target = df["outcome"].apply(
                lambda x: 1 if x in ["WIN", "TP", "PROFIT"] else 0 if x in ["LOSS", "SL", "STOP"] else None
            )
    ...
    def train_model(self, force=False):
        ...
        # Feature importance
        importances = pd.DataFrame({"feature": X.columns, "importance": self.model.feature_importances_}).sort_values(
    ...
    def publish_prediction(self, prediction):
        ...
        if prediction:
            ...
            if self.publisher:
                ...
                logger.info(
                    f"Published prediction: {prediction['signal_id']} - Win prob: {prediction['win_probability']:.2%}"
                )
