...
class HealthMonitorAgent:
        ...
        while self.is_monitoring:
            try:
                ...
                result = subprocess.run(
                    ["tasklist", "/FI", "IMAGENAME eq terminal64.exe"], capture_output=True, text=True, check=False
                )
    ...
    def check_user_mt5(self, user_id, user_path):
        ...
        try:
            ...
            result = subprocess.run(
                ["tasklist", "/FI", f"IMAGENAME eq terminal64.exe"], capture_output=True, text=True, check=False
            )
        ...
        while self.is_monitoring:
            try:
                ...
                cpu_info = subprocess.run(
                    ["wmic", "cpu", "get", "loadpercentage", "/value"], capture_output=True, text=True, check=False
                )
                ...
                disk_info = subprocess.run(
                    ["wmic", "logicaldisk", "where", "size!=0", "get", "size,freespace", "/value"],
                    capture_output=True,
                    text=True,
                    check=False,
                )
    ...
    def parse_system_metrics(self, cpu_output, memory_output, disk_output):
        ...
        metrics = {"cpu_percent": 0, "memory_percent": 0, "disk_free_gb": 0, "mt5_process_count": 0}
...
class HealthAPI(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            ...
            response = {
                "recent_checks": [
                    {"component": row[0], "status": row[1], "details": row[2], "timestamp": row[3]} for row in checks
                ]
            }
            self.wfile.write(json.dumps(response).encode())
