...
TELEGRAM_BOT_TOKEN = "7854827710:AAE9kCptkoSl8lmQwmX940UMqFWOb3TmTI0"
...
class HUDWatchdog:
    ...
    def check_health(self) -> tuple[bool, int, str]:
        ...
        """
        try:
            response = requests.get(HEALTH_CHECK_URL, timeout=HEALTH_CHECK_TIMEOUT, allow_redirects=False)
    ...
    def run_check_cycle(self):
        ...
        if restart_success:
            alert_message = (
                f"⚠️ HUD WebApp was down. Restarted by watchdog at {timestamp}.\n\n"
                f"Restart #{self.restart_count} - Server is now healthy."
            )
        else:
            alert_message = (
                f"🚨 HUD WebApp is down and restart FAILED at {timestamp}.\n\n"
