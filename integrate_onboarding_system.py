...
def add_onboarding_routes(webapp_path):
    ...
    try:
        ...
        registration_lines = ["", "# Register Press Pass API", "press_pass_manager = register_press_pass_api(app)", ""]
            ...
            if line.strip().startswith("from flask import"):
                ...
                if "redirect" not in line:
                    lines[i] = line.replace(
                        "from flask import", "from flask import redirect, send_from_directory,"
                    ).replace(",,", ",")
                import_index = i + 1
                break
...
def create_test_data():
    ...
    test_data = {
        "press_passes": [{"email": "test@example.com", "pass_id": "PRESS-TEST-001", "status": "active"}],
        "analytics": {"total_visits": 1247, "conversion_rate": 23.4, "avg_time_on_page": 45},
    }
