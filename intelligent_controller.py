...
class IntelligentController:
            ...
            if method["type"] == "http":
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        f"http://{self.target_ip}:{method['port']}/health", timeout=aiohttp.ClientTimeout(total=5)
                    ) as response:
                        return response.status == 200

            elif method["type"] == "websocket":
                async with websockets.connect(f"ws://{self.target_ip}:{method['port']}", timeout=5) as websocket:
                ...
                ssh.connect(self.target_ip, port=method["port"], username="Administrator", timeout=5)
                ssh.close()
                return True
    ...
    async def _execute_websocket(self, command: str, command_type: str) -> Dict[str, Any]:
        ...
        async with websockets.connect(f"ws://{self.target_ip}:{self.active_connection['port']}") as websocket:
            await websocket.send(json.dumps({"command": command, "type": command_type}))
