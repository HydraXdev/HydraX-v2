    ...
    def close_trade(self, trade_id: str, reason: str = "Manual close") -> Dict:
        ...
        try:
            ...
            response = requests.post(
                f"{self.farm_url}/api/close_position", json={"ticket": trade_id, "reason": reason}, timeout=5
            )
...
def register_live_trade_endpoints(app: Flask):
    ...
    def get_live_trades():
        ...
        return jsonify({"success": True, "trades": trades, "account": account, "timestamp": time.time()})
    ...
    def get_trade_details(trade_id):
        ...
        if trade:
            return jsonify({"success": True, "trade": trade, "timestamp": time.time()})
        else:
    ...
    def get_account_info():
        ...
        return jsonify({"success": True, "account": account, "timestamp": time.time()})
    ...
    def update_loop():
        ...
        while True:
            try:
                ...
                socketio.emit("live_trades_update", {"trades": trades, "account": account, "timestamp": time.time()})
