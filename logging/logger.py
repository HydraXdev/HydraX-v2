...
class DatabaseHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            # Create metadata
            metadata = {"file": record.pathname, "line": record.lineno, "function": record.funcName}
...
class ForexVPSLogger(BittenLogger):
    ...
    def log_api_request(self, endpoint: str, request_data: Dict[str, Any], user_id: str = None):
        ...
        self.info("ForexVPS API Request: {endpoint}", endpoint=endpoint, user_id=user_id, request_data=request_data)

    def log_api_response(self, endpoint: str, response_data: Dict[str, Any], response_time: int, user_id: str = None):
    ...
    def log_api_error(self, endpoint: str, error: str, user_id: str = None):
        ...
        self.error("ForexVPS API Error: {endpoint} - {error}", endpoint=endpoint, error=error, user_id=user_id)

    def log_trade_execution(self, user_id: str, symbol: str, direction: str, result: Dict[str, Any]):
        """Log trade execution"""
...
class SecurityLogger(BittenLogger):
    ...
    def log_authentication(self, user_id: str, success: bool, ip_address: str = None):
        ...
        if success:
            self.info("Authentication successful: {user_id}", user_id=user_id, ip_address=ip_address)
        else:
            self.warning("Authentication failed: {user_id}", user_id=user_id, ip_address=ip_address)
    ...
    def log_rate_limit_exceeded(self, user_id: str, endpoint: str):
        ...
        self.warning("Rate limit exceeded: {user_id} on {endpoint}", user_id=user_id, endpoint=endpoint)

    def log_suspicious_activity(self, user_id: str, activity: str, details: Dict[str, Any]):
        """Log suspicious activity"""
        self.critical(
            "Suspicious activity detected: {user_id} - {activity}", user_id=user_id, activity=activity, **details
        )
