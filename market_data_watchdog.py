...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/root/HydraX-v2/logs/market_data_watchdog.log"), logging.StreamHandler()],
)
...
class MarketDataWatchdog:
    ...
    def send_telegram_alert(self, message: str):
        ...
        try:
            ...
            payload = {"chat_id": ALERT_USER_ID, "text": alert_text, "parse_mode": "Markdown"}
    ...
    def run_health_check(self) -> bool:
        ...
        if not all_healthy:
            ...
            if self.consecutive_failures == 1 or self.consecutive_failures % 5 == 0:
                alert_message = (
                    f"Market Data Health Check Failed!\n\nConsecutive failures: {self.consecutive_failures}\n\nErrors:\n"
                    + "\n".join(errors)
                )
            ...
            if self.consecutive_failures >= 2:
                ...
                if restart_success:
                    self.send_telegram_alert(
                        f"✅ Market data service restarted successfully after {self.consecutive_failures} failures.\n\nTotal restarts today: {self.total_restarts}"
                    )
                else:
                    self.send_telegram_alert(
                        f"❌ CRITICAL: Failed to restart market data service!\n\nConsecutive failures: {self.consecutive_failures}\nManual intervention required!"
                    )
        else:
        ...
        while True:
            try:
                ...
                if int(time.time()) % 600 == 0:
                    logger.info(
                        f"📊 Watchdog status: {self.consecutive_failures} consecutive failures, {self.total_restarts} total restarts"
                    )
