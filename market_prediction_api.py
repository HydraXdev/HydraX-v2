...
def health_check():
    ...
    return jsonify(
        {"status": "healthy", "timestamp": datetime.now().isoformat(), "service": "AI Market Prediction API"}
    )
        ...
        response = {
            ...
            "data": {
                ...
                "predictions": {
                    "1h": {
                        ...
                        "change_percent": round(
                            (prediction.predicted_price_1h / prediction.current_price - 1) * 100, 2
                        ),
                    },
                    "24h": {
                        "price": prediction.predicted_price_24h,
                        "change_percent": round(
                            (prediction.predicted_price_24h / prediction.current_price - 1) * 100, 2
                        ),
                    },
                    "7d": {
                        "price": prediction.predicted_price_7d,
                        "change_percent": round(
                            (prediction.predicted_price_7d / prediction.current_price - 1) * 100, 2
                        ),
                    },
                },
                "analysis": {
                ...
                "personality_commentary": (
                    prediction.personality_commentary
                    if not personality_filter
    ...
    try:
        ...
        for symbol, pred in zip(symbols, predictions):
            if isinstance(pred, Exception):
                errors.append({"symbol": symbol, "error": str(pred)})
            else:
                results.append(
                    {
                        "symbol": pred.symbol,
        ...
        return jsonify(
            {"success": True, "data": {"results": results, "errors": errors, "timestamp": datetime.now().isoformat()}}
        )
...
def get_personalities():
    ...
    personalities = [
        {"id": p.value, "name": p.value.replace("_", " ").title(), "description": get_personality_description(p)}
        for p in TradingPersonality
    ]

    return jsonify({"success": True, "data": personalities})
...
def get_sentiments():
    ...
    sentiments = [
        {"value": s.value, "name": s.value.replace("_", " ").title(), "description": get_sentiment_description(s)}
...
async def get_report(symbol):
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "data": {"symbol": prediction.symbol, "report": report, "timestamp": prediction.timestamp.isoformat()},
            }
        )
