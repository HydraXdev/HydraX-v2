...
from crypto_ml_scorer import MarketFeatures, get_crypto_ml_scorer
    ...
    def process_smc_signal(self, smc_signal: Dict) -> Tuple[Dict, Dict]:
        ...
        try:
            ...
            enhanced_score, ml_metadata = self.ml_scorer.enhance_signal_score(
                symbol=symbol, base_score=base_score, timeframe_data=timeframe_data
            )
            ...
            logger.info(
                f"🎯 Enhanced {pattern} signal for {symbol}: "
        ...
        try:
            ...
            if "ml_metadata" in outcome_data and "features" in outcome_data["ml_metadata"]:
                ...
                self.ml_scorer.record_signal_outcome(
                    signal_id=signal_id, features=features, outcome=outcome, outcome_data=outcome_data
                )
...
def elite_guard_ml_integration_example():
    ...
    example_signals = [
        {
            ...
            "stop_loss": 44500,
            "take_profit": 46000,
            "market_data": {"bid": 45000, "ask": 45010, "spread": 0.0002, "volume": 5000, "timestamp": time.time()},
            "timeframe_data": {
                "M1": [
        ...
        },
        {
            ...
            "stop_loss": 3250,
            "take_profit": 3100,
            "market_data": {"bid": 3199, "ask": 3201, "spread": 0.0006, "volume": 2000, "timestamp": time.time()},
        },
    ]
    ...
    print(
        f"  Base Signals: {perf['base_signals']['total']} total, " f"{perf['base_signals']['win_rate']:.1f}% win rate"
    )
    print(f"  ML Enhanced: {perf['ml_enhanced']['total']} total, " f"{perf['ml_enhanced']['win_rate']:.1f}% win rate")
