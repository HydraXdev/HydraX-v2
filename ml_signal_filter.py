...
class MLSignalFilter:
    ...
    def should_fire_signal(self, signal):
        ...
        # Log the decision
        logger.info(
            f"✅ APPROVED: {signal.get('symbol')} {signal.get('pattern')} "
    ...
    def update_performance(self, signal, outcome):
        ...
        performance["trades"].append(
            {"timestamp": time.time(), "win": is_win, "confidence": signal.get("confidence", 0)}
        )
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                self.performance_history[combo_key]["trades"].append(
                    {"timestamp": time.time() - 3600, "win": is_win, "confidence": confidence}  # Historical
                )
