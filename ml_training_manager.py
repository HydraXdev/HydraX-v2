...
from crypto_ml_scorer import CryptoMLScorer, MarketFeatures, SignalOutcome
...
class MLTrainingManager:
    ...
    def __init__(
        self, truth_tracker_path: str = "/root/HydraX-v2/truth_log.jsonl", ml_scorer: Optional[CryptoMLScorer] = None
    ):
    ...
    def _scheduled_training(self):
        ...
        try:
            ...
            if new_samples >= self.config["min_new_samples"]:
                ...
                logger.info(
                    f"⏸️ Insufficient new samples ({new_samples} < {self.config['min_new_samples']}), skipping training"
                )
        ...
        try:
            ...
            for window in ["24h", "7d", "30d"]:
                ...
                if previous_rate > 0 and current_rate < previous_rate - self.config["alert_threshold_drop"]:
                    logger.warning(
                        f"🚨 Performance Alert: {window} win rate dropped "
    ...
    def get_training_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_training_check": (
                datetime.fromtimestamp(self.last_training_check).isoformat() if self.last_training_check else None
            ),
...
def main():
    ...
    if manager.performance_history:
        ...
        for perf in manager.performance_history[-3:]:
            print(
                f"  {datetime.fromtimestamp(perf['timestamp'])}: "
