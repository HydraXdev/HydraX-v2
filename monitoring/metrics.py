...
from database.models import Signal, SystemLog, Trade, User, get_db
...
class MetricsCollector:
    ...
    async def collect_system_metrics(self) -> SystemMetrics:
        ...
        try:
            ...
            app_logger.debug(
                f"System metrics collected: CPU {cpu_percent}%, Memory {memory_percent}%, Disk {disk_percent}%"
            )
            return metrics
    ...
    async def collect_database_metrics(self) -> DatabaseMetrics:
        ...
        try:
            ...
            active_users_24h = db.query(User.id).join(Trade).filter(Trade.created_at >= yesterday).distinct().count()
            ...
            successful_trades_24h = (
                db.query(Trade).filter(Trade.created_at >= yesterday, Trade.status == "executed").count()
            )
            ...
            app_logger.debug(
                f"Database metrics collected: {total_users} users, {trades_24h} trades/24h, {win_rate_24h}% win rate"
            )
            return metrics
        ...
        try:
            ...
            for metric_type, metric_data in metrics.items():
                ...
                await self.redis_client.set(
                    latest_key, str(metric_data.__dict__ if hasattr(metric_data, "__dict__") else metric_data)
                )
                    ...
                    if start_time <= timestamp <= now:
                        ...
                        if data:
                            historical_data.append(
                                {"timestamp": timestamp, "data": data.decode() if isinstance(data, bytes) else data}
                            )
    ...
    def evaluate_forexvps_health(self, forexvps_metrics: ForexVPSMetrics) -> Dict[str, Any]:
        ...
        if forexvps_metrics.avg_response_time > self.critical_thresholds["forexvps_response_time"]:
            ...
            health_status["warnings"].append(
                f"Elevated ForexVPS response time: {forexvps_metrics.avg_response_time:.3f}s"
            )
...
async def collect_all_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        all_metrics = {"system": system_metrics, "database": database_metrics, "forexvps": forexvps_metrics}
