...
class MT5InstanceAllocator:
    ...
    def allocate_instance(self, user_id, tier, preferences=None):
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            (user_id, instance["instance_id"], instance_type, instance["magic_number"], instance["port"], expires_at),
        )
    ...
    def _find_available_instance(self, instance_type):
        ...
        if result:
            conn.close()
            return {"instance_id": result[0], "magic_number": result[1], "port": result[2], "directory": result[3]}
        ...
        return {"instance_id": instance_id, "magic_number": next_magic, "port": next_port, "directory": directory}
