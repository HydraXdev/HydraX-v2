...
import numpy as np
from bitten_signals_live import check_user_access, determine_signal_type, format_bitten_signal, get_tcs_visual
...
class MT5BITTENProcessor:
    ...
    def process_mt5_tick(self, data: Dict) -> Optional[Dict]:
        ...
        if symbol in self.price_history:
            self.price_history[symbol].append({"bid": bid, "ask": ask, "volume": volume, "time": datetime.now()})
        ...
        if signal_type:
            ...
            signal = self.create_bitten_signal(symbol, direction, entry_price, tcs_score / 100, signal_type)
    ...
    def create_bitten_signal(
        self, symbol: str, direction: str, entry_price: float, confidence: float, signal_type: str
    ) -> Dict:
