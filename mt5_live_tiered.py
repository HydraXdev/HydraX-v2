...
from tiered_signal_system import (
    SESSION_SCHEDULE,
    SIGNAL_TIERS,
    apply_tiered_filters,
    check_session_active,
    format_tactical_signal,
    get_signal_tier,
)
...
TRADING_PAIRS = ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "USDCAD", "NZDUSD", "EURGBP"]
...
class MT5TieredProcessor:
    ...
    def process_mt5_tick(self, data: Dict) -> Dict:
        ...
        if symbol in self.price_history:
            self.price_history[symbol].append({"bid": bid, "ask": ask, "time": timestamp, "volume": volume})
            # Keep last 100 ticks
        ...
        tier, confidence, filters_passed = apply_tiered_filters(symbol, bid, ask, spread, indicators)

        # Check if we should generate a signal
        if tier and self.should_generate_signal(tier):
            signal = self.create_tiered_signal(
                symbol, bid, ask, indicators["direction"], confidence, tier, filters_passed
            )
            return signal
    ...
    def calculate_indicators(self, symbol: str, bid: float, ask: float, volume: int) -> Dict:
        ...
        indicators = {"volume": volume, "optimal_session": check_session_active(symbol)}
        ...
        if len(self.price_history[symbol]) >= 20:
            ...
            indicators["macd_confirmation"] = (indicators["direction"] == "BUY" and sma_5 > sma_10) or (
    ...
    def get_daily_stats(self) -> Dict:
        ...
        return {
            ...
            "remaining": {tier: SIGNAL_TIERS[tier]["max_daily"] - self.daily_counts[tier] for tier in SIGNAL_TIERS},
        }
