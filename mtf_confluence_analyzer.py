...
class MTFConfluenceAnalyzer:
        ...
        try:
            ...
            for tf in ["1M", "5M", "15M"]:
                if tf in candle_data and len(candle_data[tf]) >= 20:
                    timeframe_analyses[tf] = self.analyze_timeframe(symbol, tf, candle_data[tf])
    ...
    def calculate_momentum(self, candles: List) -> float:
        ...
        total_momentum = ema_momentum * 0.4 + rsi_momentum * 0.3 + price_momentum * 0.3
    ...
    def determine_bias(self, candles: List, momentum: float) -> Tuple[TimeframeBias, float]:
        ...
        higher_highs = sum(1 for i in range(1, min(10, len(highs))) if highs[-i] > highs[-(i + 1)])
        higher_lows = sum(1 for i in range(1, min(10, len(lows))) if lows[-i] > lows[-(i + 1)])

        lower_highs = sum(1 for i in range(1, min(10, len(highs))) if highs[-i] < highs[-(i + 1)])
        lower_lows = sum(1 for i in range(1, min(10, len(lows))) if lows[-i] < lows[-(i + 1)])

        # Bias scoring
        bullish_score = higher_highs * 10 + higher_lows * 10 + momentum * 0.3 + range_position * 30
    ...
    def calculate_confluence(self, analyses: Dict[str, TimeframeAnalysis]) -> ConfluenceResult:
        ...
        # Check for alignment
        bullish_count = sum(
            1 for a in analyses.values() if a.bias in [TimeframeBias.BULLISH, TimeframeBias.STRONGLY_BULLISH]
        )
        bearish_count = sum(
            1 for a in analyses.values() if a.bias in [TimeframeBias.BEARISH, TimeframeBias.STRONGLY_BEARISH]
        )
        ...
        strength_values = [a.strength for a in analyses.values() if a.bias != TimeframeBias.NEUTRAL]
        if strength_values:
