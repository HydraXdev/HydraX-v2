            ...
            for attempt in range(self.max_retries):
                try:
                    response = requests.get(
                        url, timeout=self.request_timeout, headers={"User-Agent": "BITTEN-Trading-Engine/1.0"}
                    )
    ...
    def evaluate_trading_environment(self) -> Dict[str, Any]:
        ...
        for event in self.economic_calendar:
            ...
            if self._is_high_impact_event(event):
                ...
                if (
                    abs(time_to_event) <= self.tier1_window_minutes
                    or abs(time_since_event) <= self.tier1_window_minutes
                ):
                ...
                if (
                    abs(time_to_event) <= self.tier2_window_minutes
                    or abs(time_since_event) <= self.tier2_window_minutes
                ):
