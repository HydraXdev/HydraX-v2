...
class NormansNotebookV2:
    ...
    def create_entry(self, user_id, entry_data):
        ...
        c.execute(
            ...
            (
                ...
                emotion,
                entry_data.get("market_state"),
                entry_data.get("entry_text"),
                entry_data.get("lessons_learned"),
                entry_data.get("pattern_noticed"),
                bit_reaction,
                family_wisdom,
                int(time.time()),
                int(time.time()),
    ...
    def _update_story_progression(self, user_id, cursor):
        ...
        cursor.execute("SELECT total_trades, chapters_unlocked FROM story_progression WHERE user_id = ?", (user_id,))
        ...
        if result:
            ...
            cursor.execute(
                """INSERT INTO story_progression
                            ...
                            VALUES (?, ?, ?, ?, ?, ?)""",
                (user_id, "discovery", 1, json.dumps(chapters), int(time.time()), int(time.time())),
            )
            return
                ...
                """INSERT OR REPLACE INTO extracted_insights
                            ...
                            VALUES (?, ?, ?, ?,
                                   COALESCE((SELECT frequency + 1 FROM extracted_insights
                                            WHERE insight_id = ?), 1),
                                   ?, ?)""",
                (insight_id, user_id, "pattern", pattern, insight_id, int(time.time()), int(time.time())),
            )
    ...
    def _check_chapter_unlock(self, user_id):
        ...
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute("SELECT chapters_unlocked, last_milestone FROM story_progression WHERE user_id = ?", (user_id,))
        result = c.fetchone()
        conn.close()
