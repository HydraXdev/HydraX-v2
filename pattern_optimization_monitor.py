...
class PatternOptimizationMonitor:
    ...
    def analyze_pattern_performance(self, signals):
        ...
        pattern_stats = defaultdict(
            lambda: {"count": 0, "confidence_sum": 0, "confidence_levels": [], "symbols": set(), "directions": []}
        )
        ...
        for pattern, stats in pattern_stats.items():
            if stats["count"] > 0:
                ...
                stats["confidence_range"] = (
                    f"{min(stats['confidence_levels']):.1f}-{max(stats['confidence_levels']):.1f}%"
                )
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "baseline_comparison": {
                ...
                "improvement_factor": (
                    round((len(signals) / hours_elapsed * 24) / self.baseline["total_signals_24h"], 2)
                    if hours_elapsed > 0
                    else 0
                ),
            },
    ...
    def print_summary(self, report):
        ...
        print(
            f"📈 Current 24h Rate: {report['baseline_comparison']['current_rate_24h']} (baseline: {report['baseline_comparison']['baseline_24h']})"
        )

        print("\n🎯 NEW PATTERN STATUS:")
        for pattern, status in report["new_pattern_status"].items():
            print(
                f"  {pattern}: {status['status']} ({status['signals']} signals @ {status.get('avg_confidence', 0):.1f}%)"
            )
