...
class PatternOptimizerV2:
    ...
    def load_session_config(self) -> Dict:
        ...
        return {
            "ASIAN": {
                "enabled_patterns": ["SUPPORT_RESISTANCE_BOUNCE", "FAIR_VALUE_GAP_FILL"],
                "disabled_patterns": ["BREAKOUT_MOMENTUM"],
            },
            "LONDON": {
                "enabled_patterns": ["LIQUIDITY_SWEEP_REVERSAL", "BREAKOUT_MOMENTUM", "VOLUME_CLIMAX_REVERSAL"],
                "disabled_patterns": [],
            },
            "NY": {
                "enabled_patterns": ["BREAKOUT_MOMENTUM", "VOLUME_CLIMAX_REVERSAL", "LIQUIDITY_SWEEP_REVERSAL"],
                "disabled_patterns": [],
            },
            "OVERLAP": {"enabled_patterns": "ALL", "disabled_patterns": []},  # All patterns active
            "OFF_HOURS": {
                "enabled_patterns": ["SUPPORT_RESISTANCE_BOUNCE"],
                "disabled_patterns": ["BREAKOUT_MOMENTUM", "VOLUME_CLIMAX_REVERSAL"],
            },
        }
    ...
    def analyze_pattern_performance(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                symbol_performance[pattern][symbol] = {"total": total, "win_rate": (wins / total) if total > 0 else 0}
    ...
    def apply_session_rules(self) -> Dict:
        ...
        return {"session": session, "pattern_status": session_status, "timestamp": datetime.now().isoformat()}
    ...
    def generate_report(self) -> str:
        ...
        for pattern, stats in sorted(performance.items(), key=lambda x: x[1].get("win_rate", 0), reverse=True):
            status = self.pattern_status.get(pattern, "UNKNOWN")
            ...
            symbol_perf = stats.get("symbol_performance", {})
            if symbol_perf:
                top_symbols = sorted(symbol_perf.items(), key=lambda x: x[1]["win_rate"], reverse=True)[:3]
                if top_symbols:
        ...
        if self.optimization_log:
            ...
            for decision in self.optimization_log[-5:]:
                report.append(
                    f"{decision['timestamp']}: {decision['action']} {decision.get('pattern', '')} - {decision['reason']}"
                )
    ...
    def run_optimization_cycle(self):
        ...
        try:
            ...
            return {"decisions": decisions, "session_rules": session_rules, "performance": performance}
