...
class RoutePerformanceProfiler:
    ...
    def __init__(self, max_samples=1000):
        ...
        console_formatter = logging.Formatter("🐌 SLOW: %(message)s")
    ...
    def _log_request(self, route_key, response_time, success=True, error=None):
        ...
        if response_time > self.slow_threshold:
            ...
            self.perf_logger.log(level, f"{route_key} took {response_time:.3f}s - SLOW REQUEST")

    def get_performance_report(self):
        """Generate comprehensive performance report"""
        report = {"timestamp": datetime.now().isoformat(), "routes": {}}
        ...
        sorted_routes = sorted(report["routes"].items(), key=lambda x: x[1]["avg_response_time"], reverse=True)
