        ...
        if stale_positions:
            ...
            for pos in stale_positions:
                ...
                if age_min > 60:  # More than 1 hour
                    logger.info("   🔴 %s - Very stale (%.1f hours) - Likely manually closed", 
                               fire_id, age_min / 60)
                elif age_min > 30:  # More than 30 minutes
                    logger.info("   🟡 %s - Moderately stale (%.1f min) - Check MT5 status", 
                               fire_id, age_min)
                else:
                    logger.info("   🟠 %s - Recently stale (%.1f min) - Monitor closely", 
                               fire_id, age_min)
