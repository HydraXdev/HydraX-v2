...
class EnhancedPrimaryAgent:
    ...
    def execute_mt5_trade(self, trade_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not MT5_AVAILABLE:
            # Simulation mode
            print(
                f"[BRIDGE] Command received: FIRE {trade_data.get('symbol', 'UNKNOWN')} {trade_data.get('lot', 0.0)} lots"
            )
            return {
        ...
        try:
            ...
            if not self.ensure_mt5_connection():
                return {"retcode": 10004, "comment": "MT5 connection failed"}  # TRADE_RETCODE_CONNECTION
            ...
            if not symbol_info:
                return {"retcode": 10013, "comment": f"Symbol {symbol} not found"}  # TRADE_RETCODE_INVALID_SYMBOL
            ...
            return {"retcode": 10008, "comment": f"Execution error: {str(e)}"}  # TRADE_RETCODE_ERROR
            ...
            try:
                ...
                while self.socket_running:
                    try:
                        ...
                        threading.Thread(
                            target=self.handle_socket_client, args=(client_socket, addr), daemon=True
                        ).start()
                    except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if command == "ping":
                    ...
                    # Log status
                    print(
                        f"[BRIDGE] Account: {status['account']} | Broker: {status['broker']} | Balance: {status['balance']}"
                    )
                    ...
                    # Unknown command
                    error_response = {"retcode": 10001, "comment": f"Unknown command: {command}"}
...
def restart_backup():
    ...
    try:
        subprocess.Popen(["python", "C:\\BITTEN_Agent\\backup_agent.py"], creationflags=subprocess.CREATE_NEW_CONSOLE)
