...
class ProcessResurrector:
    ...
    def mark_process_dead(self, user_id):
        ...
        if user_id not in self.restart_history:
            self.restart_history[user_id] = {"attempts": 0, "last_attempt": None, "total_restarts": 0}
    ...
    try:
        while True:
            ...
            if stats.get("total_resurrections_24h", 0) > 0:
                print(
                    f"📊 Resurrection Stats (24h): {stats['successful_resurrections_24h']}/{stats['total_resurrections_24h']} successful ({stats['success_rate']:.1f}%)"
                )
