...
class RealPatternAnalyzer:
    ...
    def get_regime(self, timestamp, symbol):
        ...
        regime = {"session": session, "trend": "UNKNOWN", "volatility": "UNKNOWN"}
    ...
    def check_convergence(self, new_signal):
        ...
        for sig_id, sig_data in self.active_signals.items():
            if (
                sig_data["symbol"] == symbol
                and abs(sig_data["timestamp"] - current_time) < 300
                and sig_data["pattern"] != new_signal.get("pattern_type")
            ):
                convergence_count += 1
    ...
    def record_outcome(self, signal_id, outcome, pips):
        ...
        print(
            f"📊 {pattern}: {outcome} ({pips:.1f} pips) | "
        ...
        try:
            if "TICK" in msg:
                ...
                if len(parts) >= 4:
                    ...
                    self.ticks[symbol].append({"timestamp": datetime.now(), "price": self.current_prices[symbol]})
        ...
        while True:
            try:
                ...
                if time.time() - last_save > 60:
                    ...
                    print(
                        f"Quarantined: {sum(1 for p in self.quarantine.values() if p['stage'] == 'QUARANTINE')} patterns"
                    )
