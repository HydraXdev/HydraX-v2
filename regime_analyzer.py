...
class RegimeAnalyzer:
    ...
    def __init__(
        self, db_path: str = "/root/HydraX-v2/bitten.db", regime_log: str = "/root/HydraX-v2/regime_performance.json"
    ):
    ...
    def classify_market_regime(self, signal_data: Dict, market_context: Dict = None) -> Dict:
        ...
        try:
            ...
            return {"regime": MarketRegime.RANGE_LOW_VOL.value, "session": TradingSession.ASIAN.value, "error": str(e)}
    ...
    def _get_average_atr(self, symbol: str) -> float:
        ...
        # Cache result
        self.atr_cache[cache_key] = {"value": avg_atr, "timestamp": current_time}
    ...
    def analyze_pattern_by_regime(self, pattern_type: str, days_back: int = 30) -> Dict:
        ...
        try:
            ...
            overall_stats = self._calculate_performance_stats(
                [{"outcome": s["outcome"], "pips_result": s.get("pips_result", 0)} for s in signals]
            )

            # Identify best and worst regimes
            best_regime = max(regime_stats.items(), key=lambda x: x[1]["expectancy"], default=(None, None))
            worst_regime = min(regime_stats.items(), key=lambda x: x[1]["expectancy"], default=(None, None))
            ...
            for regime, data in regime_summary.items():
                if data["patterns"]:
                    ...
                    regime_averages[regime] = {
                        ...
                        "top_patterns": sorted(data["patterns"], key=lambda x: x["expectancy"], reverse=True)[:3],
                    }
    ...
    def _generate_regime_insights(self, regime_averages: Dict) -> List[str]:
        ...
        insights.append(
            f"Best performing regime: {best_regime[0]} " f"(avg expectancy: {best_regime[1]['avg_expectancy']:.4f})"
        )

        insights.append(
            f"Worst performing regime: {worst_regime[0]} " f"(avg expectancy: {worst_regime[1]['avg_expectancy']:.4f})"
        )
        ...
        if trend_regimes and range_regimes:
            ...
            if trend_avg > range_avg:
                insights.append(f"Patterns perform better in trending markets " f"({trend_avg:.4f} vs {range_avg:.4f})")
            else:
                insights.append(f"Patterns perform better in ranging markets " f"({range_avg:.4f} vs {trend_avg:.4f})")
        ...
        if high_vol_regimes and low_vol_regimes:
            ...
            if high_vol_avg > low_vol_avg:
                insights.append(
                    f"Patterns perform better in high volatility environments "
                    f"({high_vol_avg:.4f} vs {low_vol_avg:.4f})"
                )
            else:
                insights.append(
                    f"Patterns perform better in low volatility environments "
    ...
    def get_regime_recommendations(self, pattern_type: str) -> Dict:
        ...
        try:
            ...
            sorted_regimes = sorted(regime_performance.items(), key=lambda x: x[1]["expectancy"], reverse=True)
...
def main():
    ...
    for regime, data in regime_summary.items():
        print(
            f"  {regime}: {data['avg_expectancy']:+.4f} expectancy "
