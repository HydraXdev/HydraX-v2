...
class RepairAgent:
    ...
    def diagnose_backup_agents(self):
        ...
        try:
            response = self.session.post(
                ...
                json={"command": "Get-Process | Where-Object {$_.ProcessName -eq 'python'}", "type": "powershell"},
            )
    ...
    def create_firewall_rules(self):
        ...
        for cmd in firewall_commands:
            try:
                response = self.session.post(
                    f"http://{self.target_ip}:{self.primary_port}/execute", json={"command": cmd, "type": "powershell"}
                )
    ...
    def check_port_binding(self):
        ...
        try:
            response = self.session.post(
                f"http://{self.target_ip}:{self.primary_port}/execute",
                json={"command": "netstat -an | findstr :555", "type": "cmd"},
            )
    ...
    def restart_all_agents(self):
        ...
        try:
            ...
            response = self.session.post(
                f"http://{self.target_ip}:{self.primary_port}/execute",
                json={"command": "taskkill /F /IM python.exe /T", "type": "cmd"},
            )
            ...
            response = self.session.post(
                f"http://{self.target_ip}:{self.primary_port}/execute",
                json={"command": "cd C:\\BITTEN_Agent && START_AGENTS.bat", "type": "cmd"},
            )
