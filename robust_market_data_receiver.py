...
json_buffers = defaultdict(lambda: {"buffer": "", "last_update": time.time(), "attempts": 0})
...
def receive_market_data():
    ...
    try:
        ...
        return (
            jsonify({"status": "success", "processed_ticks": valid_ticks, "uuid": uuid, "timestamp": current_time}),
            200,
        )
...
def get_all_market_data():
    ...
    with data_lock:
        for symbol in VALID_SYMBOLS:
            ...
            # Skip stale data
            if not symbol_data["last_update"] or (current_time - symbol_data["last_update"]) > STALE_DATA_THRESHOLD:
                continue
...
def health_check():
    ...
    with data_lock:
        for symbol in VALID_SYMBOLS:
            symbol_data = market_data_store[symbol]
            if symbol_data["last_update"] and (current_time - symbol_data["last_update"]) < STALE_DATA_THRESHOLD:
                active_symbols += 1
