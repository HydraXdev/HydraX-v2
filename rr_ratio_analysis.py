...
class RiskRewardAnalysis:
    ...
    def _generate_insights(self, rr_analysis: Dict) -> List[str]:
        ...
        rapid_expectancy = sum(
            rr_analysis.get(cat, {}).get("expectancy", 0) * rr_analysis.get(cat, {}).get("total_signals", 0)
        ...
        if rapid_avg > sniper_avg:
            insights.append(
                f"RAPID_ASSAULT trades (1:1.5-2.5) outperform SNIPER_OPS (1:2.5-3+): {rapid_avg:.3f}R vs {sniper_avg:.3f}R"
            )
        else:
            insights.append(
                f"SNIPER_OPS trades (1:2.5-3+) outperform RAPID_ASSAULT (1:1.5-2.5): {sniper_avg:.3f}R vs {rapid_avg:.3f}R"
            )
