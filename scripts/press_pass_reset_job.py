...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/var/log/bitten/press_pass_reset.log"), logging.StreamHandler()],
)
...
class PressPassResetJob:
    ...
    async def send_reset_notification(self, user_id: int, xp_wiped: int):
        ...
        try:
            ...
            await self.telegram.send_message(chat_id=str(user_id), text=message, parse_mode="Markdown")
                    ...
                    if self.telegram:
                        try:
                            await self.telegram.send_message(
                                chat_id=str(user["user_id"]), text=message, parse_mode="Markdown"
                            )
                            warnings_sent += 1
                        except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("action", choices=["reset", "warn_60", "warn_15"], help="Action to perform")
