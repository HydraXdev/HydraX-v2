...
class XPProgressionSimulator:
    ...
    def simulate_trade(self, player: SimulatedPlayer) -> Dict:
        ...
        return {"xp_earned": xp_earned, "outcome": outcome, "volume": volume, "is_win": is_win}
    ...
    def simulate_day(self, player: SimulatedPlayer, is_active_day: bool) -> Dict:
        ...
        return {"xp_earned": daily_xp, "trades": trades_today}
    ...
    def analyze_progression_balance(self, results: List[Dict]) -> Dict:
        ...
        if "ELITE" not in casual_result["rank_milestones"]:
            analysis["progression_issues"].append("Casual players cannot reach ELITE rank within a year")

        # Check if newbies progress too slowly
        newbie_result = next(r for r in results if r["profile"] == "newbie")
        if (
            "AUTHORIZED" not in newbie_result["rank_milestones"]
            or newbie_result["rank_milestones"].get("AUTHORIZED", 0) > 180
        ):
            analysis["progression_issues"].append("New players take too long to reach AUTHORIZED rank")
        ...
        if hardcore_result["rank_milestones"].get("ADMIN", 365) < 90:
            analysis["progression_issues"].append("Hardcore players reach max rank too quickly")
    ...
    def run_simulation(self, days: int = 365):
        ...
        for player_type, summary in analysis["player_summaries"].items():
            ...
            print(
                f"{player_type:<20} {summary['final_rank']:<15} {days_auth:<15} {days_elite:<15} {summary['avg_xp_per_day']:<15}"
            )
