...
class EnhancedXPSimulator:
    ...
    def __init__(self):
        # XP configuration
        self.xp_thresholds = {UserRank.USER: 0, UserRank.AUTHORIZED: 1000, UserRank.ELITE: 5000, UserRank.ADMIN: 10000}
    ...
    def check_achievements(self, player: SimulatedPlayer, old_xp: int) -> Tuple[int, List[str]]:
        ...
        if player.total_trades >= 20:
            ...
            for milestone, achievement in accuracy_milestones:
                if accuracy >= milestone and achievement not in player.achievements:
                    ...
                    new_achievements.append(achievement)
                    achievement_xp += self.achievement_xp[achievement]
                    player.achievement_history.append(
                        (player.days_active, achievement, self.achievement_xp[achievement])
                    )
        ...
        if player.days_active == 7 and player.total_trades >= 5:
            if "week_active" not in player.achievements:
                ...
                player.achievement_history.append(
                    (player.days_active, "week_active", self.achievement_xp["week_active"])
                )
        ...
        if player.days_active == 30 and player.total_trades >= 20:
            if "month_active" not in player.achievements:
                ...
                player.achievement_history.append(
                    (player.days_active, "month_active", self.achievement_xp["month_active"])
                )
    ...
    def simulate_day(self, player: SimulatedPlayer, is_active_day: bool, day_num: int) -> Dict:
        ...
        return {"xp_earned": daily_xp, "trades": trades_today, "xp_sources": xp_sources}
    ...
    def generate_detailed_report(self, results: List[Dict], analysis: Dict):
        ...
        print("-" * 100)
        print(
            f"{'Profile':<20} {'Final Rank':<12} {'Final XP':<10} {'Days to AUTH':<15} {'Days to ELITE':<15} {'Trades':<10} {'Active Days':<12}"
        )
        print("-" * 100)
        ...
        for profile, summary in analysis["player_summaries"].items():
            ...
            print(
                f"{profile:<20} {summary['final_rank']:<12} {summary['final_xp']:<10} {auth_days:<15} {elite_days:<15} {summary['total_trades']:<10} {summary['active_days']:<12}"
            )
        ...
        sorted_sources = sorted(
            analysis["xp_source_analysis"].items(), key=lambda x: x[1]["avg_percentage"], reverse=True
        )
        ...
        if analysis["progression_issues"]:
            ...
            print("-" * 80)
            for issue in sorted(
                analysis["progression_issues"], key=lambda x: ["critical", "high", "medium"].index(x["severity"])
            ):
        ...
        print("-" * 80)
        print(
            f"{'Rank':<15} {'Required XP':<15} {'Theoretical Days':<20} {'Actual Days (Active)':<25} {'Actual Days (Casual)':<25}"
        )
        print("-" * 80)
        ...
        for rank in [UserRank.AUTHORIZED, UserRank.ELITE, UserRank.ADMIN]:
            ...
            print(
                f"{rank.name:<15} {xp_req:<15} {theoretical_days:<20.0f} {str(active_days):<25} {str(casual_days):<25}"
            )
...
def main():
    ...
    print("\n🎮 Key Insights:")
    print(
        f"  • Rank achievement rates: AUTH={analysis['rank_achievement_rates']['AUTHORIZED']['percentage']}%, "
        f"ELITE={analysis['rank_achievement_rates']['ELITE']['percentage']}%, "
        f"ADMIN={analysis['rank_achievement_rates']['ADMIN']['percentage']}%"
    )
    print(
        f"  • Critical issues found: {len([i for i in analysis['progression_issues'] if i['severity'] == 'critical'])}"
    )
    print(
        f"  • High priority recommendations: {len([r for r in analysis['recommendations'] if r['priority'] == 'HIGH'])}"
    )
