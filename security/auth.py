...
from database.models import User, get_db, get_user_by_telegram_id
...
async def require_tier(required_tier: str):
    ...
    async def tier_check(current_user: User = Depends(get_current_user)) -> User:
        tier_hierarchy = {"NIBBLER": 1, "SCOUT": 2, "WARRIOR": 3, "COMMANDER": 4, "APEX": 5}
...
class TelegramAuth:
    ...
    def create_user_token(telegram_id: str, username: str = None) -> str:
        ...
        user_data = {"telegram_id": telegram_id, "username": username, "source": "telegram"}
...
class APIKeyAuth:
    ...
    def create_api_key(self, user_id: str, name: str) -> str:
        ...
        self.api_keys[api_key] = {"user_id": user_id, "name": name, "created_at": datetime.utcnow(), "last_used": None}
        return api_key
...
def validate_input(data: str, max_length: int = 1000) -> str:
    ...
    if len(data) > max_length:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Input too long. Maximum {max_length} characters"
        )
    ...
    for char in dangerous_chars:
        if char in data_lower:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid characters detected")
