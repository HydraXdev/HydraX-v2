...
class AdminService:
    ...
    def add_user_override(self, user_id: int, key: str, value: Any, expires_at: Optional[datetime] = None, actor: str = 'admin') -> bool:
        ...
        try:
            with self.conn.cursor() as cur:
                ...
                cur.execute("""
                    INSERT INTO admin_audit (actor, user_id, action, table_name, before, after)
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (actor, user_id, 'add_override', 'user_overrides', 
                      json.dumps({'key': key, 'value': before}),
                      json.dumps({'key': key, 'value': value, 'expires_at': expires_at.isoformat() if expires_at else None})))
...
def register_admin_routes(app):
    ...
    def get_pattern_performance_admin():
        ...
        try:
            ...
            return jsonify({
                'success': True,
                'patterns': [dict(row) for row in performance]
            })
        except Exception as e:
    ...
    def get_audit_trail_admin():
        ...
        try:
            ...
            return jsonify({
                'success': True,
                'audit_trail': [dict(row) for row in audit_trail]
            })
        except Exception as e:
