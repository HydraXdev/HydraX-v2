...
class SessionScheduler:
    ...
    def __init__(self):
        ...
        # Setup logging
        logging.basicConfig(level=logging.INFO, format="%(asctime)s - SESSION_SCHEDULER - %(levelname)s - %(message)s")
    ...
    def _restart_apex_engine(self):
        ...
        try:
            ...
            # Start new process
            subprocess.Popen(["python3", "apex_v5_lean.py"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def check_session_change(self):
        ...
        if self.current_session is None or new_session.name != self.current_session.name:
            self.logger.info(
                f"🌍 Session change detected: {self.current_session.name if self.current_session else 'STARTUP'} → {new_session.name}"
            )
