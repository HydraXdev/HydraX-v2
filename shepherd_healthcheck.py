...
class ShepherdHealthCheck:
    ...
    def check_process_running(self) -> HealthStatus:
        ...
        try:
            ...
            return self.add_result("process_check", "critical", f"Process with PID {pid} not found", {"pid": pid})

        except Exception as e:
            return self.add_result(
                "process_check", "critical", f"Failed to check process: {str(e)}", {"error": traceback.format_exc()}
            )
    ...
    def check_index_health(self) -> HealthStatus:
        ...
        try:
            if not INDEX_FILE.exists():
                return self.add_result(
                    "index_check", "critical", "Index file not found", {"index_file": str(INDEX_FILE)}
                )
            ...
            if not metadata or total_components == 0:
                return self.add_result("index_check", "critical", "Index appears to be corrupted or empty", details)
            ...
            return self.add_result(
                "index_check", "healthy", f"Index is healthy with {total_components} components", details
            )

        except Exception as e:
            return self.add_result(
                "index_check", "critical", f"Failed to check index: {str(e)}", {"error": traceback.format_exc()}
            )
    ...
    def check_system_resources(self) -> List[HealthStatus]:
        ...
        try:
            ...
            if cpu_percent > MAX_CPU_PERCENT:
                ...
                results.append(
                    self.add_result("cpu_check", "healthy", f"CPU usage: {cpu_percent}%", {"cpu_percent": cpu_percent})
                )
    ...
    def check_log_health(self) -> HealthStatus:
        ...
        try:
            ...
            if LOG_DIR.exists():
                for log_file in LOG_DIR.rglob("*.log"):
                    ...
                    if size_mb > MAX_LOG_SIZE_MB:
                        large_files.append(
                            {"file": str(log_file.relative_to(PROJECT_ROOT)), "size_mb": round(size_mb, 2)}
                        )
            ...
            if large_files:
                return self.add_result(
                    "log_check", "warning", f"Found {len(large_files)} log files exceeding {MAX_LOG_SIZE_MB}MB", details
                )

            return self.add_result(
                "log_check", "healthy", f"Log files healthy, total size: {total_size:.1f}MB", details
            )

        except Exception as e:
            return self.add_result("log_check", "warning", f"Failed to check logs: {str(e)}", {"error": str(e)})
    ...
    def check_connectivity(self) -> HealthStatus:
        ...
        try:
            ...
            if services_ok:
                return self.add_result("connectivity_check", "healthy", "Network connectivity is good", {"dns": "ok"})
            else:
                return self.add_result(
                    "connectivity_check",
                    "warning",
            ...
            return self.add_result(
                "connectivity_check", "warning", "Network connectivity issues detected", {"error": str(e)}
            )
    ...
    def check_permissions(self) -> HealthStatus:
        ...
        try:
            ...
            if issues:
                return self.add_result(
                    "permissions_check", "critical", f"Permission issues: {', '.join(issues)}", {"issues": issues}
                )

            return self.add_result("permissions_check", "healthy", "All permissions correct", {})
    ...
    def run_all_checks(self) -> Dict:
        ...
        if critical_count > 0:
            ...
            self.send_alert(f"[CRITICAL] SHEPHERD Health Check - {critical_count} issues", alert_body)
...
def main():
    ...
    if args.continuous:
        ...
        if args.json:
            print(json.dumps({"summary": summary, "results": [asdict(r) for r in checker.results]}, indent=2))
        else:
