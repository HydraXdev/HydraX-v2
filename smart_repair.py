...
class SmartRepair:
    ...
    def execute_command(self, command, cmd_type="powershell"):
        ...
        try:
            response = self.session.post(
                f"http://{self.target_ip}:{self.primary_port}/execute", json={"command": command, "type": cmd_type}
            )
    ...
    def fix_backup_agent_code(self):
        ...
        stdout, stderr = self.execute_command("Test-Path C:\\BITTEN_Agent\\backup_agent.py")
        ...
        stdout, stderr = self.execute_command("Test-Path C:\\BITTEN_Agent\\websocket_agent.py")
