...
async def get_funnel_metrics(
    ...
    try:
        ...
        return {"period": {"start": start_date.isoformat(), "end": end_date.isoformat()}, "metrics": metrics}
    except Exception as e:
...
async def get_retention_metrics(cohort_date: date = Query(..., description="Cohort date to analyze")):
...
async def get_xp_reset_metrics(date: Optional[date] = Query(None, description="Date to check (defaults to today)")):
...
async def get_churn_metrics(date: Optional[date] = Query(None, description="Date to check (defaults to today)")):
    """Get churn metrics"""
    if not date:
        date = date.today()
...
async def get_recent_alerts(hours: int = Query(24, description="Number of hours to look back")):
    """Get recent alerts"""
    try:
        ...
        return {"period_hours": hours, "total_alerts": len(alerts), "alerts": alerts}
    except Exception as e:
...
async def create_snapshot():
    ...
    try:
        ...
        return {"status": "success", "snapshot_key": snapshot_key, "timestamp": end_date.isoformat()}
    except Exception as e:
