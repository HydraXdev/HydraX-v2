...
class AnomalyDetector:
    ...
    def _detect_anomaly(self, metric_name: str, current_value: float, timestamp: datetime) -> Optional[Dict[str, Any]]:
    ...
    def _trend_detection(self, metric_name: str, values: List[float], current_value: float) -> Tuple[bool, float]:
    ...
    def _seasonal_detection(
        self, metric_name: str, history: List[Dict], current_value: float, timestamp: datetime
    ) -> Tuple[bool, float]:
    ...
    def _calculate_severity(self, anomaly_score: float, metric_name: str, current_value: float) -> str:
    ...
    def _get_recommendation(self, metric_name: str, current_value: float, historical_values: List[float]) -> str:
    ...
    def get_anomaly_summary(self) -> Dict[str, Any]:
        ...
        if not recent:
            return {"total_anomalies": 0, "by_metric": {}, "by_severity": {}, "trending_issues": []}
                ...
                trending.append(
                    {
                        ...
                        "trend": (
                            "worsening"
                            if anomalies[-1]["anomaly_score"] > anomalies[0]["anomaly_score"]
                            else "improving"
                        ),
                    }
                )
        ...
        "blocks": [
            {"type": "section", "text": {"type": "mrkdwn", "text": alert["message"]}},
            {
                ...
                "fields": [
                    ...
                    {"type": "mrkdwn", "text": f"*Metric:* {alert['anomaly']['metric']}"},
                    {"type": "mrkdwn", "text": f"*Current Value:* {alert['anomaly']['value']:.2f}"},
                    {"type": "mrkdwn", "text": f"*Anomaly Score:* {alert['anomaly']['anomaly_score']:.2f}"},
                ],
            },
        ],
    }
