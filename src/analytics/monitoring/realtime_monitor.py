...
class RealtimeMonitor:
    ...
    def _initialize_thresholds(self) -> Dict[str, MetricThreshold]:
        ...
        return {
            ...
            "error_rate": MetricThreshold(
                metric_name="error_rate", max_value=5.0, severity=AlertSeverity.ERROR  # Alert if error rate exceeds 5%
            ),
        }
        ...
        while self.is_running:
            try:
                ...
                await self._store_metric(
                    "press_pass_conversion_rate", float(result["conversion_rate"]), MetricType.GAUGE
                )
    ...
    async def _monitor_activations(self):
        ...
        while self.is_running:
            try:
                ...
                await self._store_metric(
                    "avg_activation_time", float(result["avg_hours_to_activate"] or 0), MetricType.GAUGE
                )
    ...
    async def _store_metric(self, name: str, value: float, metric_type: MetricType):
        ...
        self.metrics_buffer[name].append({"value": value, "timestamp": datetime.utcnow(), "type": metric_type.value})

        # Keep only last hour of data
        cutoff = datetime.utcnow() - timedelta(hours=1)
        self.metrics_buffer[name] = [m for m in self.metrics_buffer[name] if m["timestamp"] > cutoff]

        # Store in Redis with TTL
        await self.redis.setex(
            key,
            3600,  # 1 hour TTL
            json.dumps({"value": value, "type": metric_type.value, "timestamp": datetime.utcnow().isoformat()}),
        )
        ...
        # Check rate of change threshold
            ...
            if previous and previous != 0:
                ...
                if change_rate > threshold.rate_of_change:
                    alert_triggered = True
                    alert_message = (
                        f"{metric_name} rate of change: {change_rate:.2f}% exceeds {threshold.rate_of_change}%"
                    )
    ...
    async def _update_redis_metrics(self, metrics: Dict[str, Any]):
        ...
        for key, value in metrics.items():
            await self.redis.setex(f"realtime:{key}", 300, json.dumps(value))  # 5 minute TTL
    ...
    async def _get_database_metrics(self) -> Dict[str, float]:
        ...
        try:
            result = await self.db.fetch_one(query)
            return {"connections": result["connections"] or 0, "avg_query_time": float(result["avg_query_time"] or 0)}
        except:
