...
class ReportGenerator:
    ...
    async def _gather_daily_metrics(
        self, start_date: datetime, end_date: datetime, previous_start: datetime
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "previous": {"funnel": previous_funnel, "activation": previous_activation, "revenue": previous_revenue},
    ...
    async def _gather_weekly_metrics(
        self, start_date: datetime, end_date: datetime, previous_start: datetime, previous_end: datetime
    ) -> Dict[str, Any]:
        ...
        return {
            "current": {"funnel": current_funnel, "activation": current_activation, "revenue": current_revenue},
            "previous": {"funnel": previous_funnel, "activation": previous_activation, "revenue": previous_revenue},
            "retention": retention,
    ...
    def _calculate_changes(
        ...
        return {
            "conversions": {
                "absolute": current_funnel["funnel"]["claims"]["total"] - previous_funnel["funnel"]["claims"]["total"],
                "percentage": calc_change(
                    current_funnel["funnel"]["claims"]["total"], previous_funnel["funnel"]["claims"]["total"]
                ),
            },
            "conversion_rate": {
            ...
            "activations": {
                ...
                "percentage": calc_change(
                    current_activation["summary"]["total_activated"], previous_activation["summary"]["total_activated"]
                ),
            },
            "revenue": {
                "absolute": current_revenue["revenue"]["total"] - previous_revenue["revenue"]["total"],
                "percentage": calc_change(current_revenue["revenue"]["total"], previous_revenue["revenue"]["total"]),
            },
        }
    ...
    async def _create_weekly_charts(self, metrics: Dict[str, Any]) -> Dict[str, str]:
        ...
        plt.bar([i - width / 2 for i in x], login_retention, width, label="Login Retention", color="#2E86AB")
        plt.bar([i + width / 2 for i in x], trade_retention, width, label="Trade Activity", color="#A23B72")
    ...
    def _analyze_weekly_trends(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        conversion_trend = (
            "improving" if daily_data[-1]["conversion_rate"] > daily_data[0]["conversion_rate"] else "declining"
        )
    ...
    def _generate_recommendations(self, metrics: Dict[str, Any], insights: List[Dict[str, Any]]) -> List[str]:
        ...
        critical_insights = [
            i for i in insights if i.get("priority") == "high" and i.get("type") in ["negative", "warning"]
        ]
...
class ReportScheduler:
    ...
    async def start(self):
        ...
        await asyncio.gather(self._schedule_daily_reports(), self._schedule_weekly_reports())
