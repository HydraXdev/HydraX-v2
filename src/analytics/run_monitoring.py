...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("press_pass_monitoring.log"), logging.StreamHandler(sys.stdout)],
)
...
class MonitoringSystem:
    ...
    async def initialize(self):
        ...
        self.components["anomaly_detector"] = AnomalyDetector(sensitivity=self.config.get("anomaly_sensitivity", 2.5))
                ...
                for metric_name, metric_data in metrics.items():
                    if isinstance(metric_data, dict) and "value" in metric_data:
                        await self.components["alert_manager"].check_and_alert(
                            metric_name, metric_data["value"], datetime.utcnow()
                        )
    ...
    async def _health_check(self):
        ...
        while True:
            try:
                ...
                self.redis_client.setex("monitoring:health:status", 300, "healthy")  # 5 minute TTL

                await asyncio.sleep(300)  # Check every 5 minutes

            except Exception as e:
                logger.error(f"Health check failed: {e}")
                self.redis_client.setex("monitoring:health:status", 300, f"unhealthy: {str(e)}")
...
def load_config() -> Dict[str, Any]:
    ...
    config = {
        ...
        "alerts": {
            ...
            "suppression_rules": {"press_pass_conversions": 30, "activation_rate": 60, "error_rate": 15},  # minutes
        },
