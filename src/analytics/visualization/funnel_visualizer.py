...
class FunnelVisualizer:
    ...
    def create_funnel_chart(
        self, funnel_data: Dict[str, Any], title: str = "Press Pass Conversion Funnel"
    ) -> go.Figure:
        """Create interactive funnel chart using Plotly"""

        stages = ["Landing Page", "Press Pass Claimed", "Demo Activated", "First Trade", "Tier Upgraded"]
    ...
    def create_funnel_breakdown(self, funnel_data: Dict[str, Any]) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2,
            cols=2,
            subplot_titles=("Claims by Source", "Upgrades by Tier", "Conversion Rates Over Time", "User Journey Time"),
            specs=[[{"type": "pie"}, {"type": "bar"}], [{"type": "scatter"}, {"type": "box"}]],
        )
        ...
        for stage, times in journey_times.items():
            fig.add_trace(
                go.Box(
                    y=times / 60, name=stage, marker_color=self.colors["primary"], showlegend=False  # Convert to hours
                ),
                row=2,
                col=2,
            )
        ...
        # Update layout
        fig.update_layout(
            title="Press Pass Funnel Breakdown Analysis", height=800, showlegend=False, plot_bgcolor="white"
        )
    ...
    def create_revenue_analysis(self, revenue_data: Dict[str, Any]) -> plt.Figure:
        ...
        ax1.bar(
            tiers,
            revenues,
            color=[self.colors["primary"], self.colors["secondary"], self.colors["warning"], self.colors["danger"]],
        )
        ...
        for i, (stage, value) in enumerate(zip(stages, values)):
            ...
            # Add text
            ax2.text(0.5, i + 0.4, f"{stage}\n{value:}", ha="center", va="center", fontsize=10, weight="bold")
        ...
        ax4.axvline(
            revenue_data["time_to_upgrade"]["avg_days"], color="red", linestyle="--", linewidth=2, label="Average"
        )
        ax4.axvline(
            revenue_data["time_to_upgrade"]["median_days"], color="green", linestyle="--", linewidth=2, label="Median"
        )
    ...
    def create_real_time_dashboard(self, metrics: Dict[str, Any]) -> go.Figure:
        ...
        if alerts:
            fig.add_trace(
                go.Table(
                    header=dict(
                        values=["Time", "Alert", "Severity"], fill_color=self.colors["dark"], font_color="white"
                    ),
                    cells=dict(
                        values=[
        ...
        fig.add_trace(
            go.Indicator(
                ...
                gauge={"axis": {"range": [0, 100]}, "bar": {"color": self.colors["info"]}},
    ...
    def create_comparison_chart(
        self, current_data: Dict[str, Any], previous_data: Dict[str, Any], period: str = "Daily"
    ) -> go.Figure:
