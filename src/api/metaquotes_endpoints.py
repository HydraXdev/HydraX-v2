...
from ..bitten_core.metaquotes import DeliveryMethod, get_demo_account_service, get_pool_manager
...
async def activate_press_pass(request: ActivatePressPassRequest, api_key: str = Depends(verify_api_key)):
    """
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid delivery method: {request.delivery_method}")
...
async def check_account_health(user_id: str, api_key: str = Depends(verify_api_key)):
...
async def get_account_credentials(user_id: str, api_key: str = Depends(verify_api_key)):
    """
    ...
    try:
        ...
        if not credentials:
            raise HTTPException(status_code=404, detail="No active demo account found")
...
async def resend_credentials(request: ResendCredentialsRequest, api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        result = await manager.resend_credentials(
            user_id=request.user_id, delivery_method=delivery_method, contact_info=request.contact_info
        )
...
async def retrieve_credentials_by_link(delivery_id: str, token: str = Query(..., description="One-time token")):
    """
    ...
    try:
        ...
        if not credentials:
            raise HTTPException(status_code=404, detail="Invalid or expired credential link")
...
async def health_check():
    ...
    try:
        ...
        return {
            "status": "healthy",
            "services": {"demo_service": "active", "pool_manager": "active", "pool_health": pool_stats["pool_health"]},
            "timestamp": datetime.utcnow().isoformat(),
        }
