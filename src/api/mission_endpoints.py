...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not auth_header or not user_id:
            return (
                jsonify(
                    {"status": "error", "reason": "authentication_required", "message": "Valid authentication required"}
                ),
                401,
            )
...
def mission_status(mission_id: str):
    ...
    try:
        ...
        if not validate_mission_access(request.user_id, mission_id):
            return (
                jsonify(
                    {"status": "error", "reason": "access_denied", "message": "You don't have access to this mission"}
                ),
                403,
            )
        ...
        if not mission_data:
            return jsonify({"status": "error", "reason": "not_found", "message": "Mission not found or invalid"}), 404
        ...
        return jsonify({"status": "error", "reason": "internal_error", "message": "Internal server error"}), 500
...
def list_missions():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "missions": missions, "total": total_missions, "offset": offset, "limit": limit}
        )

    except Exception as e:
        logger.error(f"Error listing missions for user {request.user_id}: {e}")
        return jsonify({"status": "error", "reason": "internal_error", "message": "Failed to retrieve missions"}), 500
...
def handle_fire():
    ...
    try:
        ...
        if not data:
            return jsonify({"status": "error", "reason": "invalid_request", "message": "JSON payload required"}), 400

        mission_id = data.get("mission_id")
        if not mission_id:
            return (
                jsonify({"status": "error", "reason": "missing_mission_id", "message": "mission_id is required"}),
                400,
            )
        ...
        mission_data = load_mission_data(mission_id)
        if not mission_data:
            return jsonify({"status": "error", "reason": "mission_not_found", "message": "Mission not found"}), 404

        # Check if mission is expired
        time_remaining = calculate_time_remaining(mission_data["expires_at"])
        if time_remaining <= 0:
            return jsonify({"status": "error", "reason": "mission_expired", "message": "Mission has expired"}), 403

        # Check if mission already fired
        if mission_data.get("status") == "fired":
            return (
                jsonify({"status": "error", "reason": "already_fired", "message": "Mission has already been fired"}),
                409,
            )
        ...
        if not save_mission_data(mission_id, mission_data):
            return (
                jsonify({"status": "error", "reason": "save_failed", "message": "Failed to update mission status"}),
                500,
            )
        ...
        try:
            ...
            return (
                jsonify(
                    {"status": "error", "reason": "execution_failed", "message": f"Trade execution failed: {str(e)}"}
                ),
                500,
            )
...
def cancel_mission(mission_id: str):
    ...
    try:
        ...
        if mission_data.get("status") != "pending":
            return (
                jsonify(
                    {"status": "error", "reason": "cannot_cancel", "message": "Only pending missions can be cancelled"}
                ),
                400,
            )
        ...
        # Save updated mission data
        if not save_mission_data(mission_id, mission_data):
            return jsonify({"status": "error", "reason": "save_failed", "message": "Failed to cancel mission"}), 500

        logger.info(f"Mission {mission_id} cancelled by user {request.user_id}")

        return jsonify({"status": "success", "message": "Mission cancelled successfully", "mission_id": mission_id})
