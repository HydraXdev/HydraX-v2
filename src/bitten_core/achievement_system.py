...
class AchievementSystem:
    ...
    def update_progress(self, user_id: str, progress_data: Dict[str, Any]) -> List[str]:
        ...
        for achievement_id, achievement in self.achievements.items():
            ...
            if progress_updated:
                ...
                progress.progress_percentage = self._calculate_progress_percentage(progress, achievement)
    ...
    def _calculate_progress_percentage(
        self, progress: AchievementProgress, achievement: AchievementDefinition
    ) -> float:
        ...
        for req_key, req_value in achievement.requirements.items():
            ...
            if isinstance(req_value, bool):
                ...
                unlocked_count = sum(
                    1
                    for p in self.user_progress[progress.user_id].values()
    ...
    def _check_unlock_conditions(self, progress: AchievementProgress, achievement: AchievementDefinition) -> bool:
    ...
    def get_unlocked_achievements(self, user_id: str) -> List[str]:
        ...
        return [achievement_id for achievement_id, progress in self.user_progress[user_id].items() if progress.unlocked]
    ...
    def integrate_with_profile(self, user_id: str, profile_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        unlocked_badges.sort(key=lambda x: tier_order.index(AchievementTier(x["tier"])))
...
if __name__ == "__main__":
    ...
    # Win some battles
    unlocked = achievement_system.update_progress(test_user_id, {"battles_won": 1})
    print(f"Unlocked: {unlocked}")

    # Level up
    unlocked = achievement_system.update_progress(test_user_id, {"level": 10})
    print(f"Unlocked: {unlocked}")

    # Collect items
    unlocked = achievement_system.update_progress(test_user_id, {"unique_items": 15})
    print(f"Unlocked: {unlocked}")
    ...
    profile = {
        ...
        "username": "TestPlayer",
        "level": 10,
        "experience": {"current_xp": 1000, "total_xp": 2500},
    }
