...
class AIIntegrationPatch:
    ...
    def enhance_fire_command(
        self, user_id: str, signal_data: Dict[str, Any], user_profile: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            institutional_analysis = institutional_intel.analyze_market_structure(signal_data.get("symbol", "EURUSD"))

            # Generate AI-enhanced response
            ai_response = self._generate_ai_response(signal_data, coaching_analysis, institutional_analysis)
            ...
            return {
                ...
                "intervention_needed": coaching_analysis.get("psychological_state", {}).get(
        ...
        try:
            ...
            if psych_state.get("intervention_needed"):
                message_parts.append(
                    f"🧠 **PSYCHOLOGICAL ALERT**: {psych_state.get('intervention_message', 'Intervention recommended')}"
                )
            else:
                ...
                message_parts.append(
                    f"🧠 **Mental State**: {state} | Stress: {stress:.1f}/10 | Confidence: {confidence:.1f}/10"
                )
            ...
            if smart_money.get("confidence", 0) > 0.7:
                ...
                message_parts.append(
                    f"🏛️ **Smart Money**: {action} detected ({smart_money.get('confidence', 0):.0%} confidence)"
                )
            ...
            if position_adj != 1.0:
                if position_adj < 1.0:
                    message_parts.append(
                        f"📉 **AI Recommendation**: Reduce position to {position_adj:.0%} of normal size"
                    )
                else:
                    message_parts.append(
                        f"📈 **AI Recommendation**: Increase position to {position_adj:.0%} of normal size"
                    )
        ...
        try:
            ...
            if symbol:
                ...
                if recent_perf.get("trades_last_30_days", 0) > 5:
                    ...
                    insights.append(
                        f"📊 Your recent win rate: {win_rate:.0f}% over {recent_perf.get('trades_last_30_days')} trades"
                    )
    ...
    def _generate_ai_response(
        self, signal_data: Dict, coaching_analysis: Dict, institutional_analysis: Dict
    ) -> Dict[str, Any]:
...
def enhance_fire_command(
    user_id: str, signal_data: Dict[str, Any], user_profile: Optional[Dict] = None
) -> Dict[str, Any]:
