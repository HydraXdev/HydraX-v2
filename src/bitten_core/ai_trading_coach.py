...
class AITradingCoach:
    ...
    def update_trade_outcome(self, trade_id: str, outcome_data: Dict):
        ...
        for trade in reversed(self.trade_history):  # Search from most recent
            if trade.symbol == outcome_data.get("symbol") and trade.direction == outcome_data.get("direction"):
    ...
    def _detect_psychological_state(self, user_context: Dict) -> PsychologicalProfile:
        ...
        intervention_needed = (
            stress_level > 7.0
            or revenge_risk > 6.0
            or overtrading_risk > 6.0
            or current_state in [PsychologicalState.REVENGE_TRADING, PsychologicalState.TILT]
        )
    ...
    def _generate_coaching_advice(
        self, signal: Dict, psych_state: PsychologicalProfile, risk_prediction: RiskPrediction
    ) -> Dict[str, str]:
        ...
        if psych_state.current_state == PsychologicalState.REVENGE_TRADING:
            ...
            advice["psychological_guidance"] = (
                "⚖️ BALANCE: Confidence is good, overconfidence is dangerous. " "Stick to your risk management rules."
            )
        ...
        if risk_prediction.overall_risk_level == RiskLevel.HIGH:
            ...
            advice["risk_management_tip"] = (
                "⛈️ CORRELATION STORM: Multiple pairs moving together. " "Avoid additional trades in correlated pairs."
            )
    ...
    def _check_intervention_needed(
        self, psych_state: PsychologicalProfile, risk_prediction: RiskPrediction
    ) -> Dict[str, Any]:
    ...
    def _calculate_position_adjustment(
        self, risk_prediction: RiskPrediction, psych_state: PsychologicalProfile
    ) -> float:
    ...
    def _predict_drawdown_risk(self, signal: Dict, user_context: Dict) -> float:
        ...
        symbol_risk = {"XAUUSD": 8.0, "GBPJPY": 7.0, "EURJPY": 6.0, "GBPUSD": 5.0, "EURUSD": 4.0, "USDJPY": 4.5}.get(
            symbol, 5.0
        )
    ...
    def get_coaching_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_performance": {
                ...
                "avg_pnl": (
                    statistics.mean([t.pnl_percent for t in recent_trades if t.pnl_percent != 0])
                    if recent_trades
                    else 0
                ),
