...
from .user_settings import get_user_settings, should_play_sound
...
class AmbientAudioEngine:
    ...
    def update_bit_mood(self, user_id: str, new_mood: AudioMood, context: Optional[str] = None) -> None:
        ...
        bit_state.mood_history = [(mood, time) for mood, time in bit_state.mood_history if time > cutoff]
    ...
    def select_contextual_audio(
        self, user_id: str, context: TradingContext, mood: Optional[AudioMood] = None
    ) -> Optional[AudioClip]:
        ...
        context_audio_map = {
            TradingContext.SIGNAL_RECEIVED: [AudioType.CHIRP, AudioType.MEOW],
            TradingContext.TRADE_OPENED: [AudioType.CHIRP, AudioType.TRADING_FEEDBACK],
            TradingContext.PROFIT_TARGET: [AudioType.PURR, AudioType.TRADING_FEEDBACK],
            TradingContext.STOP_LOSS: [AudioType.CHIRP, AudioType.TRADING_FEEDBACK],
            TradingContext.RISK_WARNING: [AudioType.CHIRP, AudioType.MEOW],
            TradingContext.MARKET_OPEN: [AudioType.CHIRP, AudioType.ENVIRONMENTAL],
            TradingContext.MARKET_CLOSE: [AudioType.PURR, AudioType.ENVIRONMENTAL],
            TradingContext.HIGH_VOLATILITY: [AudioType.CHIRP, AudioType.ENVIRONMENTAL],
            TradingContext.LOW_VOLATILITY: [AudioType.PURR, AudioType.ENVIRONMENTAL],
        }
        ...
        matching_clips = [
            clip for clip in self.audio_clips.values() if clip.audio_type in possible_types and clip.mood == target_mood
        ]

        # If no exact mood match, find closest mood
        if not matching_clips:
            matching_clips = [clip for clip in self.audio_clips.values() if clip.audio_type in possible_types]
    ...
    async def play_contextual_audio(
        self, user_id: str, context: TradingContext, mood: Optional[AudioMood] = None, metadata: Optional[Dict] = None
    ) -> Optional[str]:
    ...
    async def _ambient_loop(self):
        ...
        while self.is_running:
            try:
                ...
                self.active_events = [event for event in self.active_events if event.timestamp > cutoff]
            ...
            if inactive_time > timedelta(minutes=30):
                if bit_state.current_mood not in [AudioMood.SLEEPY, AudioMood.CALM]:
                    ...
                    if random.random() < 0.1:  # 10% chance every cycle
                        await self.play_contextual_audio(user_id, TradingContext.LOW_VOLATILITY, AudioMood.SLEEPY)
...
async def play_trading_audio(
    user_id: str, context: TradingContext, market_data: Optional[Dict] = None
) -> Optional[str]:
...
async def bit_react_to_trade(user_id: str, trade_result: Dict[str, Any]) -> Optional[str]:
    ...
    return await ambient_audio_engine.play_contextual_audio(user_id, context, mood, metadata=trade_result)
