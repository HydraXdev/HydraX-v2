...
class AmmunitionManager:
    ...
    BASE_SHOTS = {"NIBBLER": 3, "FANG": 5, "COMMANDER": 7}

    # Upgrade configurations
    UPGRADE_CONFIG = {
        AmmoType.EXTENDED_MAG: {"extra_shots": 2, "duration_hours": 24, "min_tcs": 85, "tier_required": "FANG"},
        AmmoType.RAPID_RELOAD: {"cooldown_skips": 1, "daily_limit": 1, "tier_required": "NIBBLER"},
        AmmoType.SPECIAL_AMMO: {"risk_percent": 3, "min_tcs": 91, "uses": 1, "tier_required": "COMMANDER"},
    }
    ...
    def activate_upgrade(self, user_id: str, upgrade_type: AmmoType, user_tier: str) -> Tuple[bool, str]:
        ...
        if upgrade_type == AmmoType.EXTENDED_MAG:
            ...
            upgrade = AmmoUpgrade(
                ...
                metadata={"risk_percent": config["risk_percent"], "min_tcs": config["min_tcs"]},
            )
    ...
    def can_use_special_ammo(self, user_id: str, signal_tcs: float) -> Tuple[bool, str, Optional[float]]:
        ...
        for upgrade in status.active_upgrades:
            if upgrade.upgrade_type == AmmoType.SPECIAL_AMMO and upgrade.is_active():
                if upgrade.uses_remaining and upgrade.uses_remaining > 0:
                    ...
                    # Log usage
                    upgrade.metadata["last_used"] = {"trade_id": trade_id, "timestamp": datetime.now().isoformat()}
            ...
            try:
                ...
                for upgrade_data in data["active_upgrades"]:
                    upgrade = AmmoUpgrade(
                        ...
                        expires_at=(
                            datetime.fromisoformat(upgrade_data["expires_at"]) if upgrade_data["expires_at"] else None
                        ),
