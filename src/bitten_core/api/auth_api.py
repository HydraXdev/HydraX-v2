...
from ..user_management.auth_middleware import AuthenticationError, SessionManager
...
async def login(request: LoginRequest):
    ...
    """
    try:
        result = await user_manager.authenticate_user(telegram_id=request.telegram_id, api_key=request.api_key)
        ...
        if result["authenticated"]:
            ...
            return LoginResponse(success=False, message=result.get("message", "Authentication failed"))
...
async def validate_session(session: Dict = Depends(get_current_user)):
    ...
    return SessionResponse(
        valid=True, user_id=session["user_id"], tier=session["data"].get("tier"), expires_at=session["expires_at"]
    )
...
async def refresh_session(session: Dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {"success": True, "session_token": new_token, "message": "Session refreshed successfully"}
...
async def upgrade_tier(request: TierUpgradeRequest, session: Dict = Depends(get_current_user)):
    ...
    try:
        result = await user_manager.upgrade_user_tier(
            user_id=session["user_id"], new_tier=request.new_tier, payment_method=request.payment_method
        )
...
async def update_profile(request: ProfileUpdateRequest, session: Dict = Depends(get_current_user)):
    ...
    """
    try:
        result = await user_manager.update_user_profile(user_id=session["user_id"], profile_data=request.updates)
...
async def get_tier_features(tier: str):
    ...
    return {"tier": tier, "features": features, "limits": limits}
...
async def get_press_pass_status(session: Dict = Depends(get_current_user)):
    ...
    try:
        if session["data"].get("tier") != "PRESS_PASS":
            return {"is_press_pass": False, "message": "User is not on Press Pass tier"}
        ...
        async with get_async_db() as conn:
            ...
            if user_data and user_data["subscription_expires_at"]:
                ...
                status = await pm.check_press_pass_expiry(str(session["user_id"]), user_data["subscription_expires_at"])
        ...
        return {"is_press_pass": True, "error": "Could not retrieve Press Pass status"}
