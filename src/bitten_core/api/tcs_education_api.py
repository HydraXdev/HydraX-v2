...
def get_tutorial():
    ...
    try:
        ...
        return jsonify({"tutorial": tutorial, "content": content, "user_tier": educator._get_user_tier(user_level)})
...
def submit_quiz_answer():
    ...
    try:
        ...
        if user_id and is_correct:
            update_user_progress(user_id, "quiz_completed", {"topic": topic, "question_id": question_id})
...
def get_trading_examples():
    ...
    try:
        ...
        for example in examples:
            example["explanations"] = educator.get_score_breakdown_explanation(example["breakdown"], user_level)

        return jsonify({"examples": examples, "total": len(examples), "user_can_view_details": user_level >= 5})
