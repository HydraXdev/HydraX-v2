...
class ArcadeFilter:
    ...
    def _detect_dawn_raid(self, symbol: str, data: Dict) -> Optional[ArcadeSignal]:
        ...
        if not (
            self.dawn_raid_config["breakout_window"][0] <= current_hour < self.dawn_raid_config["breakout_window"][1]
        ):
            return None
        ...
        # Validate range
        if not (self.dawn_raid_config["min_range_pips"] <= range_pips <= self.dawn_raid_config["max_range_pips"]):
            return None
