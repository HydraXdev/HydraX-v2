...
class AudioTriggerManager:
    ...
    def _check_risk_warning(self, user_id: str, context: Dict) -> bool:
        ...
        return (
            ...
            or context.get("event_type") == "risk_warning"
            or context.get("tcs_score", 100) < 70
        )
    ...
    def _check_first_profitable_week(self, user_id: str, context: Dict) -> bool:
        ...
        return (
            weekly_pnl > 0
            and story_context.current_phase in [StoryPhase.EARLY_STRUGGLE, StoryPhase.AWAKENING]
            and context.get("milestone") == "first_profitable_week"
        )
            ...
            try:
                ...
                if trigger.condition_check(user_id, context):
                    ...
                    event_id = await ambient_audio_engine.play_contextual_audio(
                        ...
                        metadata={"trigger_id": trigger_id, "trigger_name": trigger.name, "context": context},
                    )
    ...
    def _record_trigger_activation(self, user_id: str, trigger_id: str):
        ...
        self.user_trigger_history[user_id].append({"trigger_id": trigger_id, "timestamp": datetime.now()})

        # Keep only last 24 hours of history
        cutoff = datetime.now() - timedelta(hours=24)
        self.user_trigger_history[user_id] = [
            record for record in self.user_trigger_history[user_id] if record["timestamp"] > cutoff
        ]
...
async def trigger_market_event(user_id: str, event_type: str, market_data: Dict = None):
    ...
    context = {"event_type": event_type, "market_data": market_data or {}, "timestamp": datetime.now()}
...
async def trigger_trading_event(user_id: str, event_type: str, trade_data: Dict = None):
    ...
    context = {"event_type": event_type, "trade_data": trade_data or {}, "timestamp": datetime.now()}
    return await audio_trigger_manager.process_trigger_event(user_id, context)


async def trigger_achievement_event(user_id: str, achievement_data: Dict):
    """Trigger achievement-related audio events"""
    context = {"event_type": "achievement_unlocked", "achievement_data": achievement_data, "timestamp": datetime.now()}
    return await audio_trigger_manager.process_trigger_event(user_id, context)
