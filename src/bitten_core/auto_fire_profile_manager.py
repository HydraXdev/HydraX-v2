...
class AutoFireProfileManager:
    ...
    def _ensure_default_profile(self):
        ...
        default_allowed_pairs = [
            'USDCNH', 'XAUUSD', 'USDJPY', 'EURJPY',
            'GBPCAD', 'NZDUSD', 'EURAUD'
        ]
        ...
        default_blocked_patterns = [
            'FAIR_VALUE_GAP_FILL', 'ORDER_BLOCK_BOUNCE'
        ]
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if not row:
                ...
                if default_row:
                    ...
                    """, (
                        user_id, f'Profile for {user_id}',
                        default_row[2], default_row[3],  # allowed/blocked pairs
                        default_row[4], default_row[5],  # allowed/blocked patterns
                        default_row[6], default_row[7],  # min/max confidence
                        default_row[8], default_row[9],  # optimal confidence range
    ...
    def should_auto_fire(self, user_id: str, signal_data: Dict) -> Tuple[bool, str]:
        ...
        if not (profile['min_confidence'] <= confidence <= profile['max_confidence']):
            return False, f"Confidence {confidence}% outside range {profile['min_confidence']}-{profile['max_confidence']}%"
