...
class BittenBattlePass:
    ...
    def award_xp(self, user_id: str, xp_amount: int, source: str = "gameplay") -> Dict:
        ...
        # Check for level ups
        levels_gained = 0
        while progress.xp >= self.current_season.xp_per_level and progress.level < self.current_season.max_level:
        ...
        for level in range(start_level, end_level + 1):
            if level in self.current_season.rewards:
                for reward in self.current_season.rewards[level]:
                    ...
                    required_tier_level = (
                        tier_hierarchy.index(reward.tier_requirement)
                        if reward.tier_requirement in tier_hierarchy
                        else 0
                    )
    ...
    def get_season_leaderboard(self, season_id: str = None, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            for i, row in enumerate(rows):
                leaderboard.append(
                    {"rank": i + 1, "user_id": row[0], "level": row[1], "xp": row[2], "user_tier": row[3]}
                )
...
def create_sample_season() -> BattlePassSeason:
    ...
    return BattlePassSeason(
        ...
        metadata={
            ...
            "xp_economy": {"basic_rewards": 0, "premium_rewards": 50, "elite_rewards": 200, "legendary_rewards": 500},
        },
    )
