...
from .battle_pass import BattlePassType, BittenBattlePass, RewardType, battle_pass_system
...
class BattlePassIntegration:
    ...
    async def award_trading_xp(self, user_id: str, trade_data: Dict) -> Dict:
        ...
        try:
            ...
            progression_result = self.battle_pass.award_xp(user_id=user_id, xp_amount=multiplied_xp, source="trading")
            ...
            return {"success": True, "base_xp": base_xp, "final_xp": multiplied_xp, "progression": progression_result}
    ...
    async def update_challenge_progress(self, user_id: str, trade_data: Dict):
        ...
        week_challenges = [c for c in self.battle_pass.current_season.challenges if c["week"] == current_week]
    ...
    async def check_special_rewards(self, user_id: str, trade_data: Dict) -> List[Dict]:
        ...
        if trade_data.get("tcs_score", 0) >= 90 and trade_data.get("result") == "win":
            special_rewards.append(
                {
                    "type": "perfect_trade",
    ...
    async def get_user_battle_pass_data(self, user_id: str) -> Dict:
        ...
        available_rewards = self.battle_pass.get_level_rewards(progress.level, progress.level, progress.pass_type)

        # Get claimable rewards
        claimable_levels = []
        for level in range(1, progress.level + 1):
            if level not in progress.rewards_claimed:
                level_rewards = self.battle_pass.get_level_rewards(level, level, progress.pass_type)
                if level_rewards:
        ...
        user_rank = next((i + 1 for i, entry in enumerate(leaderboard) if entry["user_id"] == user_id), None)
        ...
        return {
            ...
            "rewards": {"available": [r.to_dict() for r in available_rewards], "claimable_levels": claimable_levels},
            "challenges": week_challenges,
            "leaderboard": {"user_rank": user_rank, "total_players": len(leaderboard)},
        }
