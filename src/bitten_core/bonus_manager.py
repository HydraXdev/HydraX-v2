...
class BonusManager:
    ...
    def __init__(self, config_path: str = None):
        self.config_path = config_path or os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "..", "config", "tier_settings.yml"
        )
        self.load_config()
    ...
    def cleanup_expired_bonuses(self):
        ...
        for user_id_str, bonuses in self.config["user_bonuses"].items():
            ...
            self.config["user_bonuses"][user_id_str] = [
                ...
                if bonus.get("active", False) and datetime.fromisoformat(bonus["expires"]) > now
            ]
    ...
    def get_user_bonuses(self, user_id: int) -> List[Dict]:
        ...
        if user_id_str in self.config.get("user_bonuses", {}):
            bonuses.extend([bonus for bonus in self.config["user_bonuses"][user_id_str] if bonus.get("active", False)])
        ...
        for event_name, event_data in self.config.get("special_events", {}).items():
            ...
            if start <= now <= end:
                if tier is None or tier in event_data.get("eligible_tiers", []):
                    active_events.append({"name": event_name, **event_data})
...
def example_fourth_of_july_event():
    ...
    event_data = {
        ...
        "bonuses": {"risk_percent": 0.5, "daily_shots": 2, "positions": 0, "tcs_reduction": 0},
...
def example_grant_win_streak_bonus(user_id: int):
    ...
    bonus_id = manager.grant_user_bonus(
        ...
        duration_hours=48,
        bonuses={"risk_percent": 0.25, "daily_shots": 3, "positions": 0, "tcs_reduction": 0},
