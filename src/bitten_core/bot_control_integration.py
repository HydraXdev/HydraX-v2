...
from .telegram_bot_controls import TelegramBotControls, format_bot_message, should_show_bot_message
...
class BotControlIntegration:
    ...
    def _register_commands(self):
        ...
        def enhanced_route_command(command: str, args: list, update: TelegramUpdate) -> CommandResult:
            ...
            if command == "/disclaimer":
                ...
                return CommandResult(
                    success=result.get("success", True), message=result.get("message", ""), data=result
                )
            elif command == "/bots":
        ...
        router.command_categories["Bot Controls"] = ["disclaimer", "bots", "toggle", "immersion", "settings"]
    ...
    def check_user_consent(self, user_id: str) -> Dict:
        ...
        if not consent:
            return {"has_consent": False, "needs_onboarding": True, "message": "Please complete onboarding with /start"}
    ...
    def handle_onboarding_disclaimer(self, user_id: str, accepted: bool, preferences: Dict) -> Dict:
        ...
        result = self.bot_controls.handle_onboarding_disclaimer(str(user_id), accepted, preferences)
    ...
    def inject_bot_status_into_message(self, user_id: str, original_message: str) -> str:
        ...
        if any(keyword in original_message.lower() for keyword in ["status", "settings", "welcome"]):
            ...
            if "**System Health:**" in original_message:
                ...
                original_message = original_message.replace("**System Health:**", f"**System Health:**\n{bot_status}\n")
...
def create_bot_control_integration(telegram_router: TelegramRouter = None, bitten_core=None) -> BotControlIntegration:
...
class BotMessageMiddleware:
    ...
    def process_outgoing_message(self, user_id: str, message: Dict) -> Optional[Dict]:
        ...
        if message.get("type") == "bot_message":
            ...
            # Format message based on preferences
            formatted = self.integration.format_bot_message_for_user(user_id, bot_name, content)
        ...
        if "content" in message and isinstance(message["content"], str):
            message["content"] = self.integration.inject_bot_status_into_message(user_id, message["content"])
