...
from bitten_core.symbol_mapper import TranslationResult, symbol_mapper
...
class BridgeSymbolIntegration:
    ...
    def discover_user_symbols(self, user_id: str, bridge_id: str, host: str = "127.0.0.1") -> Tuple[bool, Dict]:
        """
        ...
        try:
            ...
            if not symbols_data or not symbols_data.get("success"):
                error_msg = (
                    symbols_data.get("error", "Unknown bridge error") if symbols_data else "Bridge connection failed"
                )
            ...
            mapping_success = symbol_mapper.initialize_user_symbols(user_id, symbols_list, broker_name)
    ...
    def translate_signal_for_user(self, user_id: str, signal: Dict) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if not translation_result.success:
                ...
                return False, {"error": translation_result.error_message, "base_pair": base_pair, "user_id": user_id}
    ...
    def _send_bridge_request(self, host: str, port: int, request: Dict, timeout: int = 30) -> Optional[Dict]:
        """
    ...
    def refresh_user_symbols(self, user_id: str) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            # Re-discover symbols
            return self.discover_user_symbols(user_id, connection_info["bridge_id"], connection_info["host"])
    ...
    def get_user_connection_status(self, user_id: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "last_discovery": (
                    connection_info.get("last_discovery", {}).isoformat()
                    if connection_info.get("last_discovery")
                    else None
                ),
            }

        except Exception as e:
            return {"user_id": user_id, "connection_active": False, "error": str(e)}
...
def test_bridge_integration():
    ...
    test_signal = {"pair": "XAUUSD", "direction": "BUY", "lot_size": 0.01, "sl": 250, "tp": 500, "risk": 2.0}

    # Mock user mapping for testing
    symbol_mapper.user_maps["test_user"] = {"XAUUSD": "XAU/USD.r", "EURUSD": "EURUSD.r"}
