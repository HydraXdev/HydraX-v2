...
class ChatNotificationService:
    ...
    def __init__(
        ...
        if enable_celery and Celery:
            try:
                self.celery_app = Celery("chat_notifications", broker=redis_url, backend=redis_url)
        ...
        try:
            if custom_settings:
                ...
                settings = NotificationSettings(
                    user_id=user_id, email_address=email, phone_number=phone, telegram_chat_id=telegram_chat_id
                )
    ...
    def process_chat_message(self, message_data: Dict[str, Any], room_members: List[str]):
            ...
            for user_id in mentioned_users:
                if user_id != sender_id:
                    self.create_notification(
                        ...
                        metadata={"full_message": content, "sender_callsign": callsign},
                    )

            # Create general squad notifications for active users
            eligible_members = [
                user_id for user_id in room_members if user_id != sender_id and user_id not in mentioned_users
            ]
    ...
    def process_file_share(self, user_id: str, filename: str, room_id: str, room_members: List[str]):
    ...
    def process_moderation_action(
        self, moderator_id: str, target_user_id: str, action: str, reason: str, duration: int = 0
    ):
        ...
        try:
            ...
            self.create_notification(
                ...
                metadata={"action": action, "reason": reason, "duration": duration, "moderator_id": moderator_id},
            )
    ...
    def _process_notification(self, notification: ChatNotification):
        ...
        try:
            ...
            delivery_methods = settings.delivery_methods.get(notification.notification_type, [DeliveryMethod.REAL_TIME])
    ...
    def _deliver_notification(
        self, notification: ChatNotification, method: DeliveryMethod, settings: NotificationSettings
    ) -> bool:
        ...
        try:
            ...
            if self.redis_client:
                self.redis_client.lpush(
                    f"real_time_notifications:{notification.user_id}", json.dumps(asdict(notification), default=str)
                )
                self.redis_client.expire(f"real_time_notifications:{notification.user_id}", 3600)  # 1 hour
                return True
    ...
    def _deliver_email(self, notification: ChatNotification, settings: NotificationSettings) -> bool:
        ...
        try:
            ...
            # Send email
            with smtplib.SMTP(self.email_config.get("smtp_host"), self.email_config.get("smtp_port", 587)) as server:
                server.starttls()
    ...
    def _store_notification_in_redis(self, notification: ChatNotification):
        ...
        try:
            if self.redis_client:
                self.redis_client.lpush(
                    f"notifications:{notification.user_id}", json.dumps(asdict(notification), default=str)
                )
    ...
    def _save_user_settings(self, user_id: str):
        ...
        try:
            ...
            data = {
                ...
                "delivery_methods": {
                    t.value: [m.value for m in methods] for t, methods in settings.delivery_methods.items()
                },
...
if __name__ == "__main__":
    ...
    notification_service.register_user(
        user_id="test_user_123", email="operator@bitten.tactical", telegram_chat_id="123456789"
    )
    ...
    notification_service.process_chat_message(
        message_data=message_data, room_members=["test_user_123", "sender_123", "other_user"]
    )
