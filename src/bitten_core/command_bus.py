...
class CommandBus:
    ...
    def partial_close(self, ticket: int, fire_id: str, seq: int, close_pct: float, milestone: str = "") -> bool:
        ...
        cmd = Command(
            ...
            args={"close_pct": close_pct, "milestone": milestone, "comment": comment},
    ...
    def modify_sl(self, ticket: int, fire_id: str, seq: int, new_sl: float, milestone: str = "") -> bool:
        ...
        cmd = Command(
            ...
            seq=seq,
            cmd_type="MODIFY_SL",
            args={"sl_price": new_sl, "milestone": milestone, "comment": comment},
            timestamp_ms=int(time.time() * 1000),
        )
    ...
    def close_all(
        self, ticket: int, fire_id: str, seq: int, reason: str = "", target_uuid: Optional[str] = None
    ) -> bool:
        ...
        if not target_uuid:
            ...
            try:
                ...
                cursor.execute(
                    "SELECT target_uuid FROM fires WHERE ticket=? AND target_uuid IS NOT NULL LIMIT 1", (ticket,)
                )
                ...
                if result:
                    ...
                    if pos and pos.user_id:
                        cursor.execute("SELECT target_uuid FROM ea_instances WHERE user_id=? LIMIT 1", (pos.user_id,))
                        result = cursor.fetchone()
                        if result:
                            target_uuid = result[0]
                conn.close()
        ...
        cmd = Command(
            ...
            seq=seq,
            cmd_type="CLOSE",
            args={"reason": reason, "comment": comment},
            timestamp_ms=int(time.time() * 1000),
    ...
    def start_trail(self, ticket: int, fire_id: str, seq: int, method: str, distance_pips: float) -> bool:
        ...
        cmd = Command(
            ...
            args={"method": method, "distance_pips": distance_pips, "comment": comment},
            timestamp_ms=int(time.time() * 1000),
        )
