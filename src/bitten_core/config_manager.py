...
class TradingPairsConfig:
    ...
    def get_position_limit_for_balance(self, balance: float) -> int:
        ...
        applicable_limits = [
            (threshold, limit) for threshold, limit in self._position_limits.items() if threshold <= balance
        ]
    ...
    def validate_trade_size(
        self, lot_size: float, stop_loss_pips: float, pip_value: float, account_balance: float, tier_name: str
    ) -> Tuple[bool, str]:
    ...
    def calculate_safe_lot_size(
        self, stop_loss_pips: float, pip_value: float, account_balance: float, tier_name: str
    ) -> float:
        ...
        max_risk_pct = min(self._trade_size_validation.max_risk_per_trade, tier_config.default_risk)
