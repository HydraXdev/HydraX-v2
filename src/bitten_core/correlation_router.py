...
from .strategies.correlation_visualizer import CorrelationVisualizer, generate_dashboard_html
from .strategies.cross_asset_correlation import AssetClass, AssetData, CrossAssetCorrelationSystem
...
def update_market_data():
    ...
    try:
        ...
        return jsonify({"success": True, "message": f'Updated data for {data["symbol"]}'})
...
def get_analysis():
    ...
    try:
        ...
        return jsonify({"success": True, "data": serialized_analysis})
...
def get_correlation_matrix():
    ...
    try:
        ...
        if matrix is None:
            return jsonify({"success": False, "error": "Insufficient data for correlation calculation"}), 400

        # Convert to JSON-serializable format
        matrix_data = {"assets": matrix.columns.tolist(), "values": matrix.values.tolist(), "period": period}

        return jsonify({"success": True, "data": matrix_data})
...
def get_divergences():
    ...
    try:
        ...
        return jsonify({"success": True, "data": divergence_data})
...
def get_risk_sentiment():
    ...
    try:
        ...
        return jsonify({"success": True, "data": {"sentiment": sentiment.value, "sector_signals": sector_signals}})
...
def get_heatmap_data():
    ...
    try:
        ...
        if matrix is None:
            return jsonify({"success": False, "error": "No correlation data available"}), 400
...
def get_correlation_prediction(asset1: str, asset2: str):
    ...
    try:
        ...
        prediction = correlation_system.predictive_model.predict_correlation_change(
            asset1.upper(), asset2.upper(), horizon
        )
...
def get_regime_changes():
    ...
    try:
        ...
        return jsonify({"success": True, "data": regime_analysis})
...
def health_check():
    ...
    return jsonify({"success": True, "status": "operational", "timestamp": datetime.now().isoformat()})
