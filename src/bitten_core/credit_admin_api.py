...
def get_user_credits(user_id: str):
    ...
    try:
        ...
        return jsonify({"success": True, "user_id": user_id, "stats": stats})
...
def manually_apply_credit():
    ...
    try:
        ...
        if result["success"]:
            ...
            return jsonify(
                {"success": True, "message": f"Applied ${amount} credit to user {user_id}", "result": result}
            )
        else:
...
def revoke_credit():
    ...
    try:
        ...
        if result["success"]:
            ...
            return jsonify(
                {"success": True, "message": f"Revoked ${amount} credit from user {user_id}", "result": result}
            )
        else:
        ...
        return jsonify(
            {
                ...
                "search_params": {"user_id": user_id, "referral_code": referral_code, "limit": limit},
            }
        )
