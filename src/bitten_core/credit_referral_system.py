...
class CreditReferralSystem:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_referral_credits_referred ON referral_credits(referred_user_id)"
            )
    ...
    def generate_referral_code(self, user_id: str, custom_code: Optional[str] = None) -> str:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute("SELECT code FROM referral_codes WHERE user_id = ? AND is_active = 1", (user_id,))
            ...
            # Insert new code
            conn.execute("INSERT INTO referral_codes (code, user_id) VALUES (?, ?)", (code, user_id))
            conn.commit()
    ...
    def use_referral_code(self, code: str, new_user_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute("SELECT user_id FROM referral_codes WHERE code = ? AND is_active = 1", (code,))
            ...
            cursor = conn.execute("SELECT id FROM referral_credits WHERE referred_user_id = ?", (new_user_id,))
            if cursor.fetchone():
            ...
            conn.execute("UPDATE referral_codes SET uses_count = uses_count + 1 WHERE code = ?", (code,))
    ...
    def confirm_payment_and_apply_credit(
        self, referred_user_id: str, invoice_id: Optional[str] = None
    ) -> Optional[str]:
    ...
    def _update_user_balance(
        self, conn, user_id: str, pending_credit: float = 0, total_credit: float = 0, applied_credit: float = 0
    ):
    ...
    def apply_credit_to_invoice(self, user_id: str, invoice_amount: float, invoice_id: str) -> Tuple[float, float]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Update balances
            self._update_user_balance(conn, user_id, total_credit=-credit_to_apply, applied_credit=credit_to_apply)
    ...
    def get_admin_stats(self) -> Dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                "SELECT SUM(credit_amount) FROM referral_credits WHERE credited = 1 AND credit_amount > 0"
            )
            ...
            # Pending credits
            cursor = conn.execute(
                "SELECT SUM(credit_amount) FROM referral_credits WHERE credited = 0 AND credit_amount > 0"
            )
            ...
            stats["top_referrers"] = [
                {"user_id": row[0], "referrals": row[1], "total_earned": row[2]} for row in cursor.fetchall()
            ]
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in results:
                    ...
                    top_referrers.append(
                        {"user_id": user_id, "referral_count": referral_count, "total_credits": total_credits or 0.0}
                    )
    ...
    def mark_gamification_rewards_processed(self, reward_ids: List[int]):
        ...
        with sqlite3.connect(self.db_path) as conn:
            for reward_id in reward_ids:
                conn.execute("UPDATE gamification_rewards SET processed = 1 WHERE id = ?", (reward_id,))
            conn.commit()
