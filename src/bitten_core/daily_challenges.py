...
class DailyChallengeManager:
    ...
    def track_trade_action(self, user_id: str, action_type: str, trade_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for challenge in self.user_challenges[user_id]:
            ...
            if updated and progress.progress >= progress.target and not progress.completed:
                ...
                completed_challenges.append(
                    {"challenge_id": challenge.challenge_id, "name": challenge.name, "xp_reward": challenge.xp_reward}
                )
    ...
    def create_weekly_event(
        self, event_type: EventType, name: str, description: str, duration_hours: int, config: Dict[str, Any]
    ) -> WeeklyEvent:
    ...
    def _generate_user_challenges(self, user_id: str) -> None:
        ...
        challenges = []
        expires_at = datetime.combine(datetime.now().date() + timedelta(days=1), time(0, 0))
    ...
    def _get_challenge_progress(self, user_id: str, challenge_id: str) -> Optional[ChallengeProgress]:
    ...
    def _get_or_create_progress(self, user_id: str, challenge: Challenge) -> ChallengeProgress:
    ...
    def _ensure_daily_challenges(self) -> None:
        ...
        for user_id in list(self.user_challenges.keys()):
            active_challenges = [c for c in self.user_challenges[user_id] if not c.is_expired()]
...
if __name__ == "__main__":
    ...
    completed = manager.track_trade_action(user_id, "trade_opened", {"execution_seconds": 45})
