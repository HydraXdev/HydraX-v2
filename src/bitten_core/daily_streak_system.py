...
from .database.models import RewardClaim, UserLoginStreak
...
class DailyStreakSystem:
    ...
    def record_login(self, user_id: str, user_timezone: str = "UTC") -> Tuple[StreakData, LoginReward]:
        ...
        self._record_daily_login(
            user_id, today_date, now.isoformat(), user_timezone, streak_data.current_streak, login_reward.total_xp
        )
    ...
    def _calculate_new_streak(
        self, user_id: str, last_login_date: Optional[str], today_date: str, timezone: str
    ) -> StreakData:
        ...
        return StreakData(
            ...
            days_until_milestone=(
                self._get_next_milestone(new_streak) - new_streak if self._get_next_milestone(new_streak) else None
            ),
    ...
    def _get_current_streak_data(self, user_id: str, timezone: str) -> StreakData:
        ...
        with get_db_session() as session:
            ...
            return StreakData(
                ...
                days_until_milestone=(
                    self._get_next_milestone(user_streak.current_streak) - user_streak.current_streak
                    if self._get_next_milestone(user_streak.current_streak)
                    else None
                ),
    ...
    def _record_daily_login(
        self, user_id: str, login_date: str, timestamp: str, timezone: str, streak_day: int, xp_earned: int
    ):
    ...
    def get_streak_calendar(self, user_id: str, year: int, month: int) -> Dict[str, Any]:
        ...
        for row in cursor.fetchall():
            calendar_data[row[0]] = {"status": row[1], "streak_count": row[2], "notes": row[3]}
    ...
    def get_user_streak_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        recent_logins = [{"date": row[0], "streak_day": row[1], "xp_earned": row[2]} for row in cursor.fetchall()]
        ...
        protection_history = [{"type": row[0], "date": row[1], "streak_saved": row[2]} for row in cursor.fetchall()]
    ...
    def get_leaderboard(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        with get_db_session() as session:
            top_streaks = (
                session.query(UserLoginStreak)
                .order_by(UserLoginStreak.current_streak.desc(), UserLoginStreak.longest_streak.desc())
                .limit(limit)
                .all()
            )
