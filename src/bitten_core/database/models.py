...
class UserCampaignProgress(Base):
    ...
    # Indexes
    __table_args__ = (Index("idx_user_campaign_progress_user_campaign", "user_id", "campaign_id", unique=True),)
...
class RewardClaim(Base):
    ...
    def __repr__(self):
        return (
            f"<RewardClaim(user_id='{self.user_id}', reward_type='{self.reward_type}', amount='{self.reward_amount}')>"
        )
...
class ComebackBonus(Base):
    ...
    def __repr__(self):
        return (
            f"<ComebackBonus(user_id='{self.user_id}', days_away={self.days_away}, multiplier={self.bonus_multiplier})>"
        )
