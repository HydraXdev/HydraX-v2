...
class DiagnosticSentry:
    ...
    def check_partial_milestone(self, ticket: int, current_r: float, milestone: str) -> bool:
        """
        ...
        if current_r < self.alert_partial_min_r:
            violation = Violation(
                ...
                details={"current_r": current_r, "min_r": self.alert_partial_min_r, "milestone": milestone},
                severity="HIGH",
            )

            self._record_violation(violation)
            logger.error(
                f"🚨 PREMATURE PARTIAL: Ticket {ticket} at {current_r:.2f}R < {self.alert_partial_min_r}R minimum"
            )

            # Alert via Telegram
            self._send_alert(
                f"⚠️ PREMATURE PARTIAL DETECTED\nTicket: {ticket}\nR: {current_r:.2f}\nExpected: >{self.alert_partial_min_r}"
            )
    ...
    def check_be_milestone(self, ticket: int, current_r: float, tp1_done: bool) -> bool:
        """
        ...
        if not tp1_done:
            violation = Violation(
                ...
                details={"current_r": current_r, "tp1_done": tp1_done},
                severity="HIGH",
            )
        ...
        if current_r < self.alert_be_min_r:
            violation = Violation(
                ...
                details={"current_r": current_r, "min_r": self.alert_be_min_r},
                severity="MEDIUM",
            )
    ...
    def check_bad_exit(self, ticket: int, exit_r: float) -> bool:
        ...
        if exit_r < 0.2:
            ...
            violation = Violation(
                ...
                details={"exit_r": exit_r, "streak": self.bad_exit_streak},
    ...
    def check_command_error(self, ticket: int, cmd_type: str, error_code: str, details: Dict) -> bool:
        """
        ...
        violation = Violation(
            ...
            details={"cmd_type": cmd_type, "error_code": error_code, **details},
            severity="MEDIUM",
        )
