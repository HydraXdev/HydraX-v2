...
class DrillReportBotIntegration:
    ...
    async def cmd_drill_report(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text(telegram_report, reply_markup=reply_markup, parse_mode="Markdown")

        except Exception as e:
            logger.error(f"Error in drill report command: {e}")
            await update.message.reply_text("⚠️ Unable to generate drill report. Try again later, soldier.")
...
— DRILL SERGEANT 🎖️"""
            ...
            await update.message.reply_text("⚠️ Unable to generate weekly summary. Systems temporarily down.")
...
def register_drill_report_handlers(bot_app, drill_system):
    ...
    bot_app.add_handler(
        CallbackQueryHandler(
            drill_integration.handle_drill_callback, pattern="^(weekly_|settings_|strategy_|achievements_)"
        )
    )
    ...
    def record_trade_completion(user_id: str, trade_result: Dict):
        ...
        if xp_economy:
            ...
            xp_result = xp_economy.award_trade_xp(
                user_id=user_id, trade_result=trade_result.get("result", "LOSS"), context=context
            )
        ...
        if not current_stats:
            ...
            stats = DailyTradingStats(
                ...
                shots_remaining=(
                    tactical_manager.TACTICAL_CONFIGS[daily_state.selected_strategy].max_shots - daily_state.shots_fired
                    if daily_state.selected_strategy
                    else 0
                ),
                max_shots=(
                    tactical_manager.TACTICAL_CONFIGS[daily_state.selected_strategy].max_shots
                    if daily_state.selected_strategy
                    else 0
                ),
        ...
        enhanced_result = {**trade_result, "xp_awarded": xp_awarded, "daily_stats": stats}
