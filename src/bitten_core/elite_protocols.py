...
class EliteProtocolManager:
    ...
    def get_protocol_config(self, user_id: str, protocol_type: ProtocolType) -> Optional[Any]:
    ...
    def update_protocol_config(self, user_id: str, protocol_type: ProtocolType, updates: Dict[str, Any]) -> bool:
    ...
    def check_trailing_guard(
        self, user_id: str, trade_id: str, current_profit_pips: float, current_sl_pips: float
    ) -> Optional[float]:
    ...
    def check_split_command(
        self, user_id: str, trade_id: str, current_profit_pips: float, tp1_pips: float
        ...
        if current_profit_pips >= tp1_pips:
            ...
            self._record_protocol_activation(user_id, trade_id, ProtocolType.SPLIT_COMMAND, action)
    ...
    def prepare_stealth_entry(
        self, user_id: str, signal_price: float, pair: str, direction: str  # "buy" or "sell"
    ...
    def check_fortress_mode(self, user_id: str, base_risk_percent: float) -> float:
                    ...
                    active.append(
                        {
                            ...
                            "active_trades": len(
                                [p for p in self.active_protocols.get(user_id, []) if p.protocol_type == protocol_type]
                            ),
                        }
                    )
    ...
    def _record_protocol_activation(
        self, user_id: str, trade_id: str, protocol_type: ProtocolType, metadata: Dict[str, Any]
    ) -> None:
