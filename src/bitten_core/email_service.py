...
class EmailService:
    ...
    def _send_sendgrid(
        ...
        # Add tracking data
        if tracking_data:
            message.tracking_settings = {"click_tracking": {"enable": True}, "open_tracking": {"enable": True}}
        ...
        if attachments:
            for attachment in attachments:
                ...
                message.add_attachment(
                    encoded_content, attachment.get("type", "application/octet-stream"), attachment["filename"]
                )
    ...
    def _send_ses(
        ...
        try:
            ...
            response = self.ses_client.send_email(
                Source=f"{self.from_name} <{self.from_email}>", Destination={"ToAddresses": [to_email]}, Message=message
            )
    ...
    def send_template_email(
        ...
        try:
            ...
            return self.send_email(
                to_email=to_email, subject=subject, html_content=html_content, tracking_data=tracking_data
            )
        except Exception as e:
    ...
    def send_batch_emails(
        self, recipients: List[Dict[str, Any]], template_name: str, default_data: Dict[str, Any]
    ) -> Dict[str, bool]:
...
class EmailQueueManager:
    ...
    def add_to_queue(
        self, to_email: str, template_name: str, template_data: Dict[str, Any], send_at: datetime, priority: int = 5
    ):
        """Add email to queue"""
        self.queue.append(
            {
    ...
    def process_queue(self) -> Dict[str, int]:
        ...
        for email in pending:
            success = self.email_service.send_template_email(
                to_email=email["to_email"], template_name=email["template_name"], template_data=email["template_data"]
            )
