...
from .emergency_stop_controller import EmergencyStopEvent, EmergencyStopLevel, EmergencyStopTrigger
...
class EmergencyNotificationSystem:
    ...
    def send_emergency_notification(
        self, event: EmergencyStopEvent, affected_users: List[int] = None, additional_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
            ...
            for channel in template.channels:
                try:
                    result = self._send_to_channel(
                        channel, template.title, formatted_message, template, notification_data
                    )
    ...
    def send_recovery_notification(
        self, recovery_data: Dict[str, Any], affected_users: List[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            formatted_message = template.message.format(
                recovery_time=recovery_data.get("recovery_time", "now"), user_id=recovery_data.get("user_id", "system")
            )
            ...
            return {"success": True, "notification_id": notification_data["id"], "delivery_results": delivery_results}
    ...
    def _send_telegram_notification(
        self, title: str, message: str, template: NotificationTemplate, notification_data: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for user_id in affected_users:
                try:
                    result = self.telegram_handler.send_message(user_id, telegram_message, **keyboard_data)
    ...
    def _send_system_log_notification(
        self, title: str, message: str, template: NotificationTemplate, notification_data: Dict
    ) -> Dict[str, Any]:
