...
class EnhancedErrorHandler:
    ...
    def log_error(self, error_event: ErrorEvent):
        ...
        try:
            # Log to file
            self.logger.error(f"[{error_event.category.value}] {error_event.error_code}: {error_event.message}")
    ...
    def attempt_recovery(self, error_event: ErrorEvent) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(f"Recovery attempt {attempt + 1}/{max_retries} for {error_event.error_code}")
    ...
    def should_break_circuit(self, error_code: str) -> bool:
        ...
        now = time.time()
        recent_failures = [timestamp for timestamp in breaker.get("failures", []) if now - timestamp < time_window]
    ...
    def update_circuit_breaker(self, error_code: str, success: bool):
        ...
        if error_code not in self.circuit_breakers:
            self.circuit_breakers[error_code] = {"failures": [], "last_success": None}
    ...
    def get_user_friendly_message(self, error_event: ErrorEvent) -> str:
        ...
        base_message = category_messages.get(error_event.category, "An unexpected error occurred.")
    ...
    def get_error_stats(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            # Total errors
            cursor.execute("SELECT COUNT(*) FROM error_events WHERE timestamp > ?", (cutoff,))
            ...
            return {
                ...
                "circuit_breakers_active": len(
                    [code for code in self.circuit_breakers if self.should_break_circuit(code)]
                ),
            }
...
def handle_errors(
    category: ErrorCategory, severity: ErrorSeverity = ErrorSeverity.MEDIUM, error_code: Optional[str] = None
):
...
def error_recovery_context(
    handler: EnhancedErrorHandler, category: ErrorCategory, error_code: str, context: Dict[str, Any]
):
    ...
    try:
        ...
        result = handler.handle_error(
            error=e, category=category, severity=ErrorSeverity.MEDIUM, error_code=error_code, context=context
        )
...
def get_system_health() -> Dict[str, Any]:
    ...
    health_status = (
        "healthy"
        if health_score >= 90
        else "warning" if health_score >= 70 else "degraded" if health_score >= 50 else "critical"
    )
