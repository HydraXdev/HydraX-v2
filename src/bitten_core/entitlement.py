...
class EntitlementManager:
    ...
    def set_user_tier(self, user_id: str, tier: str, changed_by: str = "system", reason: str = "") -> bool:
    ...
    def get_user_history(self, user_id: str) -> List[Dict]:
        ...
        for row in cursor.fetchall():
            history.append(
                {"old_tier": row[0], "new_tier": row[1], "changed_at": row[2], "changed_by": row[3], "reason": row[4]}
            )
...
def main():
    ...
    set_parser.add_argument(
        "--tier", required=True, choices=["TIER_BEGINNER", "TIER_PLUS", "TIER_PRO"], help="Tier to assign"
    )
    ...
    if args.command == "set":
        success = entitlement_manager.set_user_tier(args.user, args.tier, "CLI", args.reason)
        if success:
