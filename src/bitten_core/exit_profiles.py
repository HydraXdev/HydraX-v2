...
from .state_store import Position, PositionState, state_store
from .symbols import (
    be_offset_pips,
...
class ExitProfileManager:
    ...
    def _handle_beginner_tick(self, pos: Position, market: MarketData, config: Dict) -> bool:
        """
        ...
        if self._check_rr_reached(pos, market, config["RR"]):
            ...
            command_bus.close_all(ticket=pos.ticket, fire_id=pos.fire_id, seq=seq, reason=f"TARGET_{config['RR']}R")
    ...
    def _handle_plus_pro_tick(self, pos: Position, market: MarketData, config: Dict) -> bool:
        """
        ...
        if not pos.tp1_done and self._check_rr_reached(pos, market, config["TP1_R"]):
            ...
            if state_store.check_milestone_idempotent(pos.ticket, "TP1"):
                ...
                command_bus.partial_close(
                    ticket=pos.ticket, fire_id=pos.fire_id, seq=seq, close_pct=config["TP1_CLOSE_PCT"], milestone="TP1"
                )

                # Update position state
                remaining_lots = pos.lot_size * (1 - config["TP1_CLOSE_PCT"])
                state_store.update_position(
                    pos.ticket, tp1_done=True, lot_remaining=remaining_lots, state=PositionState.TP1_DONE
                )
    ...
    def _check_rr_reached(self, pos: Position, market: MarketData, target_r: float) -> bool:
        """
        ...
        if pos.direction.upper() == "BUY":
            ...
            target_price = price_plus_pips(pos.symbol, pos.entry_px, "BUY", target_pips)
            return current_price >= target_price
        else:
            # For shorts, use ask to check profit
            current_price = market.ask
            target_price = price_plus_pips(pos.symbol, pos.entry_px, "SELL", target_pips)
            return current_price <= target_price
    ...
    def _move_to_breakeven(self, pos: Position, market: MarketData) -> bool:
        ...
        command_bus.modify_sl(
            ticket=pos.ticket, fire_id=pos.fire_id, seq=seq, new_sl=be_price, milestone="BE_AFTER_TP1"
        )

        # Update position
        state_store.update_position(pos.ticket, be_set=True, sl_current_px=be_price, state=PositionState.BE_SET)

        logger.info(f"Moved {pos.ticket} to BE at {be_price} (offset {offset:.1f} pips)")
        return True

    def _start_trailing(self, pos: Position, market: MarketData, config: Dict) -> bool:
        ...
        # Send trail command
        seq = state_store.get_next_seq(pos.ticket)
        command_bus.start_trail(ticket=pos.ticket, fire_id=pos.fire_id, seq=seq, method=method, distance_pips=distance)

        # Update position
        state_store.update_position(pos.ticket, trail_on=True, state=PositionState.TRAILING)

        logger.info(f"Started {method} trailing for {pos.ticket} at {distance:.1f} pips")
        return True

    def _update_trailing_stop(self, pos: Position, market: MarketData, config: Dict) -> bool:
    ...
    def on_position_open(
        ...
        PositionTimer.set_max_hold_timer(ticket=ticket, fire_id=fire_id, tier=user_tier, callback=timer_callback)
    ...
    def on_timer_expired(self, ticket: int, fire_id: str, reason: str):
        ...
        if pos.user_tier == "TIER_BEGINNER" or not pos.tp1_done:
            ...
            command_bus.close_all(ticket=ticket, fire_id=fire_id, seq=seq, reason=reason)
