...
from .fire_modes import TIER_CONFIGS, ChaingunState, FireMode, TierLevel
...
class FireModeValidator:
    ...
    def _validate_fang(self, payload: Dict, profile: Dict) -> ValidationResult:
        ...
        if profile.get("shots_today", 0) >= 10:
            return ValidationResult(
                ...
                bot_responses={"drillbot": "EVEN PREDATORS NEED REST.", "bit": "*protective nuzzle*"},
            )
    ...
    def _validate_apex(self, payload: Dict, profile: Dict) -> ValidationResult:
        ...
        return ValidationResult(
            ...
            bot_responses={"drillbot": "YOU ARE THE WEAPON.", "bit": "*proud purr*"},
        )
    ...
    def _validate_chaingun_mode(self, payload: Dict, profile: Dict) -> ValidationResult:
        ...
        if not sequence:
            return ValidationResult(
                ...
                bot_responses={"drillbot": "INITIATE SEQUENCE FIRST."},
            )
    ...
    def _validate_semi_auto(self, payload: Dict, profile: Dict) -> ValidationResult:
        ...
        return ValidationResult(
            ...
            bot_responses={"drillbot": "MANUAL CONTROL ENGAGED. CHOOSE WISELY.", "bit": "*ready stance*"},
        )
    ...
    def _validate_midnight_hammer(self, payload: Dict, profile: Dict) -> ValidationResult:
        ...
        if not hammer_event:
            return ValidationResult(
                ...
                bot_responses={"drillbot": "FALSE ALARM. NO HAMMER CALLED."},
            )
        ...
        if payload.get("tcs", 0) < 95:
            return ValidationResult(
                ...
                bot_responses={"drillbot": "NOT ENOUGH FORCE FOR THE HAMMER.", "bit": "*disappointed meow*"},
            )
        ...
        if last_shot and isinstance(last_shot, datetime):
            ...
            if time_since_shot < 1800:  # 30 minutes
                ...
                return ValidationResult(
                    ...
                    bot_responses={"drillbot": "PATIENCE. RELOAD IN PROGRESS.", "bit": "*guarding the trigger*"},
                )
    ...
    def _validate_risk_limits(self, payload: Dict, profile: Dict) -> ValidationResult:
        ...
        risk_percent, risk_reason = risk_controller.get_user_risk_percent(profile["user_id"], risk_tier)
    ...
    def _check_kill_switch(self) -> ValidationResult:
        ...
        if self.emergency_controller.is_active():
            ...
            return ValidationResult(
                valid=False, reason=f"EMERGENCY STOP ACTIVE: {trigger.upper()} - {reason}", bot_responses=bot_responses
            )
    ...
    def _validate_tcs(self, tcs: float, tier: TierLevel, mode: FireMode) -> ValidationResult:
        ...
        if not tier_config:
            return ValidationResult(
                valid=False, reason="INVALID TIER", bot_responses={"drillbot": "SYSTEM ERROR. REPORT THIS."}
            )
        ...
        if tcs < 76:
            ...
            from .bit_warnings import BitWarningSystem, check_low_tcs_warning
