...
class FireModeValidator:
    ...
    def can_fire(
        self, user_id: int, tier: TierLevel, mode: FireMode, tcs_score: int, pair: str = None
    ) -> Tuple[bool, str]:
    ...
    def _get_chaingun_sequences_today(self, user_id: int) -> List[ChaingunState]:
        ...
        return [s for s in self.chaingun_sequences[user_id] if s.start_time.date() == today]
...
from .stealth_protocol import StealthLevel, get_stealth_protocol
...
class MidnightHammerEvent:
    ...
    def can_trigger(self, tcs_score: int, active_users: int, total_users: int) -> Tuple[bool, str]:
    ...
    def _get_events_this_month(self) -> List:
        ...
        return [e for e in self.event_history if e["timestamp"].month == current_month]
