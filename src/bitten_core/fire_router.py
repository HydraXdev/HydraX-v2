...
class DirectAPIManager:
    ...
    def __init__(self, api_endpoint: str = "api.broker.local", max_retries: int = 3, retry_delay: float = 1.0):
...
class AdvancedValidator:
    ...
    def validate_trade(self, request: TradeRequest, user_profile: Optional[Dict] = None) -> Tuple[bool, str]:
        ...
        if is_crypto:
            ...
            return (
                False,
                f"Unsupported symbol: {request.symbol}. Supported: {', '.join(self.crypto_symbols | self.forex_symbols)}",
            )
    ...
    def _get_min_tcs_for_profile(self, user_profile: Optional[Dict]) -> float:
        ...
        tier_requirements = {"nibbler": 10.0, "fang": 10.0, "commander": 10.0, "apex": 10.0}
    ...
    def _validate_user_profile(self, request: TradeRequest, user_profile: Dict) -> Tuple[bool, str]:
        ...
        daily_limits = {"nibbler": 6, "fang": 10, "commander": 20, "apex": 50}
        ...
        position_limits = {"nibbler": 1, "fang": 2, "commander": 3, "apex": 5}
    ...
    def _validate_risk_management(self, request: TradeRequest, user_profile: Optional[Dict]) -> Tuple[bool, str]:
        ...
        max_volumes = {"XAUUSD": 0.1, "BTCUSD": 0.01, "ETHUSD": 0.05}
...
class FireRouter:
    ...
    def __init__(self, api_endpoint: str = "api.broker.local", execution_mode: ExecutionMode = ExecutionMode.LIVE):
    ...
    def execute_trade_request(
        self, request: TradeRequest, user_profile: Optional[Dict] = None, use_fresh_packet: bool = True
        ...
        try:
            ...
            if user_profile and user_profile.get("telegram_id"):
                try:
                    ...
                    can_open, reason = mission_timing.can_open_position(user_profile["telegram_id"], user_tier)
            ...
            if user_profile and user_profile.get("telegram_id"):
                try:
                    ...
                    signal_data = {"stop_pips": request.stop_loss_pips or 20, "symbol": request.symbol}

                    can_trade, enforcement_message = enforce_handshake_before_fire(
                        user_profile["telegram_id"], signal_data
                    )
                        ...
                        if vitality_metrics.vitality_score >= 20:
                            ...
                            try:
                                market_response = requests.get(
                                    f"http://localhost:8001/market-data/venom-feed?symbol={request.symbol}", timeout=1
                                )
                            ...
                            fresh_packet = fresh_builder.build_fresh_packet(
                                mission_data, vitality_metrics, user_profile or {}, current_market
                            )
                            ...
                            if is_valid:
                                ...
                                logger.info(
                                    f"Using fresh fire packet for {request.symbol}: Entry adjusted by {fresh_packet.entry_adjustment_pips:.1f} pips"
                                )
                            else:
                                logger.warning(f"Fresh packet validation failed: {error_msg}")
                        else:
                            logger.warning(
                                f"Signal vitality too low ({vitality_metrics.vitality_score:.1f}%) - using original parameters"
                            )
            ...
            if execution_result["success"]:
                with self._stats_lock:
                    ...
                    self.stats["average_execution_time"] = (total_time + execution_time_ms) / self.stats[
                        "successful_trades"
                    ]
                ...
                if user_profile and user_profile.get("telegram_id"):
                    try:
                        ...
                        signal_type = (
                            "PRECISION_STRIKE"
                            if request.signal_type and "PRECISION" in request.signal_type.upper()
                            else "RAPID_ASSAULT"
                        )
                        mission_timing.register_position(user_profile["telegram_id"], request.mission_id, signal_type)
    ...
    def _execute_direct_api(self, request: TradeRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("success"):
                logger.info(
                    f"✅ Trade executed successfully via direct API: {request.symbol} {request.direction.value} {request.volume}"
                )
    ...
    def _log_trade_result(self, request: TradeRequest, result: TradeExecutionResult):
        ...
        if result.success:
            logger.info(
                f"Trade executed: {request.symbol} {request.direction.value} {request.volume} - {result.message}"
            )
        else:
            logger.warning(
                f"Trade failed: {request.symbol} {request.direction.value} {request.volume} - {result.message}"
            )
    ...
    def ping_api(self, user_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "message": f"API ping failed: {str(e)}", "status": "error"}
