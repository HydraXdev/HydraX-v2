...
class EnhancedFireRouter:
    ...
    def execute_trade_with_translation(
        self, user_id: str, signal: Dict, user_profile: Optional[Dict] = None
    ) -> FireRouterResult:
        """
    ...
    def _validate_translated_signal(self, translated_signal: Dict, user_profile: Optional[Dict] = None) -> Dict:
        """
        ...
        try:
            ...
            if symbol_info:
                ...
                if lot_size < min_lot:
                    return {"valid": False, "error": f"Lot size {lot_size} below minimum {min_lot}"}

                if lot_size > max_lot:
                    return {"valid": False, "error": f"Lot size {lot_size} above maximum {max_lot}"}

            # Validate direction
            direction = translated_signal.get("direction", "").upper()
            if direction not in ["BUY", "SELL"]:
                return {"valid": False, "error": f"Invalid direction: {direction}"}
            ...
            if user_profile:
                ...
                if user_tier == "press_pass":
                    ...
                    if translated_signal.get("lot_size", 0) > max_lot_tier:
                        return {"valid": False, "error": f"Lot size exceeds tier limit: {max_lot_tier}"}
    ...
    def _execute_translated_trade(
        self, user_id: str, translated_signal: Dict, user_profile: Optional[Dict] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            if not bridge_id:
                return {"success": False, "error": "No bridge connection found for user"}
    ...
    def _log_execution(self, result: FireRouterResult):
        ...
        try:
            log_entry = {
                ...
                "fallback_used": (
                    result.translation_info.get("fallback_used", False) if result.translation_info else False
                ),
...
def execute_trade_with_translation(user_id: str, signal: Dict, user_profile: Optional[Dict] = None) -> FireRouterResult:
...
class SymbolAwareFireRouter:
    ...
    def execute_trade(self, user_id: str, signal: Dict, **kwargs) -> Dict:
        ...
        result = self.enhanced_router.execute_trade_with_translation(user_id, signal, user_profile)
...
def test_fire_router_integration():
    ...
    symbol_mapper.user_maps[user_id] = {"XAUUSD": "XAU/USD.r", "EURUSD": "EURUSD.r"}

    symbol_mapper.user_symbol_info[user_id] = {
        "XAU/USD.r": {"digits": 2, "point": 0.01, "volume_min": 0.01, "volume_max": 100.0, "volume_step": 0.01}
    }

    # Mock connection status
    bridge_integration.active_connections[user_id] = {"bridge_id": "bridge_001", "mapping_ready": True}

    # Test signal
    test_signal = {"pair": "XAUUSD", "direction": "BUY", "lot_size": 0.01, "sl": 1950.0, "tp": 1970.0, "risk": 2.0}

    # Test user profile
    test_profile = {"tier": "fang", "balance": 1000.0}
