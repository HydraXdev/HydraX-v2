...
from .gear_system import GearItem, GearRarity, GearSlot, GearStats, GearSystem, GearType, Loadout
...
class GearCommands:
    ...
    async def show_crafting(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if recipes_available:
            keyboard.append([InlineKeyboardButton("🔨 Start Crafting", callback_data="craft_start")])
    ...
    async def show_gear_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        for gear_type, rarity_dict in display["items_by_type"].items():
            ...
            if all_items:
                ...
                message += (
                    f"• {gear_type.value}: {best_item.get_display_name()} (Power: {best_item.get_power_score()})\n"
                )
        ...
        if display["active_loadout"]:
            loadout_power = display["active_loadout"].get_power_score(self.gear_system.get_user_inventory(user_id))
    ...
    async def handle_loadout_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if action == "loadout_create":
            ...
            if new_loadout:
                ...
                await query.edit_message_text("*❌ Failed to create loadout*", parse_mode="Markdown")
            ...
            if len(loadouts) <= 1:
                await query.edit_message_text(
                    "*You need at least 2 loadouts to switch between them.*", parse_mode="Markdown"
                )
            else:
                keyboard = []
                ...
                reply_markup = InlineKeyboardMarkup(keyboard)
                await query.edit_message_text(
                    "*Select loadout to activate:*", parse_mode="Markdown", reply_markup=reply_markup
                )
    ...
    async def handle_craft_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if action == "craft_start":
            ...
            for recipe_id, recipe in self.gear_system.CRAFTING_RECIPES.items():
                ...
                if result_item:
                    keyboard.append(
                        [InlineKeyboardButton(result_item.get_display_name(), callback_data=f"craft_item_{recipe_id}")]
                    )

            keyboard.append([InlineKeyboardButton("Cancel", callback_data="gear_back_to_menu")])

            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text("*Select item to craft:*", parse_mode="Markdown", reply_markup=reply_markup)
            ...
            if success:
                ...
                await query.edit_message_text(f"*❌ Crafting Failed*\n\n{message}", parse_mode="Markdown")

        elif action == "craft_materials":
            inventory = self.gear_system.get_user_inventory(user_id)
            materials = [item for item in inventory.values() if "craft_material" in item.item_id]
    ...
    def get_conversation_handler(self) -> ConversationHandler:
        ...
        return ConversationHandler(
            ...
            states={
                GEAR_MENU: [CallbackQueryHandler(self.gear_menu_handler, pattern="^gear_")],
                INVENTORY_VIEW: [
...
async def equip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "*Usage:* `/equip <item_name>`\n\n" "Example: `/equip MACD Elite`", parse_mode="Markdown"
        )
        return
    ...
    if not found_item:
        await update.message.reply_text(f"*Item '{item_name}' not found in inventory*", parse_mode="Markdown")
        return
    ...
    if success:
        await update.message.reply_text(
            f"*✅ {message}*\n\n" f"Power Score: {found_item.get_power_score()}", parse_mode="Markdown"
        )
    else:
...
async def salvage_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "*Usage:* `/salvage <item_name>`\n\n" "⚠️ *Warning: This will destroy the item!*", parse_mode="Markdown"
        )
        return
    ...
    keyboard = [
        [
            InlineKeyboardButton("✅ Confirm Salvage", callback_data=f"confirm_salvage_{found_item.item_id}"),
...
async def handle_salvage_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("confirm_salvage_"):
        ...
        if success:
            await query.edit_message_text(
                f"*✅ {message}*\n\n" "Materials have been added to your inventory.", parse_mode="Markdown"
            )
        else:
...
async def gear_drop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    item = gear_system.generate_random_drop(user_level=50, luck_multiplier=1.5)  # Would get actual level
