...
from .gear_system import GearItem, GearRarity, GearSlot, GearStats, GearType, Loadout
...
class GearVisualizer:
    ...
    def format_loadout_display(
        loadout: Loadout, gear_items: Dict[str, GearItem], set_bonus: Optional[GearStats] = None
    ) -> str:
    ...
    @staticmethod
    def format_inventory_grid(inventory: Dict[str, GearItem], max_items: int = 20) -> str:
        ...
        # Sort groups
        sorted_groups = sorted(grouped.items(), key=lambda x: (-x[0][1].value_multiplier, x[0][0].value))
    ...
    @staticmethod
    def create_inventory_image(inventory: Dict[str, GearItem], title: str = "TACTICAL INVENTORY") -> bytes:
        ...
        # Draw title
        draw.text((width // 2, 20), title, fill=(255, 255, 255), font=title_font, anchor="mt")

        # Sort inventory by rarity and power
        sorted_items = sorted(
            inventory.values(), key=lambda x: (x.rarity.value_multiplier, x.get_power_score()), reverse=True
        )
        ...
        for idx, item in enumerate(sorted_items[:35]):  # Max 35 items shown
            ...
            # Draw cell border
            draw.rectangle([x, y, x + cell_size, y + cell_size], outline=color, width=3)
            ...
            # Draw level
            draw.text((x + 5, y + cell_size - 20), f"Lv{item.level}", fill=(200, 200, 200), font=item_font)

            # Draw power score
            power_text = str(item.get_power_score())
            draw.text(
                (x + cell_size - 5, y + cell_size - 20), power_text, fill=(255, 255, 0), font=item_font, anchor="ra"
            )
    ...
    @staticmethod
    def format_crafting_progress(recipe_id: str, start_time: int, completion_time: int, current_time: int) -> str:
...
class GearStatsFormatter:
    ...
    @staticmethod
    def format_enhanced_stats(base_stats: Dict[str, float], gear_stats: Dict[str, float]) -> str:
