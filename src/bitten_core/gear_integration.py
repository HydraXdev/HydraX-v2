...
from .gear_system import (
    ...
    GearSystem,
    GearType,
    award_gear_for_achievement,
    award_gear_for_milestone,
)
...
class GearDropManager:
    ...
    def check_drop_eligibility(self, user_id: int, event_type: str) -> Tuple[bool, float]:
    ...
    def calculate_luck_multiplier(self, user_id: int, user_stats: Dict[str, Any]) -> float:
    ...
    def process_trade_completion(self, user_id: int, trade_result: Dict[str, Any]) -> Optional[GearItem]:
        ...
        item = self.gear_system.generate_random_drop(
            user_level=user_level, luck_multiplier=luck, guaranteed_rarity=profit_tier
        )
        ...
        # Add to inventory
        self.gear_system.add_item_to_inventory(user_id, item, source=f"trade_profit_{trade_result['profit']:.0f}")
    ...
    def process_achievement_unlock(
        self, user_id: int, achievement_id: str, achievement_tier: AchievementTier
    ) -> Optional[GearItem]:
        ...
        # Award gear
        item = award_gear_for_achievement(self.gear_system, user_id, achievement_id, achievement_tier.value)

        if item and preferred_type:
            # Try to match the gear type to achievement theme
            if random.random() < 0.7:  # 70% chance to match theme
                themed_item = self._create_themed_item(achievement_id, achievement_tier, preferred_type)
                if themed_item:
                    self.gear_system.add_item_to_inventory(user_id, themed_item, source=f"achievement_{achievement_id}")
                    return themed_item

        return item

    def process_daily_login(self, user_id: int, consecutive_days: int) -> Optional[GearItem]:
        ...
        self.gear_system.add_item_to_inventory(user_id, item, source=f"daily_login_day_{consecutive_days}")

        self.drop_cooldowns[f"{user_id}_daily_login"] = datetime.now().timestamp()

        return item

    def process_level_up(self, user_id: int, new_level: int, level_data: Dict[str, Any]) -> Optional[GearItem]:
        ...
        item = self.gear_system.generate_random_drop(
            user_level=new_level, luck_multiplier=1.5, guaranteed_rarity=guaranteed_rarity  # Level ups are lucky
        )
        ...
        self.gear_system.add_item_to_inventory(user_id, item, source=f"level_up_{new_level}")

        return item

    def process_challenge_completion(
        self, user_id: int, challenge_id: str, challenge_data: Dict[str, Any]
    ) -> Optional[GearItem]:
        ...
        item = self.gear_system.generate_random_drop(
            user_level=challenge_data.get("user_level", 1), luck_multiplier=difficulty_multiplier.get(difficulty, 1.0)
        )
        ...
        self.gear_system.add_item_to_inventory(user_id, item, source=f"challenge_{challenge_id}")

        return item

    def _create_themed_item(self, theme: str, tier: AchievementTier, gear_type: GearType) -> Optional[GearItem]:
...
class GearEventScheduler:
    ...
    def start_themed_event(self, theme: str, duration_hours: int, bonus_gear_type: GearType):
    ...
    def get_active_events(self) -> List[Dict[str, Any]]:
        ...
        for event_id, event_data in self.active_events.items():
            if current_time > event_data["end_time"]:
                ...
                active.append(
                    {"id": event_id, **event_data, "time_remaining": (event_data["end_time"] - current_time).seconds}
                )
    ...
    def apply_event_modifiers(self, base_chance: float, gear_type: Optional[GearType] = None) -> float:
...
class GearRewardIntegration:
    ...
    def __init__(self, gear_system: GearSystem, reward_system: RewardSystem, drop_manager: GearDropManager):
        self.gear_system = gear_system
        self.reward_system = reward_system
        self.drop_manager = drop_manager

    def process_trade_rewards(self, user_id: int, trade_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process both XP rewards and gear drops for trades"""
        results = {"xp_earned": 0, "rewards_earned": 0, "gear_dropped": None, "bonuses_applied": []}

        # Calculate XP and rewards
        profit = trade_data.get("profit", 0)
        if profit > 0:
            # Get reward calculation
            reward_calc = self.reward_system.calculate_reward(user_id, profit, trade_data.get("risk_tier", "STANDARD"))
            ...
            self.reward_system.record_period_activity(
                user_id, trades=1, profit=profit, xp=trade_data.get("xp_earned", 0), rewards=reward_calc.total_amount
            )

        # Check for gear drops
        gear_drop = self.drop_manager.process_trade_completion(user_id, trade_data)
    ...
    def get_gear_enhanced_stats(self, user_id: int) -> Dict[str, float]:
