...
class GearItem:
    ...
    def get_display_name(self) -> str:
        ...
        color_codes = {"gray": "⬜", "green": "🟩", "blue": "🟦", "purple": "🟪", "orange": "🟧", "red": "🟥"}
    ...
    def get_power_score(self) -> int:
        ...
        multiplier_bonus = (self.stats.xp_multiplier - 1) * 200 + (self.stats.reward_multiplier - 1) * 300
...
class GearSystem:
    ...
    GEAR_CATALOG = {
        ...
        "quantum_predictor": GearItem(
            ...
            level=50,
            stats=GearStats(accuracy_bonus=0.50, range_bonus=0.40, critical_chance=0.25, xp_multiplier=1.5),
    ...
    CRAFTING_RECIPES = {
        "craft_macd_elite": CraftingRecipe(
            ...
            required_materials={"rsi_scanner_basic": 3, "craft_material_rare": 5},
            required_level=10,
    ...
    def add_item_to_inventory(
        self, user_id: int, item: Union[str, GearItem], quantity: int = 1, source: str = "unknown"
    ) -> bool:
        ...
        cursor.execute(
            ...
            ON CONFLICT(user_id, item_id) DO UPDATE SET
                ...
                item_data = ?
        """,
            (user_id, item_id, item_data, quantity, int(time.time()), quantity, item_data),
        )
    ...
    def create_loadout(self, user_id: int, name: str, set_active: bool = False) -> Optional[Loadout]:
        """Create a new loadout for user"""
        loadout_id = hashlib.md5(f"{user_id}_{name}_{time.time()}".encode()).hexdigest()[:12]

        loadout = Loadout(loadout_id=loadout_id, name=name, is_active=set_active)
    ...
    def equip_item(self, user_id: int, item_id: str, loadout_id: Optional[str] = None) -> Tuple[bool, str]:
    ...
    def generate_random_drop(
        self, user_level: int, luck_multiplier: float = 1.0, guaranteed_rarity: Optional[GearRarity] = None
    ) -> GearItem:
        ...
        stats = GearStats(
            ...
            xp_multiplier=1.0
            + (random.uniform(0, 0.2) * stat_multiplier if selected_rarity.value_multiplier > 3 else 0),
    ...
    def craft_item(self, user_id: int, recipe_id: str) -> Tuple[bool, str, Optional[str]]:
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
        """,
            (craft_id, user_id, recipe_id, int(time.time()), int(time.time()) + recipe.crafting_time),
        )
    ...
    def check_crafting_completion(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        for craft_id, recipe_id in cursor.fetchall():
            ...
            if random.random() <= recipe.success_rate:
                # Add crafted item
                self.add_item_to_inventory(user_id, recipe.result_item_id, source="crafting")

                completed.append({"craft_id": craft_id, "success": True, "item_id": recipe.result_item_id})
            else:
                completed.append({"craft_id": craft_id, "success": False, "item_id": recipe.result_item_id})
    ...
    def create_trade_offer(
        ...
        # Create trade offer
        trade_id = hashlib.md5(f"{sender_id}_{receiver_id}_{time.time()}".encode()).hexdigest()[:12]
    ...
    def get_inventory_display(self, user_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "set_bonus": set_bonus,
            "inventory_space": {"used": len(inventory), "max": 500},  # Could be upgraded
        }
...
def award_gear_for_achievement(
    gear_system: GearSystem, user_id: int, achievement_id: str, achievement_tier: str
) -> Optional[GearItem]:
    ...
    # Add to inventory
    gear_system.add_item_to_inventory(user_id, item, source=f"achievement_{achievement_id}")
...
def award_gear_for_milestone(
    gear_system: GearSystem, user_id: int, milestone_type: str, milestone_value: int
) -> Optional[GearItem]:
    ...
    if milestone_key in milestone_rewards:
        ...
        gear_system.add_item_to_inventory(user_id, item_id, source=f"milestone_{milestone_key}")
        return gear_system.GEAR_CATALOG.get(item_id)

    # Generate random reward for other milestones
    return award_gear_for_achievement(
        gear_system, user_id, f"milestone_{milestone_type}", "gold"  # Milestones give good rewards
    )
