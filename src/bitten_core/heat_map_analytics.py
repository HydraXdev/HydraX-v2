...
class HeatMapAnalytics:
    ...
    TRACKED_PAIRS = ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "USDCAD", "NZDUSD", "EURGBP", "EURJPY", "GBPJPY"]
    ...
    def update_pair_tcs(self, pair: str, tcs_value: float, signal_result: Optional[bool] = None) -> None:
        ...
        pair_data.historical_data = [(ts, val) for ts, val in pair_data.historical_data if ts > cutoff]
    ...
    def get_pair_details(self, pair: str, user_has_access: bool = True) -> Dict[str, Any]:
        ...
        return {
            ...
            "averages": {"24h": round(pair_data.average_tcs_24h, 1), "7d": round(pair_data.average_tcs_7d, 1)},
            "trend": {
    ...
    def _load_historical_data(self) -> None:
        ...
        for pair_file in self.data_dir.glob("*_heat.json"):
            try:
                ...
                historical = [
                    (datetime.fromisoformat(item["timestamp"]), item["tcs"]) for item in data.get("historical_data", [])
                ]
