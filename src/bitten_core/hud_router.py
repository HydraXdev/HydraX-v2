...
class HUDRouter:
    ...
    def _handle_execute_signal(self):
        ...
        if not access_info["can_execute"]:
            return jsonify({"success": False, "message": f'Tier {access_info["required_tier"]} required'}), 403
        ...
        if execution_result["success"]:
            # Update user stats
            self.profile_manager.record_trade_execution(user_id, signal_id, execution_result["position_id"])
            ...
            return jsonify({"success": False, "message": execution_result["message"]})
    ...
    def _handle_register_recruit(self, recruit_telegram_id: str):
        ...
        # Register recruit
        success = self.profile_manager.register_recruit(recruiter_id, int(recruit_telegram_id), recruit_username)
        ...
        if success:
            ...
            return jsonify(
                {"success": True, "message": "Recruit registered successfully", "updated_stats": recruiter_stats}
            )
        else:
            return jsonify({"success": False, "message": "Failed to register recruit"})
    ...
    def _check_signal_access(self, signal: HUDSignalData, user_rank: UserRank) -> Dict:
        ...
        urgency_requirements = {"MEDIUM": UserRank.USER, "HIGH": UserRank.AUTHORIZED, "CRITICAL": UserRank.ELITE}
    ...
    def _execute_trade(self, user_id: int, signal: HUDSignalData, position_size: float) -> Dict:
        ...
        if success:
            ...
            return {"success": False, "message": "Execution failed - market conditions changed"}
    ...
    def cleanup_expired_signals(self):
        ...
        expired = [sid for sid, signal in self.active_signals.items() if signal.expires_at < current_time]
