...
class HybridManager:
    ...
    def calculate_hybrid_levels(
        self, symbol: str, entry: float, sl: float, direction: str, risk_reward: float = 1.5
    ) -> Dict:
        """
        ...
        be_offset_pips = max(self.DEFAULT_CONFIG["be_offset_pips"], config.spread_buffer, config.min_stop_distance)
        be_price = self.pips_to_price(symbol, entry, be_offset_pips, direction)

        # Trail distance with safety margin
        trail_pips = max(self.DEFAULT_CONFIG["trail_distance_pips"], config.min_stop_distance + config.spread_buffer)
    ...
    def create_bitmode_config(self, symbol: str, entry: float, sl: float, direction: str, user_id: str) -> Dict:
        """
    ...
    def validate_command_timing(self, fire_id: str) -> bool:
        ...
        if now - last < self.DEFAULT_CONFIG["min_cmd_gap_ms"]:
            logger.warning(
                f"⚠️ Command rate limited for {fire_id}: {now - last:.0f}ms < {self.DEFAULT_CONFIG['min_cmd_gap_ms']}ms"
            )
            return False
    ...
    def track_position(self, fire_id: str, symbol: str, entry: float, sl: float, direction: str, lot_size: float):
    ...
    def get_partial_command(self, fire_id: str, milestone: str) -> Optional[Dict]:
        ...
        if milestone == "TP1" and pos["state"] == HybridState.ENTERED:
            ...
            return {"type": "partial_close", "fire_id": fire_id, "lots": round(partial_lots, 2), "milestone": "TP1"}
