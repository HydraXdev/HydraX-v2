...
from .intel_bot_personalities import BotPersonality, bot_personalities
from .market_analyzer import MarketAnalyzer, MarketStructure
from .signal_fusion import FusedSignal, IntelSource, SignalFusionEngine
    ...
    async def analyze(self, pair: str, market_data: Dict) -> Optional[IntelSource]:
        ...
        try:
            ...
            return IntelSource(
                ...
                confidence=confidence,
                weight=0.2,
                metadata={"factors": scores, "session": session, "day_of_week": dow},
            )
...
class AIBotAnalyzer:
    ...
    async def analyze(self, pair: str, market_data: Dict) -> Optional[IntelSource]:
        ...
        try:
            ...
            buy_weighted = sum(
                s["confidence"] * self.bot_specialties[s["bot"]]["weight"] for s in bot_signals if s["signal"] == "BUY"
            )
            sell_weighted = sum(
                s["confidence"] * self.bot_specialties[s["bot"]]["weight"] for s in bot_signals if s["signal"] == "SELL"
            )
...
class IntelligenceAggregator:
    ...
    async def _get_market_data(self, pair: str) -> Optional[Dict]:
        ...
        try:
            ...
            market_conditions["price_history"] = [1.1000 + i * 0.0001 for i in range(50)]
