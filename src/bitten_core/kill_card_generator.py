...
class KillCardGenerator:
    ...
    def _draw_rank_badge(
        self, draw: ImageDraw.Draw, position: Tuple[int, int], rank: str, badge: str, color: str, size: int = 40
    ):
        ...
        draw.text((x + size // 2 - text_width // 2, y + size // 2 - text_height // 2), badge, font=font, fill="#ffffff")

    def _draw_achievement_badges(
        self, draw: ImageDraw.Draw, achievements: List[str], position: Tuple[int, int], theme: str
    ):
        ...
        for i, achievement in enumerate(achievements[:5]):  # Max 5 achievements
            if achievement in achievement_icons:
                ...
                draw.text(
                    (badge_x + badge_size // 2 - icon_width // 2, y + badge_size // 2 - icon_height // 2),
                    icon,
                    font=font,
                    fill="#ffffff",
                )
    ...
    def _generate_qr_code(self, data: Dict[str, Any]) -> Image.Image:
        ...
        verification_data = {
            ...
            "hash": hashlib.sha256(f"{data['pair']}{data['profit_usd']}{data['timestamp']}".encode()).hexdigest()[:8],
        }
    ...
    def _add_security_watermark(self, img: Image.Image, theme: str) -> Image.Image:
        ...
        for i in range(0, width + height, 200):
            ...
            y = height - i + width
            draw.text(
                (x, y), watermark_text, font=font, fill=(*ImageColor.getrgb(self.colors[theme]["text_secondary"]), 30)
            )

        return img

    def generate_kill_card(
        self, trade_data: TradeData, theme: CardTheme = CardTheme.DARK, size: Tuple[int, int] = (1200, 675)
    ) -> Image.Image:
        ...
        # Draw header section
        header_height = 120
        draw.rectangle([0, 0, width, header_height], fill=self.colors[theme_name]["bg_secondary"])

        # Add BITTEN logo/title
        draw.text((50, 30), "BITTEN", font=self.fonts["title"], fill=self.colors[theme_name]["accent"])

        draw.text((50, 80), "KILL CONFIRMED", font=self.fonts["small"], fill=self.colors[theme_name]["text_secondary"])
        ...
        # Trade pair
        draw.text(
            (50, content_y), trade_data.pair, font=self.fonts["heading"], fill=self.colors[theme_name]["text_primary"]
        )

        # Profit information
        profit_color = (
            self.colors[theme_name]["success"] if trade_data.profit_usd > 0 else self.colors[theme_name]["danger"]
        )
        draw.text((50, content_y + 50), f"+${trade_data.profit_usd:,.2f}", font=self.fonts["title"], fill=profit_color)

        draw.text(
            (50, content_y + 110), f"+{trade_data.profit_percent:.2f}%", font=self.fonts["heading"], fill=profit_color
        )
        ...
        for i, (label, value) in enumerate(stats):
            ...
            draw.text(
                (x_pos, y_pos), f"{label}:", font=self.fonts["small"], fill=self.colors[theme_name]["text_secondary"]
            )

            draw.text(
                (x_pos + 100, y_pos), value, font=self.fonts["body"], fill=self.colors[theme_name]["text_primary"]
            )

        # XP and rank progress
        xp_y = details_y + details_box_height + 30
        draw.text((50, xp_y), f"XP EARNED: +{xp}", font=self.fonts["body"], fill=self.colors[theme_name]["warning"])
        ...
        draw.rectangle(
            [50, progress_y, 50 + progress_fill, progress_y + progress_height], fill=self.colors[theme_name]["accent"]
        )

        # Achievement badges
        if trade_data.achievements:
            self._draw_achievement_badges(draw, trade_data.achievements, (50, height - 150), theme_name)
    ...
    def generate_social_card(
        self, trade_data: TradeData, platform: str = "twitter", theme: CardTheme = CardTheme.DARK
    ) -> Image.Image:
        """Generate optimized cards for social media platforms"""
        sizes = {"twitter": (1200, 675), "instagram": (1080, 1080), "facebook": (1200, 630), "discord": (800, 450)}
    ...
    def save_card(self, card: Image.Image, output_path: str, quality: int = 95, optimize: bool = True) -> str:
        """Save the kill card to file"""
        card.save(output_path, "PNG", quality=quality, optimize=optimize)
        return output_path

    def generate_card_batch(self, trades: List[TradeData], theme: CardTheme = CardTheme.DARK) -> List[Image.Image]:
    ...
    def create_animated_card(
        self, trade_data: TradeData, theme: CardTheme = CardTheme.DARK, duration: int = 3000
    ) -> List[Image.Image]:
...
def detect_achievements(trade_data: TradeData) -> List[str]:
    ...
    if (
        trade_data.profit_percent > 0
        and abs(trade_data.entry_price - trade_data.exit_price) / trade_data.entry_price < 0.001
    ):
...
if __name__ == "__main__":
    ...
    # Save as GIF
    if frames:
        frames[0].save("kill_card_animated.gif", save_all=True, append_images=frames[1:], duration=100, loop=0)
