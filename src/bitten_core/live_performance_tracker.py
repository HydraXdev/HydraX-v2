...
class LivePerformanceTracker:
    ...
    def log_ghost_mode_action(
        self, signal_id: str, user_id: str, action: str, original_value: float, modified_value: float
    ) -> bool:
        """Log individual ghost mode actions for analysis"""
        try:
            effectiveness_score = self._calculate_ghost_effectiveness(action, original_value, modified_value)
    ...
    def get_live_performance_metrics(self, hours_back: int = 24) -> PerformanceMetrics:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            recent_signals = conn.execute(
                "SELECT COUNT(*) FROM live_signals WHERE timestamp >= ?", (cutoff_time,)
            ).fetchone()[0]
    ...
    def get_true_win_rate(self, hours_back: int = 24, include_unfired: bool = True) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "fired_signals": {"total": total_fired, "wins": total_fired_wins, "win_rate": fired_win_rate},
                "unfired_signals": {
                    "total": total_unfired,
