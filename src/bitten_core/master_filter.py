...
from .fire_modes import FireMode, FireModeValidator, TierLevel
from .position_manager import DrawdownProtection, OpenPosition, PositionManager
...
class MasterFilter:
    ...
    def process_market_update(self, market_data: Dict[str, Dict]) -> Dict[str, List[FilteredSignal]]:
        ...
        # Log summary
        logger.info(
            f"Signals generated - Arcade: {len(results['arcade'])}, "
    ...
    def validate_shot(
        self, user_id: int, signal_id: str, tier: TierLevel, balance: float
        ...
        if user_id not in self.user_states:
            self.user_states[user_id] = {"tier": tier, "balance": balance, "last_update": datetime.now()}
        ...
        pos_ok, pos_msg = self.position_manager.can_open_position(user_id, tier, balance, signal.signal_type)
        if not pos_ok:
            return False, pos_msg, None

        # 3. Check fire mode authorization
        fire_mode = FireMode.SINGLE_SHOT  # Default
        fire_ok, fire_msg = self.fire_validator.can_fire(user_id, tier, fire_mode, signal.tcs_score, signal.symbol)
        if not fire_ok:
    ...
    def _process_arcade_signal(self, signal: ArcadeSignal) -> Optional[FilteredSignal]:
        ...
        filtered = FilteredSignal(
            ...
            expected_pips=int(
                abs(signal.take_profit - signal.entry_price) / (0.0001 if not signal.symbol.endswith("JPY") else 0.01)
            ),
...
Good hunting, soldier! 🎖️"""
    ...
    def get_user_dashboard(self, user_id: int) -> Dict:
        ...
        dd_status = self.drawdown_protection.check_drawdown_status(
            user_id, self.user_states.get(user_id, {}).get("balance", 1000)
        )
        ...
        return {
            ...
            "active_signals": len([s for s in self.active_signals.values() if s.can_execute]),
            "last_update": datetime.now(),
        }

    def close_position(self, user_id: int, position_id: str, result: str, pnl: float) -> bool:
