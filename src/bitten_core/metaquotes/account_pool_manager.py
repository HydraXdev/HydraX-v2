...
from .demo_account_service import CredentialEncryption, DemoAccountConfig, DemoAccountService, MetaQuotesAPIClient
...
class AccountPoolManager:
        ...
        try:
            ...
            async with self.demo_service.api_client:
                ...
                encrypted_password = self.demo_service.encryption.encrypt_password(
                    account_data["password"], "bitten-demo-key-2025-01"
                )
        ...
        async with get_async_db() as conn:
            ...
            for account in accounts:
                task = asyncio.create_task(
                    self._check_single_account_health(account["pool_id"], account["account_number"])
                )
                tasks.append(task)
    ...
    async def get_pool_statistics(self) -> Dict[str, Any]:
        ...
        async with get_async_db() as conn:
            ...
            return {
                ...
                "pool_health": (
                    "healthy" if (stats["healthy_available"] or 0) >= self.config.min_available_accounts else "low"
                ),
            }
    ...
    async def force_replenish(self, count: int) -> Dict[str, Any]:
        ...
        async with get_async_db() as conn:
            current_total = await conn.fetchval("SELECT COUNT(*) FROM demo_account_pool WHERE status != 'expired'")
            ...
            return {"success": True, "provisioned": to_provision, "message": f"Provisioned {to_provision} new accounts"}
