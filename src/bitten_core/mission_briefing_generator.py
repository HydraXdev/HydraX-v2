...
try:
    from .norman_story_integration import integrate_norman_story, norman_story_engine
except ImportError:
    # Fallback if import fails
    norman_story_engine = None

    def integrate_norman_story(briefing_data, user_id, context):
...
class NormansStoryIntegrator:
    ...
    def __init__(self):
        ...
        self.norman_callsigns = {
            "tactical": ["DELTA WISDOM", "MAGNOLIA STRIKE", "COTTON FIELD", "MUDDY WATERS", "RIVER BEND", "PINE GROVE"],
            "family": [
                "MAMA'S BLESSING",
                "GRANDMAMA'S WATCH",
                "FAMILY HONOR",
...
class MissionBriefingGenerator:
    ...
    def generate_mission_briefing(
        self, signal_data: Dict, market_data: Dict, user_tier: str = "AUTHORIZED", user_id: str = None
    ) -> MissionBriefing:
        ...
        if signal_data.get("countdown_minutes"):
            ...
            logger.info(
                f"Using smart timer: {signal_data['countdown_minutes']} min for {signal_data.get('symbol', 'unknown')}"
            )
        else:
    ...
    def format_for_webapp(self, briefing: MissionBriefing) -> Dict:
        ...
        if hasattr(briefing, "smart_timer") or data.get("smart_timer"):
            ...
            data["timer_display"] = {"type": "standard", "countdown_seconds": briefing.time_remaining}
    ...
    def _get_tier_benefits(self, user_tier: str, mission_type: MissionType) -> Dict[str, Any]:
        ...
        benefits = {
            ...
            "AUTHORIZED": {"access_delay": 0, "partial_intel": False, "execution_limit": 3, "risk_multiplier": 1.0},
            "ELITE": {
                "access_delay": 0,
                "partial_intel": False,
                "execution_limit": 6,
    ...
    def _get_quick_actions(self, briefing: MissionBriefing) -> List[Dict]:
        ...
        actions = [
            ...
            {"id": "analyze", "label": "📊 ANALYZE", "color": briefing.color_scheme["info"], "action": "view_analysis"},
        ]
