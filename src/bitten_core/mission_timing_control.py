...
class MissionTimingControl:
    ...
    def can_open_position(self, user_id: str, user_tier: str = "NIBBLER") -> tuple[bool, str]:
        ...
        if current_count >= max_positions:
            ...
            return (
                False,
                f"⚠️ ONE AT A TIME: You have {current_count} active position(s): {', '.join(active_ids[:3])}. Close or wait for timeout.",
            )
        ...
        active_positions[user_id].append(
            {
                ...
                "expires_at": (
                    datetime.now() + timedelta(minutes=self.timing_rules[signal_type]["auto_cancel"])
                ).isoformat(),
            }
        )
    ...
    def close_position(self, user_id: str, signal_id: str):
        ...
        if user_id in active_positions:
            active_positions[user_id] = [p for p in active_positions[user_id] if p["signal_id"] != signal_id]
            ...
            for pos in positions:
                if pos["signal_id"] == signal_id:
                    ...
                    if now >= expires_at:
                        return (
                            True,
                            f"Position expired after {self.timing_rules[pos['signal_type']]['auto_cancel']} minutes",
                        )
