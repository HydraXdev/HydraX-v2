...
from .ambient_audio_system import AudioClip, AudioMood, AudioType, ambient_audio_engine
...
class MississippiAmbientEngine:
    ...
    def determine_environmental_mood(self, user_id: str, market_context: Dict[str, Any]) -> str:
        ...
        for cluster in self.environmental_clusters.values():
            ...
            if volatility == "high" and cluster.weather_pattern == WeatherPattern.APPROACHING_STORM:
                score += 15
            elif volatility == "low" and cluster.weather_pattern in [
                WeatherPattern.CALM_EVENING,
                WeatherPattern.GENTLE_BREEZE,
            ]:
                score += 12
    ...
    async def start_ambient_environment(self, user_id: str, market_context: Dict[str, Any] = None) -> Optional[str]:
        ...
        # Track user state
        self.user_ambient_states[user_id] = {"cluster_id": cluster_id, "start_time": datetime.now(), "clips_played": 0}
    ...
    def get_ambient_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "session_duration": (
                (datetime.now() - state.get("start_time")).total_seconds() if state.get("start_time") else 0
            ),
