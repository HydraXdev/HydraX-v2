...
class MT5EnhancedAdapter:
    ...
    def get_account_data(self) -> Optional[Dict]:
        ...
        try:
            ...
            if data.get("balance", 0) > 0:
                ...
                data["margin_usage_percent"] = (
                    (data.get("margin", 0) / data["balance"]) * 100 if data.get("margin", 0) > 0 else 0
                )
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "avg_pnl_percent": (
                        sum(p.get("pnl_percent", 0) for p in positions) / len(positions) if positions else 0
                    ),
                },
            }
    ...
    def execute_trade_with_risk(
        self, symbol: str, direction: str, risk_percent: float = 2.0, sl: float = 0, tp: float = 0, **kwargs
    ) -> Dict:
    ...
    def execute_trade(self, symbol: str, direction: str, volume: float, sl: float = 0, tp: float = 0, **kwargs) -> Dict:
    ...
    def should_take_trade(self, symbol: str, risk_percent: float = 2.0) -> Tuple[bool, str]:
        ...
        symbol_positions = [p for p in positions.get("positions", []) if p.get("symbol") == symbol]
    ...
    def get_lot_size_for_risk(self, symbol: str, sl_price: float, risk_percent: float = 2.0) -> float:
    ...
    def _send_instruction(self, instruction: Dict) -> Dict:
        ...
        return {"success": False, "error": "Timeout waiting for MT5 response", "instruction": instruction}
