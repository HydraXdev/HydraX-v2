...
class MT5FarmAdapter:
    ...
    def execute_trade(self, trade_params: Dict) -> Dict:
        ...
        try:
            ...
            # Send to farm
            response = requests.post(f"{self.farm_url}/execute", json=instruction, timeout=self.timeout)
            ...
            return {"success": False, "error": "Farm request timeout"}
        except Exception as e:
    ...
    def close_trade(self, ticket: int) -> Dict:
        ...
        instruction = {"action": "CLOSE_TRADE", "ticket": ticket, "timestamp": int(time.time())}
    ...
    def modify_trade(self, ticket: int, stop_loss: float = None, take_profit: float = None) -> Dict:
        ...
        instruction = {"action": "MODIFY_TRADE", "ticket": ticket, "timestamp": int(time.time())}
