...
class NewsAPIClient:
    ...
    def _cache_events(self, cache_key: str, events: List[EconomicEvent]):
        ...
        if len(self._cache) >= self._max_cache_size:
            # Remove oldest entry
            oldest_key = min(self._cache.keys(), key=lambda k: self._cache[k].get("timestamp", datetime.min))
            del self._cache[oldest_key]

        self._cache[cache_key] = {"timestamp": datetime.now(), "data": [event.to_dict() for event in events]}
    ...
    def get_upcoming_high_impact(self, hours_ahead: int = 24) -> List[EconomicEvent]:
        ...
        high_impact = [
            event for event in all_events if event.impact == NewsImpact.HIGH and now <= event.event_time <= cutoff
        ]
