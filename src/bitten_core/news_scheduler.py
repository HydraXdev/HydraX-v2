...
from .news_api_client import EconomicEvent, NewsAPIClient, NewsImpact
from .risk_management import NewsEvent, RiskManager
...
class NewsScheduler:
    ...
    def _add_event_to_risk_manager(self, event: EconomicEvent):
        ...
        news_event = NewsEvent(
            event_time=event.event_time, currency=event.currency, impact=event.impact.value, event_name=event.event_name
        )
        ...
        logger.debug(
            f"Added news event: {event.event_name} ({event.currency}) "
    ...
    def _clear_old_events(self):
        ...
        if current_count > 100:
            ...
            self.risk_manager.news_events = [event for event in self.risk_manager.news_events if event.event_time > now]
    ...
    def _log_next_event(self):
        ...
        try:
            ...
            if is_blackout and next_event:
                logger.warning(
                    f"Currently in NEWS BLACKOUT for: {next_event.event_name} "
                ...
                if next_window:
                    ...
                    logger.info(f"Next news blackout: {start} to {end} " f"for {event.event_name} ({event.currency})")
