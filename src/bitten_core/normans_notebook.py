...
Grandmama would be proud.
    ...
    def get_weekly_review(self) -> str:
        ...
        recent_entries = [e for e in self.entries if datetime.fromisoformat(e["timestamp"]) > one_week_ago]
...
The dominant feeling was {dominant_mood}. """
    ...
    def get_user_emotional_state(self) -> Dict[str, Any]:
        ...
        if not recent_entries:
            return {"confidence": "Building...", "discipline": "Growing...", "patience": "Learning..."}
    ...
    def add_trade_reflection(self, trade_id: str, reflection: str):
        ...
        entry = self.add_entry(trade_id=trade_id, phase="after", note=reflection)
        ...
        for scar in self.scars:
            if not scar["healed"] and scar["symbol"] in reflection:
                ...
                if any(
                    ...
                    for phrase in ["learned from", "remembered", "different this time", "applied lesson"]
                ):
                    scar["healed"] = True
    ...
    def get_emotional_chart_data(self) -> Dict[str, List]:
        ...
        return {"dates": [d.isoformat() for d in dates], "series": dict(mood_series)}
    ...
    def get_notes_by_symbol(self, symbol: str) -> List[Dict]:
        ...
        return [n for n in self.user_trading_notes if n.get("symbol") == symbol and not n.get("archived", False)]

    def get_notes_by_trade(self, trade_id: str) -> List[Dict]:
        """Get all notes related to a specific trade"""
        return [n for n in self.user_trading_notes if n.get("trade_id") == trade_id and not n.get("archived", False)]
...
""",
    ...
    def get_notes_stats(self) -> Dict:
        ...
        recent_week = sum(1 for n in active_notes if datetime.fromisoformat(n["updated_at"]) > week_ago)
        recent_month = sum(1 for n in active_notes if datetime.fromisoformat(n["updated_at"]) > month_ago)
...
class TradingLessonExtractor:
    ...
    def extract_winning_patterns(self) -> List[Dict]:
        ...
        profitable_entries = [e for e in self.notebook.entries if e.get("pnl", 0) > 100]
            ...
            for pattern in entry.get("patterns", []):
                if pattern in ["diamond_hands", "cut_losses", "took_profits"]:
                    winning_patterns[pattern].append(
                        {"date": entry["timestamp"], "pnl": entry["pnl"], "note": entry["note"]}
                    )
...
def create_journal_entry_from_trade(trade_data: Dict, notebook: NormansNotebook):
    ...
    if trade_data.get("status") == "open":
        notebook.add_entry(
            ...
            note=f"Opened position in {trade_data['symbol']}. " f"Risk: ${trade_data.get('risk', 0):.2f}",
        )
...
def get_journal_insights_for_symbol(symbol: str, notebook: NormansNotebook) -> Dict:
    ...
    if not history:
        return {"has_history": False, "message": f"First time trading {symbol}. Make it count."}
