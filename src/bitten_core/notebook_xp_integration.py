...
class NotebookXPIntegration:
    ...
    def record_signal_execution(
        self, signal_id: str, symbol: str, direction: str, entry_price: float, tcs_score: float
    ) -> bool:
    ...
    def update_signal_result(self, signal_id: str, exit_price: float, pnl: float, result: str) -> bool:
    ...
    def get_recent_executed_signals(self, days: int = 7) -> List[SignalExecutionHistory]:
        ...
        return [signal for signal in self.signal_history if signal.executed_at >= cutoff_date]

    def add_journal_entry_with_xp(
        self,
        title: str,
        content: str,
        ...
        result = self.notebook.add_user_note(title=title, content=content, category=category, tags=tags, **entry_data)

        # Award XP through XP Integration Manager
        actual_xp = self.xp_manager.award_xp_with_multipliers(
            self.user_id, xp_reward, xp_reason, f"Journal entry: {title[:50]}..."
        )
        ...
        result.update({"xp_earned": actual_xp, "xp_reason": xp_reason, "milestone_achieved": milestone_result})
    ...
    def _check_milestone_achievements(self) -> Optional[Dict]:
        ...
        for milestone in self.milestones:
            if current_entry_count >= milestone.entries_required and not self.user_milestones.get(milestone.id, False):
                ...
                milestone_xp = self.xp_manager.award_xp_with_multipliers(
                    self.user_id, milestone.xp_reward, f"Milestone: {milestone.name}", milestone.description
                )

                logger.info(f"User {self.user_id} achieved milestone: {milestone.name}")

                return {"milestone": milestone, "xp_awarded": milestone_xp, "achievement_unlocked": True}
    ...
    def get_signal_pairing_suggestions(self) -> List[Dict]:
        ...
        for signal in recent_signals:
            ...
            already_paired = any(entry.get("linked_signal_id") == signal.signal_id for entry in existing_entries)
    ...
    def get_notebook_xp_dashboard(self) -> Dict[str, Any]:
        ...
        total_notebook_xp = sum(entry.get("xp_earned", 0) for entry in entries)
    ...
    def _get_favorite_categories(self, entries: List[Dict]) -> List[Dict]:
        ...
        return [{"category": cat, "count": count} for cat, count in sorted_categories[:3]]
...
def award_journal_xp(user_id: str, entry_type: str = "basic", linked_signal_id: Optional[str] = None) -> int:
    ...
    return integration.xp_manager.award_xp_with_multipliers(
        user_id, xp_amount, f"Journal entry ({entry_type})", f"Signal: {linked_signal_id}" if linked_signal_id else ""
    )
