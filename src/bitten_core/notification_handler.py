...
class NotificationHandler:
    ...
    def send_tp_hit_notification(
        self, user_id: str, symbol: str, profit_pips: float, profit_currency: Optional[float] = None
    ) -> Notification:
        ...
        return self.send_notification(
            ...
            title=title,
            message=message,
            data={"symbol": symbol, "profit_pips": profit_pips, "profit_currency": profit_currency},
            priority=8,  # High priority
        )

    def send_achievement_notification(
        self, user_id: str, achievement_name: str, xp_reward: int, description: Optional[str] = None
    ) -> Notification:
        ...
        return self.send_notification(
            ...
            title=title,
            message=message,
            data={"achievement": achievement_name, "xp_reward": xp_reward},
            priority=7,
        )

    def send_xp_gain_notification(
        self, user_id: str, xp_amount: int, reason: str, is_combo: bool = False
    ) -> Notification:
        ...
        return self.send_notification(
            ...
            title=title,
            message=message,
            data={"xp_amount": xp_amount, "is_combo": is_combo},
            custom_sound=custom_sound,
            priority=5 if xp_amount < 100 else 6,
        )

    def send_warning_notification(
        self, user_id: str, warning_type: str, message: str, severity: str = "medium"  # low, medium, high, critical
    ) -> Notification:
        ...
        priority_map = {"low": 4, "medium": 6, "high": 8, "critical": 10}
        ...
        return self.send_notification(
            ...
            title=title,
            message=message,
            data={"warning_type": warning_type, "severity": severity},
    ...
    def get_user_notifications(
        self, user_id: str, limit: int = 10, notification_type: Optional[NotificationType] = None
    ) -> List[Notification]:
    ...
    def clear_old_notifications(self, hours: int = 24):
        ...
        self.notification_queue = [n for n in self.notification_queue if n.timestamp.timestamp() > cutoff]
    ...
    def format_for_telegram(self, notification: Notification) -> Dict[str, Any]:
        ...
        return {
            ...
            "metadata": {"type": notification.type.value, "sound": notification.sound_type, "data": notification.data},
        }
...
def notify_tp_hit(user_id: str, symbol: str, profit_pips: float, profit_currency: Optional[float] = None):
    ...
    return notification_handler.send_tp_hit_notification(user_id, symbol, profit_pips, profit_currency)
...
def notify_achievement(user_id: str, achievement: str, xp: int, description: Optional[str] = None):
    ...
    return notification_handler.send_achievement_notification(user_id, achievement, xp, description)


def notify_xp_gain(user_id: str, xp: int, reason: str, combo: bool = False):
    """Quick helper to notify XP gain"""
    return notification_handler.send_xp_gain_notification(user_id, xp, reason, combo)


# Example usage
if __name__ == "__main__":
    # Simulate TP hit
    notify_tp_hit(user_id="test_user", symbol="EURUSD", profit_pips=25.5, profit_currency=127.50)

    # Simulate achievement
    notify_achievement(
        user_id="test_user", achievement="First Blood", xp=100, description="Your first successful trade!"
    )
