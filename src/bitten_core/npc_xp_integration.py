...
from .xp_celebration_system import EventType, XPCelebrationSystem
...
class NPCXPIntegration:
    ...
    def handle_xp_event(
        ...
        # Get celebration
        celebration = self.celebration_system.celebrate_xp_gain(event_type, xp_amount, user_data, event_metadata)
    ...
    def provide_shop_guidance(
        self, user_id: str, affordable_items: List[Dict[str, Any]], user_profile: Dict[str, Any]
    ) -> List[NPCDialogue]:
    ...
    def celebrate_challenge_progress(
        self, user_id: str, challenge_type: str, progress: int, target: int, completed: bool
    ) -> List[NPCDialogue]:
    ...
    def handle_prestige_moment(self, user_id: str, prestige_level: int, prestige_rank: str) -> List[NPCDialogue]:
    ...
    def _calculate_user_mood(
        self, user_id: str, user_data: Dict[str, Any], event_type: EventType, xp_amount: int
    ) -> NPCMood:
    ...
    def _get_drill_response(
        self, event_type: EventType, xp_amount: int, mood: NPCMood, user_data: Dict[str, Any]
    ) -> str:
    ...
    def _get_analyst_response(
        self, event_type: EventType, xp_amount: int, event_metadata: Optional[Dict[str, Any]]
    ) -> str:
        ...
        if event_type == EventType.TRADE_TP_HIT and event_metadata:
            ...
            return (
                f"TP accuracy: {accuracy:.1f}%. XP efficiency: {xp_amount/accuracy:.1f} per percent. Optimal execution."
            )
    ...
    def _get_psych_response(self, event_type: EventType, user_data: Dict[str, Any]) -> str:
    ...
    def _is_milestone(self, user_data: Dict[str, Any], event_type: EventType) -> bool:
        ...
        return (
            ...
            or event_type == EventType.LEVEL_UP
            or any(abs(total_xp - m) < 100 for m in milestones)
        )
    ...
    def _suggest_follow_up(
        self, user_id: str, user_data: Dict[str, Any], event_type: EventType
    ) -> List[Dict[str, str]]:
        ...
        if event_type == EventType.TRADE_WIN:
            suggestions.append(
                {"action": "check_shop", "message": "New items available in XP Shop!", "npc": "DRILL_INSTRUCTOR"}
            )

        if user_data.get("total_xp", 0) >= 50000 and user_data.get("prestige_level", 0) == 0:
            suggestions.append(
                {"action": "prestige", "message": "PRESTIGE AVAILABLE! Transform and rise!", "npc": "ALL"}
            )
...
if __name__ == "__main__":
    ...
    user_data = {"rank": "SERGEANT", "total_xp": 15420, "success_rate": 72, "current_streak": 3, "tier": "FANG"}
