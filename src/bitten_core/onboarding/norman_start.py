...
class NormanOnboarding:
    ...
    async def start_sequence(self, user_id: str, username: str) -> Dict:
        ...
        intro_sequence = {"messages": [], "callbacks": [], "delays": []}

        # Message 1: Darkness
        intro_sequence["messages"].append({"text": "⬛", "parse_mode": "HTML", "disable_notification": True})
        intro_sequence["delays"].append(2.0)

        # Message 2: Norman's voice emerges
        intro_sequence["messages"].append(
            {"text": self._get_norman_intro(username), "parse_mode": "HTML", "disable_notification": False}
        )
        intro_sequence["delays"].append(4.0)

        # Message 3: The choice
        intro_sequence["messages"].append(
            {"text": self._get_the_choice(), "parse_mode": "HTML", "reply_markup": self._get_choice_buttons()}
        )
...
<b>Press /missions when you're ready for orders.</b>""",
            "parse_mode": "HTML",
            "reply_markup": {
                "keyboard": [["🎯 Missions", "📊 Signals"], ["🎖️ War Room", "👥 Squad"], ["📚 Intel", "⚙️ Settings"]],
