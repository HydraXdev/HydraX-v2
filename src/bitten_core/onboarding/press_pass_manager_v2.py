...
from ..metaquotes.credential_delivery import DeliveryMethod, SecureCredentialDelivery
...
class PressPassManagerV2:
    ...
    async def activate_press_pass(
        ...
        try:
            ...
            account_result = await self.demo_service.provision_account_for_user(user_id=user_id, tier="press_pass")
            ...
            credential_package = await self.credential_delivery.prepare_credentials(
                user_id=user_id, account_data=account_data, delivery_method=delivery_method
            )

            # Deliver credentials based on method
            delivery_result = await self._deliver_credentials(
                credential_package, delivery_method, contact_info or user_id
            )
            ...
            await self._register_press_pass_activation(
                user_id=user_id, account_number=account_data["account_number"], real_name=real_name
            )
            ...
            return {"success": False, "error": str(e), "message": "Failed to activate Press Pass. Please try again."}
    ...
    async def check_account_health(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not account_number:
                return {"success": False, "error": "no_active_account", "message": "No active demo account found"}
    ...
    async def resend_credentials(
        self, user_id: str, delivery_method: DeliveryMethod, contact_info: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not credentials:
                return {"success": False, "error": "no_active_account", "message": "No active account found"}

            # Prepare new credential package
            credential_package = await self.credential_delivery.prepare_credentials(
                user_id=user_id, account_data=credentials, delivery_method=delivery_method
            )

            # Deliver credentials
            delivery_result = await self._deliver_credentials(credential_package, delivery_method, contact_info)
    ...
    async def _deliver_credentials(
        self, credential_package, delivery_method: DeliveryMethod, contact_info: str
    ) -> Dict[str, Any]:
        """Deliver credentials using specified method"""
        if delivery_method == DeliveryMethod.TELEGRAM:
            return await self.credential_delivery.deliver_via_telegram(credential_package, contact_info)
        elif delivery_method == DeliveryMethod.EMAIL:
            return await self.credential_delivery.deliver_via_email(credential_package, contact_info)
        elif delivery_method == DeliveryMethod.QR_CODE:
            return await self.credential_delivery.generate_qr_code(credential_package)
        elif delivery_method == DeliveryMethod.API:
            return await self.credential_delivery.deliver_via_api(credential_package)
        else:
            # Default to secure link
            secure_link = await self.credential_delivery._generate_secure_link(credential_package)
            return {
                "success": True,
                "method": "secure_link",
                "link": secure_link,
    ...
    async def _register_press_pass_activation(self, user_id: str, account_number: str, real_name: str):
    ...
    async def check_weekly_limit(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with get_async_db() as conn:
                result = await conn.fetchval("SELECT check_weekly_press_pass_limit()")
    ...
    async def _increment_weekly_count(self):
        ...
        try:
            async with get_async_db() as conn:
                await conn.execute("SELECT increment_weekly_press_pass_count()")
                ...
                logger.info(
                    f"Incremented weekly Press Pass counter. Current count: {current_count}/{self.weekly_limit}"
                )
    ...
    async def get_press_pass_statistics(self) -> Dict[str, Any]:
        ...
        try:
            async with get_async_db() as conn:
                ...
                return {
                    ...
                    "conversion": {
                        ...
                        "conversion_rate": (
                            (conversion_stats["converted"] / conversion_stats["total"] * 100)
            ...
            return {"error": str(e), "weekly": {}, "overall": {}, "conversion": {}}
