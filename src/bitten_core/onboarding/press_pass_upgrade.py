...
class PressPassUpgradeHandler:
    ...
    TIER_PRICING = {
        SubscriptionTier.STARTER: {
            ...
            "features": ["Basic trading signals", "Live trading access", "Standard support", "Keep your progress"],
        },
    ...
    async def process_upgrade(self, user_id: str, selected_tier: str, payment_method: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if tier == SubscriptionTier.PRESS_PASS:
                return {"success": False, "error": "Cannot upgrade to Press Pass"}
            ...
            payment_result = await self._process_payment(
                user_id, tier_details["price"], tier_details["currency"], payment_method
            )

            if not payment_result["success"]:
                return {"success": False, "error": payment_result.get("error", "Payment processing failed")}

            # Update user subscription
            subscription_result = await self._update_user_subscription(user_id, tier, payment_result["transaction_id"])
    ...
    async def _process_payment(
        self, user_id: str, amount: float, currency: str, payment_method: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _update_user_subscription(
        self, user_id: str, tier: SubscriptionTier, transaction_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "next_billing_date": next_billing.isoformat(), "tier": tier.value}
