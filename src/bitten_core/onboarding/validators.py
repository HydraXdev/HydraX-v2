...
class OnboardingValidators:
    ...
    async def validate_callsign(self, callsign: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if len(callsign) < self.callsign_min_length:
                return (
                    False,
                    f"🚨 **Too Short**: Callsign must be at least {self.callsign_min_length} characters, recruit.",
                )

            if len(callsign) > self.callsign_max_length:
                return (
                    False,
                    f"🚨 **Too Long**: Keep it under {self.callsign_max_length} characters. Brevity is tactical.",
                )
    ...
    def get_callsign_suggestions(self, user_name: str = None) -> List[str]:
        ...
        try:
            ...
            if user_name:
                ...
                if len(name_clean) >= 3:
                    suggestions.extend(
                        [f"{name_clean}Trader", f"{name_clean}Pip", f"{name_clean}Signal", f"Tactical{name_clean}"]
                    )
    ...
    async def _is_callsign_taken(self, callsign: str) -> bool:
        ...
        try:
            ...
            taken_callsigns = {"admin", "test", "demo", "sample", "example"}
