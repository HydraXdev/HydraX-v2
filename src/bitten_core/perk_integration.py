...
from .perk_system import PerkBranch, PerkSystem
...
class PerkIntegration:
    ...
    def apply_trading_fee_reduction(self, user_id: int, base_fee: float) -> Tuple[float, List[str]]:
        ...
        return self.perk_system.apply_perk_effects(user_id, "trading_fee", base_fee)

    def apply_spread_improvement(self, user_id: int, base_spread: float) -> Tuple[float, List[str]]:
        """Apply spread improvement perks"""
        return self.perk_system.apply_perk_effects(user_id, "spread", base_spread)
    ...
    def check_double_tap(self, user_id: int, signal_id: str, last_execution_time: datetime) -> Tuple[bool, str]:
    ...
    def apply_risk_shield(self, user_id: int, loss_amount: float, is_first_daily_loss: bool) -> Tuple[float, List[str]]:
    ...
    def check_pattern_recognition(self, user_id: int, current_setup: Dict) -> List[Dict]:
        ...
        similar_patterns = [
            {"date": "2024-01-15", "similarity": 0.89, "outcome": "WIN", "profit_pips": 45},
            {"date": "2024-02-03", "similarity": 0.85, "outcome": "WIN", "profit_pips": 38},
        ]
    ...
    def get_confluence_score(self, user_id: int, signal_data: Dict) -> Optional[Dict]:
        ...
        return {
            "overall_score": 0.82,
            "timeframes": {"M5": 0.75, "M15": 0.88, "H1": 0.90, "H4": 0.78},
            "strength": "STRONG",
        }
    ...
    def get_prophet_predictions(self, user_id: int) -> List[Dict]:
        ...
        predictions = [
            {"pair": "EURUSD", "direction": "BUY", "confidence": 0.72, "estimated_time": "2 hours"},
            {"pair": "GBPJPY", "direction": "SELL", "confidence": 0.68, "estimated_time": "4 hours"},
            {"pair": "GOLD", "direction": "BUY", "confidence": 0.71, "estimated_time": "6 hours"},
        ]
    ...
    def apply_squad_xp_bonus(self, user_id: int, base_xp: int, is_squad_win: bool) -> Tuple[int, List[str]]:
    ...
    def check_rally_cry_available(self, user_id: int, last_use: Optional[datetime]) -> Tuple[bool, str]:
    ...
    def apply_rally_cry_boost(self, squad_members: List[int], boost_value: float = 0.10) -> Dict[int, float]:
    ...
    def get_recruit_starting_bonus(self, recruiter_id: int) -> Dict[str, Any]:
        ...
        return {
            "level": boost_level,
            "bonus_gear": ["starter_pack_premium", "early_access_signals", "mentor_direct_line"],
            "bonus_xp": 500,
            "bonus_ammo": 10,
        }
    ...
    def apply_xp_multipliers(self, user_id: int, base_xp: int, context: str = "general") -> Tuple[int, List[str]]:
        """Apply all XP multiplier perks"""
        final_xp, effects_applied = self.perk_system.apply_perk_effects(user_id, "xp_gain", float(base_xp))

        return int(final_xp), effects_applied

    def apply_reward_multipliers(self, user_id: int, base_reward: float) -> Tuple[float, List[str]]:
        """Apply reward multiplier perks"""
        return self.perk_system.apply_perk_effects(user_id, "reward", base_reward)

    def apply_streak_acceleration(self, user_id: int, base_streak_progress: int) -> Tuple[int, List[str]]:
    ...
    def calculate_daily_interest(self, user_id: int, yesterday_xp: int) -> Tuple[int, List[str]]:
    ...
    def calculate_prestige_retention(self, user_id: int, current_xp: int) -> Tuple[int, List[str]]:
    ...
    def calculate_passive_income(self, user_id: int, hours_elapsed: float) -> Tuple[int, List[str]]:
    ...
    def check_seasonal_bonuses(self, user_id: int, current_date: datetime) -> List[Dict]:
        ...
        if "seasonal_xp" in effects:
            if current_date.month in [12, 1, 2]:  # Winter months
                seasonal_bonuses.append({"name": "Winter Warrior", "bonus": "+50% XP", "active": True})

        # Halloween Hunter
        if current_date.month == 10 and current_date.day == 31:
            effects = self.perk_system.get_active_effects(user_id)
            if "spooky_bonus" in effects:
                seasonal_bonuses.append({"name": "Halloween Hunter", "bonus": "2x rewards", "active": True})
    ...
    def get_perk_notifications(self, user_id: int) -> List[Dict]:
        ...
        for perk_id, perk in self.perk_system.perk_catalog.items():
            ...
            can_unlock, _ = self.perk_system.unlock_perk(user_id, perk_id, 0, [])
            if can_unlock:
                notifications.append(
                    {
                        "type": "perk_available",
