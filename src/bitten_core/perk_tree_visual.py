...
class PerkTreeVisual:
    ...
    COLORS = {
        "TRADING": {"primary": "#FF6B6B", "secondary": "#FF4444", "glow": "#FF0000", "locked": "#661111"},  # Red
        "ANALYSIS": {"primary": "#4ECDC4", "secondary": "#2EA69F", "glow": "#00FFFF", "locked": "#1A4D4A"},  # Cyan
        "SOCIAL": {"primary": "#FFE66D", "secondary": "#FFD93D", "glow": "#FFFF00", "locked": "#665C1A"},  # Yellow
        "ECONOMY": {"primary": "#95E1D3", "secondary": "#6BCB77", "glow": "#00FF00", "locked": "#2D5A2D"},  # Green
    }

    # Tier badges
    TIER_BADGES = {1: "⚡", 2: "🔥", 3: "💎", 4: "👑"}  # Basic  # Advanced  # Elite  # Legendary
    ...
    def generate_tree_layout(self, user_id: int) -> Dict:
        ...
        layout = {"branches": {}, "connections": [], "player_info": tree_data["player"]}

        branch_index = 0
        for branch_name, branch_data in tree_data["tree"].items():
            branch_x = branch_index * self.BRANCH_WIDTH + 150

            layout["branches"][branch_name] = {"color": self.COLORS[branch_name], "tiers": {}, "x": branch_x}
            ...
            for tier_name, tier_data in branch_data["tiers"].items():
                ...
                for i, perk in enumerate(tier_data["perks"]):
                    ...
                    perk_node = {"x": branch_x + offset_x, "y": tier_y + offset_y, "data": perk}

                    layout["branches"][branch_name]["tiers"][tier_name]["perks"].append(perk_node)

                    # Generate connections
                    for prereq in perk["prerequisites"]:
                        layout["connections"].append({"from": prereq, "to": perk["perk_id"], "type": "prerequisite"})

                    for synergy in perk["synergies"]:
                        layout["connections"].append({"from": perk["perk_id"], "to": synergy, "type": "synergy"})
    ...
    def generate_perk_card(self, perk_id: str, user_id: int) -> Dict:
        ...
        can_unlock, reason = self.perk_system.can_unlock_perk(user_id, perk_id, player_data.level)
        ...
        for effect in perk.effects:
            ...
            effects_display.append(
                {"type": effect.effect_type, "value": display_value, "description": effect.description}
            )
        ...
        card = {
            ...
            "conflicts": [{"id": conf, "name": self.perk_system.perk_catalog[conf].name} for conf in perk.conflicts],
        }
    ...
    def generate_progression_path(self, user_id: int, target_perk_id: str) -> List[Dict]:
        ...
        if target_perk_id in player_data.unlocked_perks:
            return [{"perk_id": target_perk_id, "name": target_perk.name, "status": "already_unlocked"}]
                ...
                path.append(
                    {
                        ...
                        "can_unlock_now": (
                            player_data.level >= level_needed
                            and player_data.available_points >= points_needed
                            and all(p in player_data.unlocked_perks for p in perk.prerequisites)
                        ),
                    }
                )
    ...
    def generate_comparison_view(self, user_id: int, perk_ids: List[str]) -> Dict:
        ...
        player_data = self.perk_system.get_player_data(user_id)
        comparison = {"perks": [], "can_equip_together": True, "conflicts": []}
        ...
        for perk_id in perk_ids:
            ...
            for effect in perk.effects:
                perk_info["effects"].append(
                    {"type": effect.effect_type, "value": effect.value, "description": effect.description}
                )
            ...
            for other_id in perk_ids:
                if other_id != perk_id and other_id in perk.conflicts:
                    ...
                    comparison["conflicts"].append(
                        {"perk1": perk.name, "perk2": self.perk_system.perk_catalog[other_id].name}
                    )
        ...
        if len(branches) != len(set(branches)):
            comparison["can_equip_together"] = False
            comparison["conflicts"].append({"reason": "Multiple perks from same branch"})
    ...
    def generate_tree_stats(self, user_id: int) -> Dict:
        ...
        stats = {
            ...
            "completion_percentage": (len(player_data.unlocked_perks) / len(self.perk_system.perk_catalog) * 100),
            "by_branch": {},
            "by_tier": {},
        ...
        for branch in ["TRADING", "ANALYSIS", "SOCIAL", "ECONOMY"]:
            stats["by_branch"][branch] = {"total": 0, "unlocked": 0}

        for tier in range(1, 5):
            stats["by_tier"][tier] = {"total": 0, "unlocked": 0}
        ...
        if player_data.unlocked_perks:
            ...
            for perk_id in player_data.unlocked_perks:
                ...
                if perk and perk.tier.value > highest_tier:
                    ...
                    rarest = {"perk_id": perk_id, "name": perk.name, "tier": perk.tier.value}
