...
class BitPresence:
    ...
    def get_presence(
        self, user_state: EmotionalState, event: Optional[str] = None, context: Dict = None
    ) -> PersonaResponse:
        ...
        return PersonaResponse(persona=PersonaType.BIT, message=message, visual_cue=visual, audio_cue=audio)
...
class PersonaOrchestrator:
    ...
    def get_response(self, user_id: str, event_type: str, context: Dict) -> List[PersonaResponse]:
        """
        ...
        if event_type.startswith("user_") and random.random() < 0.2:
            ...
            responses.append(
                PersonaResponse(persona=PersonaType.OVERWATCH, message=comment, visual_cue="corner_text_overlay")
            )
...
if __name__ == "__main__":
    ...
    responses = orchestrator.get_response(
        ...
        context={
            "recent_losses": 2,
            "trades_per_hour": 15,
            "health_metrics": {"stress_level": 8, "daily_loss_percent": 4},
        },
    )
