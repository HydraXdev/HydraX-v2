...
def claim_press_pass():
    ...
    try:
        ...
        if not name or not email:
            return jsonify({"success": False, "error": "missing_fields", "message": "Name and email are required"}), 400

        # Validate email format
        if not validate_email(email):
            return (
                jsonify({"success": False, "error": "invalid_email", "message": "Please enter a valid email address"}),
                400,
            )
        ...
        if not check_rate_limit(f"press_pass_ip:{client_ip}", limit=3, window=3600):
            return (
                jsonify(
                    {"success": False, "error": "rate_limit", "message": "Too many attempts. Please try again later."}
                ),
                429,
            )
        ...
        if not check_rate_limit(f"press_pass_email:{email}", limit=2, window=86400):
            return (
                jsonify(
                    {"success": False, "error": "rate_limit", "message": "This email has already been used recently."}
                ),
                429,
            )
        ...
        if result["success"]:
            ...
            if redis_client:
                redis_client.publish(
                    "press_pass:claimed", json.dumps({"email": email, "timestamp": datetime.utcnow().isoformat()})
                )
...
def activate_press_pass():
    ...
    try:
        ...
        if not claim_token or not telegram_id:
            return (
                jsonify(
                    {"success": False, "error": "missing_fields", "message": "Claim token and Telegram ID are required"}
                ),
                400,
            )
        ...
        if result["success"]:
            # Track activation
            track_event("press_pass_activated", {"telegram_id": telegram_id})
...
def convert_press_pass():
    ...
    try:
        ...
        track_event(
            "press_pass_converted",
            {
                "telegram_id": telegram_id,
                "new_tier": new_tier,
                "conversion_value": {"NIBBLER": 39, "FANG": 89, "COMMANDER": 139, "": 188}.get(new_tier, 0),
            },
        )

        return jsonify({"success": True, "message": "Conversion tracked successfully"}), 200
...
def track_email_open(claim_id: int, campaign_type: str):
    ...
    try:
        # Track the open
        track_event("email_opened", {"claim_id": claim_id, "campaign_type": campaign_type})
...
def track_email_click():
    ...
    try:
        ...
        track_event("email_clicked", {"claim_id": claim_id, "campaign_type": campaign_type, "cta": cta})
    ...
    try:
        if hasattr(current_app, "socketio"):
            ...
            current_app.socketio.emit(
                "press_pass_update",
                {"daily_remaining": remaining, "timestamp": datetime.utcnow().isoformat()},
                namespace="/live",
            )
    except Exception as e:
