...
class PressPassManager:
    ...
    def get_weekly_remaining(self) -> int:
        ...
        try:
            ...
            week_claims = self.db.query(PressPassClaim).filter(PressPassClaim.claimed_at >= monday).count()

            remaining = max(0, WEEKLY_PRESS_PASS_LIMIT - week_claims)

            # Cache in Redis
            if redis_client:
                redis_client.setex("press_pass:weekly_remaining", 300, remaining)  # 5 minute cache
    ...
    def claim_press_pass(self, email: str, name: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            self.track_conversion_event(claim.id, "press_pass_claimed", {"ab_variant": ab_variant})
    ...
    def activate_press_pass(self, claim_token: str, telegram_id: int) -> Dict[str, Any]:
        ...
        try:
            claim = self.db.query(PressPassClaim).filter_by(claim_token=claim_token, activated=False).first()
            ...
            if claim.expires_at < datetime.utcnow():
                return {"success": False, "error": "expired", "message": "This Press Pass has expired"}
            ...
            # Track activation
            self.track_conversion_event(claim.id, "press_pass_activated", {"telegram_id": telegram_id})
    ...
    def send_welcome_email(self, claim: PressPassClaim):
        ...
        try:
            ...
            # Add tracking pixel
            tracking_pixel = (
                f'<img src="https://joinbitten.com/track/email/open/{claim.id}/welcome" width="1" height="1" />'
            )
            ...
            if SMTP_USER and SMTP_PASSWORD:
                ...
                campaign = EmailCampaign(
                    press_pass_claim_id=claim.id, campaign_type="welcome", content_variant=claim.ab_variant
                )
    ...
    def track_conversion_event(self, claim_id: int, event_type: str, event_data: Dict = None):
        ...
        try:
            event = ConversionEvent(press_pass_claim_id=claim_id, event_type=event_type, event_data=event_data or {})
            self.db.add(event)
            self.db.commit()
    ...
    def get_conversion_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for variant in self.ab_variants:
                ...
                variant_converted = self.db.query(PressPassClaim).filter_by(ab_variant=variant, converted=True).count()
