...
from database.xp_database import PressPassReset, XPDatabase
...
class PressPassResetManager:
    ...
    def __init__(
        self, xp_manager: "XPIntegrationManager", telegram: TelegramMessenger, xp_database: Optional[XPDatabase] = None
    ):
        ...
        for user_id in self.press_pass_users:
            try:
                ...
                if current_xp > 0:
                    ...
                    await self.telegram.send_message(chat_id=user_id, text=message, parse_mode="Markdown")
        ...
        if self.xp_db:
            try:
                ...
                for reset in resets:
                    ...
                    await self.telegram.send_message(chat_id=str(reset.user_id), text=message, parse_mode="Markdown")
    ...
    def _run_scheduler(self):
        ...
        schedule.every().day.at("23:00").do(lambda: asyncio.create_task(self.send_warning_notification(1)))

        # Schedule warning at 23:45 UTC (15 minutes before)
        schedule.every().day.at("23:45").do(lambda: asyncio.create_task(self.send_warning_notification(0.25)))

        # Schedule reset at 00:00 UTC
        schedule.every().day.at("00:00").do(lambda: asyncio.create_task(self.execute_xp_reset()))
