...
class PressPassRotationSystem:
    ...
    def assign_press_pass_account(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.save_vault(vault):
                return {"success": False, "message": "Failed to save account assignment", "error": "save_failed"}
            ...
            return {"success": False, "message": "System error during account assignment", "error": str(e)}
    ...
    def launch_mt5_terminal_and_bridge(self, user_id: str, account: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if bridge_process.poll() is None:
                ...
                return {"success": False, "message": "Failed to launch MT5 bridge", "error": "bridge_start_failed"}

        except Exception as e:
            logger.error(f"Error launching MT5 terminal/bridge: {e}")
            return {"success": False, "message": "Failed to launch MT5 services", "error": str(e)}
    ...
    def check_and_expire_accounts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for account in vault:
                if account.get("status") == "assigned" and account.get("expires_at"):
                    ...
                    if now >= expires_at:
                        ...
                        account.update(
                            {"assigned_to": None, "assigned_at": None, "expires_at": None, "status": "available"}
                        )

                        expired_count += 1
                        recycled_accounts.append(
                            {"login": account["login"], "user_id": user_id, "expired_at": expires_at.isoformat()}
                        )
            ...
            logger.info(
                f"Expiry check complete: {expired_count} expired, {available_count} available, {assigned_count} assigned"
            )
    ...
    def get_press_pass_status(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not user_account:
                return {"success": True, "has_press_pass": False, "message": "No active Press Pass found"}
            ...
            if time_remaining.total_seconds() <= 0:
                return {"success": True, "has_press_pass": False, "message": "Press Pass has expired", "expired": True}
    ...
    def revoke_press_pass(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if revoked:
                self.save_vault(vault)
                return {"success": True, "message": "Press Pass revoked successfully"}
            else:
                return {"success": False, "message": "No active Press Pass found for user"}
