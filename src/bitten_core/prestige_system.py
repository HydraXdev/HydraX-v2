...
class PrestigeSystem:
    ...
    def execute_prestige(
        self, user_id: str, current_xp: int
    ...
    def get_prestige_benefits(self, user_id: str) -> Dict[str, Any]:
        ...
        benefits = {
            ...
            "rank": {"name": rank.title, "icon": rank.icon, "level": rank.level},
            ...
            leaderboard_data.append(
                {
                    ...
                    "rank": {"name": rank.title, "icon": rank.icon},
    ...
    def _update_leaderboard(self) -> None:
        ...
        for user_id, progress in self.user_progress.items():
            if progress.current_level > 0:
                self.leaderboard.append((user_id, progress.current_level, progress.lifetime_xp))
    ...
    def _save_leaderboard(self) -> None:
        ...
        for user_id, level, lifetime_xp in self.leaderboard:
            data.append({"user_id": user_id, "level": level, "lifetime_xp": lifetime_xp})
    ...
    def _load_prestige_data(self) -> None:
        ...
        if leaderboard_file.exists():
            try:
                ...
                self.leaderboard = [(item["user_id"], item["level"], item["lifetime_xp"]) for item in data]
            except Exception as e:
