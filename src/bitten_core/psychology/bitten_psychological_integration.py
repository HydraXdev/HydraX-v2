...
from .tactical_mission_system import MissionTier, MissionType, TacticalMissionSystem
...
class BittenPsychologicalSystem:
    ...
    def __init__(self, db_connection, telegram_bot, lockout_system):
        ...
        self.emotion_engine = EmotionReplacementEngine(db_connection, self, lockout_system)  # personality system

        self.mission_system = TacticalMissionSystem(db_connection, self.emotion_engine, self)  # personality system
    ...
    async def process_user_onboarding(self, user_id: str, username: str) -> Dict:
        ...
        try:
            ...
            first_mission = await self.mission_system.assign_mission(user_id, "first_contact")
    ...
    async def process_trading_event(self, user_id: str, event_type: str, event_data: Dict) -> Dict:
        ...
        try:
            ...
            if event_type in ["trade_placed", "trade_closed", "loss_occurred"]:
                ...
                if emotion:
                    replacement_result = await self.emotion_engine.replace_emotion(user_id, emotion, event_data)
            ...
            mission_progress = await self.mission_system.check_mission_progress(user_id, event_data)
            ...
            if "message" in event_data:
                soul_check = self.soul_filter.evaluate_message(
                    event_data["message"], {"user_id": user_id, "event_type": event_type}
                )
                ...
                if soul_check["alignment"] in [SoulAlignment.CORRUPTED, SoulAlignment.POISON]:
                    improved_message = self.soul_filter.ensure_tactical_message(event_data["message"])
    ...
    async def send_message(self, user_id: str, personality: str, message: str, context: Dict):
        ...
        self.active_interventions[user_id].append(
            {"personality": personality, "message": message, "context": context, "timestamp": datetime.now()}
        )
    ...
    async def _log_intervention(self, user_id: str, personality: str, message: str, context: Dict):
        ...
        await self.db.execute(
            query, (user_id, personality, context.get("trigger_event", "manual"), message, datetime.now())
        )
