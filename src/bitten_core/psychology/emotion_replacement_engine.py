...
class EmotionReplacementEngine:
    ...
    def __init__(self, db_connection, personality_system, lockout_system):
        ...
        self.emotion_triggers = {
            ...
            TradingEmotion.PANIC: ["rapid_trades_in_minutes > 3", "stop_losses_removed > 1", "account_drawdown > 15"],
            TradingEmotion.FOMO: [
    ...
    async def replace_emotion(self, user_id: str, emotion: TradingEmotion, context: Dict) -> Dict:
        ...
        try:
            ...
            protocol_result = await self._execute_protocol(
                user_id, protocol_config["protocol"], protocol_config["action"], context
            )

            # Send personality intervention
            personality_message = await self._send_personality_intervention(
                user_id, protocol_config["personality"], emotion, protocol_config["message"], context
            )
            ...
            if protocol_config["lockout_minutes"] > 0:
                lockout_result = await self._apply_tactical_lockout(
                    user_id, protocol_config["lockout_minutes"], f"{emotion.value} intervention"
                )
    ...
    async def _evaluate_triggers(self, user_id: str, triggers: List[str], event: Dict, history: Dict) -> bool:
        """
    ...
    async def _evaluate_single_trigger(self, trigger: str, user_id: str, event: Dict, history: Dict) -> bool:
        """
    ...
    async def _execute_protocol(self, user_id: str, protocol: TacticalProtocol, action: str, context: Dict) -> Dict:
        """
    ...
    async def _send_personality_intervention(
        self, user_id: str, personality: str, emotion: TradingEmotion, message: str, context: Dict
    ) -> Dict:
        """
        ...
        return await self.personality.send_intervention(user_id, personality, selected_message, context)
    ...
    async def _update_psychology_profile(self, user_id: str, emotion: TradingEmotion, protocol: TacticalProtocol):
        """
    ...
    async def _get_user_trading_history(self, user_id: str, days: int = 7) -> Dict:
        ...
        return {"recent_trades": [], "current_balance": 10000, "starting_balance": 10000, "peak_balance": 10000}
