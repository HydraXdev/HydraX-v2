    ...
    def _update_daily_statistics(self, user_id: str, trade_data: Dict, now: datetime):
        ...
        try:
            ...
            if existing:
                ...
                """,
                    (1 if is_win else 0, 0 if is_win else 1, pnl, volume, now.isoformat(), user_id, today),
                )
            else:
                # Create new record
                self.conn.execute(
    ...
    def get_user_daily_performance(self, user_id: str, days: int = 30) -> List[Dict]:
        ...
        try:
            ...
            for row in rows:
                ...
                day_stats["win_rate"] = (
                    (day_stats["wins_count"] / day_stats["trades_count"]) * 100 if day_stats["trades_count"] > 0 else 0
                )
