        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                if process_name in proc.info["name"] or any(
                    process_name in cmd for cmd in (proc.info["cmdline"] or [])
                ):
                    proc.terminate()
...
class SystemRecoveryManager:
    ...
    def clear_temp_files(context: Dict[str, Any]) -> bool:
        ...
        temp_dirs = ["/tmp", "/var/tmp", "/root/HydraX-v2/logs", "/root/HydraX-v2/temp"]
    ...
    def check_memory_usage(context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for proc in processes[:3]:  # Only top 3 memory users
                if proc.info["memory_percent"] > 20 and proc.info["name"] not in ["systemd", "kernel", "init"]:
                    try:
                        proc.terminate()
                        proc.wait(timeout=5)
    ...
    def execute_recovery_sequence(self, category: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self._lock:
            if category in self.active_recoveries:
                return {"success": False, "error": f"Recovery already in progress for {category}"}

            self.active_recoveries.add(category)

        try:
            results = []
            actions = sorted(self.recovery_actions[category], key=lambda x: x.priority)

            for action in actions:
                # Check cooldown
                last_attempt = self.recovery_history.get(f"{category}_{action.name}")
                if last_attempt and (datetime.now() - last_attempt).seconds < action.cooldown_seconds:
                    continue
                ...
                try:
                    ...
                    results.append({"action": action.name, "description": action.description, "success": success})
                    ...
                    results.append(
                        {"action": action.name, "description": action.description, "success": False, "error": str(e)}
                    )
    ...
    def get_recovery_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_recovery_attempts": {
                key: timestamp.isoformat() for key, timestamp in list(self.recovery_history.items())[-10:]
            },
        }
...
def recover_database_issues(db_path: str) -> Dict[str, Any]:
    ...
    return recovery_manager.execute_recovery_sequence("database", {"db_path": db_path})
...
def recover_telegram_issues(bot_token: str) -> Dict[str, Any]:
    ...
    return recovery_manager.execute_recovery_sequence("telegram", {"bot_token": bot_token})
