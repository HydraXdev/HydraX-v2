...
class ReferralGamificationHooks:
    ...
    def trigger_referral_success_rewards(self, referrer_id: str, referral_count: int) -> Dict:
        ...
        if badge:
            ...
            rewards["notifications"].append(f"🏆 BADGE EARNED: {badge_reward['badge']} (+{badge_xp} XP)")
            rewards["notifications"].append(f"💬 {badge_reward['message']}")
            ...
            if "special_privilege" in badge_reward:
                ...
                rewards["notifications"].append(f"⭐ SPECIAL FEATURE UNLOCKED: {badge_reward['special_privilege']}")

            # War room unlock
            if "war_room_unlock" in badge_reward:
                rewards["notifications"].append(f"🏛️ WAR ROOM FEATURE UNLOCKED: {badge_reward['war_room_unlock']}")

            # Legendary status
            if badge_reward.get("legendary_status"):
                rewards["notifications"].append(f"👑 LEGENDARY STATUS ACHIEVED: Recruitment Hall of Fame!")
    ...
    def trigger_credit_milestone_rewards(self, user_id: str, total_credits_earned: float) -> Dict:
        ...
        rewards = {"xp_gained": 0, "badges_earned": [], "notifications": []}
        ...
        for milestone in credit_milestones:
            if total_credits_earned >= milestone and total_credits_earned < milestone + 10:
                ...
                rewards["notifications"].append(f"💰 CREDIT MILESTONE: ${milestone} total earned! (+{xp_reward} XP)")
    ...
    def create_war_room_recruitment_widget(self, user_id: str, stats: Dict) -> Dict:
        ...
        widget = {
            ...
            "progress_bar": {
                ...
                "percentage": (
                    min(100, (balance.referral_count / progress["next_target"]) * 100)
                    if progress["next_target"] > 0
                    else 100
                ),
            },
            "next_badge": {
