...
class ReferralSystem:
    ...
    def generate_referral_code(self, user_id: str, custom_code: Optional[str] = None) -> Tuple[bool, str, ReferralCode]:
        ...
        try:
            ...
            code = ReferralCode(code=code_str, user_id=user_id, created_at=datetime.now())
    ...
    def use_referral_code(
        self, recruit_id: str, code: str, username: str, ip_address: str
        ...
        try:
            ...
            if self.xp_economy:
                self.xp_economy.add_xp(referrer_id, xp_amount, f"New recruit: {username}")
    ...
    def track_recruit_progress(
        self, recruit_id: str, event_type: str, event_data: Optional[Dict[str, Any]] = None
    ) -> List[Tuple[str, int]]:
        ...
        try:
            ...
            if event_type == "trade_completed":
                ...
                # Milestone rewards (every 10 trades)
                    ...
                    if self.xp_economy:
                        self.xp_economy.add_xp(referrer_id, xp_amount, f"{reward.reason} ({new_trade_count} trades)")
    ...
    def get_squad_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if not squad_row:
                    return {"total_recruits": 0, "active_recruits": 0, "total_xp": 0, "squad_rank": "LONE_WOLF"}
    ...
    def get_referral_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_rewards = [{"type": row[0], "xp": row[1], "timestamp": row[2]} for row in cursor.fetchall()]
            ...
            top_recruits = [
                {"user_id": row[0], "username": row[1], "trades": row[2], "xp_contributed": row[3], "rank": row[4]}
                for row in cursor.fetchall()
            ]
    ...
    def format_telegram_response(self, user_id: str) -> str:
        ...
        if stats["referral_code"]:
            lines.extend(
                [f"📋 **Your Referral Code:** `{stats['referral_code']}`", f"Times Used: {stats['code_uses']}", ""]
            )
        else:
            lines.extend(["⚠️ No active referral code", "Use /refer generate to create one", ""])

        if stats["top_recruits"]:
            lines.append("🏆 **Top Squad Members:**")
            for i, recruit in enumerate(stats["top_recruits"], 1):
                lines.append(f"{i}. {recruit['username']} - " f"{recruit['rank']} - " f"{recruit['xp_contributed']} XP")
            lines.append("")

        if stats["next_multiplier_at"]:
            lines.append(f"📈 Next multiplier boost at " f"{stats['next_multiplier_at']} recruits")
        ...
        if tier2_result and tier2_result[0]:
            ...
            if self.xp_economy:
                self.xp_economy.add_xp(
                    tier2_referrer, int(self.REWARDS["join"].xp_amount * 0.5), "Tier 2 recruit joined"
                )
            ...
            if tier3_result and tier3_result[0]:
                ...
                if self.xp_economy:
                    self.xp_economy.add_xp(
                        tier3_referrer, int(self.REWARDS["join"].xp_amount * 0.25), "Tier 3 recruit joined"
                    )
...
class ReferralCommandHandler:
    ...
    def _format_detailed_stats(self, user_id: str) -> str:
        ...
        if stats["recent_rewards"]:
            ...
            for reward in stats["recent_rewards"][:5]:
                lines.append(f"• {reward['type'].replace('_', ' ').title()} - " f"{reward['xp']} XP")
            lines.append("")
    ...
    def _format_genealogy_tree(self, genealogy: Dict[str, Any]) -> str:
        ...
        def format_recruit(recruit: Dict[str, Any], indent: int = 0):
            ...
            lines.append(f"{prefix}{recruit['username']} " f"({recruit['rank']}) - " f"{recruit['trades']} trades")
...
class ReferralIntegration:
    ...
    def on_trade_completed(self, user_id: str, trade_data: Dict[str, Any]) -> None:
        ...
        try:
            rewards = self.referral_system.track_recruit_progress(user_id, "trade_completed", trade_data)
    ...
    def on_rank_upgraded(self, user_id: str, new_rank: str, old_rank: str) -> None:
        ...
        try:
            rewards = self.referral_system.track_recruit_progress(
                user_id, "rank_upgraded", {"new_rank": new_rank, "old_rank": old_rank}
            )
    ...
    def on_weekly_activity_check(self, user_id: str) -> None:
        ...
        try:
            rewards = self.referral_system.track_recruit_progress(user_id, "weekly_activity")
...
if __name__ == "__main__":
    ...
    # Use referral code
    success, message, result = referral_system.use_referral_code("new_user456", "ELITE", "NewRecruit", "192.168.1.1")
    print(f"Use code: {message}")

    # Track progress
    rewards = referral_system.track_recruit_progress("new_user456", "trade_completed")
