...
class ResultTracker:
    ...
    async def _save_trade_result(self, result_data: TradeResultData):
        ...
        """,
            (
                result_data.trade_id,
                result_data.signal_id,
                result_data.user_id,
                result_data.symbol,
                result_data.direction,
                result_data.entry_price,
                result_data.exit_price,
                result_data.volume,
                result_data.open_time,
                result_data.close_time,
                result_data.profit_pips,
                result_data.profit_usd,
                result_data.result.value,
                result_data.tcs_score,
                result_data.fire_mode,
                result_data.hold_time_minutes,
                result_data.sl_hit,
                result_data.tp_hit,
...
class CallbackManager:
    ...
    def _setup_default_callbacks(self):
        ...
        self.result_tracker.register_callback(TradeEvent.CLOSED, self._tcs_optimizer_callback)

        # Performance logging callback
        self.result_tracker.register_callback(TradeEvent.CLOSED, self._performance_logging_callback)

        # Alert callback for significant events
        self.result_tracker.register_callback(TradeEvent.CLOSED, self._alert_callback)
    ...
    async def _performance_logging_callback(self, result_data: TradeResultData):
        ...
        logger.info(
            f"Trade Performance: {result_data.symbol} - {result_data.result.value} "
...
async def example_usage():
    ...
    close_data = {"trade_id": "TRADE_001", "exit_price": 1.0950, "profit_pips": 50, "profit_usd": 5.0, "tp_hit": True}
