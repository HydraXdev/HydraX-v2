...
class RiskTier(Enum):
    ...
    def xp_bonus(self) -> float:
        ...
        tier_bonuses = {RiskTier.STANDARD: 0.0, RiskTier.PREMIUM: 0.15}  # 0% bonus  # 15% bonus
...
class RewardSystem:
    ...
    def __init__(self, db_path: str = "bitten_rewards.db", profile_manager=None):
    ...
    def calculate_reward(
        self, user_id: int, profit_amount: float, risk_tier: RiskTier = RiskTier.STANDARD
    ) -> RewardCalculation:
        ...
        # Calculate total
        total_reward = base_reward + tier_bonus + xp_bonus + streak_bonus + achievement_bonus
        ...
        breakdown = {
            ...
            "multipliers": {
                ...
                "achievements": sum(self.ACHIEVEMENT_MULTIPLIERS.get(m, 0) for m in earned_medals),
            },
        }
    ...
    def _calculate_achievement_bonus(self, base_reward: float, earned_medals: List[str]) -> float:
    ...
    def update_streak(self, user_id: int, streak_type: str = "daily"):
        ...
        if streak_type == "daily":
            cursor.execute(
                ...
                ON CONFLICT(user_id) DO UPDATE SET
                    ...
                    daily_last_activity = ?
            """,
                (user_id, new_streak, best_streak, current_time, new_streak, best_streak, current_time),
            )
        else:  # weekly
            cursor.execute(
                """
        ...
        if self.profile_manager and new_streak in self.STREAK_BONUSES.get(streak_type, {}):
            ...
            self.profile_manager.award_xp(user_id, bonus_xp, f"{streak_type.title()} streak milestone: {new_streak}")
    ...
    def get_streak_info(self, user_id: int, streak_type: str = "daily") -> StreakInfo:
        ...
        if streak_type == "daily":
            return StreakInfo(current_streak=row[1], best_streak=row[2], last_activity=row[3], streak_type="daily")
        else:  # weekly
            return StreakInfo(current_streak=row[4], best_streak=row[5], last_activity=row[6], streak_type="weekly")

    def record_period_activity(self, user_id: int, trades: int, profit: float, xp: int, rewards: float):
        ...
        self._update_aggregation(cursor, user_id, "daily", daily_key, trades, profit, xp, rewards)

        # Update weekly
        weekly_key = datetime.fromtimestamp(current_time).strftime("%Y-W%U")
        self._update_aggregation(cursor, user_id, "weekly", weekly_key, trades, profit, xp, rewards)

        # Update monthly
        monthly_key = datetime.fromtimestamp(current_time).strftime("%Y-%m")
        self._update_aggregation(cursor, user_id, "monthly", monthly_key, trades, profit, xp, rewards)
    ...
    def get_period_summary(self, user_id: int, period: RewardPeriod) -> Dict:
        ...
        historical = self._get_historical_comparison(cursor, user_id, period, period_key)
        summary["historical"] = historical

        conn.close()
        return summary

    def _get_historical_comparison(self, cursor, user_id: int, period: RewardPeriod, current_key: str) -> Dict:
        ...
        for row in cursor.fetchall():
            historical.append({"period_key": row[0], "trades": row[1], "profit": row[2], "rewards": row[3]})
                    ...
                    earned_medals.append(
                        {
                            ...
                            "reward_multiplier": self.ACHIEVEMENT_MULTIPLIERS.get(medal["id"], 0),
                        }
                    )
                    total_xp_from_achievements += medal["xp_reward"]

        # Calculate total reward multiplier
        total_multiplier = sum(self.ACHIEVEMENT_MULTIPLIERS.get(m["id"], 0) for m in earned_medals)
    ...
    def calculate_tier_upgrade_benefit(self, user_id: int, current_profit: float) -> Dict:
        """Calculate benefit of upgrading to tier"""
        # Calculate rewards for both tiers
        standard_reward = self.calculate_reward(user_id, current_profit, RiskTier.STANDARD)
        apex_reward = self.calculate_reward(user_id, current_profit, RiskTier.PREMIUM)
        ...
        return {
            ...
            "breakdowns": {"standard": standard_reward.breakdown, "apex": apex_reward.breakdown},
        }
    ...
    def get_comprehensive_rewards_dashboard(self, user_id: int) -> Dict:
        ...
        return {
            ...
            "xp_tier": {
                ...
                "next": next_tier,
                "progress_to_next": (
                    (user_xp - current_tier["min_xp"]) / (next_tier["min_xp"] - current_tier["min_xp"]) * 100
                    if next_tier
                    else 100
                ),
            },
    ...
    def _get_next_streak_bonus(self, current_streak: int, streak_type: str) -> Optional[Dict]:
        """Get next streak milestone and bonus"""
        bonuses = self.STREAK_BONUSES.get(streak_type, {})

        for days, bonus in sorted(bonuses.items()):
            if current_streak < days:
                return {"days_required": days, "days_remaining": days - current_streak, "bonus_percentage": bonus * 100}

        return None

    def simulate_reward(self, user_id: int, profit: float, risk_tier: str = "STANDARD") -> Dict:
