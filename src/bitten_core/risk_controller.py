...
class RiskController:
    ...
    def check_trade_allowed(
        self, user_id: int, tier: TierLevel, potential_loss: float, account_balance: float
    ) -> Tuple[bool, str]:
        """
    ...
    def record_trade_result(self, user_id: int, tier: TierLevel, won: bool, pnl: float, risk_percent: float):
        """
        ...
        with self._lock:
            ...
            if not won and risk_percent >= 1.5:  # High risk threshold
                ...
                if profile.consecutive_high_risk_losses >= 2 and not self._is_in_cooldown(user_id):
    ...
    def toggle_risk_mode(self, user_id: int, tier: TierLevel, new_mode: RiskMode) -> Tuple[bool, str]:
        """
    ...
    def get_user_status(self, user_id: int, tier: TierLevel, account_balance: float) -> Dict:
    ...
    def _activate_cooldown(
        self, user_id: int, tier: TierLevel, reason: str, consecutive_losses: int, loss_amounts: List[float]
    ):
    ...
    def _get_or_create_profile(self, user_id: int, tier: TierLevel) -> UserRiskProfile:
        ...
        if user_id not in self.profiles:
            self.profiles[user_id] = UserRiskProfile(user_id=user_id, tier=tier)
            self._save_profiles()
