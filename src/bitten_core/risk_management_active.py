...
class v5RiskParameters:
    ...
    signal_risk_multipliers: Dict[str, float] = field(
        default_factory=lambda: {
    ...
    tcs_risk_multipliers: Dict[str, float] = field(
        default_factory=lambda: {
...
class v5RiskManager:
    ...
    def calculate_position_size(self, signal: Dict, account_balance: float, current_session: str = "NORMAL") -> float:
        ...
        try:
            ...
            logger.info(
                f"Position calculated: {pair} - {lot_size} lots "
    ...
    def update_signal_count(self, signal: Dict, current_session: str = "NORMAL"):
        ...
        logger.info(
            f"Signal counters updated - Daily: {self.daily_signals}, "
    ...
    def check_correlation_limits(self, new_signal: Dict, active_trades: List[Dict]) -> bool:
        ...
        for trade in active_trades:
            ...
            # Count USD exposure
            if "USD" in pair:
                if (pair.startswith("USD") and direction == "BUY") or (pair.endswith("USD") and direction == "SELL"):
                    usd_exposure += 1
                elif (pair.startswith("USD") and direction == "SELL") or (pair.endswith("USD") and direction == "BUY"):
                    usd_exposure -= 1

            # Count EUR exposure
            if "EUR" in pair:
                if (pair.startswith("EUR") and direction == "BUY") or (pair.endswith("EUR") and direction == "SELL"):
                    eur_exposure += 1
                elif (pair.startswith("EUR") and direction == "SELL") or (pair.endswith("EUR") and direction == "BUY"):
                    eur_exposure -= 1

            # Count GBP exposure
            if "GBP" in pair:
                if (pair.startswith("GBP") and direction == "BUY") or (pair.endswith("GBP") and direction == "SELL"):
                    gbp_exposure += 1
                elif (pair.startswith("GBP") and direction == "SELL") or (pair.endswith("GBP") and direction == "BUY"):
                    gbp_exposure -= 1
    ...
    def update_performance(self, trade_result: Dict):
        ...
        logger.info(
            f"Performance updated - WR: {self.win_rate:.1f}%, "
