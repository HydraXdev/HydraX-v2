...
from .master_filter import FilteredSignal, MasterFilter
...
class SelectorIntegration:
    ...
    async def process_signals_for_user(
        self, user_id: int, tier: TierLevel, balance: float, signals: Dict[str, List[FilteredSignal]]
    ) -> List[Dict]:
        """
    ...
    async def handle_selector_action(self, user_id: int, action: Dict) -> Dict:
        ...
        if action["action"] == "display_only":
            ...
            if action.get("execution_params"):
                execution_result = await self._execute_trade(user_id, action["signal"], action["execution_params"])
    ...
    async def handle_user_confirmation(self, user_id: int, confirmation_id: str, decision: str) -> Dict:
        ...
        if result["success"] and result["action"] == "execute":
            ...
            can_execute, _, params = self.master_filter.validate_shot(
                user_id, signal["signal_id"], TierLevel.COMMANDER, self.master_filter.user_states[user_id]["balance"]
            )

            if can_execute:
                execution_result = await self._execute_trade(user_id, signal, params)
                result.update(execution_result)

        return result

    async def _execute_trade(self, user_id: int, signal: Dict, execution_params: Dict) -> Dict:
    ...
    def handle_selector_command(self, user_id: int, command: str, tier: TierLevel) -> str:
...
async def selector_telegram_handler(update: Dict, integration: SelectorIntegration):
    ...
    if text.startswith("/selector"):
        ...
        response = integration.handle_selector_command(user_id, text, TierLevel.COMMANDER)  # Would get from user data
        return {"type": "message", "text": response}

    elif text.startswith("confirm_"):
        # Handle confirmation response
        confirmation_id = text.replace("confirm_", "")
        result = await integration.handle_user_confirmation(user_id, confirmation_id, "confirm")
        return {"type": "message", "text": f"✅ {result.get('message', 'Processed')}"}

    elif text.startswith("reject_"):
        # Handle rejection
        confirmation_id = text.replace("reject_", "")
        result = await integration.handle_user_confirmation(user_id, confirmation_id, "reject")
