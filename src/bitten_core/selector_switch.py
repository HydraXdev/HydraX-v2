...
class CommanderSelector:
    ...
    def _process_semi_mode(self, signal: Dict) -> Dict:
        ...
        return {
            ...
            "popup_data": {
                ...
                "signal": signal,
                "analysis": analysis,
                "buttons": [{"text": "🔫 FIRE", "action": "confirm"}, {"text": "❌ SKIP", "action": "reject"}],
            },
        }
    ...
    def _process_full_auto(self, signal: Dict) -> Dict:
        ...
        if signal["signal_type"] == "arcade" and not self.auto_config.arcade_enabled:
            return {"action": "auto_rejected", "reason": "Arcade signals disabled in AUTO mode"}

        if signal["signal_type"] == "sniper" and not self.auto_config.sniper_enabled:
            return {"action": "auto_rejected", "reason": "Sniper signals disabled in AUTO mode"}
    ...
    def handle_confirmation(self, confirmation_id: str, decision: str) -> Dict:
        ...
        if decision == "confirm":
            ...
            return {"success": True, "action": "rejected", "message": "Signal rejected by user"}
