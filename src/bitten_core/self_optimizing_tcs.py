...
class SelfOptimizingTCS:
    ...
    async def get_optimal_tcs_threshold(self, market_condition: MarketCondition) -> float:
        ...
        new_threshold = max(
            self.config.min_tcs_threshold, min(self.config.max_tcs_threshold, self.current_tcs_threshold + adjustment)
        )
        ...
        if abs(new_threshold - self.current_tcs_threshold) >= 0.5:
            ...
            logger.info(
                f"TCS threshold adjusted: {self.current_tcs_threshold:.1f}% (Reason: {', '.join(reason_parts)})"
            )
    ...
    async def _log_adjustment(
        ...
        """,
            (
                ...
                signal_count,
                win_rate,
                market_condition.session_activity,
                market_condition.volatility_level,
            ),
        )
    ...
    async def log_signal_performance(
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (pair, tcs_score, direction, result, pips_gained, hold_time_minutes, market_condition.session_activity),
        )
...
async def main():
    ...
    market_condition = MarketCondition(
        volatility_level="MEDIUM", trend_strength=0.6, session_activity="LONDON", news_impact=0.3
    )
