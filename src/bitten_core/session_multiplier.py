...
class SessionMultiplierSystem:
    ...
    def apply_session_multiplier(self, base_tcs: int, pair: str, strategy: Optional[str] = None) -> Dict:
        """
    ...
    def get_session_schedule(self) -> str:
        ...
        sorted_sessions = sorted([(s, c) for s, c in self.sessions.items()], key=lambda x: x[1].hours[0])
