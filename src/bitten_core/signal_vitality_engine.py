...
class SignalVitalityEngine:
    ...
    def calculate_vitality(self, mission_id: str, user_balance: float = 10000) -> VitalityMetrics:
        ...
        vitality_score = self._calculate_composite_score(price_metrics, spread_metrics, volume_metrics)
        ...
        adjusted_params = self._calculate_adjusted_parameters(original, current_market, price_metrics["drift_pips"])

        # Calculate risk in dollars
        risk_dollars = self._calculate_risk_dollars(original, adjusted_params, user_balance)

        # Generate educational content
        degradation_reasons = self._analyze_degradation_reasons(price_metrics, spread_metrics, volume_metrics)

        warnings = self._generate_execution_warnings(vitality_score, price_metrics, spread_metrics)

        educational_tip = self._select_educational_tip(degradation_reasons, vitality_score)
    ...
    def _get_current_market_data(self, symbol: str) -> Dict:
        ...
        try:
            ...
            response = requests.get(f"http://localhost:8001/market-data/venom-feed?symbol={symbol}", timeout=1)
        ...
        # Fallback data
        return {"price": 0, "spread": 2.0, "volume": 1000, "timestamp": datetime.now()}
    ...
    def _calculate_composite_score(self, price: Dict, spread: Dict, volume: Dict) -> float:
        ...
        weights = {"price": 0.5, "spread": 0.3, "volume": 0.2}  # 50% weight  # 30% weight  # 20% weight
