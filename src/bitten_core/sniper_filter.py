...
class SniperFilter:
                ...
                if current_price < recent_high:  # Price back below
                    ...
                    if tcs >= self.min_tcs:
                        return self._create_sniper_signal(
                            "liquidity_hunt", symbol, direction, entry_price, stop_loss, take_profit, tcs
                        )
    ...
    def _detect_tokyo_trap(self, symbol: str, data: Dict) -> Optional[SniperSignal]:
        ...
        if current_price > range_high:
            ...
            if break_distance <= self._tokyo_config["false_break_pips"]:
                ...
                if tcs >= self.min_tcs:
                    return self._create_sniper_signal(
                        "tokyo_trap", symbol, direction, entry_price, stop_loss, take_profit, tcs
                    )
        ...
        if current_price > pre_news_price:  # Bullish spike
            ...
            if spike_pips >= self._news_fade_config["fade_threshold"]:
                ...
                if self._check_exhaustion(data):
                    ...
                    if tcs >= self.min_tcs:
                        return self._create_sniper_signal(
                            "news_fade", symbol, direction, entry_price, stop_loss, take_profit, tcs
                        )
    ...
    def _detect_pivot_magnet(self, symbol: str, data: Dict) -> Optional[SniperSignal]:
        ...
        if tcs >= self.min_tcs:
            return self._create_sniper_signal(
                "pivot_magnet", symbol, direction, entry_price, stop_loss, take_profit, tcs
            )

        return None

    def _create_sniper_signal(
        self, strategy: str, symbol: str, direction: str, entry: float, stop: float, target: float, tcs: int
    ) -> SniperSignal:
        ...
        signal_hash = hashlib.md5(f"{strategy}{symbol}{entry}{datetime.now()}".encode()).hexdigest()[:8]
    ...
    def _check_exhaustion(self, data: Dict) -> bool:
        ...
        return (
            data.get("volume_declining", False)
            or data.get("momentum_divergence", False)
            or data.get("rejection_wick", False)
        )
