...
class SniperHUDIntegration:
    ...
    def __init__(self, bot_token: str, webapp_domain: str):
        ...
        self.telegram_router = TelegramRouter(
            bitten_core=None, hud_webapp_url=self.hud_webapp_url  # Your trading system
        )

        self.signal_alert_system = SignalAlertSystem(bot_token=bot_token, hud_webapp_url=self.hud_webapp_url)

        self.hud_router = HUDRouter(app=self.app, bot_token=bot_token)
    ...
    async def send_trading_signal(
        ...
        # Send alerts
        results = await self.signal_alert_system.send_batch_alerts(signal, target_users)

        print(
            f"Signal sent: {results['sent']} successful, "
...
if __name__ == "__main__":
    # Initialize system
    integration = SniperHUDIntegration(bot_token="YOUR_BOT_TOKEN", webapp_domain="https://your-domain.com")
