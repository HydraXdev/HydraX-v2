...
class SniperXPHandler:
    ...
    def register_sniper_trade(
        self, trade_id: str, user_id: int, symbol: str, entry_price: float, tp_price: float, sl_price: float
    ):
    ...
    def calculate_trade_xp(self, trade_id: str, exit_price: float, exit_reason: str, pnl: float) -> Dict:
        """
    ...
    def get_active_sniper_count(self, user_id: int) -> int:
        ...
        return sum(1 for trade in self.active_sniper_trades.values() if trade["user_id"] == user_id)
...
def calculate_fang_trade_xp(
    trade_id: str, signal_type: str, exit_price: float, exit_reason: str, pnl: float, entry_data: Optional[Dict] = None
) -> Dict:
    """
