...
try:
    from .chat_notifications import ChatNotificationService, NotificationPriority
    from .chat_notifications import NotificationType as ChatNotificationType
...
class SocialBragSystem:
    ...
    def notify_tactical_strategy_unlock(
        ...
        try:
            ...
            messages = self.TACTICAL_STRATEGY_MESSAGES.get(
                strategy_name,
                {
            ...
            # Format messages
            brag_message = messages["brag_message"].format(username=username, strategy_name=strategy_display_name)
            drill_sergeant_message = messages["drill_sergeant"].format(
                username=username, strategy_name=strategy_display_name
            )
    ...
    def notify_rank_promotion(
        self, user_id: str, username: str, new_rank: str, old_rank: str
    ) -> Optional[BragNotification]:
    ...
    def notify_prestige_achievement(
        self, user_id: str, username: str, prestige_level: int
    ) -> Optional[BragNotification]:
        ...
        try:
            ...
            # Get message templates
            messages = self.PRESTIGE_MESSAGES.get(
                prestige_level,
                {
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.referral_system, "db_path"):
                    ...
                    cursor = conn.cursor()
                    cursor.execute("SELECT referrer_id FROM recruits WHERE recruit_id = ?", (user_id,))
...
def notify_rank_promotion(user_id: str, username: str, new_rank: str, old_rank: str) -> Optional[BragNotification]:
...
def notify_prestige_achievement(user_id: str, username: str, prestige_level: int) -> Optional[BragNotification]:
...
if __name__ == "__main__":
    ...
    rank_brag = brag_system.notify_rank_promotion(
        user_id="test_user_456", username="BRAVO-2", new_rank="FANG", old_rank="NIBBLER"
    )
