...
class SocialSharingManager:
    ...
    def __init__(self):
        ...
        self.visual_elements = {
            "logos": {"bitten": self._get_bitten_logo(), "watermark": self._get_watermark()},
            "gradients": {
    ...
    def generate_share_content(self, card: AchievementCard, platform: str, referral_link: str) -> Dict[str, Any]:
        ...
        if card.type == "medal":
            context.update({"medal_name": card.title, "medal_desc": card.description})
        elif card.type == "milestone":
            context.update({"milestone_desc": card.description, "stats": f"{card.value} completed"})
        elif card.type == "streak":
            context.update({"streak_count": card.value})
        elif card.type == "profit":
            context.update({"profit_amount": f"{card.value:,.2f}"})
    ...
    def create_recruitment_card(self, user_data: Dict) -> Dict[str, Any]:
        ...
        card_data = {
            ...
            "visual_style": {"theme": "elite", "accent": "#f39c12", "pattern": "network"},
        }
    ...
    def create_leaderboard_card(self, leaderboard_data: Dict, user_position: Dict) -> Dict[str, Any]:
        ...
        card_data = {
            ...
            "visual_style": {"theme": "competitive", "show_podium": user_position["rank"] <= 3},
        }
    ...
    def _create_discord_embed(self, card: AchievementCard, referral_link: str) -> Dict[str, Any]:
        ...
        embed = {
            ...
            "footer": {"text": "BITTEN Trading Network", "icon_url": "https://example.com/bitten-icon.png"},
            "author": {
        ...
        if card.type == "medal":
            embed["fields"].append({"name": "XP Earned", "value": f"+{card.value.get('xp_reward', 0)}", "inline": True})
            embed["fields"].append({"name": "Tier", "value": card.value.get("tier", "Bronze"), "inline": True})
        elif card.type == "streak":
            embed["fields"].append({"name": "Streak Count", "value": str(card.value), "inline": True})
        elif card.type == "profit":
            embed["fields"].append({"name": "Total Profit", "value": f"${card.value:,.2f}", "inline": True})

        # Add CTA
        embed["fields"].append(
            {"name": "Join BITTEN", "value": f"[Start Trading Now]({referral_link})", "inline": False}
        )
    ...
    def track_share_event(self, user_id: int, card_id: str, platform: str) -> bool:
        ...
        share_event = {"user_id": user_id, "card_id": card_id, "platform": platform, "timestamp": int(time.time())}

        # Award XP for sharing
        share_xp = {"telegram": 10, "twitter": 15, "discord": 10, "whatsapp": 5}
    ...
    def get_share_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        return {
            "total_shares": 0,
            "platforms": {"telegram": 0, "twitter": 0, "discord": 0, "whatsapp": 0},
