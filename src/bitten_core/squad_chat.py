...
class SquadChatSystem:
    ...
    def init_app(self, app: Flask):
        ...
        self.socketio = SocketIO(
            app, cors_allowed_origins="*", async_mode="eventlet", logger=True, engineio_logger=False
        )
    ...
    def _register_socket_events(self):
        ...
        def handle_connect():
            ...
            emit(
                ...
                {"status": "connected", "user_id": user_id, "timestamp": datetime.now().isoformat()},
            )
        ...
        def handle_join_room(data):
            ...
            emit(
                "room_history",
                {"room_id": room_id, "messages": [self._serialize_message(msg) for msg in recent_messages]},
            )
        ...
        def handle_add_reaction(data):
            ...
            if user_id not in message.reactions[emoji]:
                ...
                emit(
                    "reaction_added",
                    {"message_id": message_id, "emoji": emoji, "user_id": user_id, "reactions": message.reactions},
        ...
        def handle_moderate_user(data):
            ...
            success = self._apply_moderation_action(moderator_id, target_user_id, action, duration, reason)
    ...
    def _store_message(self, message: ChatMessage):
        ...
        if self.redis_client:
            try:
                self.redis_client.lpush(f"chat_messages:{room_id}", json.dumps(asdict(message), default=str))
    ...
    def _clear_user_typing(self, user_id: str):
        ...
        for room_id in list(self.typing_users.keys()):
            if user_id in self.typing_users[room_id]:
                ...
                self.socketio.emit("user_stopped_typing", {"user_id": user_id, "room_id": room_id}, room=room_id)
    ...
    def _apply_moderation_action(
        self, moderator_id: str, target_user_id: str, action: str, duration: int, reason: str
    ) -> bool:
        """Apply moderation action"""
        try:
    ...
    def _schedule_cleanup_tasks(self):
        ...
        def cleanup_typing_timeouts():
            ...
            for user_id, user in self.active_users.items():
                if user.is_typing and user.typing_timeout and current_time > user.typing_timeout:
                    self._clear_user_typing(user_id)
    ...
    def register_user(
        self, user_id: str, username: str, callsign: str, role: UserRole, squad_id: str, rank: str
    ) -> bool:
        """Register a new user in the chat system"""
        try:
            user = ChatUser(
                user_id=user_id, username=username, callsign=callsign, role=role, squad_id=squad_id, rank=rank
            )
    ...
    def create_squad_room(
        self, squad_id: str, name: str, description: str, created_by: str, is_secure: bool = False
    ) -> Optional[str]:
    ...
    def send_system_message(self, room_id: str, content: str, metadata: Optional[Dict[str, Any]] = None):
