...
class StealthProtocol:
    ...
    def entry_delay(self, trade_params: Dict) -> float:
        ...
        self._log_action(
            StealthAction(
                ...
                details={"pair": trade_params.get("symbol", "UNKNOWN"), "delay_seconds": delay},
            )
        )
    ...
    def lot_size_jitter(self, original_lot_size: float, pair: str = None) -> float:
        ...
        jitter_min, jitter_max = level_ranges.get(
            self.config.level, (self.config.lot_jitter_min, self.config.lot_jitter_max)
        )
    ...
    def tp_sl_offset(self, tp_price: float, sl_price: float, pair: str = None) -> Tuple[float, float]:
    ...
    def execution_shuffle(self, trade_queue: List[Dict]) -> List[Dict]:
        ...
        for i, trade in enumerate(shuffled_queue):
            if i > 0:  # Not first trade
                ...
                random_delay = self.config.shuffle_delay_min + (secrets.randbelow(1000) / 1000.0 * delay_range)
        ...
        self._log_action(
            StealthAction(
                ...
                level=self.config.level,
                details={"queue_size": len(trade_queue), "shuffle_applied": original_order != shuffled_order},
            )
        )
    ...
    def apply_full_stealth(self, trade_params: Dict) -> Dict:
        ...
        if "volume" in stealth_params:
            stealth_params["volume"] = self.lot_size_jitter(stealth_params["volume"], asset)
        ...
        if "tp" in stealth_params and "sl" in stealth_params:
            stealth_params["tp"], stealth_params["sl"] = self.tp_sl_offset(
                stealth_params["tp"], stealth_params["sl"], asset
            )
    ...
    def get_stealth_stats(self) -> Dict:
        ...
        stats = {
            ...
            "active_trades": {asset: len(trades) for asset, trades in self.active_trades.items()},
        ...
        for action in self.action_log[-10:]:
            stats["recent_actions"].append(
                {"timestamp": action.timestamp.isoformat(), "type": action.action_type, "level": action.level.value}
            )
    ...
    def set_level(self, level: StealthLevel):
        ...
        self._log_action(
            StealthAction(
                ...
                level=level,
                details={"changed_by": "manual", "reason": "user_request"},
            )
        )

    def export_logs(self, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> List[Dict]:
