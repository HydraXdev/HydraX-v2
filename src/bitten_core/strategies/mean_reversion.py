...
class MeanReversionStrategy(StrategyBase):
    ...
    def analyze_setup(self, market_data: List[MarketData], indicators: TechnicalIndicators) -> Optional[TradingSignal]:
        ...
        reversal_signal = self._confirm_reversal_signal(current_data, market_data[-5:], deviation_type, indicators)
    ...
    def _detect_extreme_deviation(
        self, current_data: MarketData, market_data: List[MarketData], indicators: TechnicalIndicators
    ) -> Optional[Tuple[str, float, float, List[str], List[str]]]:
        """
    ...
    def _check_momentum_divergence(
        self, recent_data: List[MarketData], direction: SignalDirection, indicators: TechnicalIndicators
    ) -> bool:
    ...
    def _calculate_mean_reversion_targets(
        self, entry_price: float, direction: SignalDirection, mean_price: float, deviation_magnitude: float, atr: float
    ) -> Tuple[float, float]:
    ...
    def _calculate_mean_reversion_tcs_factors(
        self,
        current_data: MarketData,
        indicators: TechnicalIndicators,
