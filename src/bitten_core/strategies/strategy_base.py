...
class StrategyBase(ABC):
    ...
    def calculate_tcs_score(self, factors: Dict[str, float]) -> int:
        ...
        if "timeframe_alignment" in factors:
            ...
            if alignment > 0.8:
                signal_data.update({"M15_aligned": True, "H1_aligned": True, "H4_aligned": True, "D1_aligned": True})
            elif alignment > 0.6:
    ...
    def calculate_position_size(
        self, account_balance: float, risk_percent: float, stop_loss_pips: int, tier_multiplier: float = 1.0
    ) -> float:
