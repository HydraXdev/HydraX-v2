...
class StrategyBase(ABC):
    ...
    def calculate_tcs_score(self, factors: Dict[str, float]) -> int:
        """Tactical Confidence Score Algorithm - BITTEN's Secret Weapon"""
        base_score = 50
        
        # Volume confirmation (institutional participation)
        if factors.get('volume_ratio', 1.0) > 1.5:
            base_score += 10
        elif factors.get('volume_ratio', 1.0) < 0.8:
            base_score -= 5
        
        # Multi-timeframe alignment (confluence power)
        alignment_score = factors.get('timeframe_alignment', 0.5)
        base_score += int(alignment_score * 15)
        
        # Support/Resistance confluence (critical levels)
        sr_strength = factors.get('sr_strength', 0.5)
        base_score += int(sr_strength * 15)
        
        # Momentum agreement (trend strength)
        momentum_score = factors.get('momentum_strength', 0.5)
        base_score += int(momentum_score * 10)
        
        # Market structure integrity (clean setup)
        structure_score = factors.get('structure_integrity', 0.5)
        base_score += int(structure_score * 10)
        
        # Risk factors (protect the warriors)
        if factors.get('spread_ratio', 1.0) > 2.0:
            base_score -= 5
        if factors.get('news_risk', False):
            base_score -= 5
        if factors.get('session_overlap', False):
            base_score += 5
        
        # Advanced factors (elite edge)
        if factors.get('divergence_present', False):
            base_score += 8
        if factors.get('pattern_completion', False):
            base_score += 7
        if factors.get('institutional_levels', False):
            base_score += 12
