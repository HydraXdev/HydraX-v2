...
from .strategy_base import MarketData, SignalType, TechnicalIndicators, TradingSignal
...
class StrategyOrchestrator:
    ...
    def __init__(self, symbols: List[str]):
        ...
        self.strategy_stats = defaultdict(lambda: {"total": 0, "wins": 0, "losses": 0, "win_rate": 0.0})

    def process_market_update(
        self, symbol: str, market_data: MarketData, indicators: TechnicalIndicators, user_id: str = None
        ...
        logger.info(
            f"{symbol} - Market: {market_structure.trend} "
        ...
        if signal:
            ...
            validation = self.validator.validate_signal(signal, self.recent_signals[symbol], market_conditions)
    ...
    def _try_backup_strategies(
        self, symbol: str, market_data: MarketData, indicators: TechnicalIndicators, excluded_strategy: str
    ) -> Optional[TradingSignal]:
    ...
    def _record_signal(self, symbol: str, signal: TradingSignal) -> None:
        ...
        self.recent_signals[symbol] = [s for s in self.recent_signals[symbol] if s.timestamp > cutoff_time]
