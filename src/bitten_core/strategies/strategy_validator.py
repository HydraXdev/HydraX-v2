...
from .strategy_base import MarketSession, SignalDirection, TradingSignal
...
class StrategyValidator:
    ...
    def validate_signal(
        self, signal: TradingSignal, recent_signals: List[TradingSignal], market_conditions: Dict
    ) -> ValidationResult:
        """
    ...
    def _check_signal_correlation(self, signal: TradingSignal, recent_signals: List[TradingSignal]) -> Optional[str]:
        ...
        recent_same_direction = [
            ...
            if s.timestamp > cutoff_time and s.direction == signal.direction and s.symbol == signal.symbol
        ]
    ...
    def _check_pattern_confluence(self, signal: TradingSignal, market_conditions: Dict) -> float:
        ...
        if "trend_direction" in market_conditions:
            if market_conditions["trend_direction"] == "up" and signal.direction == SignalDirection.BUY:
                confluence_score += 5
            elif market_conditions["trend_direction"] == "down" and signal.direction == SignalDirection.SELL:
                confluence_score += 5
