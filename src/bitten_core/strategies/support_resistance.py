...
class SupportResistanceStrategy(StrategyBase):
    ...
    def _count_level_touches(
        self, market_data: List[MarketData], level_price: float, level_type: str
    ) -> Tuple[int, float]:
    ...
    def _calculate_level_strength(
        self, level_price: float, touches: int, age_hours: float, market_data: List[MarketData], level_type: str
    ) -> float:
    ...
    def _analyze_approach(
        self, market_data: List[MarketData], level_price: float, level_type: str
    ) -> Optional[Tuple[float, float, List[str], List[str]]]:
    ...
    def _detect_rejection(
        ...
        if level_type in ["resistance", "psychological"]:
            ...
            if current_data.high >= level_price and current_price < current_data.high - (
                self.rejection_candle_size * pip_value
            ):
            ...
            if current_data.low <= level_price and current_price > current_data.low + (
                self.rejection_candle_size * pip_value
            ):
    ...
    def _calculate_sr_targets(
        self, entry_price: float, direction: SignalDirection, level_price: float, level_strength: float, atr: float
    ) -> Tuple[float, float]:
