...
class StripePaymentProcessor:
    ...
    async def create_customer(self, user_id: int, email: Optional[str] = None, metadata: Optional[Dict] = None) -> Dict:
        """Create Stripe customer for user"""

        try:
            customer_data = {"metadata": {"bitten_user_id": str(user_id), "platform": "telegram", **(metadata or {})}}
    ...
    async def create_setup_intent(self, customer_id: str) -> Dict:
        ...
        try:
            setup_intent = stripe.SetupIntent.create(
                ...
                usage="off_session",
                metadata={"type": "subscription_payment_method"},
            )

            return {"success": True, "client_secret": setup_intent.client_secret, "setup_intent_id": setup_intent.id}

        except stripe.error.StripeError as e:
            logger.error(f"Stripe error creating setup intent: {e}")
            return {"success": False, "error": str(e)}

    async def create_subscription(self, customer_id: str, tier: str, billing_cycle: str = "monthly") -> Dict:
        ...
        try:
            ...
            if not price_id:
                return {"success": False, "error": f"Invalid tier/billing combination: {tier}/{billing_cycle}"}
    ...
    async def update_subscription(self, subscription_id: str, new_tier: str, new_billing_cycle: str = None) -> Dict:
        ...
        try:
            ...
            # Get new price ID
            billing_cycle = new_billing_cycle or (
                "annual" if "annual" in subscription.items.data[0].price.id else "monthly"
            )
            price_key = f"{new_tier.upper()}_{billing_cycle.upper()}"
            new_price_id = self.price_ids.get(price_key)

            if not new_price_id:
                return {"success": False, "error": f"Invalid tier/billing combination: {new_tier}/{billing_cycle}"}
            ...
            updated_subscription = stripe.Subscription.modify(
                ...
                metadata={"tier": new_tier, "billing_cycle": billing_cycle, "upgraded_at": datetime.now().isoformat()},
            )
        ...
        try:
            if immediate:
                ...
                subscription = stripe.Subscription.modify(
                    subscription_id, cancel_at_period_end=True, metadata={"cancelled_at": datetime.now().isoformat()}
                )
            ...
            return {
                ...
                "effective_date": (
                    datetime.now() if immediate else datetime.fromtimestamp(subscription.current_period_end)
                ),
            }
    ...
    async def reactivate_subscription(self, subscription_id: str) -> Dict:
        ...
        try:
            subscription = stripe.Subscription.modify(
                subscription_id, cancel_at_period_end=False, metadata={"reactivated_at": datetime.now().isoformat()}
            )
    ...
    async def create_checkout_session(
        self, customer_id: str, tier: str, billing_cycle: str, success_url: str, cancel_url: str
    ) -> Dict:
        ...
        try:
            ...
            session = stripe.checkout.Session.create(
                ...
                subscription_data={"trial_period_days": 15, "metadata": {"tier": tier, "billing_cycle": billing_cycle}},
            )
    ...
    def _handle_payment_failed(self, event: Dict) -> Dict:
        ...
        return {
            ...
            "next_payment_attempt": (
                datetime.fromtimestamp(invoice["next_payment_attempt"]) if invoice.get("next_payment_attempt") else None
            ),
        }
