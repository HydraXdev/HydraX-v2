...
class SimpleStripeProcessor:
    ...
    def __init__(self):
        ...
        self.stripe_secret_key = os.getenv(
            "STRIPE_SECRET_KEY",
            "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",
        )
        stripe.api_key = self.stripe_secret_key

        # Monthly prices for each tier
        self.tier_prices = {"NIBBLER": 39, "FANG": 89, "COMMANDER": 189}
        ...
        try:
            ...
            for tier, price_cents in [
                ...
                product = stripe.Product.create(
                    name=f"BITTEN {tier.title()}", description=f"{tier} tier subscription", metadata={"tier": tier}
                )
    ...
    async def create_customer(self, user_id: int, email: Optional[str] = None) -> Dict:
        ...
        try:
            customer = stripe.Customer.create(
                metadata={"bitten_user_id": str(user_id), "platform": "telegram"}, email=email
            )
    ...
    async def create_payment_link(self, customer_id: str, tier: str) -> Dict:
        ...
        try:
            ...
            if not price_id:
                return {"success": False, "error": f"Price not configured for {tier}. Run setup_products first."}
            ...
            return {"success": True, "payment_url": link.url, "payment_link_id": link.id}

        except stripe.error.StripeError as e:
            logger.error(f"Error creating payment link: {e}")
            return {"success": False, "error": str(e)}

    async def create_checkout_session(self, customer_id: str, tier: str, success_url: str, cancel_url: str) -> Dict:
        ...
        try:
            ...
            if not price_id:
                return {"success": False, "error": f"Price not configured for {tier}"}
    ...
    async def update_subscription_tier(self, subscription_id: str, new_tier: str) -> Dict:
        ...
        try:
            ...
            if not new_price_id:
                return {"success": False, "error": f"Price not configured for {new_tier}"}
            ...
            updated = stripe.Subscription.modify(
                ...
                metadata={"tier": new_tier, "upgraded_at": datetime.now().isoformat()},
            )

            return {"success": True, "new_tier": new_tier, "effective_date": datetime.now()}
        ...
        try:
            ...
            if event["type"] == "customer.subscription.created":
                ...
                return {
                    ...
                    "customer_id": invoice["customer"],
                    "next_attempt": (
                        datetime.fromtimestamp(invoice["next_payment_attempt"])
