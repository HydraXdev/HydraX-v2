...
from .database.models import SubscriptionStatus, User, UserSubscription
    ...
    if event == "subscription_created":
        ...
        with get_db_session() as session:
            ...
            user_sub = (
                ...
                .filter(UserSubscription.stripe_customer_id == result["customer_id"])
                .first()
            )

            if user_sub:
                user = session.query(User).filter(User.user_id == user_sub.user_id).first()
        ...
        with get_db_session() as session:
            user_sub = (
                session.query(UserSubscription)
                .filter(UserSubscription.stripe_subscription_id == result["subscription_id"])
                .first()
            )
