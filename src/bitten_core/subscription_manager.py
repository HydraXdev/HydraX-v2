...
class SubscriptionManager:
    ...
    def __init__(self, payment_processor=None, notification_service=None):
        ...
        self.tier_pricing = {
            BillingCycle.MONTHLY: {"PRESS_PASS": 0, "NIBBLER": 39, "FANG": 89, "COMMANDER": 189},  # Free tier
        ...
        self.trial_allowed_tiers = ["PRESS_PASS", "NIBBLER"]

        logger.info("Subscription Manager initialized")

    async def create_subscription(
        self, user_id: int, tier: str, billing_cycle: BillingCycle = BillingCycle.MONTHLY, trial: bool = False
    ) -> CommandResult:
            ...
            try:
                ...
                if not trial:
                    payment_result = await self._process_payment(
                        user_id, amount, f"New {tier} subscription - {billing_cycle.value}"
                    )
                ...
                subscription = UserSubscription(
                    ...
                    auto_renew=True,
                    metadata=json.dumps(
                        {"original_tier": user.tier, "billing_cycle": billing_cycle.value, "is_trial": trial}
                    ),
                )
    ...
    async def renew_subscription(self, user_id: int, auto_renew: bool = True) -> CommandResult:
        ...
        with get_db_session() as session:
            try:
                ...
                # Process payment
                payment_result = await self._process_payment(
                    user_id, amount, f"Renewal - {subscription.plan_name} {billing_cycle.value}"
                )
                ...
                if not payment_result["success"]:
                    ...
                    await self._track_event(
                        SubscriptionEvent.PAYMENT_FAILED, user_id, {"error": payment_result.get("error")}
                    )
                ...
                await self._track_event(
                    ...
                    user_id,
                    {"tier": subscription.plan_name, "amount": amount, "new_period_end": new_end.isoformat()},
                )
...
Your {subscription.plan_name} features continue uninterrupted!"""
    ...
    async def cancel_subscription(
        self, user_id: int, immediate: bool = False, reason: Optional[str] = None
    ) -> CommandResult:
        ...
        with get_db_session() as session:
            try:
                ...
                subscription = (
                    session.query(UserSubscription)
                    .filter(
                        UserSubscription.user_id == user_id, UserSubscription.status == SubscriptionStatus.ACTIVE.value
                    )
                    .first()
                )
                ...
                if immediate:
                    ...
                    if refund_amount > 0:
                        refund_result = await self._process_refund(
                            user_id, refund_amount, f"Immediate cancellation refund - {days_remaining} days"
                        )
...
Thank you for being part of the BITTEN network."""
                    ...
                    subscription.cancelled_at = datetime.now()
                    subscription.metadata = json.dumps(
                        {**json.loads(subscription.metadata or "{}"), "cancellation_reason": reason, "immediate": False}
                    )
...
You can reactivate anytime before expiry using `/upgrade`."""
                ...
                await self._track_event(
                    ...
                    user_id,
                    {"tier": subscription.plan_name, "immediate": immediate, "reason": reason},
                )
            ...
            try:
                ...
                for trial in trials:
                    ...
                    if days_until_end in [2, 1, 0]:
                        await self._track_event(
                            SubscriptionEvent.TRIAL_ENDING, trial.user_id, {"days_remaining": days_until_end}
                        )
        ...
        with get_db_session() as session:
            try:
                ...
                for sub in expired:
                    ...
                    # Track event
                    await self._track_event(SubscriptionEvent.EXPIRED, sub.user_id, {"previous_tier": sub.plan_name})

                    # Send expiry notification
                    if self.notification_service:
                        await self.notification_service.send_subscription_expired(sub.user_id, sub.plan_name)
    ...
    async def retry_failed_payments(self) -> int:
        ...
        with get_db_session() as session:
            try:
                ...
                past_due = (
                    session.query(UserSubscription)
                    .filter(UserSubscription.status == SubscriptionStatus.PAST_DUE.value)
                    .all()
                )
                ...
                for sub in past_due:
                    ...
                    if retry_result.success:
                        ...
                        if days_since_attempt >= max(self.retry_intervals) + self.grace_period_days:
                            # Cancel subscription
                            await self.cancel_subscription(sub.user_id, immediate=True, reason="Payment retry failed")
    ...
    async def get_subscription_status(self, user_id: int) -> Dict:
        ...
        with get_db_session() as session:
            try:
                ...
                if not subscription:
                    return {"status": "never_subscribed", "tier": user.tier, "can_subscribe": True}
