...
class MetricCollector:
    ...
    def _get_recent_error_rates(self) -> Dict[str, float]:
        ...
        try:
            if os.path.exists("data/error_tracking.db"):
                ...
                cursor.execute("SELECT COUNT(*) FROM error_events WHERE timestamp > ?", (hour_ago,))
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM error_events WHERE timestamp > ? AND severity = 'critical'", (day_ago,)
                )
    ...
    def get_metric_trend(self, metric_name: str, minutes: int = 30) -> List[tuple]:
        ...
        with self._lock:
            ...
            return [
                (timestamp, value) for timestamp, value in self.metrics_history[metric_name] if timestamp > cutoff_time
            ]
...
class AlertManager:
    ...
    def _load_default_thresholds(self) -> Dict[str, Dict[str, float]]:
        ...
        return {
            ...
            "webapp_response_time": {"warning": 2.0, "critical": 5.0},
            "errors_per_hour": {"warning": 10.0, "critical": 25.0},
            "critical_errors_per_day": {"warning": 1.0, "critical": 5.0},
    ...
    def _process_alert(self, alert: MonitoringAlert):
        ...
        # Log the alert
        logger.log(logging.CRITICAL if alert.level == AlertLevel.CRITICAL else logging.WARNING, alert.message)
    ...
    def _resolve_alerts_for_metric(self, metric_name: str):
        ...
        with self._lock:
            to_resolve = [key for key in self.active_alerts.keys() if key.startswith(f"{metric_name}_")]
    ...
    def get_alert_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        relevant_alerts = [alert for alert in self.alert_history if alert.timestamp > cutoff_time]
...
class SystemMonitor:
        ...
        while self.running:
            try:
                ...
                if int(time.time()) % 300 == 0:  # Every 5 minutes
                    logger.info(
                        f"System metrics: CPU {system_metrics.get('cpu_percent', 0):.1f}%, "
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        active_critical = len(
            [alert for alert in self.alert_manager.get_active_alerts() if alert.level == AlertLevel.CRITICAL]
        )
