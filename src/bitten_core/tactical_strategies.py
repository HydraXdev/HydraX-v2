...
class TacticalStrategyManager:
    ...
    def filter_signal_for_user(self, user_id: str, signal):
        ...
        try:
            ...
            can_fire, reason, shot_info = self.can_fire_shot(user_id, signal.tcs_score, signal.direction.value)
    ...
    def _get_shot_requirement(
        self, config: TacticalConfig, shot_number: int, state: DailyTacticalState
    ...
    def _check_strategy_specific_requirements(
        self, state: DailyTacticalState, signal_direction: str, config: TacticalConfig
    ) -> Tuple[bool, str]:
    ...
    def _reset_daily_state(self, user_id: str) -> DailyTacticalState:
        ...
        new_state = DailyTacticalState(
            user_id=user_id, selected_strategy=TacticalStrategy.LONE_WOLF, last_reset=datetime.now()  # Default
        )
    ...
    def _load_daily_state(self, user_id: str) -> None:
        ...
        try:
            ...
            self.daily_states[user_id] = DailyTacticalState(
                user_id=user_id, selected_strategy=TacticalStrategy.LONE_WOLF
            )
