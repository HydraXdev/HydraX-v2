...
class TCSPerformanceDatabase:
    ...
    def store_trade_performance(self, performance: TradePerformance):
        ...
        market_condition_json = (
            json.dumps(asdict(performance.market_condition)) if performance.market_condition else None
        )
        ...
        """,
            (
                performance.signal_id,
                performance.ticket,
                performance.pair,
                performance.direction,
                performance.confidence,
                performance.tcs_threshold,
                performance.entry_time,
                performance.entry_price,
                performance.lot_size,
                performance.exit_time,
                performance.exit_price,
                performance.pips_gained,
                performance.profit_usd,
                performance.hold_time_minutes,
                performance.max_drawdown,
                performance.max_profit,
                performance.volatility_at_entry,
                performance.spread_at_entry,
                performance.result,
                performance.exit_type,
                performance.user_id,
                performance.user_tier,
                market_condition_json,
            ),
        )
    ...
    def store_threshold_adjustment(
        self,
        pair: str,
        old_threshold: float,
        new_threshold: float,
...
class AdvancedTCSPerformanceTracker:
    ...
    async def _finalize_trade(self, ticket: int):
        ...
        logger.info(
            f"Finalized trade {ticket}: {trade.result} " f"({trade.pips_gained:.1f} pips, ${trade.profit_usd:.2f})"
        )
    ...
    def get_real_time_stats(self) -> Dict[str, Any]:
        ...
        return {
            "active_trades": {
                ...
                "avg_hold_time": (
                    np.mean([trade.hold_time_minutes for trade in self.active_trades.values()])
    ...
    def get_threshold_analysis(self, pair: str) -> Dict[str, Any]:
        ...
        if not effectiveness:
            return {"optimal_threshold": 75.0, "confidence_level": 0.5, "analysis": "Insufficient data"}
