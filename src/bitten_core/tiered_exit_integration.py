...
from .exit_profiles import MarketData, exit_profile_manager
...
def on_market_tick(ticket: int, bid: float, ask: float, atr: Optional[float] = None):
    ...
    """
    try:
        market = MarketData(bid=bid, ask=ask, spread=ask - bid, atr=atr)
...
def _check_position_timeouts():
    ...
    from .state_store import state_store
    from .timers import clear_timeout_meta, get_timeout_meta
    ...
    try:
        ...
        for ticket, pos in active_positions.items():
            ...
            if elapsed_minutes >= max_hold_min:
                logger.info(
                    f"⏱️ Position {ticket} exceeded max hold time ({max_hold_min} min, elapsed={elapsed_minutes:.1f} min) - closing"
                )

                # Use close_all method from command_bus
                fire_id = getattr(pos, "fire_id", f"timeout_{ticket}")
                seq = getattr(pos, "last_seq", 0) + 1
                command_bus.close_all(ticket=ticket, fire_id=fire_id, seq=seq, reason=f"timeout_{max_hold_min}min")
...
def check_system_health() -> Dict:
    ...
    positions_by_tier = {"TIER_BEGINNER": 0, "TIER_PLUS": 0, "TIER_PRO": 0}
...
def enhanced_create_fire_command(
    mission_id: str, user_id: str, symbol: str, direction: str, entry: float, sl: float, tp: float, lot: float
) -> Dict:
    """
