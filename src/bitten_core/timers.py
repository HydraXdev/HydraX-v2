...
class TimerManager:
    ...
    def set_timer(
        self, ticket: int, fire_id: str, duration_seconds: float, callback: Callable, reason: str = "MAX_HOLD"
    ) -> bool:
        """
        ...
        with self.timer_lock:
            ...
            timer = Timer(ticket=ticket, fire_id=fire_id, expiry_time=expiry_time, callback=callback, reason=reason)
...
class PositionTimer:
    ...
    @staticmethod
    def set_max_hold_timer(ticket: int, fire_id: str, tier: str, callback: Callable) -> bool:
...
def set_timeout_meta(ticket: int, open_ts_utc: str, max_hold_min: int):
    ...
    with sqlite3.connect(DB) as cx:
        cx.execute(
            "INSERT OR REPLACE INTO position_meta(ticket,open_ts_utc,pre_tp1_max_hold_min) VALUES(?,?,?)",
...
def get_timeout_meta(ticket: int):
    ...
    with sqlite3.connect(DB) as cx:
        row = cx.execute(
            "SELECT open_ts_utc, pre_tp1_max_hold_min FROM position_meta WHERE ticket=?", (ticket,)
        ).fetchone()
