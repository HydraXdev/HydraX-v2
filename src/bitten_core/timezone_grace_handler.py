...
class TimezoneGraceHandler:
    ...
    def detect_user_timezone(self, user_id: str, client_timezone: str = None, ip_address: str = None) -> TimezoneInfo:
        """
    ...
    def calculate_grace_period(
        ...
        can_extend = (
            grace_type in [GracePeriodType.HOLIDAY, GracePeriodType.MAINTENANCE]
            or current_streak >= self.config.extended_grace_threshold
        )
    ...
    def use_grace_period(self, user_id: str, grace_status: GracePeriodStatus) -> bool:
        ...
        logger.info(
            f"Grace period activated for user {user_id}: {grace_status.grace_type.value} - {grace_status.reason}"
        )
        return True

    def check_login_window(
        self, user_id: str, timezone_info: TimezoneInfo, last_login: Optional[datetime] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not in_valid_window and last_login:
            ...
            grace_status = self.calculate_grace_period(user_id, current_streak, last_login, timezone_info)
    ...
    def handle_timezone_change(
        self, user_id: str, old_timezone: str, new_timezone: str, reason: str = "user_change"
    ) -> Dict[str, Any]:
        """
        ...
        is_reasonable_change = self._is_reasonable_timezone_change(old_timezone, new_timezone, time_diff_hours, reason)
    ...
    def _determine_grace_period_type(
        self, current_streak: int, special_circumstances: List[str], timezone_info: TimezoneInfo
    ) -> Tuple[GracePeriodType, int, str]:
        ...
        if "maintenance" in special_circumstances:
            return (GracePeriodType.MAINTENANCE, self.config.maintenance_hours, "System maintenance grace period")

        if "holiday" in special_circumstances or self._is_holiday_period():
            return (GracePeriodType.HOLIDAY, self.config.holiday_hours, "Holiday grace period")

        # Check for weekend
        user_tz = pytz.timezone(timezone_info.current_timezone)
        now_user = datetime.now(user_tz)
        if now_user.weekday() >= 5:  # Saturday or Sunday
            return (GracePeriodType.WEEKEND, self.config.weekend_hours, "Weekend grace period")

        # Extended grace for long streaks
        if current_streak >= self.config.extended_grace_threshold:
            return (
                GracePeriodType.EXTENDED,
                self.config.extended_hours,
                f"Extended grace for {current_streak}-day streak",
            )

        # Standard grace period
        return (GracePeriodType.STANDARD, self.config.standard_hours, "Standard grace period")
    ...
    def _calculate_login_windows(self, date, user_tz) -> List[Dict[str, Any]]:
        ...
        return [{"type": "main", "start": main_start, "end": main_end, "description": "Main login window"}]
    ...
    def _is_reasonable_timezone_change(self, old_tz: str, new_tz: str, time_diff_hours: float, reason: str) -> bool:
    ...
    def _generate_login_recommendations(
        self, current_time: datetime, optimal_start: datetime, optimal_end: datetime
    ) -> List[str]:
...
if __name__ == "__main__":
    ...
    timezone_info = handler.detect_user_timezone(
        user_id=user_id, client_timezone="America/New_York", ip_address="192.168.1.1"
    )
    ...
    change_result = handler.handle_timezone_change(
        user_id=user_id, old_timezone="America/New_York", new_timezone="Europe/London", reason="business_trip"
    )
