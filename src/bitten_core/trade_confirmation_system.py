...
class TradeConfirmationSystem:
    ...
    def __init__(self, telegram_sender: Optional[Callable] = None):
        ...
        # Processing stats
        self.stats = {"total_sent": 0, "failed_sends": 0, "last_send_time": None, "queue_size": 0}
    ...
    def _initialize_templates(self) -> Dict[ConfirmationType, Dict]:
        ...
        return {
            ...
            ConfirmationType.MARGIN_CALL: {"emoji": "⚠️", "title": "MARGIN CALL", "color": "🔴", "sound": "alarm"},
    ...
    async def _process_confirmations(self):
        ...
        while self.processing:
            try:
                ...
                confirmation = await asyncio.wait_for(self.confirmation_queue.get(), timeout=1.0)
    ...
    async def send_parachute_exit_confirmation(self, user_id: int, parachute_data: Dict):
        ...
        confirmation = TradeConfirmation(
            ...
            metadata={"xp_penalty_waived": True, **parachute_data.get("metadata", {})},
        )
        ...
        try:
            ...
            if self.telegram_sender:
                ...
                if success:
                    ...
                    logger.info(
                        f"Sent {confirmation.confirmation_type.value} confirmation to user {confirmation.user_id}"
                    )
                else:
                    self.stats["failed_sends"] += 1
    ...
    def get_confirmation_history(self, user_id: Optional[int] = None, limit: int = 50) -> List[TradeConfirmation]:
