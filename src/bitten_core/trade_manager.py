...
from .risk_management import RiskProfile, TradeManagementFeature, TradeManagementPlan
...
class TradeManager:
    ...
    async def _send_trade_opened_confirmation(self, trade: ActiveTrade):
        ...
        await self.confirmation_system.send_trade_opened_confirmation(trade.user_id, trade_data)
    ...
    async def _send_trade_closed_confirmation(self, trade: ActiveTrade):
        ...
        if trade.exit_type == ExitType.STOP_LOSS:
            await self.confirmation_system.send_stop_loss_hit_confirmation(trade.user_id, trade_data)
        elif trade.exit_type == ExitType.TAKE_PROFIT:
            await self.confirmation_system.send_take_profit_hit_confirmation(trade.user_id, trade_data)
        elif trade.exit_type == ExitType.EMERGENCY:
            await self.confirmation_system.send_emergency_close_confirmation(trade.user_id, trade_data)
        elif trade.exit_type == ExitType.PARACHUTE:
            await self.confirmation_system.send_parachute_exit_confirmation(trade.user_id, trade_data)
        else:
            await self.confirmation_system.send_trade_closed_confirmation(trade.user_id, trade_data)
    ...
    def _send_exit_to_xp_system(self, exit_data: Dict):
        ...
        logger.info(
            f"Sending exit data to XP system: {exit_data['trade_id']} - "
    ...
    async def _execute_plan(
        self, trade: ActiveTrade, plan: TradeManagementPlan, current_price: float, pip_value: float
    ):
        """Execute a specific management plan"""

        # Breakeven
        if (
            plan.feature == TradeManagementFeature.BREAKEVEN
            and not trade.is_breakeven
            and trade.current_pnl_pips >= plan.breakeven_trigger_pips
        ):
        ...
        # Breakeven Plus
            ...
            plan.feature == TradeManagementFeature.BREAKEVEN_PLUS
            and not trade.is_breakeven
            and trade.current_pnl_pips >= plan.breakeven_trigger_pips
        ):
        ...
        # Trailing Stop
            ...
            plan.feature == TradeManagementFeature.TRAILING_STOP and trade.current_pnl_pips >= plan.trailing_start_pips
        ):
        ...
        # Partial Close
            ...
            plan.feature == TradeManagementFeature.PARTIAL_CLOSE
            and trade.partial_closed < plan.partial_close_percent
            and trade.current_pnl_pips >= plan.partial_close_at_pips
        ):
        ...
        # Runner Mode
            ...
            plan.feature == TradeManagementFeature.RUNNER_MODE and trade.current_pnl_pips >= plan.partial_close_at_pips
        ):
        ...
        # Zero Risk Runner
            ...
            plan.feature == TradeManagementFeature.ZERO_RISK_RUNNER
            and trade.current_pnl_pips >= plan.partial_close_at_pips
        ):
    ...
    async def _send_breakeven_confirmation(self, trade: ActiveTrade, old_sl: float, plus_pips: float):
        ...
        await self.confirmation_system.send_breakeven_moved_confirmation(trade.user_id, breakeven_data)

    async def _trail_stop(self, trade: ActiveTrade, plan: TradeManagementPlan, current_price: float, pip_value: float):
        """Trail the stop loss"""
        old_sl = trade.current_sl

        if trade.direction == "BUY":
            trail_price = current_price - (plan.trailing_distance_pips * pip_value)
            if trail_price > trade.current_sl and (
                not trade.last_trail_price
                or current_price >= trade.last_trail_price + plan.trailing_step_pips * pip_value
            ):
                ...
                if success:
                    ...
                    if self.confirmation_system:
                        await self._send_trailing_stop_confirmation(
                            trade, old_sl, plan.trailing_distance_pips, current_price
                        )
        else:
            trail_price = current_price + (plan.trailing_distance_pips * pip_value)
            if trail_price < trade.current_sl and (
    ...
    async def _send_trailing_stop_confirmation(
        self, trade: ActiveTrade, old_sl: float, trail_distance: float, current_price: float
    ):
        ...
        trail_data = {
            ...
            "new_stop_loss": trade.current_sl,
            "current_pnl_pips": trade.current_pnl_pips,
            "metadata": {"old_stop_loss": old_sl, "trail_distance": trail_distance, "current_price": current_price},
        }

        await self.confirmation_system.send_trailing_stop_moved_confirmation(trade.user_id, trail_data)
    ...
    async def _send_partial_close_confirmation(
        self, trade: ActiveTrade, closed_volume: float, closed_percent: float, original_volume: float
    ):
        """Send partial close confirmation"""
        ...
        await self.confirmation_system.send_partial_close_confirmation(trade.user_id, partial_data)
    ...
    async def _modify_stop_loss(self, trade: ActiveTrade, new_sl: float) -> bool:
        ...
        instruction = {"action": "modify", "ticket": trade.ticket, "sl": new_sl, "tp": trade.current_tp}
    ...
    async def _close_partial_position(self, trade: ActiveTrade, volume: float) -> bool:
        ...
        instruction = {"action": "close_partial", "ticket": trade.ticket, "volume": volume}
    ...
    async def activate_parachute_exit(self, trade_id: str) -> bool:
        ...
        if result:
            ...
            self._notify(
                f"🪂 PARACHUTE EXIT on {trade.symbol}! Position closed at {trade.current_pnl_pips:.1f} pips (NO XP PENALTY)"
            )
    ...
    async def execute_profit_runner_exit(self, trade_id: str, exit_percent: float) -> bool:
        ...
        if result:
            self._notify(
                f"🏃 PROFIT RUNNER EXIT! Secured {exit_percent}% of {trade.symbol} at {trade.current_pnl_pips:.1f} pips!"
            )
    ...
    async def approve_emergency_exit(self, trade_id: str, approval_code: str) -> bool:
        ...
        if approval_code.startswith("EMRG-"):
            ...
            if result:
                ...
                self._notify(
                    f"🚨 EMERGENCY EXIT APPROVED on {trade.symbol}! Position closed at {trade.exit_pnl_pips:.1f} pips"
                )
                self.remove_trade(trade_id)
    ...
    async def _close_full_position(self, trade: ActiveTrade) -> bool:
        ...
        instruction = {"action": "close", "ticket": trade.ticket, "volume": trade.volume}
    ...
    async def check_exit_conditions(self, trade: ActiveTrade, current_price: float):
        ...
        if trade.current_tp > 0:
            if trade.direction == "BUY" and current_price >= trade.current_tp:
                ...
                await self._close_full_position(trade)
                self._notify(
                    f"🎯 TAKE PROFIT HIT on {trade.symbol} at {trade.exit_pnl_pips:.1f} pips profit!",
                    sound_type="cash_register",
                    event_type="tp_hit",
                )
                self.remove_trade(trade.trade_id)
...
async def example_usage():
    ...
    profile = RiskProfile(user_id=123456, current_xp=20000, tier_level=TierLevel.COMMANDER)  # Unlocks Leroy Jenkins
