...
class TradeWriter:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                self._update_daily_summary(
                    row[1], close_time.date().isoformat(), outcome, profit_loss, pip_profit, row[5]
                )
    ...
    def get_trade_history(
        self, user_id: int, limit: int = 100, symbol: str = None, start_date: str = None, end_date: str = None
    ) -> List[TradeRecord]:
    ...
    def _update_daily_summary(
        self, user_id: int, date: str, outcome: TradeOutcome, profit_loss: float, pip_profit: float, volume: float
    ):
...
• XP Earned: {stats['total_xp']} points"""
            ...
            if stats["symbol_breakdown"]:
                ...
                for symbol_data in stats["symbol_breakdown"][:3]:
                    summary += (
                        f"• {symbol_data['symbol']}: {symbol_data['trades']} trades, ${symbol_data['profit']:.2f}\n"
                    )
            ...
            if recent_trades:
                ...
                for trade in recent_trades[:3]:
                    status_emoji = (
                        ...
                        else "❌" if trade.outcome == TradeOutcome.LOSS else "⏳"
                    )
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if old_trades:
                    ...
                    with open(backup_path, "w") as f:
                        json.dump(
                            [dict(zip([col[0] for col in cursor.description], row)) for row in old_trades],
                            f,
                            indent=2,
                            default=str,
                        )
