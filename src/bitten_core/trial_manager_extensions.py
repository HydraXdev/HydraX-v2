...
async def _process_subscription(self, user_id: int, tier: str) -> CommandResult:
    ...
    with get_db_session() as session:
        ...
        # Create payment link
        link_result = await stripe.create_payment_link(subscription.stripe_customer_id, tier)
...
def handle_subscription_callback_updated(self, user_id: int, callback_data: str):
    ...
    if callback_data == "trial_subscribe":
        ...
        return CommandResult(True, "✅ We'll remind you tomorrow!", data={"callback_answer": True})

    elif callback_data == "trial_back":
        return CommandResult(True, "Going back...", data={"delete_message": True})
