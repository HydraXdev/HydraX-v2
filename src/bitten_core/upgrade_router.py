...
class TierUpgradeRouter:
    ...
    def __init__(self, payment_processor=None):
        ...
        self.tier_pricing = {"NIBBLER": 39, "FANG": 89, "COMMANDER": 189}
                    ...
                    keyboard.append(
                        [
                            InlineKeyboardButton(
                                f"⬆️ Upgrade to {next_tier} (+${price_diff}/mo)", callback_data=f"upgrade_{next_tier}"
                            )
                        ]
                    )
...
Type `/fire` to experience your upgraded firepower!"""
    ...
    def reactivate_subscription(self, user_id: int) -> CommandResult:
        ...
        with get_db_session() as session:
            ...
            payment_result = self._process_payment(user_id, self.tier_pricing[user.tier], f"Reactivate {user.tier}")
