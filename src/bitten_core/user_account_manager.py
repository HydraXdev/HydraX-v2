...
class UserAccountManager:
    ...
    def setup_logging(self):
        ...
        log_formatter = logging.Formatter("%(asctime)s - [ACCOUNT_MGR] %(levelname)s - %(message)s")
        ...
        try:
            ...
            with sqlite3.connect(self.engagement_db_path) as conn:
                ...
                if existing_record:
                    ...
                    self.logger.info(
                        f"✅ Created new account record for user {telegram_user_id}, account {account_number}"
                    )
    ...
    def store_account_json(self, telegram_user_id: str, mt5_account_info: Dict[str, Any]):
        ...
        try:
            ...
            if telegram_user_id not in user_accounts:
                user_accounts[telegram_user_id] = {"accounts": [], "created_at": datetime.now().isoformat()}
    ...
    def update_account_balance(
        self, telegram_user_id: str, account_number: str, new_balance: float, new_equity: float
    ) -> bool:
        ...
        try:
            with sqlite3.connect(self.engagement_db_path) as conn:
                ...
                cursor.execute(
                    ...
                    WHERE telegram_user_id = ? AND mt5_account_number = ?
                """,
                    (new_balance, new_equity, datetime.now().isoformat(), telegram_user_id, str(account_number)),
                )
                ...
                if cursor.rowcount > 0:
                    ...
                    self.logger.warning(
                        f"No account found to update for user {telegram_user_id}, account {account_number}"
                    )
                    return False
