...
*Confirmation expires in 5 minutes*"""
            ...
            return {"success": False, "message": f"Failed to prepare emergency stop: {str(e)}"}
    ...
    def execute_emergency_stop(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not pending:
                return {"success": False, "message": "❌ No pending emergency stop found. Please request again."}

            # Check if confirmation expired
            if datetime.now() > pending["expires_at"]:
                del self.pending_confirmations[user_id]
                return {"success": False, "message": "❌ Confirmation expired. Please request emergency stop again."}
...
🔓 Use `/emergency_recover` when ready"""
            ...
            return {"success": True, "message": success_msg, "emergency_state": emergency_state, "results": results}
    ...
    def cancel_emergency_stop(self, user_id: int) -> Dict[str, Any]:
        ...
        if user_id in self.pending_confirmations:
            del self.pending_confirmations[user_id]
            return {"success": True, "message": "✅ Emergency stop request cancelled"}
        else:
            return {"success": False, "message": "❌ No pending emergency stop found"}
    ...
    def get_user_emergency_status(self, user_id: int) -> Dict[str, Any]:
        ...
        if not state or not state.is_active:
            return {"is_active": False, "message": "🟢 **NORMAL OPERATIONS**\nNo emergency stop active"}
...
**Recovery:** {'Automatic' if state.recovery_time else 'Manual'}"""
        ...
        return {"is_active": True, "level": state.level, "message": status_msg, "state": state}
    ...
    def recover_from_emergency(self, user_id: int, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if not state or not state.is_active:
                return {"success": False, "message": "❌ No active emergency stop found"}
...
🛡️ **Stay safe out there, soldier!**"""
            ...
            return {"success": True, "message": recovery_msg, "recovery_time": datetime.now()}
    ...
    def _execute_user_emergency_stop(self, user_id: int, level: UserEmergencyLevel, reason: str) -> Dict[str, Any]:
        ...
        results = {"actions_taken": [], "closed_positions": [], "blocked_systems": []}
            ...
            try:
                ...
                for user_id_str, state_data in data.items():
                    ...
                    state = UserEmergencyState(
                        ...
                        recovery_time=(
                            datetime.fromisoformat(state_data["recovery_time"])
