...
def require_auth(min_tier: Optional[str] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if min_tier:
                tier_hierarchy = {"PRESS_PASS": 0, "NIBBLER": 1, "FANG": 2, "COMMANDER": 3, "": 4}
...
class SessionManager:
    ...
    async def refresh_session(session_token: str) -> str:
        ...
        new_token = await user_manager._create_user_session(session["user_id"], session["telegram_id"], session["data"])
