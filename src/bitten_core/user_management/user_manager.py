...
from ...database.models import SubscriptionPlan, SubscriptionStatus, TierLevel, User, UserProfile, UserSubscription
...
class UserManager:
    ...
    async def create_user_from_onboarding(self, onboarding_session: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            async with get_async_db() as conn:
                async with conn.transaction():
                    ...
                    existing_user = await conn.fetchrow("SELECT user_id FROM users WHERE telegram_id = $1", telegram_id)
                    ...
                    if is_press_pass:
                        press_pass_result = await self._activate_press_pass(
                            user_id, telegram_id, first_name, onboarding_session
                        )
                    ...
                    await self._create_initial_subscription(user_id, initial_tier, is_press_pass)

                    # Create authentication session
                    session_token = await self._create_user_session(
                        user_id, telegram_id, {"tier": initial_tier, "is_press_pass": is_press_pass}
                    )
            ...
            return {"success": False, "error": "database_error", "message": "User creation failed due to data conflict"}
        except Exception as e:
            logger.error(f"Error creating user from onboarding: {e}")
            return {"success": False, "error": str(e), "message": "Failed to create user account"}

    async def authenticate_user(self, telegram_id: int, api_key: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            async with get_async_db() as conn:
                ...
                if not user:
                    return {"authenticated": False, "error": "user_not_found", "message": "User not registered"}

                # Check if user is banned
                if user["is_banned"]:
                    return {"authenticated": False, "error": "user_banned", "message": "Account has been suspended"}

                # Verify API key if provided
                if api_key and user["api_key"] != api_key:
                    return {"authenticated": False, "error": "invalid_api_key", "message": "Invalid API key"}
                ...
                session_token = await self._create_user_session(user["user_id"], telegram_id, session_data)
            ...
            return {"authenticated": False, "error": str(e), "message": "Authentication failed"}

    async def upgrade_user_tier(
        self, user_id: int, new_tier: str, payment_method: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if new_tier not in [t.value for t in TierLevel]:
                return {"success": False, "error": "invalid_tier", "message": f"Invalid tier: {new_tier}"}
            ...
            async with get_async_db() as conn:
                async with conn.transaction():
                    ...
                    if current_tier == TierLevel.PRESS_PASS.value:
                        press_pass_result = await self.press_pass_manager.upgrade_to_paid_tier(str(user_id), new_tier)
            ...
            return {"success": False, "error": str(e), "message": "Tier upgrade failed"}
        ...
        try:
            async with get_async_db() as conn:
                ...
                if not update_fields:
                    return {"success": False, "error": "no_valid_fields", "message": "No valid fields to update"}
            ...
            return {"success": False, "error": str(e), "message": "Profile update failed"}
    ...
    async def _activate_press_pass(
        self, user_id: int, telegram_id: int, first_name: str, onboarding_session: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            demo_result = await self.press_pass_manager.provision_demo_account(str(user_id), first_name)
            ...
            return {"success": True, "demo_account": demo_result["account"], "expires_at": press_pass_expiry}

        except Exception as e:
            logger.error(f"Error activating Press Pass: {e}")
            return {"success": False, "error": str(e)}

    async def _create_initial_subscription(self, user_id: int, tier: str, is_press_pass: bool) -> None:
        """Create initial subscription record"""
        try:
            async with get_async_db() as conn:
                # Get plan for tier
    ...
    async def _create_user_session(self, user_id: int, telegram_id: int, session_data: Dict[str, Any]) -> str:
            ...
            async with get_async_db() as conn:
                ...
                if user and user["subscription_expires_at"]:
                    return await self.press_pass_manager.check_press_pass_expiry(
                        str(user_id), user["subscription_expires_at"]
                    )
    ...
    async def _log_tier_upgrade(self, user_id: int, old_tier: str, new_tier: str) -> None:
