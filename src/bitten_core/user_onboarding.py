    ...
    def onboard_new_user(self, telegram_id: int, tier: str, broker_config: Dict) -> Dict:
        ...
        try:
            ...
            profile_result = self.profile_manager.create_user_profile(
                telegram_id=telegram_id, tier=tier, broker=broker_config.get("broker", "MT5")
            )
        ...
        try:
            ...
            for field in required_fields:
                if not broker_config.get(field):
                    return {"success": False, "error": f"Missing {field} in broker configuration"}

            # TODO: Add actual MT5 connection test
            return {"success": True, "message": "Configuration validated (connection test pending)"}
    ...
    def create_demo_users_for_testing(self) -> Dict:
        ...
        try:
            ...
            for user_data in demo_users:
                result = self.onboard_new_user(user_data["telegram_id"], user_data["tier"], user_data["broker_config"])
                results.append(result)

            return {"success": True, "demo_users_created": len(results), "results": results}
