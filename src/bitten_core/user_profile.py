...
class UserProfileManager:
    ...
    MEDALS = [
        ...
        Medal(
            "legendary_sniper",
            "Legendary Sniper",
            "Complete 500 successful trades",
            "⚡",
            1000,
            "trades",
            500,
    ...
    def create_user_profile(self, user_id: int) -> UserStats:
        ...
        stats = UserStats(user_id=user_id, joined_at=int(time.time()), last_active=int(time.time()))
    ...
    def record_trade_execution(self, user_id: int, signal_id: str, position_id: str):
        ...
        self.award_xp(user_id, self.XP_REWARDS["trade_executed"], f"Executed trade {position_id}")
    ...
    def record_trade_result(self, user_id: int, position_id: str, profit: float, is_success: bool):
        """Record trade result"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        if is_success:
            ...
            # Award success XP
            self.award_xp(user_id, self.XP_REWARDS["trade_success"], f"Successful trade: +${profit:.2f}")
        else:
            cursor.execute(
                """
                UPDATE user_stats
            ...
            self.award_xp(user_id, self.XP_REWARDS["trade_failed"], "Trade completed")
    ...
    def register_recruit(self, recruiter_id: int, recruit_id: int, username: str) -> bool:
        ...
        try:
            ...
            self.award_xp(recruiter_id, self.XP_REWARDS["recruit_joined"], f"Recruited {username}")
    ...
    def get_recruiting_stats(self, user_id: int) -> Dict:
        ...
        recent_recruits = [
            {"username": row[0], "joined_at": row[1], "trades_completed": row[2]} for row in cursor.fetchall()
        ]
    ...
    def _check_medal_progress(self, user_id: int):
        ...
        for medal in self.MEDALS:
            ...
            if earned:
                ...
                # Award medal XP
                self.award_xp(user_id, medal.xp_reward, f"Medal earned: {medal.name}")
    ...
    def _update_recruiter_stats(self, recruit_id: int):
        ...
        if new_trades == 1:
            # First trade bonus
            self.award_xp(recruiter_id, self.XP_REWARDS["recruit_first_trade"], "Recruit completed first trade")
        elif new_trades % 10 == 0:
            # Milestone bonus
            self.award_xp(recruiter_id, self.XP_REWARDS["recruit_milestone"], f"Recruit reached {new_trades} trades")
    ...
    def _get_next_recruitment_milestone(self, current_recruits: int) -> Dict:
        ...
        return {"target": 1000, "remaining": 1000 - current_recruits, "xp_reward": 10000, "medal": "Mythical"}
