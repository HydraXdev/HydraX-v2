...
class CharacterEventDispatcher:
    ...
    def _load_character_modules(self):
        ...
        try:
            ...
            from .athena_personality import (
                get_execution_failure,
                get_execution_success,
                get_fire_confirmation,
        ...
        try:
            # BIT - System Core
            from .bit_personality import bit_react_trade, get_bit_response

            self.characters["BIT"] = {"system_status": get_bit_response, "trade_reaction": bit_react_trade}
        except ImportError:
            pass
        ...
        try:
            ...
            from .nexus_personality import (
                get_execution_celebration,
                get_recruitment_message,
                get_take_profit_celebration,
            )
        ...
        try:
            ...
            from .doc_personality import (
                get_execution_failure_protection,
                get_reassurance,
                get_risk_management_warning,
                get_stop_loss_protection,
            )
    ...
    def _generate_character_response(
        self, character: str, event_type: TradeEventType, event_data: Dict, user_context: Dict
    ) -> str:
        ...
        if event_type == TradeEventType.MISSION_BRIEFING:
            if "ATHENA" in self.characters and "mission_briefing" in self.characters["ATHENA"]:
                return self.characters["ATHENA"]["mission_briefing"](
                    event_data, user_context.get("tier", "NIBBLER"), event_data.get("mission_id", "UNKNOWN")
                )
            ...
            if "ATHENA" in self.characters and "fire_confirmation" in self.characters["ATHENA"]:
                return self.characters["ATHENA"]["fire_confirmation"](event_data, datetime.now().isoformat())
    ...
    def _generate_stealth_response(self, event_type: TradeEventType, event_data: Dict, user_context: Dict) -> str:
        ...
        stealth_responses = {
            TradeEventType.PRECISION_TRADE: ["Target acquired.", "Precision required.", "Silent approach optimal."],
            TradeEventType.EXECUTION_SUCCESS: ["Confirmed.", "Precision achieved.", "..."],
        }
...
def get_mission_briefing_response(signal_data: Dict, user_tier: str, mission_id: str) -> Dict:
    ...
    return character_dispatcher.route_event(
        TradeEventType.MISSION_BRIEFING, signal_data, {"tier": user_tier, "mission_id": mission_id}
    )
...
def get_trade_execution_response(signal_data: Dict, execution_result: Dict, user_context: Dict = None) -> Dict:
    ...
    event_type = (
        TradeEventType.EXECUTION_SUCCESS if execution_result.get("success") else TradeEventType.EXECUTION_FAILURE
    )
