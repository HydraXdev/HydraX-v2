...
class ElevenLabsVoiceDriver:
    ...
    def check_usage_limits(self, text_length: int) -> bool:
        ...
        if self.usage_data["characters_used"] + text_length > self.monthly_limit:
            logger.warning(
                f"Monthly character limit reached: {self.usage_data['characters_used']}/{self.monthly_limit}"
            )
            return False
    ...
    async def generate_voice(self, text: str, voice_id: str, voice_settings: Dict = None) -> Optional[bytes]:
        ...
        if voice_settings is None:
            voice_settings = {"stability": 0.7, "similarity_boost": 0.8, "style": 0.5, "use_speaker_boost": True}
        ...
        data = {"text": text, "model_id": "eleven_monolingual_v1", "voice_settings": voice_settings}
    ...
    def clean_text_for_voice(self, text: str) -> str:
        ...
        if lines and any(
            keyword in lines[0] for keyword in ["DRILL_SERGEANT", "DOC_AEGIS", "RECRUITER", "OVERWATCH", "STEALTH"]
        ):
            clean_text = "\n".join(lines[1:])
    ...
    def get_usage_stats(self) -> Dict:
        ...
        return {
            ...
            "cache_hit_rate": (
                self.usage_data["cache_hits"] / max(1, self.usage_data["cache_hits"] + self.usage_data["cache_misses"])
            )
            * 100,
