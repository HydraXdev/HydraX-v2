...
from bitten_core.persona_system import EmotionalState, MarketSentiment, PersonaOrchestrator, PersonaType
...
class UnifiedPersonalityOrchestrator:
    ...
    async def process_user_interaction(
        ...
        responses = await self._generate_unified_responses(message_text, unified_context, user_action)
    ...
    async def _generate_unified_responses(self, message_text: str, context: Dict, user_action: str) -> Dict:
        """Generate responses from all personality systems"""

        responses = {}

        # 1. Adaptive personality response
        adaptive_response = self.adaptive_engine.format_personality_message(context["user_id"], message_text)
        ...
        if context.get("primary_personality"):
            persona_responses = self.core_personas.get_response(
                context["user_id"], user_action or "general_interaction", context
            )
        ...
        if self.intel_personalities and context.get("intel_role"):
            intel_response = self.intel_personalities.get_response(
                context["intel_role"], user_action or "general_interaction", context
            )
    ...
    async def _combine_responses(self, responses: Dict, context: Dict) -> UnifiedResponse:
        ...
        combined_message = self._weave_personality_layers(adaptive_msg, persona_elements, intel_enhancement, context)
        ...
        if voice_config and context.get("voice_enabled"):
            try:
                voice_file = await self.voice_driver.generate_personality_voice(combined_message, voice_config)
    ...
    def _weave_personality_layers(
        self, adaptive_msg: str, persona_elements: List[str], intel_enhancement: str, context: Dict
    ) -> str:
        ...
        if intel_enhancement and context.get("user_action") in ["trade_analysis", "risk_assessment", "strategy_review"]:
    ...
    def _add_norman_story_context(self, context: Dict) -> Optional[str]:
        ...
        if random.random() < 0.1:
            personality_config = self.personality_mapping.get(context["primary_personality"], {})
    ...
    def _analyze_emotional_state(self, user_id: str, user_action: str, market_context: Dict) -> str:
    ...
    def _check_evolution_triggers(self, user_id: str, context: Dict) -> Optional[str]:
        ...
        if current_tier != profile_tier:
            result = self.adaptive_engine.evolve_personality(user_id, "tier_advancement", {"new_tier": current_tier})
            if result:
    ...
    def _get_visual_cue(self, context: Dict) -> Optional[str]:
        ...
        visual_cues = {
            ...
            "DOC_AEGIS": {"focused": "health_monitor_stable", "stressed": "medical_alert", "defeated": "recovery_mode"},
            "RECRUITER": {"focused": "network_pulse", "euphoric": "celebration_burst", "learning": "knowledge_flow"},
            "OVERWATCH": {
                "focused": "tactical_overview",
                "stressed": "threat_assessment",
                "euphoric": "market_dominance",
            },
            "STEALTH": {"focused": "minimal_indicator", "stressed": "shadow_mode", "euphoric": "subtle_glow"},
        }
    ...
    def _get_audio_cue(self, context: Dict) -> Optional[str]:
        ...
        audio_cues = {
            "DRILL_SERGEANT": {"focused": "sharp_beep", "stressed": "alarm_chirp", "euphoric": "victory_fanfare"},
            "DOC_AEGIS": {"focused": "steady_hum", "stressed": "medical_beep", "defeated": "comforting_tone"},
            "RECRUITER": {"focused": "connection_chime", "euphoric": "celebration_sound", "learning": "knowledge_ping"},
            "OVERWATCH": {"focused": "radar_ping", "stressed": "alert_tone", "euphoric": "success_notification"},
            "STEALTH": {"focused": "soft_purr", "stressed": "warning_chirp", "euphoric": "content_purr"},
        }
    ...
    async def get_personality_stats(self, user_id: str) -> Dict:
        ...
        unified_stats = {
            "primary_personality": primary_personality,
            "deep_persona": (
                personality_config.get("core_persona", "").value if personality_config.get("core_persona") else None
            ),
            "intel_role": personality_config.get("intel_role"),
