...
class VolatilityIntegration:
    ...
    async def check_volatility_requirements(
        self, symbol: str, tcs: float, tier: str, fire_mode: FireMode
    ) -> Dict[str, Any]:
        """
        ...
        should_block, reason = self.volatility_manager.should_block_trade(level, tcs, tier)

        # Get base TCS for tier
        base_tcs = {"NIBBLER": 72, "FANG": 85, "COMMANDER": 91, "": 91}.get(tier, 72)
        ...
        if level != VolatilityLevel.NORMAL:
            data = await self._fetch_volatility_data(symbol)
            result["mission_brief"] = self.volatility_manager.get_mission_brief(level, data, symbol, base_tcs)
    ...
    def store_pending_confirmation(self, user_id: int, trade_details: Dict):
        ...
        self._pending_confirmations[user_id] = {"details": trade_details, "timestamp": datetime.utcnow()}
    ...
    async def process_volatility_confirmation(self, user_id: int, response: str) -> Dict[str, Any]:
        """
        ...
        if not pending:
            return {"confirmed": False, "message": "No pending trade awaiting confirmation."}
        ...
        if response.upper() == "CONFIRM":
            ...
            # User aborted
            self.clear_pending_confirmation(user_id)
            return {"confirmed": False, "message": "Trade aborted. Standing down."}
        else:
            return {"confirmed": False, "message": 'Invalid response. Type "CONFIRM" or "ABORT".'}

    def adjust_stop_loss(self, base_stop_pips: float, symbol: str) -> Tuple[float, float]:
        """
        ...
        adjusted_stop = self.volatility_manager.calculate_adjusted_stop(base_stop_pips, level)
    ...
    def get_volatility_status(self) -> Dict[str, Any]:
        ...
        status = {"timestamp": datetime.utcnow().isoformat(), "symbols": {}}
...
async def check_trade_volatility(symbol: str, tcs: float, tier: str, fire_mode: FireMode) -> Dict[str, Any]:
    """Quick check for trade volatility requirements"""
    integration = get_volatility_integration()
    return await integration.check_volatility_requirements(symbol, tcs, tier, fire_mode)
