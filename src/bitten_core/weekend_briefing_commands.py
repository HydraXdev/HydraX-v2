...
async def weekend_brief_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Generate briefing
        briefing = briefing_system.get_weekend_warning_message(user_profile.tier_level, len(positions))

        # Add stats if available
        if hasattr(user_profile, "weekend_stats"):
            briefing += briefing_system.get_weekend_stats_summary(user_profile.weekend_stats)
        ...
        await update.message.reply_text("❌ Error generating briefing. Try again later.")
...
async def force_weekend_briefing_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.message.reply_text("🎖️ Initiating weekend safety briefing for all operators...")
        ...
        await update.message.reply_text("✅ Weekend safety briefing complete. Check logs for details.")

    except Exception as e:
        logger.error(f"Error in force_weekend_briefing: {e}")
        await update.message.reply_text(f"❌ Briefing failed: {str(e)}")
...
async def weekend_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Error retrieving stats. Try again later.")
