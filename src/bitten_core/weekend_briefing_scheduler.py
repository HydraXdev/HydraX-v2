...
class WeekendBriefingScheduler:
        ...
        try:
            ...
            for user in active_users:
                try:
                    ...
                    positions = await self.telegram_router.mt5_bridge.get_open_positions(user["user_id"])

                    if positions:
                        with_positions += 1

                    # Generate appropriate briefing
                    briefing_message = self.briefing_system.get_weekend_warning_message(user["tier"], len(positions))

                    # Add user's weekend performance stats
                    if user.get("weekend_stats"):
                        briefing_message += self.briefing_system.get_weekend_stats_summary(user["weekend_stats"])
                    ...
                    await self.telegram_router.send_message(
                        chat_id=user["chat_id"], text=briefing_message, parse_mode="Markdown"
                    )
    ...
    def schedule_briefings(self):
        ...
        schedule.every().friday.at("12:00").do(lambda: asyncio.create_task(self.execute_weekend_briefing()))
