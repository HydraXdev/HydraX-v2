...
class XPCalculator:
    ...
    EDUCATION_MULTIPLIERS = {
        ...
        "perfect_score": {"quiz": 1.5, "module": 1.75, "assessment": 2.0},
        # Tier progression multipliers
        "difficulty_bonus": {"beginner": 1.0, "intermediate": 1.25, "advanced": 1.5, "expert": 1.75},
        # Special event multipliers
        "event_multiplier": {"double_xp_weekend": 2.0, "learning_marathon": 1.5, "community_challenge": 1.75},
    }
    ...
    # Penalties
    PENALTIES = {"panic_exit": -50, "overtrading": -25, "revenge_trading": -30}
    ...
    def calculate_trade_xp(
        ...
        active_multipliers = self._calculate_multipliers(multipliers or {}, performance_metrics or {})
    ...
    def _calculate_multipliers(
        self, active_multipliers: Dict[str, float], performance_metrics: Dict[str, float]
    ) -> Dict[str, float]:
    ...
    def _calculate_penalties(self, exit_type: ExitType, performance_metrics: Dict[str, float]) -> Dict[str, int]:
    ...
    def calculate_milestone_progress(
        self, current_xp: int, daily_avg_xp: float, current_multipliers: Dict[str, float]
    ) -> Dict[str, any]:
        """
        ...
        for milestone in TradingMilestone:
            if milestone.value <= current_xp:
                ...
                days_estimate = self._estimate_days_to_milestone(xp_needed, daily_avg_xp, current_multipliers)
        ...
        days_to_elite = self._estimate_days_to_milestone(
            max(0, TradingMilestone.ELITE.value - current_xp), daily_avg_xp, current_multipliers
        )
    ...
    def _estimate_days_to_milestone(self, xp_needed: int, daily_avg_xp: float, multipliers: Dict[str, float]) -> int:
    ...
    def validate_progression_rate(
        self, current_xp: int, start_date: datetime, target_milestone: TradingMilestone = TradingMilestone.ELITE
    ) -> Dict[str, any]:
        """
    ...
    def calculate_session_summary(
        self, trades: List[Dict[str, any]], session_multipliers: Dict[str, float]
    ) -> Dict[str, any]:
        """
        ...
        for trade in trades:
            ...
            trade_breakdown.append({"trade_id": trade.get("id"), "xp_earned": calc.net_xp, "breakdown": calc})
    ...
    def calculate_education_xp(
        ...
        total_multiplier = min(self._apply_multiplier_cap(multipliers), self.MAX_EDUCATION_MULTIPLIER)
    ...
    def _calculate_education_multipliers(
        ...
        if streak_days > 0:
            ...
            streak_mult = min(streak_config["base"] + (streak_days * streak_config["increment"]), streak_config["max"])
        ...
        for event in active_events:
            if event in self.EDUCATION_MULTIPLIERS["event_multiplier"]:
                multipliers["event_multiplier"] = max(
                    multipliers.get("event_multiplier", 1.0), self.EDUCATION_MULTIPLIERS["event_multiplier"][event]
                )
...
if __name__ == "__main__":
    ...
    group_result = calc.calculate_education_xp(
        activity=EducationActivity.ATTEND_WORKSHOP, user_id="user123", group_size=8, streak_days=10
    )
    ...
    progress = calc.calculate_milestone_progress(
        current_xp=25000, daily_avg_xp=850, current_multipliers={"win_rate": 1.5, "consistency": 1.3}
    )
