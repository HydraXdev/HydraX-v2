...
class XPCelebrationSystem:
    ...
    SOUND_EFFECTS = {
        CelebrationTier.MICRO: {"name": "coin_drop", "config": {"type": "coin", "pitch": 1.0, "duration": 200}},
        CelebrationTier.SMALL: {
            "name": "coin_cascade",
            "config": {"type": "coin_cascade", "count": 3, "pitch_variance": 0.1, "duration": 500},
        },
        CelebrationTier.MEDIUM: {
            "name": "cash_register",
            "config": {"type": "kaching", "pitch": 1.0, "bell_count": 1, "duration": 800},
        },
        CelebrationTier.LARGE: {
            "name": "cash_register_jackpot",
            "config": {"type": "kaching_multi", "pitch": 1.1, "bell_count": 3, "coin_shower": True, "duration": 1500},
        },
        CelebrationTier.EPIC: {
            "name": "victory_fanfare",
            "config": {"type": "fanfare", "cash_register": True, "duration": 2000},
        },
        CelebrationTier.LEGENDARY: {
            "name": "legendary_achievement",
            "config": {"type": "legendary", "orchestral": True, "duration": 3000},
        },
    }
    ...
    def _generate_message(
        self, event_type: EventType, xp_amount: int, user_data: Dict[str, Any], event_metadata: Optional[Dict[str, Any]]
    ) -> str:
        ...
        if event_type == EventType.TRADE_WIN:
            ...
            return (
                f"{rank}, you've earned {xp_amount} XP from a ${profit:.2f} victory! Total: {total_xp + xp_amount:} XP"
            )
            ...
            challenge_name = (
                event_metadata.get("challenge_name", "Daily Challenge") if event_metadata else "Daily Challenge"
            )
    ...
    def _generate_npc_reactions(self, tier: CelebrationTier, event_type: EventType) -> List[NPCResponse]:
        ...
        drill_responses = self.NPC_CELEBRATIONS[NPCPersonality.DRILL_INSTRUCTOR].get(
            tier, self.NPC_CELEBRATIONS[NPCPersonality.DRILL_INSTRUCTOR][CelebrationTier.MICRO]
        )
        reactions.append(
            NPCResponse(
    ...
    def get_milestone_celebration(self, milestone_type: str, user_data: Dict[str, Any]) -> CelebrationEvent:
        ...
        config = milestone_configs.get(
            milestone_type, {"xp": 500, "title": "🎉 MILESTONE REACHED!", "message": "Incredible achievement unlocked!"}
        )

        return self.celebrate_xp_gain(
            EventType.LEVEL_UP, config["xp"], user_data, {"milestone": milestone_type, "special": True}
        )
...
def generate_cash_register_sound_config() -> Dict[str, Any]:
    ...
    return {
        "type": "cash_register",
        "components": [
            {"type": "bell", "frequency": 2800, "duration": 100, "decay": 0.3, "volume": 0.8},
            {
                "type": "coins",
                "count": 5,
                "base_frequency": 4000,
            ...
            {"type": "drawer", "frequency": 150, "duration": 200, "attack": 0.01, "decay": 0.5, "volume": 0.4},
        ],
        "reverb": {"enabled": True, "roomSize": 0.3, "decay": 1.5, "wet": 0.2},
    }
...
def generate_tp_hit_celebration_sound() -> Dict[str, Any]:
    ...
    return {
        "type": "tp_celebration",
        "sequence": [
            {"type": "rise", "start_frequency": 400, "end_frequency": 800, "duration": 200, "volume": 0.5},
            {
                "type": "bell_cascade",
...
if __name__ == "__main__":
    ...
    user_data = {"rank": "SERGEANT", "total_xp": 5420}

    celebration = celebration_system.celebrate_xp_gain(EventType.TRADE_WIN, 250, user_data, {"profit": 125.50})
