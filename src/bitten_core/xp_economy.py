...
class XPEconomy:
        ...
        if user_id not in self.user_balances:
            ...
            if balance_file.exists():
                ...
                self.user_balances[user_id] = UserXPBalance(
                    user_id=user_id, current_balance=0, lifetime_earned=0, lifetime_spent=0
                )
    ...
    def add_xp(self, user_id: str, amount: int, reason: str = "", context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if self.achievement_system and ACHIEVEMENT_SYSTEM_AVAILABLE:
            achievement_progress = {"total_xp": balance.current_balance, "lifetime_earned": balance.lifetime_earned}
    ...
    def purchase_item(
        self, user_id: str, item_id: str, user_tier: str, user_stats: Optional[Dict[str, Any]] = None
    ...
    def refund_purchase(self, user_id: str, transaction_id: str, reason: str = "Admin refund") -> Tuple[bool, str]:
        """Refund a purchase"""
        balance = self.get_user_balance(user_id)
    ...
    def get_active_items(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        for item_id in balance.active_purchases:
            if item_id in self.shop_catalog:
                item = self.shop_catalog[item_id]
                active_items.append({"item_id": item_id, "name": item.name, "type": "permanent", "expires_at": None})
    ...
    def _check_tactical_unlocks(self, old_xp: int, new_xp: int, user_id: str) -> List[Dict[str, Any]]:
        ...
        unlock_thresholds = {120: "FIRST_BLOOD", 240: "DOUBLE_TAP", 360: "TACTICAL_COMMAND"}
...
— DRILL SERGEANT 🎖️"""
    ...
    def award_daily_login_xp(self, user_id: str) -> Dict[str, Any]:
        ...
        return self.add_xp(user_id=user_id, amount=xp_amount, reason="Daily login bonus", context={"daily_login": True})
    ...
    def get_drill_report_xp_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        today_transactions = [t for t in balance.purchase_history if t.timestamp.date() == today and t.amount > 0]

        today_xp = sum(t.amount for t in today_transactions)

        # Get week's XP transactions
        week_start = today - timedelta(days=today.weekday())
        week_transactions = [t for t in balance.purchase_history if t.timestamp.date() >= week_start and t.amount > 0]
    ...
    def _get_recent_tactical_unlocks(self, user_id: str, days: int = 7) -> List[Dict[str, Any]]:
        ...
        for transaction in balance.purchase_history:
            if (
                ...
                and "strategy_unlocks" in transaction.metadata
            ):
...
if __name__ == "__main__":
    ...
    # First win
    result = economy.award_trade_xp(user_id, "WIN", {"pair": "EURUSD", "win_streak": 1, "first_strategy_use": True})
    ...
    for i in range(2, 12):
        result = economy.award_trade_xp(user_id, "WIN", {"pair": "GBPUSD", "win_streak": i})
