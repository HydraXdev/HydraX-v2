...
from database.xp_database import XPBalance, XPDatabase, XPTransaction
...
class XPEconomyDB:
    ...
    async def get_user_balance(self, user_id: str) -> UserXPBalanceDB:
        ...
        try:
            ...
            return UserXPBalanceDB(user_id=user_id, current_balance=0, lifetime_earned=0, lifetime_spent=0)

    async def add_xp(
        self, user_id: str, amount: int, source: str = "unknown", metadata: Optional[Dict[str, Any]] = None
    ) -> UserXPBalanceDB:
        ...
        try:
            user_id_int = int(user_id)
            balance, transaction = await self.xp_db.add_xp(user_id_int, amount, f"XP earned from {source}", metadata)
    ...
    async def spend_xp(
        self, user_id: str, amount: int, item_name: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Tuple[bool, str]:
        ...
        try:
            user_id_int = int(user_id)
            balance, transaction = await self.xp_db.spend_xp(user_id_int, amount, f"Purchased: {item_name}", metadata)
    ...
    async def purchase_item(self, user_id: str, item_id: str, user_tier: str = "NIBBLER") -> Tuple[bool, str]:
        ...
        success, message = await self.spend_xp(
            ...
            item.cost,
            item.name,
            {"item_id": item_id, "purchase_type": item.purchase_type.value, "tier": user_tier},
        )
    ...
    async def migrate_from_file_system(self, old_economy):
        ...
        for user_id, old_balance in old_economy.users.items():
            try:
                ...
                balance = await self.xp_db.create_user_balance(user_id_int, old_balance.current_balance)
