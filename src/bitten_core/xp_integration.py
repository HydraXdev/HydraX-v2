...
from .press_pass_reset import PressPassResetManager, ShadowStats
from .prestige_system import PrestigeSystem
from .user_profile import UserProfileManager
from .xp_economy import PurchaseType, XPEconomy
...
class XPIntegrationManager:
    ...
    def __init__(
        ...
        # Initialize Press Pass reset manager if telegram is available
        self.press_pass_manager = None
        if telegram_messenger:
            self.press_pass_manager = PressPassResetManager(xp_manager=self, telegram=telegram_messenger)
    ...
    def get_user_xp_status(self, user_id: str) -> Dict[str, Any]:
        ...
        # Check if Press Pass user
        is_press_pass = False
        shadow_stats = None
        if self.press_pass_manager:
            is_press_pass = self.press_pass_manager.is_press_pass_user(user_id)
            if is_press_pass:
                shadow_stats = self.press_pass_manager.get_shadow_stats(user_id)
        ...
        result = {
            ...
            "press_pass": {"active": is_press_pass, "shadow_stats": asdict(shadow_stats) if shadow_stats else None},
        }

        return result

    def award_xp_with_multipliers(self, user_id: str, base_amount: int, reason: str, details: str = "") -> int:
        ...
        # Update shadow stats for Press Pass users
        if self.press_pass_manager and self.press_pass_manager.is_press_pass_user(user_id):
            shadow = self.press_pass_manager.shadow_stats.get(user_id)
            if shadow:
                shadow.real_total_xp += final_amount
                shadow.real_lifetime_earned += final_amount
                self.press_pass_manager._save_shadow_stats()
    ...
    def purchase_xp_item(self, user_id: str, item_id: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        # Attempt purchase
        success, message, transaction = self.xp_economy.purchase_item(user_id, item_id, user_tier, user_stats)
    ...
    def attempt_prestige(self, user_id: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        # Execute prestige
        success, progress, reward = self.prestige_system.execute_prestige(user_id, current_xp)
        ...
        if success:
            ...
            return (
                True,
                f"Prestiged to {progress.current_level}!",
                {
    ...
    def record_trade_with_xp(
        self, user_id: str, signal_id: str, position_id: str, profit: float, is_success: bool
    ) -> Dict[str, Any]:
        ...
        if bonus_xp > 0:
            self.award_xp_with_multipliers(user_id, bonus_xp, f"Trade profit bonus", f"Profit: ${profit:.2f}")
    ...
    def check_trade_protocols(
        self, user_id: str, trade_id: str, current_profit_pips: float, current_sl_pips: float, tp1_pips: float
    ) -> Dict[str, Any]:
        """Check all active protocols for a trade"""
        actions = {}

        # Check trailing guard
        new_sl = self.protocol_manager.check_trailing_guard(user_id, trade_id, current_profit_pips, current_sl_pips)
        if new_sl:
            actions["trailing_guard"] = {"action": "trail_sl", "new_sl_pips": new_sl}

        # Check split command
        split_action = self.protocol_manager.check_split_command(user_id, trade_id, current_profit_pips, tp1_pips)
        if split_action:
    ...
    def prepare_stealth_orders(
        self, user_id: str, signal_price: float, pair: str, direction: str
    ) -> Optional[List[Dict[str, Any]]]:
        """Prepare stealth entry orders if enabled"""
        return self.protocol_manager.prepare_stealth_entry(user_id, signal_price, pair, direction)

    # Press Pass specific methods
    def enable_press_pass(self, user_id: str) -> Tuple[bool, str]:
        """Enable Press Pass for a user"""
        if not self.press_pass_manager:
            return False, "Press Pass system not initialized"

        self.press_pass_manager.add_press_pass_user(user_id)
        return True, "Press Pass activated! Your XP will reset daily at 00:00 UTC."

    def disable_press_pass(self, user_id: str) -> Tuple[bool, str]:
        """Disable Press Pass for a user"""
        if not self.press_pass_manager:
            return False, "Press Pass system not initialized"

        self.press_pass_manager.remove_press_pass_user(user_id)
        return True, "Press Pass deactivated. Your XP is now safe from resets."

    def get_press_pass_status(self, user_id: str) -> Dict[str, Any]:
        """Get detailed Press Pass status for a user"""
        if not self.press_pass_manager:
            return {"active": False, "message": "Press Pass system not available"}

        is_active = self.press_pass_manager.is_press_pass_user(user_id)
        shadow_stats = self.press_pass_manager.get_shadow_stats(user_id) if is_active else None

        return {
            "active": is_active,
            "shadow_stats": asdict(shadow_stats) if shadow_stats else None,
            "current_xp": self.xp_economy.get_user_balance(user_id).current_balance,
            "next_reset": "00:00 UTC" if is_active else None,
        }

    def start_press_pass_scheduler(self):
        """Start the Press Pass reset scheduler"""
        if self.press_pass_manager:
            self.press_pass_manager.start_scheduler()
            logger.info("Press Pass scheduler started")

    def stop_press_pass_scheduler(self):
        """Stop the Press Pass reset scheduler"""
        if self.press_pass_manager:
...
if __name__ == "__main__":
    ...
    xp_earned = manager.award_xp_with_multipliers(user_id, 100, "Test award")
