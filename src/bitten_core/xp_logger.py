...
class XPLogger:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    ...
                    WHERE trade_id = ?
                """,
                    (exit_price, outcome.value, profit_loss, close_time.isoformat(), hold_duration, total_xp, trade_id),
                )
                ...
                if row:
                    ...
                    """,
                        (
                            new_xp,
                            new_rank.value,
                            datetime.now().isoformat() if new_rank != current_rank else row[1],
                            user_id,
                        ),
                    )
                else:
    ...
    def format_performance_report(self, user_id: int) -> str:
        ...
        try:
            ...
            if next_threshold:
                progress_pct = (
                    (performance.current_xp - current_threshold) / (next_threshold - current_threshold)
                ) * 100
