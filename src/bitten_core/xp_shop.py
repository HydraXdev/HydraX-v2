...
from .xp_economy import PurchaseType, XPEconomy, XPItem
...
class XPShop:
    ...
    # Category mappings
    CATEGORY_MAP = {
        ShopCategory.TACTICAL_INTEL: [PurchaseType.HEAT_MAP, PurchaseType.SQUAD_RADAR, PurchaseType.CONFIDENCE_BOOST],
        ...
        ShopCategory.AMMUNITION: [PurchaseType.EXTENDED_MAG, PurchaseType.RAPID_RELOAD, PurchaseType.SPECIAL_AMMO],
    }

    # Category display info
    CATEGORY_INFO = {
        ShopCategory.TACTICAL_INTEL: {"icon": "📊", "description": "Information advantages and market intelligence"},
        ShopCategory.ELITE_PROTOCOLS: {"icon": "🎯", "description": "Advanced trading strategies and automation"},
        ShopCategory.AMMUNITION: {"icon": "🔫", "description": "Extra shots and enhanced firepower"},
        ShopCategory.SPECIAL: {"icon": "⭐", "description": "Special offers and limited items"},
    }
    ...
    def get_shop_display(self, user_id: str, user_tier: str, filter_affordable: bool = False) -> List[ShopDisplay]:
            ...
            for purchase_type in purchase_types:
                ...
                for item_id, item in self.economy.shop_catalog.items():
                    ...
                    if item_display["is_locked"]:
                        item_display["unlock_progress"] = self._get_unlock_progress(user_tier_index, item_tier_index)

                    category_items.append(item_display)

            if category_items:
                info = self.CATEGORY_INFO[category]
                shop_displays.append(
                    ShopDisplay(
                        category=category,
    ...
    def get_item_details(self, item_id: str, user_id: str, user_tier: str) -> Optional[Dict[str, Any]]:
        ...
        purchase_count = sum(
            1 for trans in balance.purchase_history if trans.item_id == item_id and trans.status.value == "completed"
        )
        ...
        # Add purchase availability
        can_buy, reason = self.economy.can_purchase(user_id, item_id, user_tier)
        details["purchase"] = {"can_purchase": can_buy, "reason": reason if not can_buy else "Ready to purchase"}
    ...
    def _get_item_benefits(self, item: XPItem) -> List[str]:
        ...
        if item.purchase_type == PurchaseType.TRAILING_GUARD:
            benefits.extend(
                ["✅ Automatically protects profits", "✅ No manual intervention needed", "✅ Works 24/7 on all trades"]
            )
        elif item.purchase_type == PurchaseType.SPLIT_COMMAND:
            benefits.extend(
                ["✅ Lock in profits early", "✅ Let winners run further", "✅ Reduces emotional decisions"]
            )
        elif item.purchase_type == PurchaseType.EXTENDED_MAG:
            benefits.extend(
                ["✅ More trading opportunities", "✅ Only on high-confidence signals", "✅ Maintains safety standards"]
            )
...
if __name__ == "__main__":
    ...
    shop.create_special_offer(offer_id="weekend_special", item_id="heat_map", discount_percent=50, valid_hours=48)
