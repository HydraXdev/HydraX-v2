...
from ..monitoring.logger import IntelligenceLogger, LoggerManager
from ..monitoring.metrics import JSONExporter, MetricsMonitor, PrometheusExporter
from ..normalization.normalizer import DataNormalizer, NormalizerFactory
from .base import DataIngestionComponent, DataSource, IntelligenceComponent, IntelligenceError, Signal, SignalGenerator
...
class IntelligenceOrchestrator:
    ...
    async def add_data_source(self, name: str, source_type: str, config: Dict[str, Any]) -> None:
        ...
        # Create data source
        data_source = DataSource(name=name, type=source_type, config=config)

        # Create ingester based on type
        ingester_class = self._get_ingester_class(source_type)
        ingester = ingester_class(name=f"{name}_ingester", data_source=data_source, config=config)

        # Create normalizer
        normalizer = NormalizerFactory.create_normalizer(
            source_type=source_type, name=f"{name}_normalizer", config=config
        )
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        for component in self.registry.get_all():
            try:
                ...
                components_health.append({"name": component.name, "running": False, "error": str(e)})
