...
def time_window_key(timestamp: datetime, window_minutes: int) -> str:
    ...
    window_start = timestamp.replace(
        minute=(timestamp.minute // window_minutes) * window_minutes, second=0, microsecond=0
    )
    return window_start.isoformat()


async def parallel_map(func: Callable[[T], Any], items: List[T], max_concurrent: int = 10) -> List[Any]:
