...
class MarketDataPreprocessor:
    ...
    def fit(self, data: pd.DataFrame) -> "MarketDataPreprocessor":
        ...
        feature_groups = {
            ...
            "indicators": [
                col for col in featured_data.columns if any(ind in col for ind in ["rsi", "macd", "bb", "atr", "stoch"])
            ],
            "others": [],
        }
        ...
        feature_groups["others"] = [col for col in featured_data.columns if col not in assigned_features]
...
class DataPipeline:
    ...
    def prepare_data(self, data: pd.DataFrame, target_column: str = "close") -> Dict[str, DataLoader]:
        ...
        # Create sequences
        sequences, target_sequences = self.preprocessor.create_sequences(processed_data, targets)
        ...
        # Create datasets
        train_dataset = MarketDataset(train_sequences, train_targets, augment=self.augment_train)
        val_dataset = MarketDataset(val_sequences, val_targets)
        test_dataset = MarketDataset(test_sequences, test_targets)

        # Create data loaders
        dataloaders = {
            "train": DataLoader(train_dataset, batch_size=self.batch_size, shuffle=self.shuffle),
