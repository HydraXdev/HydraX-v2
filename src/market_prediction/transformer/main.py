...
import pandas as pd
import torch
from data.preprocessing import DataPipeline, MarketDataPreprocessor
from inference.inference_engine import InferenceEngine, ModelVersionManager, StreamingInferenceEngine
from models.attention_mechanisms import (
    CrossTimeScaleAttention,
    DilatedAttention,
    HierarchicalAttention,
    LocalAttention,
    PatternMatchingAttention,
)
...
from utils.model_versioning import AutomatedModelUpdater
from utils.model_versioning import ModelVersionManager as VersionManager
from utils.uncertainty_estimation import AdaptiveUncertaintyEstimator, ConfidenceScorer, UncertaintyEstimator
...
class MarketPredictionSystem:
    ...
    def prepare_data(self, data_path: str) -> Dict:
        ...
        dataloaders = pipeline.prepare_data(data, target_column=self.config["data"].get("target_column", "close"))

        logger.info(
            f"Prepared data loaders - Train: {len(dataloaders['train'])}, "
    ...
    def evaluate(self, test_loader: torch.utils.data.DataLoader) -> Dict:
        ...
        self.uncertainty_estimator = UncertaintyEstimator(
            model=self.model, n_samples=self.config["uncertainty"]["n_mc_samples"]
        )
        ...
        for batch_data, batch_targets in test_loader:
            ...
            uncertainty_metrics = self.uncertainty_estimator.estimate_uncertainty(batch_data, method="mc_dropout")
    ...
    def setup_inference(self):
        ...
        self.confidence_scorer = ConfidenceScorer(history_window=self.config["uncertainty"]["history_window"])
    ...
    def backtest(self, data: pd.DataFrame, start_date: Optional[str] = None) -> Dict:
        ...
        pred_direction = np.sign(
            predictions[:, -1] - data["close"].iloc[window_size - 1 : -self.config["data"]["prediction_horizon"]].values
        )
        true_direction = np.sign(
            targets[:, -1] - data["close"].iloc[window_size - 1 : -self.config["data"]["prediction_horizon"]].values
        )
...
def main():
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["train", "evaluate", "predict", "backtest", "serve"],
        required=True,
        help="Operation mode",
    )
    ...
    try:
        if args.mode == "train":
            ...
            system.trainer = MarketPredictionTrainer(
                model=system.build_model(), device=system.device, config=system.config["training"]
            )
...
def create_default_config() -> Dict:
    ...
    config = {
        ...
        "training": {
            ...
            "scheduler": {"type": "reduce_on_plateau", "factor": 0.5, "patience": 10, "min_lr": 1e-7},
        },
        "inference": {
            "device": "cuda",
            "batch_timeout": 0.1,
            ...
            "streaming": {"enabled": True, "buffer_size": 100, "update_frequency": 1.0},
        },
        "uncertainty": {
            "n_mc_samples": 100,
