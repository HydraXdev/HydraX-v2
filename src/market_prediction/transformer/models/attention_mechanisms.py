...
class DilatedAttention(nn.Module):
    ...
    def __init__(self, d_model: int, dilation_rates: list = [1, 2, 4, 8], dropout: float = 0.1):
        ...
        self.rate_projections = nn.ModuleList([nn.Linear(d_model, d_model) for _ in dilation_rates])
...
class CrossTimeScaleAttention(nn.Module):
    ...
    def __init__(self, d_model: int, scales: list = [1, 5, 10, 20], dropout: float = 0.1):
        ...
        self.downsamplers = nn.ModuleList(
            [
                nn.Conv1d(d_model, d_model, kernel_size=scale, stride=scale) if scale > 1 else nn.Identity()
                for scale in scales
            ]
        )
        ...
        self.scale_attentions = nn.ModuleList(
            [nn.MultiheadAttention(d_model, num_heads=4, dropout=dropout) for _ in scales]
        )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        for i, (scale, downsampler, attention) in enumerate(zip(self.scales, self.downsamplers, self.scale_attentions)):
            ...
            if x_scaled.size(1) != seq_len:
                x_scaled = F.interpolate(
                    x_scaled.transpose(1, 2), size=seq_len, mode="linear", align_corners=False
                ).transpose(1, 2)
...
class PatternMatchingAttention(nn.Module):
    ...
    def __init__(self, d_model: int, n_patterns: int = 32, pattern_length: int = 20, dropout: float = 0.1):
        super().__init__()
        self.d_model = d_model
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        pattern_importance = self.pattern_weights(pattern_matches.mean(dim=1).transpose(1, 2)).squeeze(
            -1
        )  # (batch, n_patterns)
...
class HierarchicalAttention(nn.Module):
    ...
    def __init__(self, d_model: int, segment_length: int = 10, n_heads: int = 4, dropout: float = 0.1):
        super().__init__()
        self.d_model = d_model
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        attention_info = {"intra_weights": torch.stack(intra_weights, dim=1), "inter_weights": inter_weight}
