...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int, dropout: float = 0.1):
        ...
        self.feed_forward = nn.Sequential(
            nn.Linear(d_model, d_ff), nn.ReLU(), nn.Dropout(dropout), nn.Linear(d_ff, d_model), nn.Dropout(dropout)
        )
...
class MarketTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.prediction_head = nn.Sequential(
            nn.Linear(d_model, d_model // 2), nn.ReLU(), nn.Dropout(dropout), nn.Linear(d_model // 2, n_outputs)
        )
    ...
    def forward(self, x: torch.Tensor, lengths: Optional[torch.Tensor] = None, return_attention: bool = False) -> dict:
        """
        ...
        outputs = {"predictions": predictions, "uncertainty": uncertainty, "confidence": confidence}
...
class TemporalTransformer(MarketTransformer):
    ...
    def __init__(self, *args, kernel_sizes: list = [3, 5, 7], **kwargs):
        ...
        self.temporal_convs = nn.ModuleList(
            [nn.Conv1d(self.d_model, self.d_model, kernel_size=k, padding=k // 2) for k in kernel_sizes]
        )
