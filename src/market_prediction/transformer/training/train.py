...
class ProbabilisticLoss(nn.Module):
    ...
    def forward(self, predictions: torch.Tensor, uncertainty: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """
...
class MarketPredictionTrainer:
    ...
    def __init__(self, model: nn.Module, device: torch.device, config: Dict, use_wandb: bool = True):
        ...
        self.training_history = {"train_loss": [], "val_loss": [], "train_metrics": [], "val_metrics": []}
    ...
    def _get_scheduler(self) -> Optional[object]:
        ...
        if sched_type == "reduce_on_plateau":
            ...
            return CosineAnnealingWarmRestarts(
                self.optimizer, T_0=sched_config.get("T_0", 10), T_mult=sched_config.get("T_mult", 2)
            )
    ...
    def train_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        with tqdm(train_loader, desc=f"Training Epoch {self.current_epoch}") as pbar:
            for batch_idx, (sequences, targets) in enumerate(pbar):
                ...
                # Compute losses
                pred_loss = self.prediction_loss(outputs["predictions"], outputs["uncertainty"], targets)
                ...
                # Update progress bar
                pbar.set_postfix({"loss": f"{loss.item():.4f}", "mse": f"{mse.item():.4f}", "mae": f"{mae.item():.4f}"})
    ...
    def validate(self, val_loader: DataLoader) -> Dict[str, float]:
        ...
        with torch.no_grad():
            with tqdm(val_loader, desc="Validation") as pbar:
                for sequences, targets in pbar:
                    ...
                    # Update progress bar
                    pbar.set_postfix(
                        {"loss": f"{loss.item():.4f}", "mse": f"{mse.item():.4f}", "mae": f"{mae.item():.4f}"}
                    )
        ...
        calibration_metrics = self._calculate_calibration_metrics(all_predictions, all_targets, all_uncertainties)
        metrics.update(calibration_metrics)

        # Calculate confidence correlation
        prediction_errors = torch.abs(all_predictions - all_targets).mean(dim=1)
        confidence_correlation = torch.corrcoef(torch.stack([prediction_errors, all_confidences.squeeze()]))[
            0, 1
        ].item()
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader, n_epochs: int, patience: int = 20):
...
def create_training_config() -> Dict:
    ...
    return {
        ...
        "training": {"batch_size": 32, "n_epochs": 100, "patience": 20, "gradient_clip": 1.0, "uncertainty_beta": 0.5},
