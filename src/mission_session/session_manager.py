    ...
    def create_session(
        ...
        try:
            with self._get_connection() as conn:
                ...
                ''', (
                    mission_session_id, user_id, alert_id, signal_id,
                    'PENDING', pair, timeframe, risk_max_usd, nonce,
                    now, expires_at
                ))
                conn.commit()

            logger.info(f"✅ Created mission session {mission_session_id} for user {user_id}, expires in {ttl_seconds}s")
            return session_data
