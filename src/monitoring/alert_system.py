...
class WebhookNotifier(AlertNotifier):
    ...
    async def send_alert(self, alert: Alert) -> bool:
        ...
        try:
            ...
            # Send webhook
            response = requests.post(url, json=payload, headers=headers, auth=auth if auth else None, timeout=10)
            response.raise_for_status()
...
class AlertDatabase:
    ...
    def log_notification(self, alert_id: str, notifier_type: str, success: bool, error_message: str = None):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (alert_id, notifier_type, success, error_message, datetime.now().isoformat()),
            )
...
class AlertManager:
    ...
    def _resolve_alert(self, alert_id: str):
        ...
        self.logger.info(
            ...
            extra={"alert_id": alert_id, "service": alert.service, "metric": alert.metric},
        )
    ...
    def _send_notification(self, notifier_type: str, notifier: AlertNotifier, alert: Alert):
        ...
        try:
            ...
            # Log notification attempt
            self.db.log_notification(alert.id, notifier_type, success, None if success else "Unknown error")
            ...
            self.db.log_notification(alert.id, notifier_type, False, error_msg)
    ...
    def _send_resolution_notification(self, notifier_type: str, notifier: AlertNotifier, alert: Alert):
        ...
        try:
            ...
            self.db.log_notification(
                alert.id, f"{notifier_type}_resolution", success, None if success else "Unknown error"
            )
            ...
            # Log failed notification
            self.db.log_notification(alert.id, f"{notifier_type}_resolution", False, error_msg)
...
def get_default_alert_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "notifiers": {
            ...
            "slack": {"webhook_url": ""},  # Set via environment
            "telegram": {"bot_token": "", "chat_id": ""},  # Set via environment  # Set via environment
        },
    }
...
if __name__ == "__main__":
    ...
    alert_manager.check_metric(
        service="signal-generator", metric="signals_today", value=45, tags={"environment": "production"}  # Below target
    )
