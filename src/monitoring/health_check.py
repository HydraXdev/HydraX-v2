...
class HTTPHealthChecker(HealthChecker):
    ...
    async def check_health(self) -> HealthCheckResult:
        ...
        try:
            ...
            response = requests.request(method=method, url=url, headers=headers, timeout=self.timeout)
    ...
    async def check_health(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                timestamp=datetime.now(),
                response_time_ms=response_time,
                details={"active_accounts": active_accounts, "farm_url": farm_url, **self._get_system_metrics()},
            )
...
class HealthCheckManager:
    ...
    def _initialize_checkers(self):
        ...
        for db_name, db_config in self.config.get("databases", {}).items():
            self.checkers[f"database-{db_name}"] = DatabaseHealthChecker(f"database-{db_name}", db_config)
        ...
        for endpoint_name, endpoint_config in self.config.get("endpoints", {}).items():
            self.checkers[f"endpoint-{endpoint_name}"] = HTTPHealthChecker(f"endpoint-{endpoint_name}", endpoint_config)

        # Process checkers
        for process_name, process_config in self.config.get("processes", {}).items():
            self.checkers[f"process-{process_name}"] = ProcessHealthChecker(f"process-{process_name}", process_config)
    ...
    def get_overall_health(self) -> Dict[str, Any]:
        ...
        if not self.results:
            return {"status": "unknown", "message": "No health checks run yet", "timestamp": datetime.now().isoformat()}
...
class HealthCheckAPI:
    ...
    def _setup_routes(self):
        ...
        def health_check():
            ...
            return jsonify(
                {"status": "healthy", "timestamp": datetime.now().isoformat(), "service": "bitten-health-api"}
            )
        ...
        def readiness_check():
            ...
            if overall_health["status"] == "critical":
                return jsonify({"status": "not_ready", "reason": "Critical services failing"}), 503
...
def get_default_config() -> Dict[str, Any]:
    ...
    return {
        "databases": {
            ...
            "performance": {"type": "sqlite", "path": "/var/log/bitten/performance.db"},
        },
        "redis": {"host": "localhost", "port": 6379, "db": 0},
        "endpoints": {
            "webapp": {"url": "http://localhost:9001/health", "method": "GET", "timeout": 5, "expected_status": 200},
            "telegram-webhook": {
        ...
        "processes": {"bitten-core": {"name": "python", "pid_file": "/var/run/bitten/bitten-core.pid"}},
        "mt5_farm": {"url": "http://129.212.185.102:8001"},
        "api_port": 8080,
    }
