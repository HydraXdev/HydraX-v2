    ...
    def _check_tcs_adjustment(self):
        ...
        if projected_daily < self.target_signals_per_day * 0.8:
            ...
            if new_threshold != self.current_tcs_threshold:
                ...
                self.logger.warning(
                    f"TCS threshold adjusted to {new_threshold} " f"(projected daily: {projected_daily:.1f})"
                )
...
class TradePerformanceMonitor:
    ...
    def record_trade_execution(
        ...
        # Log performance
        self.logger.info(
            f"Trade execution: {trade_id} {symbol} {direction} " f"@ {entry_price} (success: {success})",
            extra={
                "trade_id": trade_id,
                "symbol": symbol,
                "direction": direction,
    ...
    def record_trade_close(self, trade_id: str, exit_price: float, pnl: float):
        ...
        self.logger.info(
            ...
            extra={"trade_id": trade_id, "exit_price": exit_price, "pnl": pnl, "action": "trade_close"},
        )
...
class PerformanceMonitor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.db = PerformanceDatabase(self.config.get("db_path", "/var/log/bitten/performance.db"))
