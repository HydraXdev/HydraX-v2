...
class BittenMonitoringIntegrator:
        ...
        try:
            ...
            if signal_perf.get("signals_today", 0) < 52:  # 80% of target
                self.alert_manager.check_metric(
                    service="signal-generator", metric="signals_today", value=signal_perf.get("signals_today", 0)
                )
            ...
            if trade_perf.get("win_rate", 0) < 80:  # Below warning threshold
                self.alert_manager.check_metric(
                    service="trade-executor", metric="win_rate", value=trade_perf.get("win_rate", 0)
                )
    ...
    def record_trade_close(
        ...
        try:
            ...
            self.performance_monitor.trade_monitor.record_trade_close(trade_id=trade_id, exit_price=exit_price, pnl=pnl)
