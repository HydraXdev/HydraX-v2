...
from src.database.models import RiskSession, Trade, User, XPTransaction
...
class MT5BridgeIntegration:
    ...
    async def process_mt5_result(self, result_string: str, user_id: int, fire_mode: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if parsed["type"] == "trade_opened":
                ...
                return {"success": True, "type": parsed["type"], "result": trade_result.to_dict()}

        except Exception as e:
            logger.error(f"Error processing MT5 result: {e}")
            return {"success": False, "error": str(e)}

    async def _process_trade_opened(self, trade_result: TradeResult, user_id: int, fire_mode: str) -> Dict[str, Any]:
        ...
        with self.db.session_scope() as session:
            ...
            risk_session = session.query(RiskSession).filter_by(user_id=user_id, session_date=today).first()
            ...
            if self.telegram:
                ...
                await self.telegram.send_message(user.telegram_id, message, parse_mode="HTML")
    ...
    async def _process_trade_closed(self, trade_result: TradeResult, user_id: int) -> Dict[str, Any]:
        ...
        with self.db.session_scope() as session:
            ...
            # Find existing trade
            db_trade = session.query(Trade).filter_by(user_id=user_id, mt5_ticket=trade_result.ticket).first()
            ...
            if is_early_exit:
                ...
                xp_earned = self.xp_calculator.calculate_xp(xp_event, tier=user.tier, profit=trade_result.net_profit)
            else:
    ...
    async def _process_trade_error(self, trade_result: TradeResult, user_id: int) -> Dict[str, Any]:
        ...
        with self.db.session_scope() as session:
            ...
            logger.error(
                f"MT5 trade error for user {user_id}: " f"Code {trade_result.error_code} - {trade_result.error_message}"
            )
    ...
    async def process_batch_results(self, results: List[str], user_id: int, fire_mode: str = None) -> Dict[str, Any]:
        """
        ...
        summary = {"total": len(results), "successful": successful_count, "failed": len(results) - successful_count}
        ...
        return {"success": True, "summary": summary, "results": processed_results}
        ...
        while self.processing:
            try:
                ...
                await self.process_mt5_result(
                    result_data["result_string"], result_data["user_id"], result_data.get("fire_mode")
                )
    ...
    async def queue_result(self, result_string: str, user_id: int, fire_mode: str = None):
        """Queue result for processing"""
        await self.result_queue.put({"result_string": result_string, "user_id": user_id, "fire_mode": fire_mode})
...
async def process_mt5_result(result_string: str, user_id: int, fire_mode: str = None) -> Dict[str, Any]:
