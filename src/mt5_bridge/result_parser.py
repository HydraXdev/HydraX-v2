...
class MT5ResultParser:
    ...
    PATTERNS = {
        ...
        "position_modified": re.compile(
            r"POSITION_MODIFIED\|ticket:(\d+)\|sl:([\d.]+)\|tp:([\d.]+)\|" r"success:(true|false)"
        ),
        # Error: "ERROR|code:10006|message:Request rejected|context:TRADE_OPEN"
        "error": re.compile(r"ERROR\|code:(\d+)\|message:([^|]+)(?:\|context:(.+))?"),
        ...
        "account_update": re.compile(
            r"ACCOUNT_UPDATE\|balance:([\d.]+)\|equity:([\d.]+)\|" r"margin:([\d.]+)\|free_margin:([\d.]+)"
        ),
    ...
    def get_summary(self) -> Dict[str, any]:
        ...
        for result in self.results:
            if result["type"] == "trade_closed":
                ...
                errors.append({"code": result.get("error_code"), "message": result.get("error_message")})
