...
class AbsorptionPatternDetector:
    ...
    def __init__(
        ...
        # Thresholds
        self.absorption_rate_thresholds = {"weak": 10.0, "moderate": 50.0, "strong": 100.0}
    ...
    def _detect_level_absorption(self, key: str, current_snapshot: OrderBookSnapshot) -> Optional[AbsorptionEvent]:
        ...
        ask_absorption = self._check_side_absorption(
            initial_snapshot.asks[:5], current_snapshot.asks[:5], snapshots, "ask"
        )
        ...
        bid_absorption = self._check_side_absorption(
            initial_snapshot.bids[:5], current_snapshot.bids[:5], snapshots, "bid"
        )
    ...
    def _classify_absorption_strength(self, rate: float) -> str:
        ...
        for strength, threshold in sorted(self.absorption_rate_thresholds.items(), key=lambda x: x[1], reverse=True):
            if rate >= threshold:
                return strength
        return "weak"
    ...
    def get_active_patterns(self, symbol: str, exchange: str = None) -> List[AbsorptionPattern]:
        ...
        active = [p for p in self.active_patterns[key] if current_time - p.end_time < 1800]
    ...
    def get_statistics(self, symbol: str, exchange: str = None) -> Dict:
        ...
        if key not in self.absorption_events:
            return {"total_events": 0, "total_volume_absorbed": 0.0, "pattern_count": 0, "dominant_side": "neutral"}
...
async def main():
    ...
    detector = AbsorptionPatternDetector(min_volume_threshold=10.0, max_price_movement=0.001, min_duration=5.0)
