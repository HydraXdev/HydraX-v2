...
class CumulativeDeltaCalculator:
    ...
    def get_current_delta(self, symbol: str, exchange: str) -> Dict:
        ...
        if key not in self.cumulative_delta:
            return {"cumulative_delta": 0.0, "current_bar_delta": 0.0, "delta_trend": "neutral"}
    ...
    def get_delta_profile(self, symbol: str, exchange: str, periods: int = 20) -> Dict:
        ...
        if key not in self.delta_bars or len(self.delta_bars[key]) == 0:
            return {"bars": [], "statistics": {}, "divergences": []}
        ...
        if key in self.divergences:
            ...
            recent_divergences = [div.to_dict() for div in self.divergences[key] if div.end_time > cutoff_time]

        return {"bars": [bar.to_dict() for bar in bars], "statistics": stats, "divergences": recent_divergences}
    ...
    def get_delta_extremes(self, symbol: str, exchange: str) -> Dict:
        ...
        if key not in self.delta_bars or len(self.delta_bars[key]) < 10:
            return {"max_delta": 0.0, "min_delta": 0.0, "extreme_levels": []}
        ...
        for i in range(1, len(deltas) - 1):
            if deltas[i] > deltas[i - 1] and deltas[i] > deltas[i + 1]:
                extreme_levels.append(
                    {"type": "peak", "delta": deltas[i], "price": bars[i].close_price, "timestamp": bars[i].timestamp}
                )
            elif deltas[i] < deltas[i - 1] and deltas[i] < deltas[i + 1]:
                extreme_levels.append(
                    {"type": "trough", "delta": deltas[i], "price": bars[i].close_price, "timestamp": bars[i].timestamp}
                )
...
# Example usage
async def main():
    calculator = CumulativeDeltaCalculator(bar_period=60, lookback_bars=100)  # 1-minute bars
    ...
    for i in range(100):
        ...
        if i % 10 == 0:
            ...
            print(
                f"Bar {i}: Cumulative Delta: {delta_info['cumulative_delta']:.2f}, "
