...
class DarkPoolActivityScanner:
    ...
    def scan_for_activity(
        self, symbol: str, recent_trades: List[Dict], order_book_snapshot: Optional[Dict] = None
    ) -> List[DarkPoolPrint]:
        """
    ...
    def _detect_volume_spike(self, symbol: str, trades: List[Dict], order_book: Optional[Dict]) -> List[DarkPoolPrint]:
        ...
        for window, window_trades in time_windows.items():
            ...
            if notional > self.min_print_size:
                ...
                confidence = self._calculate_confidence_volume_spike(notional, len(window_trades), symbol)
    ...
    def _detect_price_deviation(
        self, symbol: str, trades: List[Dict], order_book: Optional[Dict]
    ) -> List[DarkPoolPrint]:
    ...
    def _detect_time_pattern(self, symbol: str, trades: List[Dict], order_book: Optional[Dict]) -> List[DarkPoolPrint]:
    ...
    def _detect_size_anomaly(self, symbol: str, trades: List[Dict], order_book: Optional[Dict]) -> List[DarkPoolPrint]:
    ...
    def get_flow_analysis(self, symbol: str, time_period: str = "1h") -> DarkPoolFlow:
        ...
        period_hours = {"1h": 1, "4h": 4, "1d": 24, "1w": 168}.get(time_period, 1)
        ...
        large_prints = [p for p in recent_prints if p.notional_value > avg_print_size * self.large_print_multiplier]
    ...
    def get_statistics(self, symbol: str) -> Dict:
        ...
        if symbol not in self.prints:
            return {"total_prints": 0, "total_volume": 0, "average_confidence": 0, "detection_methods": {}}
...
# Example usage
async def main():
    scanner = DarkPoolActivityScanner(min_print_size=50000, large_print_multiplier=10)  # $50k minimum
    ...
    for print_data in detected_prints:
        print(
            f"  - ${print_data.notional_value:,.0f} at {print_data.price:.2f} "
