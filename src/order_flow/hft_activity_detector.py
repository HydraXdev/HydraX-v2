...
class HFTActivityDetector:
    ...
    def process_event(
        self, timestamp: datetime, event_type: str, price: float, size: float, side: str, aggressive: bool = False
    ) -> Optional[HFTSignature]:
        """
        ...
        event = MicrostructureEvent(
            timestamp=timestamp, event_type=event_type, price=price, size=size, side=side, aggressive=aggressive
        )
    ...
    def _estimate_participation_rate(self, events: List[MicrostructureEvent]) -> float:
        ...
        hft_volume = sum(e.size for e in events if e.event_type == "trade" and self._is_likely_hft_trade(e))
    ...
    def _check_two_sided_activity(self) -> bool:
        ...
        bid_quotes = sum(1 for e in recent_events if e.event_type == "quote" and e.side == "bid")
        ask_quotes = sum(1 for e in recent_events if e.event_type == "quote" and e.side == "ask")
    ...
    def _get_events_near_timestamp(self, timestamp: datetime, window: timedelta) -> List[MicrostructureEvent]:
        ...
        return [e for e in self.event_stream if start_time <= e.timestamp <= end_time]
    ...
    def get_current_hft_activity(self) -> Dict[str, Any]:
        ...
        recent_signatures = [s for s in self.hft_signatures if s.timestamp >= datetime.now() - timedelta(minutes=5)]

        if not recent_signatures:
            return {"active": False, "dominant_type": None, "intensity": 0.0, "message_rate": 0.0}
