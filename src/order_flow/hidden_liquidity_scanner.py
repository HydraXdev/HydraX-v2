...
class HiddenLiquidityScanner:
    ...
    def analyze_execution(
        ...
        side: str,
    ) -> Optional[HiddenLiquiditySignal]:
        """
        ...
        signal = self._detect_hidden_liquidity(timestamp, price, executed_size, displayed_liquidity, side)
    ...
    def update_order_book(self, timestamp: datetime, bid_book: Dict[float, float], ask_book: Dict[float, float]):
        ...
        self.typical_book_depths.append((snapshot["total_bid_depth"] + snapshot["total_ask_depth"]) / 2)
    ...
    def _detect_hidden_liquidity(
        ...
        excess_execution = self._check_excess_execution(price, executed_size, displayed_liquidity, side)
        if excess_execution:
            return excess_execution

        # Method 2: Price improvement detection
        price_improvement = self._check_price_improvement(timestamp, price, executed_size, side)
        if price_improvement:
            return price_improvement

        # Method 3: Rapid refill pattern
        refill_signal = self._check_refill_behavior(timestamp, price, side)
        if refill_signal:
            return refill_signal

        # Method 4: Dark pool signature
        dark_pool = self._check_dark_pool_signature(timestamp, price, executed_size, side)
        if dark_pool:
            return dark_pool

        return None

    def _check_excess_execution(
        self, price: float, executed_size: float, displayed_liquidity: Dict[float, float], side: str
    ) -> Optional[HiddenLiquiditySignal]:
    ...
    def _check_price_improvement(
        self, timestamp: datetime, price: float, executed_size: float, side: str
    ) -> Optional[HiddenLiquiditySignal]:
    ...
    def _check_refill_behavior(self, timestamp: datetime, price: float, side: str) -> Optional[HiddenLiquiditySignal]:
        ...
        recent_executions = [
            ...
            if abs(e["price"] - price) < self.tick_size and e["timestamp"] >= timestamp - timedelta(seconds=5)
        ]
    ...
    def _check_dark_pool_signature(
        self, timestamp: datetime, price: float, executed_size: float, side: str
    ) -> Optional[HiddenLiquiditySignal]:
    ...
    def get_hidden_liquidity_map(self) -> Dict[str, Dict[float, float]]:
        ...
        liquidity_map = {"bid": defaultdict(float), "ask": defaultdict(float)}

        # Aggregate recent detections
        cutoff_time = datetime.now() - timedelta(minutes=5)
        recent_signals = [s for s in self.detected_liquidity if s.timestamp >= cutoff_time]
        ...
        return {"bid": dict(liquidity_map["bid"]), "ask": dict(liquidity_map["ask"])}
    ...
    def get_dark_pool_probability(self, price_range: Tuple[float, float]) -> float:
        ...
        dark_pool_signals = [
            s
            for s in self.detected_liquidity
            if s.liquidity_type == "dark_pool" and price_range[0] <= s.price_level <= price_range[1]
        ]
    ...
    def get_hidden_liquidity_summary(self) -> Dict[str, Any]:
        ...
        if not self.detected_liquidity:
            return {"total_detections": 0, "liquidity_types": {}, "detection_methods": {}, "estimated_hidden_volume": 0}
