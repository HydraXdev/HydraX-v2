...
class ImbalanceDetector:
    ...
    def __init__(self, levels_to_analyze: int = 20, imbalance_threshold: float = 1.5, lookback_period: int = 100):
        """
        ...
        self.strength_thresholds = {"weak": 1.5, "moderate": 2.0, "strong": 3.0, "extreme": 5.0}
    ...
    def detect_dynamic_imbalance(self, snapshot: OrderBookSnapshot) -> Optional[ImbalanceSignal]:
        ...
        weighted_imbalance = self._calculate_weighted_imbalance(
            snapshot, bid_levels=significant_levels["bid"], ask_levels=significant_levels["ask"]
        )
    ...
    def get_imbalance_trend(self, symbol: str, exchange: str = None) -> Dict:
        ...
        if key not in self.signal_history or len(self.signal_history[key]) < 2:
            return {"trend": "neutral", "consistency": 0.0, "average_ratio": 0.0, "signal_count": 0}
    ...
    def _calculate_weighted_imbalance(
        self, snapshot: OrderBookSnapshot, bid_levels: int = None, ask_levels: int = None
    ) -> float:
    ...
    def _find_significant_levels(self, snapshot: OrderBookSnapshot) -> Dict[str, int]:
        ...
        return {"bid": bid_significant, "ask": ask_significant}
    ...
    def get_statistics(self, symbol: str, exchange: str = None) -> Dict:
        ...
        if key not in self.signal_history:
            return {"total_signals": 0, "average_ratio": 0.0, "max_ratio": 0.0, "strength_distribution": {}}
