...
class LiquidityProfile:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "has_critical_voids": (self.largest_bid_void and self.largest_bid_void.severity == "critical")
            or (self.largest_ask_void and self.largest_ask_void.severity == "critical"),
        }
...
class LiquidityVoidDetector:
    ...
    def analyze_liquidity(self, snapshot: OrderBookSnapshot) -> LiquidityProfile:
        ...
        total_bid_liquidity = sum(level.quantity * level.price for level in snapshot.bids[: self.levels_to_analyze])
        total_ask_liquidity = sum(level.quantity * level.price for level in snapshot.asks[: self.levels_to_analyze])
    ...
    def _detect_side_voids(self, snapshot: OrderBookSnapshot, side: str) -> List[LiquidityVoid]:
        ...
        for i in range(min(len(levels) - 1, self.levels_to_analyze - 1)):
            ...
            potential_slippage = self._estimate_slippage(volume_to_cross, gap, mid_price)
    ...
    def _calculate_liquidity_score(
        ...
        severe_voids = sum(1 for v in bid_voids + ask_voids if v.severity in ["severe", "critical"])
        score -= severe_voids * 5

        # Penalize for liquidity imbalance
        if total_bid_liquidity + total_ask_liquidity > 0:
            imbalance = abs(total_bid_liquidity - total_ask_liquidity) / (total_bid_liquidity + total_ask_liquidity)
            score -= imbalance * 10
    ...
    def _calculate_effective_spread(
        self, snapshot: OrderBookSnapshot, bid_voids: List[LiquidityVoid], ask_voids: List[LiquidityVoid]
    ) -> float:
    ...
    def _classify_severity(self, gap_percentage: float) -> str:
        ...
        for severity, threshold in sorted(self.severity_thresholds.items(), key=lambda x: x[1], reverse=True):
    ...
    def _analyze_cluster(self, voids: List[LiquidityVoid]) -> Dict:
        ...
        price_range = (min(v.start_price for v in voids), max(v.end_price for v in voids))

        total_gap = sum(v.gap_size for v in voids)
        avg_severity = np.mean([list(self.severity_thresholds.keys()).index(v.severity) for v in voids])
        ...
        return {
            ...
            "dominant_side": max(set(v.side for v in voids), key=lambda s: sum(1 for v in voids if v.side == s)),
    ...
    def get_statistics(self, symbol: str, exchange: str = None) -> Dict:
        ...
        return {
            ...
            "has_recent_critical_voids": any(
                p.largest_bid_void
                and p.largest_bid_void.severity == "critical"
                or p.largest_ask_void
                and p.largest_ask_void.severity == "critical"
...
# Example usage
async def main():
    from order_book_reader import OrderBookLevel, OrderBookSnapshot
    ...
    # Create detector
    detector = LiquidityVoidDetector(min_gap_percentage=0.0001, min_volume_threshold=0.1, levels_to_analyze=20)
    ...
    if profile.largest_ask_void:
        print(
            f"Largest ask void: {profile.largest_ask_void.gap_percentage*100:.3f}% "
