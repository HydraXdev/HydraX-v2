...
class MarketMakerAnalyzer:
    ...
    def analyze_quote_update(
        ...
        maker_id = self._identify_market_maker(timestamp, bid_price, bid_size, ask_price, ask_size, quote_id)

        if not maker_id:
            return None

        # Analyze action type
        action = self._analyze_maker_action(maker_id, timestamp, bid_price, bid_size, ask_price, ask_size)
    ...
    def _identify_market_maker(
        ...
        quote_signature = self._calculate_quote_signature(bid_price, bid_size, ask_price, ask_size)
    ...
    def _calculate_quote_signature(
    ...
    def _matches_maker_pattern(self, maker_id: str, signature: Dict[str, Any], timestamp: datetime) -> bool:
    ...
    def _is_market_maker_behavior(self, bid_price: float, bid_size: float, ask_price: float, ask_size: float) -> bool:
    ...
    def _analyze_maker_action(
        self, maker_id: str, timestamp: datetime, bid_price: float, bid_size: float, ask_price: float, ask_size: float
    ) -> Optional[MarketMakerAction]:
        ...
        inventory_pressure = self._estimate_inventory_pressure(maker_id, bid_size, ask_size)
    ...
    def _estimate_inventory_pressure(self, maker_id: str, bid_size: float, ask_size: float) -> float:
    ...
    def _check_maker_competition(self, action: MarketMakerAction):
        ...
        for maker_id, actions in self.maker_actions.items():
            maker_recent = [a for a in actions if a.timestamp >= cutoff_time and a.action_type == "improve"]
    ...
    def get_market_quality_metrics(self) -> Dict[str, Any]:
        ...
        if not self.spread_history:
            return {"avg_spread": 0, "spread_volatility": 0, "maker_count": 0, "competition_level": "low"}

        recent_spreads = [s[1] for s in list(self.spread_history)[-100:]]

        # Active makers
        active_makers = [
            m for m, p in self.maker_profiles.items() if p.last_seen >= datetime.now() - timedelta(minutes=5)
        ]
        ...
        improve_actions = sum(
            1 for actions in self.maker_actions.values() for a in actions if a.action_type == "improve"
        )
        ...
        return {
            ...
            "recent_competition_events": len(
                [e for e in self.maker_competition_events if e["timestamp"] >= datetime.now() - timedelta(minutes=30)]
            ),
        }
    ...
    def get_maker_impact_analysis(self, maker_id: str) -> Dict[str, Any]:
        ...
        for timestamp, spread in self.spread_history:
            ...
            maker_active = any(abs((timestamp - mt).total_seconds()) < 60 for mt in maker_timestamps)
