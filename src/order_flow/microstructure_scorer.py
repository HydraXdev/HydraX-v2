...
from .market_maker_analyzer import MarketMakerAction, MarketMakerAnalyzer
from .quote_stuffing_identifier import QuoteStuffingEvent, QuoteStuffingIdentifier
...
class MicrostructureScorer:
    ...
    def __init__(self, symbol: str, tick_size: float = 0.0001):
        ...
        # Scoring weights
        self.weights = {"liquidity": 0.25, "stability": 0.20, "fairness": 0.30, "efficiency": 0.25}
        ...
        # Thresholds
        self.manipulation_thresholds = {"low": 20, "moderate": 40, "high": 60, "extreme": 80}
    ...
    def update_market_data(
        ...
        if bid_book and ask_book:
            ...
            self.maker_analyzer.analyze_quote_update(
                timestamp, best_bid, bid_book[best_bid], best_ask, ask_book[best_ask]
            )
        ...
        for trade in trades:
            ...
            self.iceberg_detector.update_order_flow(
                trade["timestamp"], trade["price"], trade["volume"], trade["side"], trade.get("spread", 2.0)
            )
            ...
            self.liquidity_scanner.analyze_execution(
                trade["timestamp"], trade["price"], trade["volume"], displayed_liquidity, trade["side"]
            )
    ...
    def _calculate_manipulation_risk(self) -> float:
        ...
        if stuffing_stats["event_count"] > 0:
            severity_weight = {"low": 5, "medium": 10, "high": 20, "extreme": 30}
    ...
    def _calculate_institutional_presence(self) -> float:
        ...
        # Dark pool activity
        dark_pool_prob = self.liquidity_scanner.get_dark_pool_probability((0, float("inf")))  # Full price range
