...
class OrderBookReader:
    ...
    async def subscribe(self, symbol: str, exchange: str = None, callback: Callable = None):
        ...
        if key not in self.tasks or self.tasks[key].done():
            self.tasks[key] = asyncio.create_task(self._update_loop(symbol, exchange))
    ...
    async def get_historical_snapshots(
        self, symbol: str, exchange: str = None, limit: int = 100
...
# Example usage
async def main():
    from exchange_manager import ExchangeConfig, ExchangeManager, RateLimitConfig

    # Setup exchange manager
    exchange_manager = ExchangeManager()
    await exchange_manager.add_exchange(
        ExchangeConfig(name="binance", rate_limits=RateLimitConfig(requests_per_second=10, requests_per_minute=1200))
    )
    ...
    def on_update(snapshot: OrderBookSnapshot):
        print(
            f"Update: {snapshot.symbol} - Spread: {snapshot.get_spread():.2f}, " f"Mid: {snapshot.get_mid_price():.2f}"
        )
