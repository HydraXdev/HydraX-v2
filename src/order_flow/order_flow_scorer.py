...
from .absorption_detector import AbsorptionEvent, AbsorptionPattern
...
class OrderFlowScorer:
    ...
    def __init__(
        self, weights: Optional[Dict[str, float]] = None, min_confidence: float = 0.6, lookback_periods: int = 20
    ):
        """
    ...
    def calculate_score(
        ...
        confidence = self._calculate_confidence(imbalance, absorption, liquidity, delta_info, dark_pool)

        # Generate insights and warnings
        insights = self._generate_insights(imbalance, absorption, liquidity, delta_info, dark_pool)
        warnings = self._generate_warnings(imbalance, absorption, liquidity, delta_info, dark_pool)
    ...
    def _score_absorption(self, absorption: Optional[AbsorptionPattern]) -> float:
        ...
        pattern_scores = {"accumulation": 80, "distribution": 20, "support": 70, "resistance": 30}
    ...
    def _generate_insights(self, *args) -> List[str]:
        ...
        if args[0]:
            imbalance = args[0]
            insights.append(
                f"{imbalance.strength.capitalize()} {imbalance.direction} "
        ...
        if args[1]:
            absorption = args[1]
            insights.append(
                f"{absorption.pattern_type.capitalize()} pattern detected "
        ...
        if args[4]:
            ...
            if abs(dark_pool.flow_score) > 50:
                ...
                insights.append(f"Strong dark pool {direction} detected " f"(flow score: {dark_pool.flow_score:.0f})")
    ...
    def _check_opportunity(
        self, score: OrderFlowScore, order_book: Optional[OrderBookSnapshot]
    ...
    def get_statistics(self, symbol: str, exchange: str = None) -> Dict:
        ...
        return {
            ...
            "hit_rate": (
                len([o for o in opportunities if o.confidence > 0.7]) / len(opportunities) if opportunities else 0
            ),
        }
...
# Example usage
async def main():
    scorer = OrderFlowScorer(min_confidence=0.6, lookback_periods=20)
    ...
    # Sample delta info
    delta_info = {"cumulative_delta": 500, "current_bar_delta": 50, "delta_trend": "bullish"}
