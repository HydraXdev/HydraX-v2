...
class QuoteStuffingIdentifier:
    ...
    def process_quote_message(
        self, timestamp: datetime, message_id: str, action: str, price: float, size: float, side: str
        ...
        message = QuoteMessage(
            timestamp=timestamp, message_id=message_id, action=action, price=price, size=size, side=side
        )
    ...
    def _detect_oscillating_stuffing(self) -> Optional[QuoteStuffingEvent]:
        ...
        if len(price_groups) >= 2:
            ...
            sorted_groups = sorted(price_groups.items(), key=lambda x: len(x[1]), reverse=True)
    ...
    def _analyze_stuffing_event(self, messages: List[QuoteMessage], pattern_type: str) -> QuoteStuffingEvent:
    ...
    def _calculate_peak_rate(self, messages: List[QuoteMessage]) -> float:
        ...
        for i in range(len(messages)):
            ...
            window_messages = [m for m in messages[i:] if m.timestamp <= window_end]

            if len(window_messages) > 1:
                time_span = (window_messages[-1].timestamp - window_messages[0].timestamp).total_seconds()
                if time_span > 0:
    ...
    def _determine_severity(self, peak_rate: float, duration_ms: float) -> str:
        ...
        for severity, thresholds in reversed(list(self.severity_levels.items())):
            if peak_rate >= thresholds["rate"] and duration_ms >= thresholds["duration"]:
                return severity
    ...
    def _calculate_market_impact(self, messages: List[QuoteMessage]) -> Dict[str, float]:
        ...
        during_rate = len(messages) / ((messages[-1].timestamp - messages[0].timestamp).total_seconds() or 1)
    ...
    def _get_messages_before(self, timestamp: datetime, window: timedelta) -> List[QuoteMessage]:
        ...
        return [m for m in self.message_stream if start_time <= m.timestamp < timestamp]
    ...
    def _get_messages_after(self, timestamp: datetime, window: timedelta) -> List[QuoteMessage]:
        ...
        return [m for m in self.message_stream if timestamp < m.timestamp <= end_time]
    ...
    def get_stuffing_statistics(self, minutes: int = 60) -> Dict[str, Any]:
        ...
        if not recent_events:
            return {"event_count": 0, "total_messages_involved": 0, "avg_severity": "none", "patterns": {}}
