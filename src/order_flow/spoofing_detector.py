...
class SpoofingDetector:
    ...
    def update_order_book(
        self, timestamp: datetime, bid_levels: List[Tuple[float, float]], ask_levels: List[Tuple[float, float]]
    ) -> Optional[SpoofingEvent]:
        """
    ...
    def track_order_event(
        self, order_id: str, timestamp: datetime, event_type: str, price: float, size: float, side: str
    ):
        """
            ...
            if len(suspicious_levels) >= 3:  # Multiple layers
                ...
                if self._check_layer_cancellation(side, suspicious_levels):
                    return self._create_spoofing_event(
                        snapshot=snapshot, side=side, pattern_type="layering", suspicious_levels=suspicious_levels
                    )
    ...
    def _create_spoofing_event(
        self, snapshot: OrderBookSnapshot, side: str, pattern_type: str, suspicious_levels: List[Tuple[float, float]]
    ) -> SpoofingEvent:
    ...
    def _calculate_spoofing_confidence(self, pattern_type: str, suspicious_levels: List[Tuple[float, float]]) -> float:
        """Calculate confidence score for spoofing detection"""

        base_confidence = {"layering": 0.7, "momentum_ignition": 0.8, "flash": 0.75}.get(pattern_type, 0.5)
    ...
    def get_spoofing_statistics(self) -> Dict[str, Any]:
        ...
        if not self.detected_spoofs:
            return {"total_events": 0, "events_by_type": {}, "avg_confidence": 0, "avg_price_impact": 0}
