...
class RateLimiter:
    ...
    def check_rate_limit(self, key: str, limit: int, window: int = 60, increment: bool = True) -> Dict[str, Any]:
        """
    ...
    def _check_redis_rate_limit(self, key: str, limit: int, window: int, increment: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"allowed": allowed, "remaining": remaining, "reset_time": reset_time, "retry_after": retry_after}

        except Exception as e:
            logger.error(f"Redis rate limit error: {e}, falling back to file storage")
            return self._check_file_rate_limit(key, limit, window, increment)

    def _check_file_rate_limit(self, key: str, limit: int, window: int, increment: bool) -> Dict[str, Any]:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not result["allowed"]:
                raise RateLimitExceeded(
                    f"Rate limit exceeded for {key}. " f"Try again in {result['retry_after']} seconds"
                )
