...
class TradeResultWatcher(FileSystemEventHandler):
    ...
    def _send_result_to_toc(self, result_data: Dict):
        ...
        try:
            ...
            payload = {"user_id": self.user_id, "terminal_id": self.terminal_id, "trade_result": result_data}
...
def launch_terminal():
    ...
    try:
        ...
        if success:
            ...
            return jsonify({"success": True, "terminal_id": terminal_id, "message": "Terminal launched successfully"})
        else:
            return jsonify({"success": False, "error": "Failed to launch terminal"}), 500
...
def list_terminals():
    ...
    with terminal_lock:
        ...
        for tid, info in active_terminals.items():
            ...
            terminals.append(
                {"terminal_id": tid, "path": info["path"], "status": status, "launched_at": info["launched_at"]}
            )
...
def stop_terminal(terminal_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": f"Terminal {terminal_id} stopped"})
...
def _launch_mt5_terminal(terminal_id: str, terminal_path: Path) -> Optional[int]:
    ...
    try:
        ...
        cmd = [str(terminal_exe), "/portable", f"/config:{terminal_path}"]

        process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=str(terminal_path))
