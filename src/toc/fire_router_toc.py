...
from terminal_assignment import TerminalAssignment, TerminalStatus, TerminalType
...
class FireRouterTOC:
    ...
    def __init__(self, terminal_db_path: str = "terminal_assignments.db", config_path: Optional[str] = None):
        """
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict:
        ...
        default_config = {
            ...
            "http_headers": {"Content-Type": "application/json", "X-Signal-Source": "HydraX-TOC"},
        }
    ...
    def route_signal(
        ...
        try:
            ...
            assignment = self._get_or_assign_terminal(signal.user_id, terminal_type, preferred_terminal_id)
            ...
            return SignalResponse(success=False, message=f"Routing error: {str(e)}", error_code="ROUTING_ERROR")

    def _get_or_assign_terminal(
        self, user_id: str, terminal_type: TerminalType, preferred_terminal_id: Optional[int] = None
    ) -> Optional[Dict]:
        ...
        assignment = self.terminal_manager.assign_terminal(
            ...
            metadata={"source": "fire_router_toc", "timestamp": datetime.now().isoformat()},
        )

        return assignment

    def _route_with_retry(self, signal: TradeSignal, assignment: Dict, method: DeliveryMethod) -> SignalResponse:
        """Route signal with retry logic"""
        retry_count = 0
        last_error = None
    ...
    def _send_http(self, signal: TradeSignal, assignment: Dict, retry_count: int) -> SignalResponse:
        ...
        try:
            ...
            response = requests.post(
                url, json=payload, headers=self.config.get("http_headers", {}), timeout=self.timeout
            )
    ...
    def get_user_routing_info(self, user_id: str) -> Dict[str, Any]:
        ...
        routing_info = {"user_id": user_id, "active_assignments": len(assignments), "terminals": []}
        ...
        user_history = [entry for entry in self.signal_history if entry["user_id"] == user_id][-10:]  # Last 10 signals
    ...
    def get_terminal_health(self, terminal_id: int) -> Dict[str, Any]:
        ...
        if not terminal_info:
            return {"terminal_id": terminal_id, "status": "NOT_FOUND", "healthy": False}
        ...
        health["healthy"] = health["status"] == TerminalStatus.AVAILABLE.value and health.get(
        ...
        try:
            ...
            for assignment in assignments:
                ...
                if new_assignment:
                    ...
                    results["failures"].append({"user_id": user_id, "reason": "No available terminals"})

            results["success"] = len(results["failures"]) == 0
            results["message"] = (
                f"Failover completed. {len(results['reassignments'])} users reassigned, {len(results['failures'])} failures"
            )
            ...
            return {"success": False, "message": f"Failover failed: {str(e)}", "terminal_id": terminal_id}
    ...
    def fire_endpoint():
        ...
        try:
            ...
            response = router.route_signal(
                signal=signal, terminal_type=terminal_type, preferred_terminal_id=data.get("preferred_terminal_id")
            )
