...
class TerminalAssignment:
        ...
        with self._lock:
            with self._get_connection() as conn:
                ...
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                    (terminal_name, terminal_type.value, ip_address, port, folder_path, max_users, metadata_json),
                )
    ...
    def release_terminal(
        self, user_id: str, terminal_id: Optional[int] = None, terminal_type: Optional[TerminalType] = None
    ) -> bool:
        """
    ...
    def get_assignment_history(
        self, user_id: Optional[str] = None, terminal_id: Optional[int] = None, limit: int = 100
    ) -> List[Dict]:
        """
...
if __name__ == "__main__":
    ...
    assignment = manager.assign_terminal(
        user_id="user123", terminal_type=TerminalType.PRESS_PASS, metadata={"session_type": "training"}
    )
