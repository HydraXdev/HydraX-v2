...
class EducationHUD {
  ...
  selectMission(missionId) {
    ...
    const selectedMission = document.querySelector(
    ...
    setInterval(() => {
      if (Math.random() < 0.2) {
        const achievements = [
          {
            title: "Speed Demon",
            desc: "Complete 5 missions in 1 hour",
            xp: 300,
          },
          {
            title: "Perfect Execution",
            desc: "100% accuracy on Algorithm Training",
            xp: 500,
          },
          { title: "Night Owl", desc: "Study past midnight", xp: 200 },
          {
            title: "Quick Learner",
            desc: "Master a concept in record time",
            xp: 400,
          },
        ];
        const achievement =
  ...
  levelUp() {
    ...
    setTimeout(() => {
      ...
      document.getElementById("neededXP").textContent =
        this.neededXP.toLocaleString();
      this.updateXPDisplay();
    }, 3000);

    // Add to feed
    this.addTacticalFeedItem(
      `LEVEL UP! You are now Level ${this.currentLevel}`,
    );
  ...
  updateMissionProgress(missionIndex) {
    ...
    if (mission.progress === 100) {
      ...
      this.addTacticalFeedItem(
        `Mission Progress: ${mission.name} +${progressIncrease}%`,
      );
    }
  }
