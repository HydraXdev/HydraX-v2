...
function initializeHUD() {
  ...
  document
    ...
    .querySelector(".fire-text").textContent = "AWAITING SIGNAL";
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("input", calculatePosition);
  document
...
function updateUserStats() {
  ...
  document.getElementById("todayPnL").textContent =
    stats.todayPnL > 0 ? `+${stats.todayPnL}%` : `${stats.todayPnL}%`;

  // Color code P&L
  const pnlElement = document.getElementById("todayPnL");
  pnlElement.style.color =
    stats.todayPnL > 0 ? FANG_CONFIG.colors.success : FANG_CONFIG.colors.danger;

  // Show fire emoji for streaks > 5
  document.getElementById("streakFire").style.display =
    stats.currentStreak > 5 ? "inline" : "none";
}
...
function updateSignalDisplay() {
  ...
  document.getElementById("direction").className =
    `direction ${currentSignal.direction.toLowerCase()}`;
  document.getElementById("currentPrice").textContent =
    currentSignal.currentPrice.toFixed(2);
  document.getElementById("priceChange").textContent =
    currentSignal.priceChange > 0
      ? `+${currentSignal.priceChange}%`
      : `${currentSignal.priceChange}%`;
  ...
  document.getElementById("entryPrice").textContent =
    currentSignal.entry.price.toFixed(2);
  ...
  // Risk calculation
  const riskPercent = (
    ((currentSignal.entry.price - currentSignal.stopLoss) /
      currentSignal.entry.price) *
    100
  ).toFixed(2);
  ...
  currentSignal.takeProfits.forEach((tp, index) => {
    ...
    document
      .querySelector(`#tp${index + 1}`)
      .parentElement.querySelector(".tp-percent").textContent =
      `+${tp.percent}%`;
  });
  ...
  // Update Bit message
  updateBitMessage(
    `Signal detected: ${currentSignal.symbol} ${currentSignal.direction}. TCS Score: ${currentSignal.tcs.composite}`,
  );
}
...
function updateTechnicalIndicators() {
  ...
  const visibleCount = Math.ceil(
    currentSignal.indicators.length * FANG_CONFIG.dataVisibility,
  );
...
function startCountdown() {
  ...
  const updateCountdown = () => {
    ...
    if (remaining <= 0) {
      ...
      document.getElementById("urgencyLevel").style.color =
        FANG_CONFIG.colors.danger;
      document.getElementById("fireButton").disabled = true;
      return;
    }
...
function calculatePosition() {
  ...
  const balance =
    parseFloat(document.getElementById("accountBalance").value) || 10000;
  const riskPercent =
    parseFloat(document.getElementById("riskPercent").value) || 2;
...
async function handleFireButton() {
  ...
  try {
    ...
    showAlert(
      "TRADE EXECUTED",
      `${currentSignal.symbol} ${currentSignal.direction} position opened successfully.`,
    );
...
async function refreshData() {
  ...
  await Promise.all([loadUserData(), loadSignalData()]);
...
function showAnalytics() {
  showAlert(
    "ANALYTICS",
    "Full analytics available in FANG tier. Access your performance dashboard in the main app.",
  );
}
