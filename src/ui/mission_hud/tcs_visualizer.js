...
class TCSVisualizer {
  ...
  createDOM() {
    ...
    this.elements = {
      ...
      predictiveDisplay: this.container.querySelector(
  ...
  updateScore(newScore, animated = true) {
    ...
    this.state.targetScore = Math.max(
      0,
  ...
  showPredictiveScore(predictedScore, tradeDetails = {}) {
    ...
    this.state.predictedScore = Math.max(
      0,
      Math.min(this.config.maxScore, predictedScore),
    );
    ...
    this.elements.predictiveValue.textContent =
      this.state.predictedScore.toFixed(1);
    this.elements.predictiveChange.textContent = `(${change > 0 ? "+" : ""}${change.toFixed(1)}%)`;

    // Update arrow direction
    this.elements.predictiveDisplay.querySelector(
      ".tcs-predictive-arrow",
    ).className = `tcs-predictive-arrow ${change > 0 ? "up" : "down"}`;

    // Update predicted ring
    this.updatePredictedRing();

    // Show warnings if crossing tier boundaries
    this.checkPredictiveWarnings(
  ...
  checkPredictiveWarnings(currentScore, predictedScore) {
    ...
    if (currentTier !== predictedTier) {
      const improving =
        this.tiers[predictedTier].min > this.tiers[currentTier].min;
      const message = improving
  ...
  toggleHistory() {
    ...
    this.elements.historyCanvas.parentElement.style.display = this.config
      .showHistory
      ? "block"
      : "none";
