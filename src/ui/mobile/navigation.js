...
class HydraXMobileNavigation {
  ...
  initializeDOM() {
    ...
    this.navContainer =
      document.getElementById("mobile-nav") || this.createNavigationContainer();

    // Get content area
    this.contentArea =
      document.getElementById("content-area") || document.querySelector("main");
  ...
  createTabButton(config) {
    ...
    button.setAttribute(
      "tabindex",
      config.id === this.state.currentPage ? "0" : "-1",
    );
  ...
  setupEventListeners() {
    ...
    if (window.matchMedia) {
      window
        ...
        .addListener(this.handleThemeChange.bind(this));
      window
        .matchMedia("(prefers-reduced-motion: reduce)")
        .addListener(this.handleMotionPreferenceChange.bind(this));
    }
  ...
  processCompletedGesture(deltaX, deltaY, velocity, duration) {
    ...
    if (
      ...
      deltaY > this.options.pullToRefreshThreshold
    ) {
      this.triggerRefresh();
      return;
    }
  ...
  animatePageTransition(fromPage, toPage, direction) {
    return new Promise((resolve) => {
      ...
      this.applyTransitionAnimation(fromElement, toElement, direction).then(
        () => {
          this.switchPageContent(fromPage, toPage);
          resolve();
        },
  ...
  updateOneHandedMode() {
    document.body.classList.toggle(
      "one-handed-mode",
      this.state.isOneHandedMode,
    );

    if (this.oneHandedToggle) {
      this.oneHandedToggle.setAttribute(
        "aria-pressed",
        this.state.isOneHandedMode,
      );
    ...
    try {
      if (this.isInTelegram && this.telegramApp.HapticFeedback) {
        ...
        const feedbackMap = {
          ...
          medium: () =>
            this.telegramApp.HapticFeedback.impactOccurred("medium"),
          heavy: () => this.telegramApp.HapticFeedback.impactOccurred("heavy"),
          success: () =>
            this.telegramApp.HapticFeedback.notificationOccurred("success"),
          error: () =>
            this.telegramApp.HapticFeedback.notificationOccurred("error"),
        };
  ...
  updateGestureVisuals(deltaX, deltaY) {
    ...
    if (absX > absY) {
      ...
      deltaY > 0 &&
      this.shouldHandlePullToRefresh(deltaY, window.pageYOffset)
    ) {
  ...
  manageFocusOnPageChange(pageId) {
    ...
    if (newPanel) {
      const focusTarget = newPanel.querySelector(
        'h1, [tabindex="0"], button, input, select, textarea, a[href]',
      );
      if (focusTarget) {
        focusTarget.focus();
      }
  ...
  handlePopState(event) {
    const page =
      event.state?.page || window.location.hash.slice(1) || "dashboard";
