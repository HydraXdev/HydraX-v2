...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", refreshSignalData);
  document
  ...
  document
    ...
    .addEventListener("click", copyRecruitmentLink);

  // Share button
  document
    .getElementById("shareButton")
    .addEventListener("click", shareRecruitmentLink);
...
async function loadSignalData() {
  ...
  try {
    const response = await fetch(
      `/api/hud/signal/${HUDState.signalData.signal_id}`,
      {
        headers: {
...
function updateSignalDisplay(signal) {
  // Update signal ID
  document.getElementById("signalId").textContent =
    `SIG-${signal.id.substr(-5).toUpperCase()}`;
  ...
  document.getElementById("urgencyLevel").className =
    `urgency-level ${signal.urgency.toLowerCase()}`;

  // Update trade parameters
  document.getElementById("entryPrice").textContent = formatPrice(
    signal.entry_price,
  );
  document.getElementById("stopLoss").textContent = formatPrice(
    signal.stop_loss,
  );
  document.getElementById("takeProfit").textContent = formatPrice(
    signal.take_profit,
  );
...
function updateStrategyDisplay(strategy) {
  ...
  if (strategy.conditions) {
    const conditionsHtml = strategy.conditions
      .map(
        (condition) => `
...
function updateFireControl(access) {
  ...
  if (access.can_execute) {
    ...
    if (tierLock) {
      ...
      document.getElementById("requiredTier").textContent =
...
function updatePersonalStats(stats) {
  ...
  document.getElementById("recruitCount").textContent =
    stats.recruits_count || "0";
...
function shareRecruitmentLink() {
  ...
  if (tg.isVersionAtLeast("6.1")) {
    ...
    // Fallback to URL
    window.open(
      `https://t.me/share/url?url=${encodeURIComponent(recruitLink)}&text=${encodeURIComponent(shareText)}`,
    );
  }
}
