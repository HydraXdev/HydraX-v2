...
def start_apex_engine():
    ...
    try:
        ...
        proc = subprocess.Popen(
            [sys.executable, "apex_v5_live_real.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def start_telegram_connector():
    ...
    try:
        os.chdir("/root/HydraX-v2")
        proc = subprocess.Popen(
            [sys.executable, "apex_telegram_connector.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def start_webapp_server():
    ...
    try:
        os.chdir("/root/HydraX-v2")
        proc = subprocess.Popen([sys.executable, "webapp_server.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def monitor_system(processes):
    ...
    while True:
        try:
            ...
            print(
                f"📊 Status: {len([p for p in processes.values() if p and p.poll() is None])}/3 processes running, "
...
def shutdown_handler(signum, frame):
    ...
    try:
        ...
        children = subprocess.check_output(f"pgrep -P {current_pid}", shell=True).decode().strip().split("\n")
