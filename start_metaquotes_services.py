...
from src.bitten_core.metaquotes import PoolConfig, get_demo_account_service, get_pool_manager

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    handlers=[logging.FileHandler("logs/metaquotes_services.log"), logging.StreamHandler()],
)
...
class MetaQuotesServiceManager:
    ...
    async def start(self):
        ...
        self.tasks.append(asyncio.create_task(self._monitor_services()))

        # Start credential delivery processor
        self.tasks.append(asyncio.create_task(self._process_credential_deliveries()))
    ...
    async def _monitor_services(self):
        ...
        while self.is_running:
            try:
                ...
                logger.info(
                    f"Pool Stats - Available: {pool_stats['healthy_available']}, "
