...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/root/HydraX-v2/logs/system_offline.log"), logging.StreamHandler()],
)
...
class SystemOfflineManager:
    ...
    def take_component_offline(self, component: str, reason: OfflineReason, details: str = ""):
        ...
        try:
            ...
            messages = self.offline_messages.get(
                reason,
                {
    ...
    def get_system_status(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "offline_since_longest": (
                    min([s.offline_since for s in self.offline_components.values()])
                    if self.offline_components
                    else None
                ),
            ...
            return {"system_state": "ERROR", "system_health": 0, "error": str(e)}
    ...
    def _attempt_recovery(self, component: str, offline_status: OfflineStatus):
        ...
        try:
            ...
            logger.info(
                f"🔄 Recovery attempt {offline_status.attempt_count}/{offline_status.max_attempts} for {component_name}"
            )
                    ...
                    for dependent in dependents:
                        if dependent not in self.offline_components:
                            self.take_component_offline(
                                dependent, OfflineReason.NO_REAL_DATA, f"Dependency {component} is offline"
                            )
