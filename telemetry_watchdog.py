...
logging.basicConfig(level=logging.INFO, format="%(asctime)s - TelemetryWatchdog - %(levelname)s - %(message)s")
...
class TelemetryWatchdog:
    ...
    def check_signal_activity(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["tail", "-1", "/root/HydraX-v2/optimized_tracking.jsonl"], capture_output=True, text=True, timeout=5
            )
    ...
    def restart_telemetry_bridge(self):
        ...
        try:
            # Restart via PM2
            subprocess.run(["pm2", "restart", "zmq_telemetry_bridge"], capture_output=True, timeout=10)
