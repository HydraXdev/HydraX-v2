...
def test_mission_session_ttl() -> bool:
    ...
    print_test("Mission session TTL configured (5-10 min)", ttl_in_range,
               f"TTL: {ttl_seconds}s ({ttl_seconds/60:.1f} min)")
    ...
    try:
        ...
        ''', (
            test_ms_id, "test_user", 999, "test_signal",
            "PENDING", "EURUSD", "M5", None, test_nonce,
            now, now + 2  # 2 second expiration
        ))
        conn.commit()

        session_created = True
        print_test("Mission session creation", session_created,
                  f"Session ID: {test_ms_id}")
...
def test_nonce_and_idempotency() -> bool:
    ...
    try:
        ...
        print_test("Duplicate returns same opId", same_op_id,
...
def test_risk_fuse() -> bool:
    ...
    try:
        ...
        print_test("Token includes riskMaxUsd claim", has_risk_claim,
                  f"Claim present: {has_risk_claim}")
        record_result(category, "riskMaxUsd claim present", has_risk_claim)
        all_passed &= has_risk_claim

        print_test("riskMaxUsd value correct", correct_risk_value,
                  f"Expected: {test_risk_limit}, Got: {claims.get('riskMaxUsd')}")
        record_result(category, "riskMaxUsd value correct", correct_risk_value,
                     f"{claims.get('riskMaxUsd')}")
        ...
        if not has_risk_enforcement:
            print_test("Risk enforcement code exists in webapp", False,
                      f"{YELLOW}WARNING: Not yet implemented (architectural requirement){RESET}")
            record_result(category, "Risk enforcement code exists (WARNING)", False,
                         "Not implemented - architectural requirement")
        else:
            print_test("Risk enforcement code exists in webapp", True,
                      f"Found risk validation logic")
        ...
        print_test("Risk limit validation logic", should_reject,
                  f"Request ${test_request_amount} > Limit ${test_risk_limit}")
        record_result(category, "Risk limit validation", should_reject,
                     f"${test_request_amount} > ${test_risk_limit}")
...
def test_rooms_and_topics() -> bool:
    ...
    print_test("User-scoped rooms architecture", user_scoped_rooms,
              "Rooms scoped by user_id")
    ...
    print_test("Topic authorization architecture", topic_auth,
              "JWT-based topic authorization")
...
def test_audit_logs() -> bool:
    ...
    try:
        ...
        print_test("Logs contain required fields", logs_safe_fields,
                  f"Fields: {', '.join(safe_log_fields)}")
        record_result(category, "Required log fields", logs_safe_fields,
                     ', '.join(safe_log_fields))
        ...
        print_test("No PII/balances in logs", no_pii_in_logs,
                  f"Excluded: {', '.join(unsafe_fields)}")
        record_result(category, "No PII in logs", no_pii_in_logs,
                     ', '.join(unsafe_fields))
...
def print_summary(summary: Dict):
    ...
    for category, data in test_results['categories'].items():
        ...
        print(f"  {status} {category.replace('_', ' ').title()}: {data['passed']}/{data['passed'] + data['failed']} passed")
