...
def calculate_dollar_amounts(
    user_id: str, symbol: str, stop_pips: float, target_pips: float, risk_percent: float = 2.0
) -> dict:
...
def mask_price(price: float, decimals_to_mask: int = 2) -> str:
    ...
    if "." in price_str:
        ...
        masked = (
            str(price)[:-decimals_to_mask] + "X" * decimals_to_mask if len(str(price)) > decimals_to_mask else "XXX"
        )
...
def format_sl_tp_with_privacy(entry: float, sl: float, tp: float, show_partial: bool = True) -> dict:
    ...
    if show_partial:
        ...
        return {"entry": mask_price(entry, 2), "sl": mask_price(sl, 2), "tp": mask_price(tp, 2)}
    else:
        # Full privacy - only show existence
        return {"entry": "•••••", "sl": "Protected", "tp": "Protected"}


if __name__ == "__main__":
    # Test with commander user
    test_event = {"symbol": "EURUSD", "stop_pips": 20, "target_pips": 40, "entry_price": 1.0845}
