...
def gates(series):
    ...
    if bull:
        return ("engulf_bull", None)
    if bear:
        return ("engulf_bear", None)
    return None
...
if hits:
    ...
    report.append(
        "[EG-DOCTOR] no simple structures detected; either market is quiet or higher-level gates are stricter than these."
    )
print("\n".join(report))
