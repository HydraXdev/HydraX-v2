...
try:
    ...
    class UUIDTradeTracker:
        def get_system_stats(self):
            return {"total_trades": 0, "active_trades": 0}
...
class FireLoopVerifier:
    ...
    def check_file_status(self, file_path: Path) -> dict:
        ...
        if not file_path.exists():
            return {"status": "❌ NOT FOUND", "age": "N/A", "size": "N/A", "content_preview": "File does not exist"}
        ...
        try:
            ...
            return {"status": status, "age": f"{age:.1f}s", "size": f"{size}B", "content_preview": preview}
        except Exception as e:
            return {"status": "❌ ERROR", "age": "N/A", "size": "N/A", "content_preview": f"Error: {e}"}
    ...
    def check_bridge_heartbeat(self) -> dict:
        ...
        if heartbeat_info["status"] == "❌ NOT FOUND":
            return {"status": "❌ BRIDGE OFFLINE", "message": "No heartbeat file found"}
        ...
        try:
            ...
            if "[HEARTBEAT]" in heartbeat_content:
                ...
                return {"status": status, "message": f"Last heartbeat: {age:.1f}s ago"}
        except Exception as e:
            return {"status": "❌ BRIDGE ERROR", "message": f"Heartbeat read error: {e}"}
    ...
    def check_aws_bridge_signals(self) -> dict:
        ...
        try:
            ...
            return {"total_signals": 0, "recent_signals": [], "last_signal_time": "Error", "error": str(e)}
    ...
    def display_fire_loop_status(self):
        ...
        print(
            f"[1] Signal Generation: {flow_analysis['stage_1_signal']['status']} ({flow_analysis['stage_1_signal']['age']})"
        )
        print(f"[2] Troll Relay: {flow_analysis['stage_2_relay']['status']} ({flow_analysis['stage_2_relay']['age']})")
        print(
            f"[3] Trade Result: {flow_analysis['stage_3_result']['status']} ({flow_analysis['stage_3_result']['age']})"
        )
        ...
        if UUID_TRACKING_AVAILABLE:
            try:
                ...
                if active_trades:
                    print(
                        f"    Current: {active_trades[0]['trade_uuid'][:30]}... ({active_trades[0]['current_stage']})"
                    )
