...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/root/HydraX-v2/logs/midnight_slot_cleanup.log"), logging.StreamHandler()],
)
...
def cleanup_stuck_slots():
    ...
    try:
        ...
        if stuck_slots > 0:
            ...
            # Get affected users
            cursor.execute(
                "SELECT DISTINCT user_id FROM user_fire_modes WHERE manual_slots_in_use = 0 AND auto_slots_in_use = 0"
            )
