...
def send(text, retry=2, signal_id="", event_data=None):
    ...
    message_data = {"chat_id": TG_CHAT, "text": text}
    ...
    if signal_id and event_data:
        ...
        inline_keyboard = {"inline_keyboard": [[{"text": btn_text, "url": mission_url}]]}
    ...
    for attempt in range(retry):
        try:
            ...
            resp = requests.post(f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage", data=message_data, timeout=5)
            resp_json = resp.json()
