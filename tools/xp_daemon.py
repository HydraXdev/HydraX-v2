    ...
    while True:
        ...
        try:
            ...
            for uid, lst in per_user.items():
                ...
                for k in kinds:
                    exec_(
                        "INSERT OR IGNORE INTO xp_events(event_id,user_id,pattern_type,delta,reason,created_at) VALUES (?,?,?,?,?,?)",
                ...
                for i in range(2, len(lst)):
                    if lst[i]["pattern_type"] == lst[i - 1]["pattern_type"] == lst[i - 2]["pattern_type"]:
                        exec_(
                            "INSERT OR IGNORE INTO xp_events(event_id,user_id,pattern_type,delta,reason,created_at) VALUES (?,?,?,?,?,?)",
                            (
                                f"streak:{uid}:{lst[i]['pattern_type']}:{i}:{now//3600}",
                                uid,
                                lst[i]["pattern_type"],
                                STREAK_BONUS,
                                "streak_triple",
