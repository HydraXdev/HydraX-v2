...
class UltimateCORECryptoEngine:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler("/root/HydraX-v2/ultimate_core.log"), logging.StreamHandler()],
        )
    ...
    def build_timeframe_candles(
        self, symbol: str, ticks: List[CryptoTick], current_time: datetime, timeframe: str, seconds: int
    ):
        ...
        while self.running:
            try:
                for symbol in self.crypto_symbols:
                    ...
                    if (
                        len(self.m1_candles[symbol]) >= 10
                        and len(self.m5_candles[symbol]) >= 5
                        and len(self.tick_data[symbol]) >= 50
                    ):
    ...
    def detect_crypto_momentum_breakout(
        self, symbol: str, m1_candles: List[CryptoCandle], m5_candles: List[CryptoCandle], ticks: List[CryptoTick]
        ...
        if strong_momentum and volume_surge:
            ...
            self.logger.info(
                f"🚀 RAPID_ASSAULT detected: {symbol} {direction} @ {entry:.5f} (conf: {final_confidence:.1f}%)"
            )
            return signal

        return None

    def detect_crypto_liquidity_sweep(
        self, symbol: str, m1_candles: List[CryptoCandle], m5_candles: List[CryptoCandle], ticks: List[CryptoTick]
    ) -> Optional[CryptoSignal]:
        ...
        if swept_high or swept_low:
            ...
            self.logger.info(
                f"💎 LIQUIDITY_SWEEP detected: {symbol} {direction} @ {entry:.5f} (conf: {final_confidence:.1f}%)"
            )
            return signal

        return None

    def detect_crypto_order_block(
        self, symbol: str, m1_candles: List[CryptoCandle], m5_candles: List[CryptoCandle], ticks: List[CryptoTick]
    ) -> Optional[CryptoSignal]:
        ...
        near_boundary = current_price <= min(lows[-5:]) * 1.001 or current_price >= max(highs[-5:]) * 0.999
        ...
        if consolidation and near_boundary:
            ...
            self.logger.info(
                f"🎯 ORDER_BLOCK detected: {symbol} {direction} @ {entry:.5f} (conf: {final_confidence:.1f}%)"
            )
            return signal

        return None

    def detect_crypto_fair_value_gap(
        self, symbol: str, m1_candles: List[CryptoCandle], m5_candles: List[CryptoCandle], ticks: List[CryptoTick]
    ) -> Optional[CryptoSignal]:
        ...
        for i in range(3, len(m1_candles) - 2):
            ...
            if bullish_gap or bearish_gap:
                ...
                if approaching and gap_size > self.calculate_crypto_atr(m1_candles, 14) * 0.5:
                    ...
                    self.logger.info(
                        f"⚡ FAIR_VALUE_GAP detected: {symbol} {direction} @ {entry:.5f} (conf: {final_confidence:.1f}%)"
                    )
                    return signal
    ...
    def print_status(self):
        ...
        print(
            f"  🚀 RAPID_ASSAULT: {self.rapid_assault_count} ({self.rapid_assault_count/max(1,self.signals_generated)*100:.1f}%)"
        )
        print(
            f"  💎 PRECISION_STRIKE: {self.precision_strike_count} ({self.precision_strike_count/max(1,self.signals_generated)*100:.1f}%)"
        )
