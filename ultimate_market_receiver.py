...
class BrokerIntelligence:
    ...
    def classify_broker(self, broker_name: str) -> Dict:
        ...
        return self.broker_profiles.get(broker_name, {"type": "unknown", "spread_multiplier": 1.0})
    ...
    async def process_tick_batch(self, tick_data: Dict) -> Dict:
        ...
        try:
            ...
            if tick_data.get("source") != "MT5_LIVE":
                raise HTTPException(
                    status_code=403, detail=f"Only MT5_LIVE data accepted. Got: {tick_data.get('source', 'unknown')}"
                )
    ...
    async def store_tick(self, symbol: str, tick_data: Dict):
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.redis.setex(key, CONFIG["tick_ttl"], json.dumps(tick_data))
            )

            # Store in broker-specific stream
            broker_key = f"broker:{tick_data['broker']}:{symbol}"
            await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.redis.lpush(broker_key, json.dumps(tick_data))
            )
            await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.redis.ltrim(broker_key, 0, 49)  # Keep last 50 ticks
            )
            await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.redis.expire(broker_key, CONFIG["tick_ttl"])
            )
...
class SpreadAnalyzer:
    ...
    async def get_broker_spreads(self, symbol: str) -> Dict:
        ...
        try:
            ...
            keys = await asyncio.get_event_loop().run_in_executor(None, lambda: self.redis.keys(pattern))
            ...
            for key in keys:
                ...
                latest_tick = await asyncio.get_event_loop().run_in_executor(None, lambda: self.redis.lindex(key, 0))
...
async def get_all_market_data():
    ...
    try:
        ...
        for symbol in symbols:
            try:
                key = f"tick:{symbol}:latest"
                tick_json = await asyncio.get_event_loop().run_in_executor(None, lambda: redis_client.get(key))
        ...
        return JSONResponse(
            content={"data": market_data, "total_symbols": len(market_data), "timestamp": datetime.now().isoformat()}
        )
...
async def health_check():
    ...
    try:
        ...
        if redis_client:
            try:
                await asyncio.get_event_loop().run_in_executor(None, lambda: redis_client.ping())
            except:
        ...
        if redis_client:
            try:
                keys = await asyncio.get_event_loop().run_in_executor(None, lambda: redis_client.keys("tick:*:latest"))
