...
market_data = defaultdict(
    lambda: {"bid": 0.0, "ask": 0.0, "spread": 0.0, "volume": 0, "last_update": None, "source": None}
)
...
def receive_data():
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Raw data around error: {raw_data[max(0, e.pos-50):e.pos+50] if hasattr(e, 'pos') and raw_data else 'N/A'}"
            )
