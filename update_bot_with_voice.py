...
class VoiceEnabledPersonalityBot:
    ...
    def save_voice_settings(self):
        ...
        with open(self.voice_settings_file, "w") as f:
            json.dump({"enabled_users": list(self.voice_enabled_users)}, f, indent=2)
        ...
        try:
            ...
            if audio_file and os.path.exists(audio_file):
                ...
                with open(audio_file, "rb") as audio:
                    self.bot.send_voice(
                        chat_id, audio, caption=f"🎙️ {voice_personality}", duration=None  # Let Telegram calculate
                    )
            ...
            if enabled:
                ...
                asyncio.run(
                    self.send_voice_message(
                        message.chat.id, "Voice system activated. Ready to serve, soldier!", "DRILL_SERGEANT"
                    )
                )
            else:
...
def apply_voice_patch():
    ...
    try:
        ...
        bitten_personality_bot.BittenPersonalityBot.send_personality_message = integrate_voice_with_personality_bot(
            original_send
        )
