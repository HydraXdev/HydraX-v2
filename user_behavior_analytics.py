...
class UserBehaviorAnalytics:
    ...
    def track_signal_skipped(self, user_id: str, signal_id: str, reason: str = "user_choice"):
        ...
        skip_data = {"timestamp": time.time(), "reason": reason}  # user_choice, expired, low_confidence, etc.
            ...
            with open(self.truth_log_path, "r") as f:
                for line in f:
                    if line.strip():
                        ...
                        if (
                            user_id in trade.get("users_fired", []) or trade.get("user_count", 0) > 0
                        ):  # Assuming user executed
                            trades.append(trade)
    ...
    def calculate_user_win_rate(self, trades: List[Dict]) -> float:
        ...
        wins = sum(
            1
            for trade in trades
            if trade.get("outcome") == "WIN" or trade.get("hit_tp_first", False) or trade.get("pips_result", 0) > 0
        )
    ...
    def classify_user(self, execution_rate: float, win_rate: float, total_trades: int) -> str:
        ...
        if total_trades < 5:
            ...
            execution_rate >= self.power_user_thresholds["execution_rate"]
            and win_rate >= self.power_user_thresholds["win_rate"]
        ):
            return "POWER_USER"
    ...
    def identify_user_education_needs(self) -> Dict[str, List[str]]:
        ...
        education_map = {
            ...
            "SKILLED": ["Advanced pattern analysis", "Multi-timeframe strategies", "Position sizing optimization"],
        }
