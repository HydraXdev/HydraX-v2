...
from webapp.database import FireOperations, MissionOperations, SignalOperations, UserOperations
from webapp.utils import can_user_fire, get_signal_class, get_user_tier
...
def fire_mission():
    ...
    try:
        ...
        FireOperations.create_fire_record({"fire_id": fire_id, "mission_id": mission_id, "user_id": user_id})
        ...
        try:
            result = subprocess.run(
                ["python3", "/root/HydraX-v2/enqueue_fire.py", json.dumps(fire_command)],
                capture_output=True,
                text=True,
                timeout=5,
            )
            ...
            if result.returncode == 0:
                ...
                return (
                    jsonify({"success": False, "error": "Failed to execute trade command", "details": result.stderr}),
                    500,
                )
            ...
            return jsonify({"success": False, "error": "Trade command timed out"}), 500
...
def mission_status(signal_id):
    ...
    try:
        ...
        if signal:
            return jsonify({"success": True, "signal_id": signal_id, "status": "AVAILABLE", "signal": signal}), 200

        return jsonify({"success": False, "error": "Mission/Signal not found"}), 404
