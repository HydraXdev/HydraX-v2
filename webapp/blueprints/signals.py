...
from webapp.database import MissionOperations, SignalOperations
from webapp.utils import calculate_sl_tp, get_bitten_db, pip_size
...
def api_signals():
    ...
    try:
        if request.method == "POST":
            ...
            return jsonify({"success": True, "signal_id": signal_id, "message": "Signal received and stored"}), 200
...
def venom_signals():
    ...
    try:
        ...
        return (
            jsonify({"signals": formatted, "count": len(formatted), "source": "elite_guard"}),  # Indicate actual source
            200,
        )
