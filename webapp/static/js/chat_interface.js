...
class SquadChatInterface {
  ...
  async init() {
    try {
      ...
      console.log(
        "BITTEN Squad Chat initialized for user:",
    ...
    const sound = {
      play: () => {
        ...
        gainNode.gain.exponentialRampToValueAtTime(
          0.01,
          this.audioContext.currentTime +
            (duration * frequencies.length) / 1000,
        );
  ...
  handleUserJoined(data) {
    this.addSystemMessage(
      `${data.username} joined the tactical channel`,
      data.timestamp,
    );
  }
  ...
  createMessageElement(message) {
    ...
    content.innerHTML = this.formatMessageContent(
      message.content,
  ...
  validateFile(file) {
    if (file.size > this.maxFileSize) {
      this.showError(
        `File too large: ${file.name} (max ${this.maxFileSize / 1024 / 1024}MB)`,
      );
      return false;
    }
    ...
    menuItems.forEach((item) => {
      if (item.divider) {
        ...
        divider.style.cssText =
          "margin: 5px 0; border: 0; border-top: 1px solid var(--bitten-border);";
  ...
  hasModeratorPermissions() {
    return ["commander", "sergeant"].includes(
  ...
  showBrowserNotification(message) {
    if ("Notification" in window && Notification.permission === "granted") {
      new Notification(
        `${message.callsign}: ${message.content.substring(0, 50)}...`,
        {
