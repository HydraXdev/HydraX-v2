...
def test_tier(tier):
    ...
    template_content = template_content.replace(
        ...
        f'id="currentStreak">{"5" if tier == "nibbler" else "12" if tier == "fang" else "28"}</span>',
    )
    template_content = template_content.replace(
        'id="winRate">0%</span>',
        f'id="winRate">{"58" if tier == "nibbler" else "68" if tier == "fang" else "78"}%</span>',
    )
    template_content = template_content.replace(
        'id="todayPnL">+0%</span>',
        f'id="todayPnL">+{"1.2" if tier == "nibbler" else "3.8" if tier == "fang" else "7.5"}%</span>',
    )
    ...
    template_content = template_content.replace(
        'id="tcsScore">--</div>',
        f'id="tcsScore">{"72" if tier == "nibbler" else "78" if tier == "fang" else "88"}</div>',
    )
    ...
    template_content = template_content.replace(
        "Analyzing...",
        f'{"Good Signal Quality" if tier == "nibbler" else "High Confidence Signal" if tier == "fang" else "Elite Level Signal"}',
    )
...
def mission_briefing():
    ...
    try:
        ...
        template_content = template_content.replace(
            'id="currentStreak">0</span>', f'id="currentStreak">{user_stats.get("streak_days", "0")}</span>'
        )
        template_content = template_content.replace(
            'id="winRate">0%</span>', f'id="winRate">{user_stats.get("win_rate", "0")}%</span>'
        )
        template_content = template_content.replace(
            'id="todayPnL">+0%</span>', f'id="todayPnL">+{user_stats.get("last_7d_pips", "0")}%</span>'
        )

        # Replace signal data
        template_content = template_content.replace(
            'id="symbol">LOADING...</span>', f'id="symbol">{signal_data.get("symbol", "LOADING...")}</span>'
        )
        template_content = template_content.replace(
            'id="direction">--</span>', f'id="direction">{signal_data.get("direction", "--")}</span>'
        )
        template_content = template_content.replace(
            'id="tcsScore">--</div>', f'id="tcsScore">{signal_data.get("tcs_score", "--")}</div>'
        )
        template_content = template_content.replace(
            'id="entryPrice">0.00000</div>', f'id="entryPrice">{signal_data.get("entry_price", "0.00000")}</div>'
        )
        template_content = template_content.replace(
            'id="stopLoss">0.00000</div>', f'id="stopLoss">{signal_data.get("stop_loss", "0.00000")}</div>'
        )
        template_content = template_content.replace(
            'id="takeProfit">0.00000</div>', f'id="takeProfit">{signal_data.get("take_profit", "0.00000")}</div>'
        )
        template_content = template_content.replace(
            'id="riskRatio">1:0</div>', f'id="riskRatio">1:{signal_data.get("risk_reward_ratio", "0")}</div>'
        )
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=8889, debug=os.getenv("FLASK_DEBUG") == "1")
