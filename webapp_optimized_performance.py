...
from webapp_performance_patch import create_api_cache, performance_monitor
    ...
    try:
        ...
        if user_id and mission_id:
            try:
                ...
                threading.Thread(target=log_hud_access, args=(user_id, mission_id), daemon=True).start()
            except:
                pass
        ...
        return (
            render_template("error_hud.html", error=f"Error loading mission {mission_id}: {str(e)}", error_code=500),
            500,
        )
...
def fire_mission():
    ...
    try:
        ...
        if not mission_id or not user_id:
            return jsonify({"error": "Missing mission_id or user_id", "success": False}), 400
...
def cache_control():
    ...
    try:
        ...
        if action == "clear_mission_cache":
            ...
            return jsonify({"mission_cache": mission_loader.cache.get_cache_stats(), "api_cache_enabled": True})

        elif action == "cleanup":
            expired = mission_loader.cache.cleanup_expired()
            return jsonify({"message": f"Cleaned up {expired} expired entries", "success": True})
...
if __name__ == "__main__":
    ...
    app.run(
        host="0.0.0.0", port=8889, debug=os.getenv("FLASK_DEBUG") == "1", threaded=True
    )  # Different port to test alongside main server
