    ...
    try:
        ...
        for pos in positions:
            ...
            if ticket:
                ...
                pos["hybrid_events"] = [
                    {"type": row[0], "volume": row[1], "pips": row[2], "time": row[3]} for row in cur.fetchall()
                ]
...
def get_hybrid_statistics(uuid, hours=24):
    ...
    try:
        ...
        stats = {}
        for row in cur.fetchall():
            stats[row[0]] = {"count": row[1], "total_volume": row[2], "avg_pips": row[3]}
        ...
        closure_stats = {}
        for row in cur.fetchall():
            closure_stats[row[0]] = {"count": row[1], "total_profit": row[2], "avg_profit": row[3]}

        con.close()

        return {"hybrid_events": stats, "closure_reasons": closure_stats, "period_hours": hours}
...
if __name__ == "__main__":
    ...
    if positions:
        print(f"✅ Position Snapshot: {positions['open_positions']} open, " f"{positions['hybrid_positions']} hybrid")
