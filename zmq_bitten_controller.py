...
class BITTENZMQController:
    ...
    def send_command(self, command: str, params: Dict = None) -> bool:
        ...
        try:
            message = {"type": "command", "command": command, "timestamp": datetime.utcnow().isoformat()}
    ...
    def _handle_telemetry(self, data: Dict):
        ...
        # Log key changes
        logger.info(
            f"📊 Telemetry: {uuid} - Balance: ${telemetry.balance:.2f}, "
    ...
    def _handle_error(self, data: Dict):
        ...
        # Create error result
        result = TradeResult(signal_id=signal_id, status="error", message=error)
        ...
        while True:
            try:
                ...
                if cmd == "q":
                    ...
                    signal = {"symbol": "XAUUSD", "action": "buy", "lot": 0.1, "sl": 200, "tp": 400}
                    controller.send_signal(signal, on_result)

                elif cmd == "2":
                    # SELL EURUSD
                    signal = {"symbol": "EURUSD", "action": "sell", "lot": 0.01, "sl": 50, "tp": 100}
                    ...
                    for uuid, telem in controller.get_all_telemetry().items():
                        print(
                            f"  {uuid}: Balance=${telem.balance:.2f}, "
