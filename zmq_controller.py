...
class ZMQController:
    ...
    def publish_to_venom(self, tick_data: Dict):
        ...
        try:
            ...
            # Format for VENOM
            venom_msg = {"type": "market_tick", "data": tick_data, "controller_time": datetime.utcnow().isoformat()}
    ...
    def print_stats(self):
        ...
        # Top symbols
        top_symbols = sorted(self.tick_count.items(), key=lambda x: x[1], reverse=True)[:5]
        logger.info("\nTop 5 symbols:")
        ...
        current_time = time.time()
        recent_eas = sorted(self.ea_connections.items(), key=lambda x: x[1], reverse=True)[:5]
...
def main():
    ...
    parser.add_argument("--pull-port", type=int, default=5555, help="Port to receive data from EAs")
    parser.add_argument("--push-port", type=int, default=5556, help="Port to send commands to EAs")
    parser.add_argument("--bind", type=str, default="0.0.0.0", help="Interface to bind on")
    ...
    controller = ZMQController(pull_port=args.pull_port, push_port=args.push_port, bind_address=args.bind)
