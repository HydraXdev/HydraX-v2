...
class MigrationHelper:
    ...
    def __init__(self):
        ...
        self.migration_stats = {"fire_txt_writes": 0, "zmq_writes": 0, "dual_writes": 0, "failures": 0}
    ...
    def execute_trade(
        self, user_id: str, signal_data: Dict, fire_path: Optional[str] = None, callback: Optional[Callable] = None
    ) -> Dict:
        """
        ...
        """
        result = {"success": False, "method": "none", "message": "No execution method available"}

        # Log migration attempt
        if ZMQ_LOG_MIGRATIONS:
            logger.info(
                f"🔄 Migration: Executing trade for {user_id} - " f"USE_ZMQ={USE_ZMQ}, DUAL_WRITE={ZMQ_DUAL_WRITE}"
            )
    ...
    def get_migration_stats(self) -> Dict:
        ...
        """
        total = (
            sum([self.migration_stats["fire_txt_writes"], self.migration_stats["zmq_writes"]])
...
def migrate_execute_trade(user_id: str, signal_data: Dict, fire_path: Optional[str] = None) -> Dict:
    """
...
if __name__ == "__main__":
    ...
    test_signal = {"symbol": "EURUSD", "action": "buy", "lot": 0.01, "sl": 50, "tp": 100}
