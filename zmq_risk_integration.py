...
class ZMQRiskIntegration:
    ...
    def analyze_telemetry_risk(self, telemetry: Dict, user_tier: str = "nibbler") -> Dict:
        ...
        if alerts:
            self.risk_alerts[uuid].extend([{**alert, "timestamp": datetime.now().isoformat()} for alert in alerts])
    ...
    def _get_risk_recommendations(self, risk_level: str, alerts: List[Dict]) -> List[str]:
        ...
        if risk_level == "danger":
            ...
            recommendations.extend(
                ["Risk levels normal", "Continue following your trading plan", "Maintain disciplined position sizing"]
            )
    ...
    def get_user_risk_summary(self, uuid: str) -> Dict:
        ...
        """
        if uuid not in self.user_risk_state:
            return {"risk_level": "unknown", "alerts": [], "recommendations": ["No data available"]}

        state = self.user_risk_state[uuid]
        recent_alerts = [
            a
            for a in self.risk_alerts[uuid]
