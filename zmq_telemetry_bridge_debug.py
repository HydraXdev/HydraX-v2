        ...
        try:
            ...
            if message.startswith("OHLC "):
                ...
                if message_count % 30 == 0:
                    logger.info(
                        f"💓 Heartbeat #{message_count} | Ticks: {tick_count}, OHLC: {ohlc_count}, HB: {heartbeat_count}"
                    )
                ...
                try:
                    ...
                    if "symbol" in data and "bid" in data and "ask" in data:
                        symbol = data["symbol"]
                        quotes[symbol] = {"bid": float(data["bid"]), "ask": float(data["ask"])}
                        ...
                        if now - last_drive_ts >= DRIVE_MIN_GAP:
                            ...
                            snapshot = {
                                s: quotes[s]
                                for s in quotes.keys() & symbols_we_manage
