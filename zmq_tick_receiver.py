...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/var/log/zmq_tick_receiver.log"), logging.StreamHandler(sys.stdout)],
)
...
class ZMQTickReceiver:
    ...
    def process_tick(self, symbol: str, data: Dict):
        ...
        if symbol in self.gold_symbols:
            data["gold"] = True
            logger.info(
                f"⚡ GOLD tick received - {symbol}: " f"bid={data.get('bid', 'N/A')}, " f"ask={data.get('ask', 'N/A')}"
            )
    ...
    def process_market_data(self, payload: Dict):
        ...
        if self.market_data_handler:
            ...
            print(f"[{symbol}] Bid: {bid:.5f}, Ask: {ask:.5f}, " f"Gold: {is_gold}, Ticks: {self.tick_counts[symbol]}")
    ...
    def print_statistics(self):
        ...
        for symbol, count in sorted(self.tick_counts.items(), key=lambda x: x[1], reverse=True):
...
def main():
    ...
    parser.add_argument("--enable-gold-routing", action="store_true", help="Enable separate gold routing")
